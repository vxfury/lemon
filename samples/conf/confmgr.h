...
template <typename T, typename std::enable_if<std::is_base_of<google::protobuf::Message, T>::value>::type * = nullptr>
int derive(const std::string &from, T &to)
{
...
class DistributedManager : public manager<Key, bool, int, uint32_t, uint64_t, std::string> {
  public:
    ...
    DistributedManager(std::string repo, std::string branch) : repo_(std::move(repo)), branch_(std::move(branch)) {}
