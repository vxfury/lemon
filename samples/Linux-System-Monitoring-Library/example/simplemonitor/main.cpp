...
int main(int argc, char *argv[])
{
    ...
    for (auto elem : ethernetMonitoring) {
        std::cout << "get Mac adr of dev: " << elem->getDeviceName() << " : "
                  << linuxUtil::getIFaceMacAddress(elem.get()->getDeviceName()) << std::endl;
    }
    ...
    Timer::periodicShot(
        [&processes]() {
            ...
            for (const auto &elem : procCPUUsage) {
                if (elem.second > 1.0) {
                    std::cout << " proc: " << elem.first << " usage: " << elem.second << " % " << std::endl;
                }
            }
        },
    ...
    Timer::periodicShot(
        [&]() {
            std::cout << "----------------------------------------------";
            std::cout << "----------------------------------------------" << std::endl;
            std::cout << " memory load: " << memoryMonitoring->getCurrentMemUsageInPercent()
                      << "% maxmemory: " << memoryMonitoring->getTotalMemoryInKB()
                      << " Kb used: " << memoryMonitoring->getCurrentMemUsageInKB() << " Kb  Memload of this Process "
                      << memoryMonitoring->getMemoryUsageByThisProcess() << " KB " << std::endl;
        },
