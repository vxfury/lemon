...
linuxsysmonitor::linuxsysmonitor(const std::chrono::milliseconds &interval, const std::string &basepath)
    : interval(interval)
{
    this->init(basepath);
...
void linuxsysmonitor::init(const std::string &basepath)
{
    ...
    sysMemory = std::make_unique<memoryLoad>(basepath + "meminfo", basepath + "self/status", basepath + "self/");
    cpu->initCpuUsage();
...
linuxmonitoring_data::DataLinuxmonitoring linuxsysmonitor::getlinuxSysMonitoringData()
{
    ...
    linuxDataModel.get_mutable_linuxsystemmonitoring().get_mutable_cpu().set_num_of_cores(
        cpu->getCurrentMultiCoreUsage().size());
    ...
    for (const auto &elem : this->sysEthernet_v) {
        ...
        obj.set_bytes_rx_total(elem->getParamSinceStartup(networkLoad::mapEnumToString(networkLoad::RXbytes)));
        ...
        obj.set_bytes_tx_total(elem->getParamSinceStartup(networkLoad::mapEnumToString(networkLoad::TXbytes)));
