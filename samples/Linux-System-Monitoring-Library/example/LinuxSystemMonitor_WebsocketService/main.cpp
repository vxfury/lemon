...
class PerSocketData : public IObserver {
  public:
    ...
    void setConnectedWebsocket(uWS::WebSocket<false, true> *socket,
    ...
    void setConnectedWebsocket(uWS::WebSocket<true, true> *socket,
                               std::shared_ptr<linuxsysmonitor> linuxSysMonitorInstance)
    {
...
class LinuxMonitorWebSocketBridge {
  private:
    ...
    void startServer(std::string route)
    {
        if (!this->privateKey.empty() && !this->publicCerts.empty()) {
            uWS::SSLApp({.key_file_name = this->privateKey.c_str(),
                ...
                .listen(this->portToListen,
                        [&](us_listen_socket_t *token) {
                            if (token) {
                                std::cout << "Secure WebsocketServer Listening on port " << this->portToListen
                                          << " with route " << route << std::endl;
                            }
                        })
                .run();
        } else {
            uWS::App()
                ...
                .listen(this->portToListen,
                        [&](us_listen_socket_t *token) {
                            if (token) {
                                std::cout << "Unsecure WebsocketServer Listening on port " << this->portToListen
                                          << " with route " << route << std::endl;
                            }
                        })
                .run();
  ...
  public:
    explicit LinuxMonitorWebSocketBridge(
        const std::string &privateKeyPath, const std::string &publicCertPath, const std::string &passphrase = "",
    ...
    {
        spawnThread = std::make_unique<std::thread>([&]() {
...
int main(int argc, const char *argv[])
{
    ...
    options.add_options()("d,debug", "Enable debugging")("c,public_cert", "path to public cert",
        ...
        cxxopts::value<int>()->default_value("1000"))("f,json", "File name", cxxopts::value<std::string>())(
        "b,abspath", "Path to access kernel informationm", cxxopts::value<std::string>()->default_value("/proc/"));
    auto result = options.parse(argc, argv);

    auto webSocketLinuxMonitor = std::make_unique<LinuxMonitorWebSocketBridge>(
        std::string(result["private_key"].as<std::string>()), std::string(result["public_cert"].as<std::string>()), "",
        result["websocketport_num"].as<int>(), std::chrono::milliseconds(result["sendInterval"].as<int>()),
