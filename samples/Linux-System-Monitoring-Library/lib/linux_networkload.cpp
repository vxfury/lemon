...
void networkLoad::networkParser::parse(const std::string &netFile)
{
    ...
    for (std::string line; std::getline(ethFile, line); lineCnt++) {
        ...
        while (strStream >> strPart) {
            if (ifName.empty()) {
                strPart.erase(std::remove_if(strPart.begin(), strPart.end(),
                                             [](auto c) {
...
const std::unordered_map<std::string, uint64_t> &networkLoad::networkParser::getEthObj(
    const std::string &ethDevice) const
{
    return this->ethObj.at(ethDevice);
}

const std::unordered_map<std::string, uint64_t> &networkLoad::networkParser::getEthObjOld(
    const std::string &ethDevice) const
{
