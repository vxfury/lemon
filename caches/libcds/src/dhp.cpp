...
CDS_EXPORT_API guard_block *hp_allocator::alloc()
{
    ...
    if (block)
        ...
        // allocate new block
        gb = new (s_alloc_memory(sizeof(guard_block) + sizeof(guard) * defaults::c_extended_guard_block_size))
            guard_block;
...
CDS_EXPORT_API retired_block *retired_allocator::alloc()
{
    ...
    if (block)
        ...
        // allocate new block
        rb =
            new (s_alloc_memory(sizeof(retired_block) + sizeof(retired_ptr) * retired_block::c_capacity)) retired_block;
...
struct smr::thread_record : thread_data {
    ...
    thread_record(guard *guards, size_t guard_count) : thread_data(guards, guard_count) {}
};
...
/*static*/ CDS_EXPORT_API void smr::set_memory_allocator(void *(*alloc_func)(size_t size), void (*free_func)(void *p))
{
...
{
    if (instance_) {
        if (bDetachAll) instance_->detach_all_thread();
...
CDS_EXPORT_API smr::~smr()
{
    ...
    for (thread_record *hprec = pHead; hprec; hprec = pNext) {
        ...
        // delete retired data
        for (retired_block *block = retired.list_head_; block && block != retired.current_block_;
             block = block->next_) {
...
{
    if (!tls_) tls_ = instance().alloc_thread_data();
}
...
CDS_EXPORT_API smr::thread_record *smr::create_thread_data()
{
    ...
    */
    ...
    return new (mem) thread_record(reinterpret_cast<guard *>(mem + sizeof(thread_record)), initial_hazard_count_);
}
...
CDS_EXPORT_API smr::thread_record *smr::alloc_thread_data()
{
    ...
    for (hprec = thread_list_.load(atomics::memory_order_acquire); hprec; hprec = hprec->next_) {
        ...
        if (!hprec->thread_id_.compare_exchange_strong(thId, curThreadId, atomics::memory_order_relaxed,
    ...
    if (!hprec) {
        ...
        } while (!thread_list_.compare_exchange_weak(pOldHead, hprec, atomics::memory_order_release,
...
CDS_EXPORT_API void smr::free_thread_data(thread_record *pRec, bool callHelpScan)
{
    ...
    if (callHelpScan) help_scan(pRec);
...
CDS_EXPORT_API void smr::scan(thread_data *pThreadRec)
{
    ...
    while (pNode) {
        if (pNode->thread_id_.load(std::memory_order_relaxed) != cds::OS::c_NullThreadId) {
            ...
            for (guard_block *block = pNode->hazards_.extended_list_.load(atomics::memory_order_acquire); block;
    ...
    if (plist.size() > plist_size)
        last_plist_size_.compare_exchange_weak(plist_size, plist.size(), std::memory_order_relaxed,
    ...
    if (free_count < retired_count / 4 && last_block == pRec->retired_.list_tail_
        && last_block_cell == last_block->last())
        pRec->retired_.extend();
}

CDS_EXPORT_API void smr::help_scan(thread_data *pThis)
{
    assert(static_cast<thread_record *>(pThis)->thread_id_.load(atomics::memory_order_relaxed)
           == cds::OS::get_current_thread_id());
    ...
    for (thread_record *hprec = thread_list_.load(atomics::memory_order_acquire); hprec; hprec = hprec->next_) {
        if (hprec == static_cast<thread_record *>(pThis)) continue;
        ...
        // Several threads may work concurrently so we use atomic technique
        {
            ...
            if (curOwner == nullThreadId) {
                if (!hprec->thread_id_.compare_exchange_strong(curOwner, curThreadId, atomics::memory_order_acquire,
                                                               atomics::memory_order_relaxed))
                    continue;
            } else
                continue;
        ...
        for (retired_block *block = src.list_head_; block; block = block->next_) {
            ...
            for (retired_ptr *p = block->first(); p != last; ++p) {
                if (!dest.push(*p)) scan(pThis);
            }

            if (block == src.current_block_) break;
        }
