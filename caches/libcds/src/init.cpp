...
#ifdef CDS_CXX11_THREAD_LOCAL_SUPPORT
thread_local threading::cxx11_internal::ThreadDataPlaceholder CDS_DATA_ALIGNMENT(
    8) threading::cxx11_internal::s_threadData;
...
void CDS_EXPORT_API check_hpstat_enabled(bool enabled)
{
    ...
    if (!enabled) {
        fprintf(stderr, "inconsistency: libcds has been built with CDS_ENABLE_HPSTAT flag set but the application is "
                        "without that flag");
        abort();
    }
#else
    if (enabled) {
        fprintf(stderr, "inconsistency: libcds has been built without CDS_ENABLE_HPSTAT flag set but the application "
                        "is with that flag");
        abort();
    }
#endif
