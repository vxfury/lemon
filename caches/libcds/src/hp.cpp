...
/*static*/ CDS_EXPORT_API void basic_smr::set_memory_allocator(void *(*alloc_func)(size_t size),
...
/*static*/ CDS_EXPORT_API void basic_smr::construct(size_t nHazardPtrCount, size_t nMaxThreadCount,
                                                    size_t nMaxRetiredPtrCount, scan_type nScanType)
{
    if (!instance_) {
        instance_ = new (s_alloc_memory(sizeof(basic_smr)))
            basic_smr(nHazardPtrCount, nMaxThreadCount, nMaxRetiredPtrCount, nScanType);
    }
}
...
CDS_EXPORT_API basic_smr::basic_smr(size_t nHazardPtrCount, size_t nMaxThreadCount, size_t nMaxRetiredPtrCount,
                                    scan_type nScanType)
...
CDS_EXPORT_API basic_smr::thread_record *basic_smr::create_thread_data()
{
    ...
    */
    ...
    return new (mem) thread_record(reinterpret_cast<guard *>(mem + sizeof(thread_record)), get_hazard_ptr_count(),
...
CDS_EXPORT_API basic_smr::thread_record *basic_smr::alloc_thread_data()
{
    ...
    for (hprec = thread_list_.load(atomics::memory_order_acquire); hprec; hprec = hprec->next_) {
        ...
        if (!hprec->owner_rec_.compare_exchange_strong(null_rec, hprec, atomics::memory_order_relaxed,
...
CDS_EXPORT_API void basic_smr::inplace_scan(thread_data *pThreadRec)
{
    ...
    if (first_retired == last_retired) return;
            ...
            if (pNode->owner_rec_.load(atomics::memory_order_relaxed) != nullptr) {
                ...
                for (auto hp = hpstg.begin(), end = hpstg.end(); hp != end; ++hp) {
                    ...
                    if (hptr) {
                        ...
                        retired_ptr *it =
                            std::lower_bound(first_retired, last_retired, dummy_retired, retired_ptr::less);
    ...
    // Move all marked pointers to head of array
    {
        ...
        for (retired_ptr *it = first_retired; it != last_retired; ++it) {
            if (it->m_n & 1) {
                it->m_n &= ~uintptr_t(1);
                if (insert_pos != it) *insert_pos = *it;
                ++insert_pos;
            } else {
...
{
    assert(static_cast<thread_record *>(pThis)->owner_rec_.load(atomics::memory_order_relaxed)
           == static_cast<thread_record *>(pThis));
    ...
    for (thread_record *hprec = thread_list_.load(atomics::memory_order_acquire); hprec; hprec = hprec->next_) {
        ...
        if (hprec->free_.load(atomics::memory_order_acquire)) continue;
        ...
        // Several threads may work concurrently so we use atomic technique only.
        {
            ...
            if (curOwner == nullptr) {
                if (!hprec->owner_rec_.compare_exchange_strong(curOwner, hprec, atomics::memory_order_acquire,
                                                               atomics::memory_order_relaxed))
                    continue;
            } else
                continue;
        ...
        for (; src_first != src_last; ++src_first) {
            if (!dest.push(std::move(*src_first))) scan(pThis);
        }
