...
static void prepare_current_processor_call()
{
    ...
    if (s_fnGetCurrentProcessorNumber == nullptr)
        s_fnGetCurrentProcessorNumber =
            (fnGetCurrentProcessorNumber)get_proc_addr("ntdll", "NtGetCurrentProcessorNumber");
}
...
{
    if (s_fnGetCurrentProcessorNumber != nullptr) return s_fnGetCurrentProcessorNumber();
    return 0;
}
} // namespace Win32
} // namespace OS
