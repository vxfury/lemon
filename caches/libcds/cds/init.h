...
*/
static inline void Initialize(unsigned int nFeatureFlags = 0 ///< for future use, must be zero.
)
{
    ...
    if (cds::details::init_first_call()) {
        ...
        if (cds::threading::ThreadData::s_nProcCount == 0) cds::threading::ThreadData::s_nProcCount = 1;
