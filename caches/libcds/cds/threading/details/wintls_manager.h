...
class Manager {
  private:
    ...
    //@cond
    enum EThreadAction { do_getData, do_attachThread, do_detachThread, do_checkData };
    //@endcond
    ...
    struct Holder {
        ...
        {
            if (m_key != TLS_OUT_OF_INDEXES) {
                if (::TlsFree(m_key) == 0) CDS_THROW_EXCEPTION(api_exception(::GetLastError(), "TlsFree"));
        ...
        static void alloc()
        {
            ...
            if (!::TlsSetValue(m_key, pData)) CDS_THROW_EXCEPTION(api_exception(::GetLastError(), "TlsSetValue"));
        }
        static void free()
        {
            ThreadData *p = get();
    ...
    static ThreadData *_threadData(EThreadAction nAction)
    {
        switch (nAction) {
            ...
            case do_attachThread:
                if (Holder::get() == nullptr) Holder::alloc();
                return Holder::get();
  ...
  public:
    ...
    */
    static void detachThread()
    {
        ...
        if (pData) {
            if (pData->fini()) _threadData(do_detachThread);
        } else
