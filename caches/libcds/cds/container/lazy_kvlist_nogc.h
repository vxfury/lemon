...
#endif
{
  ...
  protected:
    ...
    class iterator_type : protected base_class::template iterator_type<IsConst> {
        ...
        iterator_type(head_type const &refNode) : iterator_base(const_cast<head_type *>(&refNode)) {}

        explicit iterator_type(const iterator_base &it) : iterator_base(it) {}

        friend class LazyKVList;

      protected:
        explicit iterator_type(node_type &pNode) : iterator_base(&pNode) {}
      ...
      public:
        ...
        iterator_type() : iterator_base() {}

        iterator_type(const iterator_type &src) : iterator_base(src) {}
  ...
  protected:
    ...
    node_type *insert_node_at(head_type &refHead, node_type *pNode)
    {
        ...
        if (base_class::insert_at(&refHead, *p)) return p.release();
    ...
    std::pair<node_type *, bool> update_at(head_type &refHead, const K &key, bool bAllowInsert)
    {
        ...
        std::pair<bool, bool> ret = base_class::update_at(
            &refHead, *pNode,
            [&pItemFound](bool, node_type &item, node_type &) {
                pItemFound = &item;
            },
            bAllowInsert);

        if (ret.second) pNode.release();
