...
struct make_cuckoo_set {
    ...
    static unsigned int const store_hash_count =
        store_hash ? ((unsigned int)std::tuple_size<typename original_traits::hash::hash_tuple_type>::value) : 0;

    struct node_type : public intrusive::cuckoo::node<probeset_type, store_hash_count> {
        value_type m_val;

        template <typename Q>
        node_type(Q const &v) : m_val(v)
        {
        }

        template <typename... Args>
        node_type(Args &&...args) : m_val(std::forward<Args>(args)...)
        {
        }
    };
    ...
    struct intrusive_traits : public original_traits {
        typedef intrusive::cuckoo::base_hook<cds::intrusive::cuckoo::probeset_type<probeset_type>,
                                             cds::intrusive::cuckoo::store_hash<store_hash_count> >
            hook;

        typedef cds::intrusive::cuckoo::traits::disposer disposer;

        typedef typename std::conditional<std::is_same<typename original_traits::equal_to, opt::none>::value, opt::none,
                                          predicate_wrapper<typename original_traits::equal_to, bool> >::type equal_to;

        typedef typename std::conditional<std::is_same<typename original_traits::compare, opt::none>::value, opt::none,
                                          predicate_wrapper<typename original_traits::compare, int> >::type compare;

        typedef typename std::conditional<std::is_same<typename original_traits::less, opt::none>::value, opt::none,
                                          predicate_wrapper<typename original_traits::less, bool> >::type less;
...
/** @ingroup cds_nonintrusive_set
   ...
   items. For a hash set f size N = 2k we use a two-entry array of tables, and two independent hash functions, <tt> h0,
   ...
   <tt>table[1][h1(x)]</tt>, ad removes it if found.
    ...
    - \p Traits - type traits. See cuckoo::traits for explanation.
        It is possible to declare option-based set with cuckoo::make_traits metafunction result as \p Traits template
   argument.
...
#endif
{
  ...
  public:
    ...
    typedef
        typename base_class::mutex_policy mutex_policy; ///< Concurrent access policy, see cuckoo::traits::mutex_policy
    ...
    static size_t const c_nArity =
        base_class::c_nArity; ///< the arity of cuckoo hashing: the number of hash functors provided; minimum 2.

    typedef typename base_class::key_equal_to key_equal_to; ///< Key equality functor; used only for unordered probe-set

    typedef typename base_class::key_comparator
        key_comparator; ///< key comparing functor based on \p Traits::compare and \p Traits::less option setter. Used
                        ///< only for ordered probe set

    typedef typename base_class::allocator allocator; ///< allocator type used for internal bucket table allocations

    /// Node allocator type
    typedef typename std::conditional<std::is_same<typename traits::node_allocator, opt::none>::value, allocator,
                                      typename traits::node_allocator>::type node_allocator;
  ...
  public:
    ...
    static unsigned int const c_nRelocateLimit =
        base_class::c_nRelocateLimit; ///< Count of attempts to relocate before giving up
  ...
  public:
    ...
    */
    CuckooSet(size_t nInitialSize ///< Initial set size; if 0 - use default initial size \ref c_nDefaultInitialSize
              ,
              ...
              unsigned int nProbesetThreshold = 0 ///< probe set threshold, <tt>nProbesetThreshold < nProbesetSize</tt>.
                                                  ///< If 0, nProbesetThreshold = nProbesetSize - 1
              )
    ...
    */
    CuckooSet(size_t nInitialSize ///< Initial set size; if 0 - use default initial size \ref c_nDefaultInitialSize
              ...
              unsigned int nProbesetThreshold ///< probe set threshold, <tt>nProbesetThreshold < nProbesetSize</tt>. If
                                              ///< 0, nProbesetThreshold = nProbesetSize - 1
              ,
  ...
  public:
    ...
    */
    ...
    bool insert(Q const &val, Func f)
    {
        ...
        if (base_class::insert(*pNode, [&f](node_type &node) {
                f(node.m_val);
            })) {
            pNode.release();
            return true;
    ...
    */
    ...
    {
        scoped_node_ptr pNode(alloc_node(val));
        std::pair<bool, bool> res = base_class::update(
            *pNode,
            [&val, &func](bool bNew, node_type &item, node_type const &) {
                func(bNew, item.m_val, val);
            },
            bAllowInsert);
        if (res.first && res.second) pNode.release();
        return res;
    }
    //@cond
    template <typename Q, typename Func>
    ...
    */
    ...
    {
        return base_class::find(val, [&f](node_type &item, Q &v) {
            f(item.m_val, v);
        });
    }
    //@cond
    ...
    {
        return base_class::find(val, [&f](node_type &item, Q const &v) {
            f(item.m_val, v);
        });
    }
    //@endcond
    ...
    */
    ...
    {
        CDS_UNUSED(pred);
        return base_class::find_with(key, typename maker::template predicate_wrapper<Predicate, bool>(),
                                     [](node_type &, Q const &) {
                                     });
    }
    //@cond
