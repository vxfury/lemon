...
/** @ingroup cds_nonintrusive_map
   ...
   dereferenced.
    ...
    is original data structure based on an ordered list. Suppose, you want construct split-list map based on \p
   cds::urcu::general_buffered<> GC and \p MichaelList as ordered list implementation. Your map should map \p int key to
   \p std::string value. So, you beginning your program with following include: \code #include
   <cds/urcu/general_buffered.h> #include <cds/container/michael_list_rcu.h> #include
   <cds/container/split_list_map_rcu.h>
    ...
    - second, include the header of ordered-list implementation (for this example,
   <tt>cds/container/michael_list_rcu.h</tt>),
    - then, the header for RCU-based split-list map <tt>cds/container/split_list_map_rcu.h</tt>.

    Now, you should declare traits for split-list map. The main parts of traits are a hash functor for the map key and a
   comparing functor for ordered list. We use \p std::hash<int> and \p std::less<int>.

    The second attention: instead of using \p %MichaelList in \p %SplitListMap traits we use a tag \p
   ds::contaner::michael_list_tag for the Michael's list. The split-list requires significant support from underlying
   ...
   list interrelations. The tag paradigm simplifies split-list interface.
    ...
    \code
    typedef cc::SplitListMap< cds::urcu::gc<cds::urcu::general_buffered<> >, int, std::string, foo_set_traits >
   int_string_map; \endcode
    ...
    >  int_string_map;
    \endcode
    In case of option-based declaration using \p split_list::make_traits metafunction the struct \p foo_set_traits is
   not required.
   ...
   inherited from cds::container::split_list::traits. There are many other useful options for deep tuning the split-list
   and ordered-list containers.
*/
template <class RCU, typename Key, typename Value,
...
class SplitListMap<cds::urcu::gc<RCU>, Key, Value, Traits>
    : protected container::SplitListSet<cds::urcu::gc<RCU>, std::pair<Key const, Value>,
                                        split_list::details::wrap_map_traits<Key, Value, Traits> > {
    //@cond
    typedef container::SplitListSet<cds::urcu::gc<RCU>, std::pair<Key const, Value>,
                                    split_list::details::wrap_map_traits<Key, Value, Traits> >
        base_class;
    //@endcond
  ...
  public:
    ...
    */
    ...
    /**
        ...
        @warning For \ref cds_nonintrusive_MichaelKVList_gc "MichaelKVList" as the ordered list see \ref
