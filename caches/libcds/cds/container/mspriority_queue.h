...
/**
    \p Options are:
    - \p opt::buffer - the buffer type for heap array. Possible type are: \p opt::v::initiaized_static_buffer, \p
   opt::v::initialized_dynamic_buffer. Default is \p %opt::v::initialized_dynamic_buffer. You may specify any type of
   values for the buffer since at instantiation time the \p buffer::rebind member metafunction is called to change the
...
class MSPriorityQueue : protected cds::intrusive::MSPriorityQueue<T, Traits> {
  ...
  public:
    ...
    typedef typename std::allocator_traits<typename traits::allocator>::template rebind_alloc<value_type>
        allocator_type;                               ///< Value allocator
  ...
  public:
    ...
    */
    ...
    {
        return pop_with([&dest](value_type &src) {
            move_policy()(dest, std::move(src));
        });
    }
    ...
    */
    void clear()
    {
        base_class::clear_with([](value_type &src) {
            value_deleter()(&src);
        });
    }
    ...
    */
    ...
    {
        base_class::clear_with([&f](value_type &val) {
            f(val);
            value_deleter()(&val);
        });
    }
