...
/** @ingroup cds_nonintrusive_list
    ...
    It is possible to declare option-based list with cds::container::michael_list::make_traits metafunction istead of \p
   Traits template argument. For example, the following traits-based declaration of gc::HP Michael's list \code #include
...
#endif
{
  ...
  public:
    ...
    static constexpr const size_t c_nHazardPtrCount =
        base_class::c_nHazardPtrCount; ///< Count of hazard pointer required for the algorithm
  ...
  protected:
    ...
    class iterator_type : protected base_class::template iterator_type<IsConst> {
        ...
        iterator_type(head_type const &pNode) : iterator_base(pNode) {}
      ...
      public:
        ...
        iterator_type(iterator_type const &src) : iterator_base(src) {}
  ...
  public:
    ...
    */
    ...
    explicit MichaelList(Stat &st) : base_class(st)
    {
    }
    //@endcond
    ...
    */
    ...
    /**
        ...
        \p Less functor has the semantics like \p std::less but it should accept arguments of type \p value_type and \p
       Q in any order. \p pred must imply the same element order as the comparator used for building the list.
    */
    template <typename Q, typename Less>
  ...
  protected:
    ...
    bool insert_at(head_type &refHead, Q &&key, Func f)
    {
        ...
        if (base_class::insert_at(refHead, *pNode, [&f](node_type &node) {
                f(node_to_value(node));
            })) {
            pNode.release();
            return true;
        }
    ...
    {
        return base_class::erase_at(refHead, key, cmp, [&f](node_type const &node) {
            f(node_to_value(node));
        });
    }
    ...
    std::pair<bool, bool> update_at(head_type &refHead, Q const &key, Func f, bool bAllowInsert)
    {
        ...
        std::pair<bool, bool> ret = base_class::update_at(
            refHead, *pNode,
            [&f, &key](bool bNew, node_type &node, node_type &) {
                f(bNew, node_to_value(node), key);
            },
            bAllowInsert);
        if (ret.first && ret.second) pNode.release();
    ...
    {
        return base_class::find_at(refHead, val, cmp, [&f](node_type &node, Q &v) {
            f(node_to_value(node), v);
        });
    }
