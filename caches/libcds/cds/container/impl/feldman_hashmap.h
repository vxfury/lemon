...
/** @ingroup cds_nonintrusive_map
   ...
   \p dataNode by a bitmark on the second-least significant bit.
    ...
    \p %FeldmanHashMap multi-level array is similar to a tree in that we keep a pointer to the root, which is a memory
   ...
   reach the \p arrayNode on which we need to operate; this is initially one, because of \p head.
    ...
    That approach to the structure of the hash map uses an extensible hashing scheme; <b> the hash value is treated as a
   bit string</b> and rehash incrementally.
    ...
    - all keys is converted to fixed-size bit-string by hash functor provided.
      ...
      For the strings you may use well-known hashing algorithms like <a
   ...
   fixed-length bit-strings, and such hash values will be the keys in \p %FeldmanHashMap. If your key is fixed-sized the
   hash functor is optional, see \p feldman_hashmap::traits::hash for explanation and examples.
    - \p %FeldmanHashMap uses a perfect hashing. It means that if two different keys, for example, of type \p
   std::string, have identical hash then you cannot insert both that keys in the map. \p %FeldmanHashMap does not
   maintain the key, it maintains its fixed-size hash value.
    ...
    - \p Traits - type traits, the structure based on \p feldman_hashmap::traits or result of \p
   feldman_hashmap::make_traits metafunction.
    ...
    - <tt><cds/container/feldman_hashmap_rcu.h></tt> for \ref cds_container_FeldmanHashMap_rcu "RCU type". RCU
   specialization has a slightly different interface.
*/
template <class GC, typename Key, typename T
...
#endif
{
  ...
  public:
    ...
    typedef typename base_class::hash_comparator
        hash_comparator; ///< hash compare functor based on \p Traits::compare and \p Traits::less
...
#else
    typedef typename gc::template guarded_ptr<node_type, value_type,
                                              cds::container::details::guarded_ptr_cast_set<node_type, value_type> >
        guarded_ptr;
#endif

#ifdef CDS_DOXYGEN_INVOKED
    typedef implementation_defined
        iterator; ///< @ref cds_container_FeldmanHashMap_iterators "bidirectional iterator" type
    typedef implementation_defined
        const_iterator; ///< @ref cds_container_FeldmanHashMap_iterators "bidirectional const iterator" type
    typedef implementation_defined
        reverse_iterator; ///< @ref cds_container_FeldmanHashMap_iterators "bidirectional reverse iterator" type
    typedef implementation_defined const_reverse_iterator; ///< @ref cds_container_FeldmanHashMap_iterators
  ...
  public:
    ...
    */
    FeldmanHashMap(size_t head_bits = 8, size_t array_bits = 4) : base_class(head_bits, array_bits) {}
    ...
    */
    ...
    {
        scoped_node_ptr sp(cxx_node_allocator().MoveNew(m_Hasher, std::forward<K>(key)));
        if (base_class::insert(*sp, [&func](node_type &item) {
                func(item.m_Value);
            })) {
            sp.release();
            return true;
    ...
    */
    ...
    {
        scoped_node_ptr sp(cxx_node_allocator().MoveNew(m_Hasher, std::forward<K>(key)));
        std::pair<bool, bool> result = base_class::do_update(
            *sp,
            [&func](node_type &node, node_type *old) {
                func(node.m_Value, old ? &old->m_Value : nullptr);
            },
            bInsert);
        if (result.first) sp.release();
        return result;
    }
    ...
    */
    ...
    {
        return base_class::erase(m_Hasher(key_type(key)), [&f](node_type &node) {
            f(node.m_Value);
        });
    }
    ...
    */
    ...
    {
        return base_class::find(m_Hasher(key_type(key)), [&f](node_type &node) {
            f(node.m_Value);
        });
    }
  ...
  public:
    ...
    */
    ...
    /// Returns an iterator to the element following the last element of the map. This element acts as a placeholder;
    ...
    /// Returns a const iterator to the element following the last element of the map. This element acts as a
