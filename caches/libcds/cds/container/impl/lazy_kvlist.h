...
/** @ingroup cds_nonintrusive_list
    ...
    - \p Traits - type traits, default is \p lazy_list::traits
        It is possible to declare option-based list with cds::container::lazy_list::make_traits metafunction istead of
...
*/
template <typename GC, typename Key, typename Value,
...
#endif
{
  ...
  public:
    ...
    struct rebind_traits {
        typedef LazyKVList<gc, key_type, mapped_type, typename cds::opt::make_options<traits, Options...>::type> type;
    };
  ...
  public:
    /// Guarded pointer
    typedef
        typename gc::template guarded_ptr<node_type, value_type, details::guarded_ptr_cast_map<node_type, value_type>>
            guarded_ptr;
  ...
  public:
    ...
    /**
       ...
       concurrent deleting operations it is no guarantee that you iterate all item in the list.
        ...
        - <tt> operator *</tt> - returns a reference (a const reference for \p const_iterator) to \ref value_type for
       iterator
  ...
  public:
    ...
    explicit LazyKVList(Stat &st) : base_class(st)
    {
    }
    //@endcond
  ...
  protected:
    ...
    bool erase_at(head_type &refHead, K const &key, Compare cmp, Func f)
    {
        return base_class::erase_at(&refHead, key, cmp, [&f](node_type const &node) {
            f(const_cast<value_type &>(node.m_Data));
        });
    }
    ...
    std::pair<bool, bool> update_at(head_type &refHead, K &&key, Func f, bool bAllowInsert)
    {
        ...
        std::pair<bool, bool> ret = base_class::update_at(
            &refHead, *pNode,
            [&f](bool bNew, node_type &node, node_type &) {
                f(bNew, node.m_Data);
            },
            bAllowInsert);
    ...
    {
        return base_class::find_at(&refHead, key, cmp, [&f](node_type &node, K &) {
            f(node.m_Data);
        });
    }
