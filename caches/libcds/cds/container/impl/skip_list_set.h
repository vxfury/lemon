...
/** @ingroup cds_nonintrusive_set
    ...
    - <tt><cds/container/skip_list_set_nogc.h></tt> for \ref cds_nonintrusive_SkipListSet_nogc "non-deletable
   SkipListSet"
...
#endif
{
  ...
  public:
    ...
    typedef
        typename traits::allocator allocator_type; ///< Allocator type used for allocate/deallocate the skip-list nodes
    ...
    static size_t const c_nHazardPtrCount =
        base_class::c_nHazardPtrCount; ///< Count of hazard pointer required for the skip-list
  ...
  public:
    ...
    */
    ...
    {
        scoped_node_ptr sp(node_allocator().New(random_level(), val));
        if (base_class::insert(*sp.get(), [&f](node_type &v) {
                f(v.m_Value);
            })) {
            sp.release();
            return true;
    ...
    */
    ...
    {
        scoped_node_ptr sp(node_allocator().New(random_level(), val));
        std::pair<bool, bool> bRes = base_class::update(
            *sp,
            [&func, &val](bool bNew, node_type &node, node_type &) {
                func(bNew, node.m_Value, val);
            },
            bInsert);
        if (bRes.first && bRes.second) sp.release();
        return bRes;
    }
    //@cond
    template <typename Q, typename Func>
    ...
    */
    ...
    bool erase_with(Q const &key, Less pred)
    {
        ...
        return base_class::erase_with(
            key, cds::details::predicate_wrapper<node_type, Less, typename maker::value_accessor>());
    }
    ...
    */
    ...
    {
        return base_class::erase(key, [&f](node_type const &node) {
            f(node.m_Value);
        });
    }
    ...
    */
    ...
    /**
        The function searches an item with minimal key, unlinks it, and returns pointer to the item found as \p
       guarded_ptr. If the skip-list is empty the function returns an empty guarded pointer.
    ...
    */
    ...
    /**
        The function searches an item with maximal key, unlinks it, and returns the pointer to item found as \p
       guarded_ptr. If the skip-list is empty the function returns an empty guarded pointer.
    ...
    */
    ...
    {
        return base_class::find(key, [&f](node_type &node, Q &v) {
            f(node.m_Value, v);
        });
    }
    //@cond
