...
/** @ingroup cds_nonintrusive_map
    ...
    @note Before including <tt><cds/container/bronson_avltree_map_rcu.h></tt> you should include appropriate RCU header
   file, see \ref cds_urcu_gc "RCU type" for list of existing RCU class and corresponding header files.
*/
template <typename RCU, typename Key, typename T,
...
class BronsonAVLTreeMap<cds::urcu::gc<RCU>, Key, T *, Traits> {
  public:
    ...
    typedef implementation_defined
        key_comparator; ///< key compare functor based on \p Traits::compare and \p Traits::less
#else
    ...
    typedef typename traits::sync_monitor
        sync_monitor; ///< @ref cds_sync_monitor "Synchronization monitor" type for node-level locking
...
#else
    typedef cds::urcu::exempt_ptr<gc, typename std::remove_pointer<mapped_type>::type,
                                  typename std::remove_pointer<mapped_type>::type, disposer, void>
        exempt_ptr;
#endif
  ...
  protected:
    ...
    enum class find_result { not_found, found, retry };
    ...
    enum node_condition { nothing_required = -3, rebalance_required = -2, unlink_required = -1 };

    enum direction { left_child = -1, right_child = 1 };
  ...
  protected:
    //@cond
    template <typename K>
    static node_type *alloc_node(K &&key, int nHeight, version_type version, node_type *pParent, node_type *pLeft,
                                 node_type *pRight)
    {
    ...
    class rcu_disposer {
      ...
      public:
        rcu_disposer() : m_pRetiredList(nullptr), m_pRetiredValue(nullptr) {}
      ...
      private:
        ...
        void clean()
        {
            ...
            // Dispose value
            if (m_pRetiredValue) gc::template retire_ptr<disposer>(m_pRetiredValue);
        }
    };
  ...
  public:
    /// Creates empty map
    BronsonAVLTreeMap() : m_pRoot(static_cast<node_type *>(&m_Root)) {}
    ...
    */
    ...
    /**
        ...
        then the new node created from \p key will be inserted into the map; note that in this case the \ref key_type
       should be constructible from type \p K. Otherwise, the value for \p key will be changed to \p pVal.
    ...
    */
    ...
    {
        return do_remove(key, key_comparator(), [](key_type const &, mapped_type pVal, rcu_disposer &disp) -> bool {
    ...
    */
    ...
    {
        CDS_UNUSED(pred);
        return do_remove(key, cds::opt::details::make_comparator_from_less<Less>(),
                         [](key_type const &, mapped_type pVal, rcu_disposer &disp) -> bool {
                             disp.dispose_value(pVal);
                             return true;
                         });
    }
    ...
    */
    ...
    {
        return do_remove(key, key_comparator(), [&f](key_type const &k, mapped_type pVal, rcu_disposer &disp) -> bool {
            assert(pVal);
            f(k, *pVal);
            disp.dispose_value(pVal);
            return true;
    ...
    */
    ...
    {
        return exempt_ptr(do_extract_min([&f](key_type const &key) {
            f(key);
        }));
    }
    ...
    */
    typename std::enable_if<std::is_copy_assignable<key_type>::value, exempt_ptr>::type extract_min_key(
        key_type &min_key)
    {
        return exempt_ptr(do_extract_min([&min_key](key_type const &key) {
            min_key = key;
        }));
    }
    ...
    */
    ...
    {
        return exempt_ptr(do_extract_max([&f](key_type const &key) {
            f(key);
        }));
    }
    ...
    */
    typename std::enable_if<std::is_copy_assignable<key_type>::value, exempt_ptr>::type extract_max_key(
        key_type &max_key)
    {
        return exempt_ptr(do_extract_max([&max_key](key_type const &key) {
            max_key = key;
        }));
    }
    ...
    */
    ...
    {
        CDS_UNUSED(pred);
        return do_find(key, cds::opt::details::make_comparator_from_less<Less>(), [&f](node_type *pNode) -> bool {
            assert(pNode != nullptr);
            mapped_type pVal = pNode->m_pValue.load(memory_model::memory_order_relaxed);
            if (pVal) {
                f(pNode->m_key, *pVal);
    ...
    */
    ...
    {
        return do_find(key, key_comparator(), [](node_type *) -> bool {
            return true;
        });
    }
    ...
    */
    ...
    {
        CDS_UNUSED(pred);
        return do_find(key, cds::opt::details::make_comparator_from_less<Less>(), [](node_type *) -> bool {
            return true;
        });
    }
  ...
  protected:
    ...
    size_t do_check_consistency(node_type *pNode, size_t nLevel, Func f, size_t &nErrors) const
    {
        if (pNode) {
            ...
            if (pLeft && cmp(pLeft->m_key, pNode->m_key) > 0) ++nErrors;
            if (pRight && cmp(pNode->m_key, pRight->m_key) > 0) ++nErrors;
    ...
    mapped_type do_extract_min(Func f)
    {
        ...
        do_extract_minmax(left_child, [&pExtracted, &f](key_type const &key, mapped_type pVal, rcu_disposer &) -> bool {
            f(key);
            pExtracted = pVal;
            return false;
        });
    ...
    {
        mapped_type pExtracted = nullptr;
        do_extract_minmax(right_child,
                          [&pExtracted, &f](key_type const &key, mapped_type pVal, rcu_disposer &) -> bool {
                              f(key);
                              pExtracted = pVal;
                              return false;
                          });
    ...
    {
        mapped_type pExtracted = nullptr;
        do_remove(key, key_comparator(), [&pExtracted](key_type const &, mapped_type pVal, rcu_disposer &) -> bool {
            pExtracted = pVal;
            return false;
        });
    ...
    mapped_type do_extract_with(Q const &key, Less pred)
    {
        CDS_UNUSED(pred);
        mapped_type pExtracted = nullptr;
        do_remove(key, cds::opt::details::make_comparator_from_less<Less>(),
                  [&pExtracted](key_type const &, mapped_type pVal, rcu_disposer &) -> bool {
                      pExtracted = pVal;
                      return false;
                  });
        m_stat.onExtract(pExtracted != nullptr);
  ...
  private:
                    ...
                    if (nChildVersion & node_type::shrinking) {
                        ...
                        if (pNode->version(memory_model::memory_order_acquire) != nVersion) {
                            ...
                            break; // retry
                               ...
                               && child(pNode, nDir, memory_model::memory_order_acquire) == pChild) {
                        if (pNode->version(memory_model::memory_order_acquire) != nVersion) {
                            --pos;
                            m_stat.onFindRetry();
                            break; // retry
    ...
    int try_update_root(K const &key, Compare cmp, int nFlags, Func funcUpdate, rcu_disposer &disp)
    {
        ...
        while (true) {
            ...
            if (result != update_flags::retry) return result;
        }
    }
    ...
    template <typename K, typename Compare, typename Func>
    int try_update(K const &key, Compare cmp, int nFlags, Func funcUpdate, node_type *pNode, version_type nVersion,
                   rcu_disposer &disp)
    {
        assert(gc::is_locked());
    ...
    template <typename K, typename Compare, typename Func>
    int try_remove(K const &key, Compare cmp, Func func, node_type *pParent, node_type *pNode, version_type nVersion,
                   rcu_disposer &disp)
    {
        assert(gc::is_locked());
        assert(nVersion != node_type::unlinked);
        ...
        while (pos >= 0) {
            ...
            while (true) {
                ...
                if (pChild == nullptr) return update_flags::failed;
    ...
    template <typename Func>
    int try_extract_minmax(int nDir, Func func, node_type *pParent, node_type *pNode, version_type nVersion,
                           rcu_disposer &disp)
    {
        assert(gc::is_locked());
        assert(nVersion != node_type::unlinked);
            ...
            while (true) {
                ...
                if (!pChild) {
                    ...
                    if (pNode->is_valued(memory_model::memory_order_acquire)) {
                        ...
                        if (result == update_flags::result_removed) return result;
                        ...
                        rcu_disposer &disp)
    {
        ...
        node_type *pDamaged;
        {
            ...
            constexpr_if(!c_bRelaxedInsert) fnCreateNode(pNew = alloc_node(key, 1, 0, pNode, nullptr, nullptr));
    ...
    int try_update_node(int nFlags, Func funcUpdate, node_type *pNode, version_type nVersion, rcu_disposer &disp)
    {
        ...
        assert(pNode != nullptr);
        {
            ...
            if (pNode->version(memory_model::memory_order_acquire) != nVersion) return update_flags::retry;
    ...
    int try_remove_node(node_type *pParent, node_type *pNode, version_type nVersion, Func func, rcu_disposer &disp)
    {
        ...
        if (!pNode->is_valued(memory_model::memory_order_acquire)) return update_flags::failed;
        ...
        if (child(pNode, left_child, memory_model::memory_order_acquire) == nullptr
            ...
            mapped_type pOld;
            {
                ...
                if (pParent->is_unlinked(memory_model::memory_order_acquire)
                    || parent(pNode, memory_model::memory_order_acquire) != pParent)
                    return update_flags::retry;

                {
                    ...
                    pOld = pNode->value(memory_model::memory_order_relaxed);
                    if (!pOld) return update_flags::failed;

                    if (!try_unlink_locked(pParent, pNode, disp)) return update_flags::retry;
                }
            ...
            mapped_type pOld;
            {
                node_scoped_lock ln(m_Monitor, *pNode);
                pOld = pNode->value(memory_model::memory_order_relaxed);
                if (pNode->version(memory_model::memory_order_relaxed) != nVersion) return update_flags::retry;
                if (!pOld) return update_flags::failed;
    ...
    bool try_unlink_locked(node_type *pParent, node_type *pNode, rcu_disposer &disp)
    {
        ...
        if (pSplice) pSplice->parent(pParent, memory_model::memory_order_release);
  ...
  private: // rotations
    ...
    int estimate_node_condition(node_type *pNode)
    {
        ...
        if (nBalance < -1 || nBalance > 1) return rebalance_required;
    ...
    void fix_height_and_rebalance(node_type *pNode, rcu_disposer &disp)
    {
        while (pNode && parent(pNode, memory_model::memory_order_acquire)) {
            ...
            if (nCond == nothing_required || pNode->is_unlinked(memory_model::memory_order_acquire)) return;
    ...
    node_type *rebalance_to_right_locked(node_type *pParent, node_type *pNode, node_type *pLeft, int hR)
    {
        ...
        if (pNode->m_pLeft.load(memory_model::memory_order_relaxed) != pLeft) return pNode; // retry for pNode

        assert(check_node_ordering(pNode, pLeft) > 0);

        int hL = height(pLeft, memory_model::memory_order_acquire);
        if (hL - hR <= 1) return pNode; // retry
        ...
        if (pLRight) {
            {
                ...
                if (pLeft->m_pRight.load(memory_model::memory_order_acquire) != pLRight) return pNode; // retry

                assert(check_node_ordering(pLeft, pLRight) < 0);

                hLR = height(pLRight, memory_model::memory_order_acquire);
                if (hLL > hLR) return rotate_right_locked(pParent, pNode, pLeft, hR, hLL, pLRight, hLR);

                int hLRL = height_null(child(pLRight, left_child, memory_model::memory_order_relaxed),
                                       memory_model::memory_order_acquire);
                int balance = hLL - hLRL;
                if (balance >= -1 && balance <= 1
                    && !((hLL == 0 || hLRL == 0) && !pLeft->is_valued(memory_model::memory_order_relaxed))) {
    ...
    node_type *rebalance_to_left_locked(node_type *pParent, node_type *pNode, node_type *pRight, int hL)
    {
        ...
        if (pNode->m_pRight.load(memory_model::memory_order_relaxed) != pRight) return pNode; // retry for pNode
        ...
        if (pRLeft) {
            {
                ...
                if (pRight->m_pLeft.load(memory_model::memory_order_acquire) != pRLeft) return pNode; // retry

                assert(check_node_ordering(pRight, pRLeft) > 0);

                hRL = height(pRLeft, memory_model::memory_order_acquire);
                if (hRR >= hRL) return rotate_left_locked(pParent, pNode, hL, pRight, pRLeft, hRL, hRR);
                ...
                if (balance >= -1 && balance <= 1
                    && !((hRR == 0 || hRLR == 0) && !pRight->is_valued(memory_model::memory_order_relaxed)))
    ...
    node_type *rotate_right_locked(node_type *pParent, node_type *pNode, node_type *pLeft, int hR, int hLL,
                                   node_type *pLRight, int hLR)
    {
    ...
    node_type *rotate_left_locked(node_type *pParent, node_type *pNode, int hL, node_type *pRight, node_type *pRLeft,
                                  int hRL, int hRR)
    {
        version_type nodeVersion = pNode->version(memory_model::memory_order_relaxed);
        node_type *pParentLeft = child(pParent, left_child, memory_model::memory_order_relaxed);
    ...
    node_type *rotate_right_over_left_locked(node_type *pParent, node_type *pNode, node_type *pLeft, int hR, int hLL,
                                             node_type *pLRight, int hLRL)
    {
        version_type nodeVersion = pNode->version(memory_model::memory_order_relaxed);
    ...
    node_type *rotate_left_over_right_locked(node_type *pParent, node_type *pNode, int hL, node_type *pRight,
                                             node_type *pRLeft, int hRR, int hRLR)
    {
        version_type nodeVersion = pNode->version(memory_model::memory_order_relaxed);
