...
/** @ingroup cds_nonintrusive_set
    ...
    You should decide what garbage collector you want, and what ordered list you want to use as a base. Split-ordered
   list is original data structure based on an ordered list.
    ...
    \endcode
    The inclusion order is important: first, include header for ordered-list implementation (for this example,
   <tt>cds/container/lazy_list_dhp.h</tt>), then the header for split-list set <tt>cds/container/split_list_set.h</tt>.

    Now, you should declare traits for split-list set. The main parts of traits are a hash functor for the set and a
   comparing functor for ordered list. Note that we define several function in <tt>foo_hash</tt> and <tt>foo_less</tt>
   functors for different argument types since we want call our \p %SplitListSet object by the key of type <tt>int</tt>
   and by the value of type <tt>foo</tt>.

    The second attention: instead of using \p %LazyList in \p %SplitListSet traits we use a tag \p
   cds::contaner::lazy_list_tag for the lazy list. The split-list requires significant support from underlying ordered
   list class and it is not good idea to dive you into deep implementation details of split-list and ordered list
   interrelations. The tag paradigm simplifies split-list interface.
    ...
    Note that in this example we show only mandatory \p traits parts, optional ones is the default and they are
   inherited from \p cds::container::split_list::traits. There are many other options for deep tuning the split-list and
   ordered-list containers.
*/
template <class GC, class T,
...
#else
    protected details::make_split_list_set<GC, T, typename Traits::ordered_list,
                                           split_list::details::wrap_set_traits<T, Traits> >::type
#endif
{
  protected:
    //@cond
    typedef details::make_split_list_set<GC, T, typename Traits::ordered_list,
                                         split_list::details::wrap_set_traits<T, Traits> >
        maker;
  ...
  public:
    /// Guarded pointer
    typedef
        typename gc::template guarded_ptr<node_type, value_type, details::guarded_ptr_cast_set<node_type, value_type> >
            guarded_ptr;
  ...
  protected:
    ...
    class iterator_type : protected base_class::template iterator_type<IsConst> {
      ...
      public:
        ...
        /// Copy ctor
        iterator_type(iterator_type const &src) : iterator_base_class(src) {}

      protected:
        explicit iterator_type(iterator_base_class const &src) : iterator_base_class(src) {}
  ...
  public:
    ...
    */
    SplitListSet() : base_class() {}

    /// Initializes split-ordered list
    SplitListSet(size_t nItemCount ///< estimated average of item count
                 ,
                 size_t nLoadFactor =
                     1 ///< the load factor - average item count per bucket. Small integer up to 8, default is 1.
                 )
  ...
  public:
    ...
    /**
        ...
        - The iterator object cannot be moved across thread boundary because it contains GC's guard that is
       thread-private GC data.
        - Iterator ensures thread-safety even if you delete the item that iterator points to. However, in case of
       concurrent deleting operations it is no guarantee that you iterate all item in the split-list. Moreover, a crash
       is possible when you try to iterate the next element that has been deleted by concurrent thread.
  ...
  public:
    ...
    */
    ...
    /**
        ...
        @warning For \ref cds_intrusive_MichaelList_hp "MichaelList" as the bucket see \ref cds_intrusive_item_creating
       "insert item troubleshooting". \ref cds_intrusive_LazyList_hp "LazyList" provides exclusive access to inserted
       item and does not require any node-level synchronization.
    */
    ...
    */
    ...
    std::pair<bool, bool>
#else
    typename std::enable_if<std::is_same<Q, Q>::value && is_iterable_list<ordered_list>::value,
    ...
    /**
        ...
        @warning For \ref cds_intrusive_MichaelList_hp "MichaelList" and \ref cds_nonintrusive_IterableList_gc
       "IterableList" as the bucket see \ref cds_intrusive_item_creating "insert item troubleshooting". \ref
       cds_intrusive_LazyList_hp "LazyList" provides exclusive access to inserted item and does not require any
    ...
    */
    ...
    update(Q &&val, Func func, bool bAllowInsert = true)
    {
        ...
        if (bRet.first && bRet.second) pNode.release();
        return bRet;
    }
    //@cond
    template <typename Q, typename Func>
    ...
    */
    ...
    {
        CDS_UNUSED(pred);
        return base_class::erase_with(key, typename maker::template predicate_wrapper<Less>(), [&f](node_type &node) {
            f(node.m_Value);
        });
    }
    ...
    */
    ...
    typename std::enable_if<std::is_same<Iterator, iterator>::value && is_iterable_list<ordered_list>::value,
                            bool>::type
    ...
    */
    ...
    //@cond
    template <typename Q>
    typename std::enable_if<std::is_same<Q, Q>::value && is_iterable_list<ordered_list>::value, iterator>::type find(
        Q const &key)
    {
        return find_iterator_(key);
    }
