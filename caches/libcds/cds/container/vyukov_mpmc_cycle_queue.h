...
/** @ingroup cds_nonintrusive_queue
    ...
    It's multi-producer multi-consumer (MPMC), array-based, fails on overflow, does not require GC, w/o priorities,
   causal FIFO, blocking producers and consumers queue. The algorithm is pretty simple and fast. It's not lock-free in
   the official meaning, just implemented by means of atomic RMW operations w/o mutexes.

    The cost of enqueue/dequeue is 1 CAS per operation.
    No dynamic memory allocation/management during operation. Producers and consumers are separated from each other (as
   in the two-lock queue), i.e. do not touch the same data while queue is not empty.
...
class VyukovMPMCCycleQueue : public cds::bounded_container {
  ...
  public:
    ...
    */
    VyukovMPMCCycleQueue(size_t nCapacity = 0) : m_buffer(nCapacity), m_nBufferMask(m_buffer.capacity() - 1)
    {
        ...
        for (size_t i = 0; i != nCapacity; ++i) m_buffer[i].sequence.store(i, memory_model::memory_order_relaxed);
    ...
    */
    ...
    {
        return enqueue_with([&val](value_type &dest) {
            new (&dest) value_type(val);
        });
    }
    ...
    {
        return enqueue_with([&val](value_type &dest) {
            new (&dest) value_type(std::move(val));
        });
    }
...
#else
        return enqueue_with([&args...](value_type &dest) {
    ...
    bool dequeue_with(Func f)
    {
        ...
        for (;;) {
            ...
            if (dif == 0) {
                ...
                // Queue empty?
                if (pos - m_posEnqueue.load(memory_model::memory_order_relaxed) == 0) return false; // queue empty
                bkoff();
    ...
    typename std::enable_if<SC, value_type *>::type front()
    {
        ...
        for (;;) {
            ...
            if (dif == 0)
                ...
                // Queue empty?
                if (pos - m_posEnqueue.load(memory_model::memory_order_relaxed) == 0) return nullptr; // queue empty
                bkoff();
                pos = m_posDequeue.load(memory_model::memory_order_relaxed);
            } else
                pos = m_posDequeue.load(memory_model::memory_order_relaxed);
    ...
    bool empty() const
    {
        ...
        for (;;) {
            ...
            if (dif == 0)
                ...
                if (pos - m_posEnqueue.load(memory_model::memory_order_relaxed) == 0) return true;
            }
            bkoff();
            pos = m_posDequeue.load(memory_model::memory_order_relaxed);
        }
