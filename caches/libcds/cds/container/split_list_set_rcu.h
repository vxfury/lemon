...
template <typename T, class OrdList, typename OrdListTag>
class make_raw_ptr;
...
/** @ingroup cds_nonintrusive_set
   ...
   dereferenced.
    ...
    is an original data structure based on an ordered list. Suppose, you want construct split-list set based on \p
   cds::urcu::general_buffered<> GC and \p LazyList as ordered list implementation. So, you beginning your program with
   following include: \code #include <cds/urcu/general_buffered.h> #include <cds/container/lazy_list_rcu.h> #include
   ...
   comparing functor for ordered list. Note that we define several function in \p foo_hash and \p foo_less functors for
   different argument types since we want call our \p %SplitListSet object by the key of type \p int and by the value of
   type \p foo.

    The second attention: instead of using \p %LazyList in \p %SplitListSet traits we use \p
   cds::contaner::lazy_list_tag tag for the lazy list. The split-list requires significant support from underlying
...
#else
    protected details::make_split_list_set<cds::urcu::gc<RCU>, T, typename Traits::ordered_list,
                                           split_list::details::wrap_set_traits<T, Traits> >::type
#endif
{
  protected:
    //@cond
    typedef details::make_split_list_set<cds::urcu::gc<RCU>, T, typename Traits::ordered_list,
  ...
  public:
    ...
    */
    ...
    /**
        ...
        @warning For \ref cds_intrusive_MichaelList_rcu "MichaelList" as the bucket see \ref cds_intrusive_item_creating
       "insert item troubleshooting". \ref cds_intrusive_LazyList_rcu "LazyList" provides exclusive access to inserted
    ...
    */
    ...
    std::pair<bool, bool> update(Q const &val, Func func, bool bAllowInsert = true)
    {
        ...
        std::pair<bool, bool> bRet = base_class::update(
            *pNode,
