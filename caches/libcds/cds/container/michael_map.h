...
/** @ingroup cds_nonintrusive_map
    ...
    - \p OrderedList - ordered key-value list implementation used as bucket for hash map, for example, \p MichaelKVList,
        ...
        stored in the hash-map, the reclamation schema \p GC used by hash-map, the comparison functor for the type \p
   Key and other features specific for the ordered list.
    - \p Traits - map traits, default is \p michael_map::traits.
        Instead of defining \p Traits struct you may use option-based syntax with \p michael_map::make_traits
   metafunction.
    ...
    \endcode
*/
template <class GC, class OrderedList,
...
class MichaelHashMap {
  public:
    ...
    static constexpr const size_t c_nHazardPtrCount =
        ordered_list::c_nHazardPtrCount; ///< Count of hazard pointer required

    //@cond
    typedef typename ordered_list::template select_stat_wrapper<typename ordered_list::stat> bucket_stat;

    typedef typename ordered_list::template rebind_traits<cds::opt::item_counter<cds::atomicity::empty_item_counter>,
                                                          cds::opt::stat<typename bucket_stat::wrapped_stat> >::type
        internal_bucket_type;

    typedef typename internal_bucket_type::guarded_ptr guarded_ptr;

    typedef typename std::allocator_traits<allocator>::template rebind_alloc<internal_bucket_type>
  ...
  protected:
    ...
    class iterator_type : protected cds::intrusive::michael_set::details::iterator<internal_bucket_type, IsConst> {
      ...
      public:
        ...
        typedef
            typename cds::details::make_const_type<typename MichaelHashMap::mapped_type, IsConst>::pointer value_ptr;
        /// Value reference type (const for const_iterator)
        typedef
            typename cds::details::make_const_type<typename MichaelHashMap::mapped_type, IsConst>::reference value_ref;
        ...
        typedef
            typename cds::details::make_const_type<typename MichaelHashMap::value_type, IsConst>::reference pair_ref;

      protected:
        iterator_type(list_iterator const &it, bucket_ptr pFirst, bucket_ptr pLast) : base_class(it, pFirst, pLast) {}

      public:
        /// Default ctor
        iterator_type() : base_class() {}

        /// Copy ctor
        iterator_type(const iterator_type &src) : base_class(src) {}
  ...
  public:
    ...
    /**
        ...
        - to protect the value, the iterator contains a GC-specific guard + another guard is required locally for
       increment operator. For some GC (like as \p gc::HP), a guard is a limited resource per thread, so an exception
       (or assertion) "no free guard" may be thrown if the limit of guard count per thread is exceeded.
        ...
        - for \p MichaelKVList and \p LazyKVList: iterator guarantees safety even if you delete the item that iterator
       points to because that item is guarded by hazard pointer. However, in case of concurrent deleting operations it
       is no guarantee that you iterate all item in the map. Moreover, a crash is possible when you try to iterate the
       next element that has been deleted by concurrent thread. Use this iterator on the concurrent container for
       debugging purpose only.
        ...
        @note The iterator object returned by \p end(), \p cend() member functions points to \p nullptr and should not
       be dereferenced.
    */
  ...
  public:
    ...
    /** @anchor cds_nonintrusive_MichaelHashMap_hp_ctor
        ...
        \p nLoadFactor parameter defines average count of items per bucket and it should be small number between 1
       and 10. Remember, since the bucket implementation is an ordered list, searching in the bucket is linear
       [<tt>O(nLoadFactor)</tt>]. Note, that many popular STL hash map implementation uses load factor 1.

        The ctor defines hash table size as rounding <tt>nMacItemCount / nLoadFactor</tt> up to nearest power of two.
    */
    MichaelHashMap(size_t nMaxItemCount, ///< estimation of max item count in the hash map
    ...
    ~MichaelHashMap()
    {
        ...
        for (auto it = m_Buckets, itEnd = m_Buckets + bucket_count(); it != itEnd; ++it) it->~internal_bucket_type();
    ...
    */
    ...
    /**
        ...
        @warning For \ref cds_nonintrusive_MichaelKVList_gc "MichaelKVList" as the bucket see \ref
       cds_intrusive_item_creating "insert item troubleshooting". \ref cds_nonintrusive_LazyKVList_gc "LazyKVList"
       provides exclusive access to inserted item and does not require any node-level synchronization.
    */
    ...
    /**
        ...
        @warning For \ref cds_nonintrusive_MichaelKVList_gc "MichaelKVList" and \ref cds_nonintrusive_IterableKVList_gc
       "IterableKVList" as the bucket see \ref cds_intrusive_item_creating "insert item troubleshooting". \ref
       cds_nonintrusive_LazyKVList_gc "LazyKVList" provides exclusive access to inserted item and does not require any
    ...
    */
    ...
    std::pair<bool, bool> update(K &&key, Func func, bool bAllowInsert = true)
    {
        ...
        if (bRet.first && bRet.second) ++m_ItemCounter;
        return bRet;
    }
    //@cond
    template <typename K, typename Func>
    ...
    */
    ...
    void clear()
    {
        for (size_t i = 0; i < bucket_count(); ++i) m_Buckets[i].clear();
        m_ItemCounter.reset();
    }
