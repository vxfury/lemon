...
#endif
{
  public:
    ...
    static constexpr const bool c_bEliminationEnabled =
        traits::enable_elimination; ///< \p true if elimination is enabled
  ...
  public:
    ...
    FCDeque(unsigned int nCompactFactor ///< Flat combining: publication list compacting factor
            ,
    ...
    */
    bool push_front(value_type const &val ///< Value to be copied to inserted element
    )
    {
        auto pRec = m_FlatCombining.acquire_record();
        pRec->pValPush = &val;

        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_push_front, pRec, *this);
        else m_FlatCombining.combine(op_push_front, pRec, *this);
    ...
    /**
        The function always returns \p true
    */
    bool push_front(value_type &&val ///< Value to be moved to inserted element
    )
    {
        auto pRec = m_FlatCombining.acquire_record();
        pRec->pValPush = &val;

        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_push_front_move, pRec, *this);
        else m_FlatCombining.combine(op_push_front_move, pRec, *this);
    ...
    /**
        The function always returns \p true
    */
    bool push_back(value_type const &val ///< Value to be copied to inserted element
    )
    {
        auto pRec = m_FlatCombining.acquire_record();
        pRec->pValPush = &val;

        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_push_back, pRec, *this);
        else m_FlatCombining.combine(op_push_back, pRec, *this);
    ...
    /**
        The function always returns \p true
    */
    bool push_back(value_type &&val ///< Value to be moved to inserted element
    )
    {
        auto pRec = m_FlatCombining.acquire_record();
        pRec->pValPush = &val;

        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_push_back_move, pRec, *this);
        else m_FlatCombining.combine(op_push_back_move, pRec, *this);
    ...
    */
    bool pop_front(value_type &val ///< Target to be received the copy of removed element
    )
    {
        auto pRec = m_FlatCombining.acquire_record();
        pRec->pValPop = &val;

        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_pop_front, pRec, *this);
        else m_FlatCombining.combine(op_pop_front, pRec, *this);
    ...
    /**
        ...
        If the deque is empty \p val is not changed.
    */
    bool pop_back(value_type &val ///< Target to be received the copy of removed element
    )
    {
        auto pRec = m_FlatCombining.acquire_record();
        pRec->pValPop = &val;

        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_pop_back, pRec, *this);
        else m_FlatCombining.combine(op_pop_back, pRec, *this);
    ...
    void clear()
    {
        ...
        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_clear, pRec, *this);
        else m_FlatCombining.combine(op_clear, pRec, *this);
    ...
    */
    ...
    {
        auto &deque = m_Deque;
        m_FlatCombining.invoke_exclusive([&deque, &f]() {
            f(deque);
        });
    }
    ...
    */
    bool empty() const
    {
        ...
        m_FlatCombining.invoke_exclusive([&deq, &bRet]() {
            bRet = deq.empty();
        });
        return bRet;
    }
    ...
    void fc_apply(fc_record *pRec)
    {
        ...
        switch (pRec->op()) {
            ...
            case op_pop_back:
                ...
                break;
            case op_clear:
                while (!m_Deque.empty()) m_Deque.pop_front();
                break;
            default:
                assert(false);
                break;
