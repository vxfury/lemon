...
class WeakRingBuffer : public cds::bounded_container {
  public:
    ...
    typedef
        typename traits::value_cleaner value_cleaner; ///< Value cleaner, see \p weak_ringbuffer::traits::value_cleaner
  ...
  public:
    ...
    */
    WeakRingBuffer(size_t capacity = 0) : front_(0), pfront_(0), cback_(0), buffer_(capacity)
    {
    ...
    */
    ...
    bool push(Q *arr, size_t count, CopyFunc copy)
    {
        ...
        // copy data
        for (size_t i = 0; i < count; ++i, ++back) copy(buffer_[buffer_.mod(back)], arr[i]);
    ...
    */
    template <typename Q>
    typename std::enable_if<std::is_constructible<value_type, Q>::value, bool>::type push(Q *arr, size_t count)
    {
        return push(arr, count, [](value_type &dest, Q const &src) {
    ...
    */
    ...
    typename std::enable_if<std::is_constructible<value_type, Args...>::value, bool>::type emplace(Args &&...args)
    {
        counter_type back = back_.load(memory_model::memory_order_relaxed);
    ...
    */
    ...
    bool pop(Q *arr, size_t count, CopyFunc copy)
    {
        ...
        if (static_cast<size_t>(cback_ - front) < count) {
            ...
            if (static_cast<size_t>(cback_ - front) < count) return false;
        }
    ...
    */
    template <typename Q>
    typename std::enable_if<std::is_assignable<Q &, value_type const &>::value, bool>::type pop(Q *arr, size_t count)
    {
        return pop(arr, count, [](Q &dest, value_type &src) {
            dest = src;
        });
    }
    ...
    */
    template <typename Q>
    typename std::enable_if<std::is_assignable<Q &, value_type const &>::value, bool>::type dequeue(Q &val)
    {
        return pop(&val, 1);
    }
    ...
    template <typename Q>
    typename std::enable_if<std::is_assignable<Q &, value_type const &>::value, bool>::type pop(Q &val)
    {
        return dequeue(val);
    }
    ...
    */
    value_type *front()
    {
        ...
        if (cback_ - front < 1) {
            cback_ = back_.load(memory_model::memory_order_acquire);
            if (cback_ - front < 1) return nullptr;
        }
    ...
    */
    ...
    size_t size() const
    {
        return static_cast<size_t>(back_.load(memory_model::memory_order_relaxed)
  ...
  public:
    ...
    */
    std::pair<void *, size_t> front()
    {
        ...
        if (cback_ - front < sizeof(size_t)) {
            cback_ = back_.load(memory_model::memory_order_acquire);
            if (cback_ - front < sizeof(size_t)) return std::make_pair(nullptr, 0u);
        }
    ...
    */
    bool pop_front()
    {
        ...
        if (cback_ - front < sizeof(size_t)) {
            cback_ = back_.load(memory_model::memory_order_acquire);
            if (cback_ - front < sizeof(size_t)) return false;
        }
    ...
    void clear()
    {
        for (auto el = front(); el.first; el = front()) pop_front();
    }
