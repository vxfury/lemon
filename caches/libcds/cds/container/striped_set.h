...
/** @ingroup cds_nonintrusive_set
   ...
   bucket. Instead, the class supports different container type for the bucket, for exampe, \p std::list, \p std::set
   and others.
   ...
   you specify among \p Options template arguments.
    ...
    The \p Options are:
        ...
        - \p opt::item_counter - item counter type. Default is \p atomicity::item_counter since some operation on the
   ...
   counter like as \p atomicity::empty_item_counter is not suitable.
        - \p opt::allocator - the allocator type using for memory allocation of bucket table and lock array. Default is
   ...
   <tt>striped_set::load_factor_resizing<4> </tt>; for other type of containers like \p std::set, \p std::unordered_set
   the resizing policy is \p striped_set::no_resizing. See \ref cds_striped_resizing_policy "available resizing policy".
            ...
            For other, non-sequential types of \p Container (like a \p std::set) the resizing policy is not so
   important.
        - \p opt::copy_policy - the copy policy which is used to copy items from the old set to the new one when
   resizing. The policy can be optionally used in adapted bucket container for performance reasons of resizing. The
   detail of copy algorithm depends on type of bucket container and explains below.
    ...
    <b>Internal details</b>
        ...
        The \p %StripedSet class cannot utilize the \p Container container specified directly, but only its adapted
   variant which supports an unified interface. Internally, the adaptation is made via striped_set::adapt metafunction
   that wraps bucket container and provides the unified bucket interface suitable for \p %StripedSet. Such adaptation is
   completely transparent for you - you don't need to call \p adapt metafunction directly, \p %StripedSet class's
   internal machinery itself invokes appropriate \p adapt metafunction to adjust your \p Container container class to \p
   %StripedSet bucket's internal interface. All you need is to include a right header before
   <tt>striped_hash_set.h</tt>.
   ...
   AnyContainer. However, there are a lot of specializations of <tt>striped_set::adapt</tt> for well-known containers,
   see table below. Any of this specialization wraps corresponding container making it suitable for the set's bucket.
        Remember, you should include the proper header file for \p adapt <b>before</b> including
   <tt>striped_hash_set.h</tt>. <table> <tr> <th>Container</th> <th>.h-file for \p adapt</th> <th>Example</th>
                <th>Notes</th>
            </tr>
            <tr>
                <td> \p std::list</td>
   ...
   type \p T stored in the list
            ...
            <tr>
                ...
                <td>
                    You should provide two different hash function \p h1 and \p h2 - one for \p std::unordered_set and
   other for \p %StripedSet. For the best result, \p h1 and \p h2 must be orthogonal i.e. <tt> h1(X) != h2(X) </tt> for
   any value \p X.
                </td>
            </tr>
            <tr>
   ...
   type \p T stored in the vector
            ...
            <tr>
                ...
                <td>
                    You should provide two different hash function \p h1 and \p h2 - one for \p boost::unordered_set and
   other for \p %StripedSet. For the best result, \p h1 and \p h2 must be orthogonal i.e. <tt> h1(X) != h2(X) </tt> for
   any value \p X.
                </td>
            </tr>
        ...
        Suppose, you have a container class \p MyBestContainer and you want to integrate it with \p %StripedSet as
   bucket type. There are two possibility:
        ...
        - or your \p MyBestContainer class does not support bucket's interface, which means, that you should develop a
   specialization <tt>cds::container::striped_set::adapt<MyBestContainer> </tt> metafunction providing necessary
   interface.
   ...
   std::swap. Not all containers support this copy policy, see details in table below.
        ...
        <table>
            ...
            <tr>
                ...
                <td>\code
                    struct copy_item {
                        void operator()( std::list<T>& list, std::list<T>::iterator itInsert, std::list<T>::iterator
   itWhat )
                        {
   ...
   itWhat )
            ...
            <tr>
                ...
                <td>\code
                    struct copy_item {
                        void operator()( bc::slist<T>& list, bc::slist<T>::iterator itInsert, bc::slist<T>::iterator
   itWhat )
                        {
...
class StripedSet : protected intrusive::StripedSet<Container, Options...> {
  ...
  public:
    ...
    */
    template <typename Q, typename Less, typename Bucket = bucket_type,
    ...
    */
    ...
    bool erase(Q const &key, Func f)
    {
        ...
        if (bOk) --base_class::m_ItemCounter;
        return bOk;
    }
    ...
    /**
        ...
        supports \p %erase_with feature.
    */
    template <typename Q, typename Less, typename Func, typename Bucket = bucket_type,
              typename = typename std::enable_if<Bucket::has_erase_with>::type>
