...
/** @ingroup cds_nonintrusive_map
    ...
    is original data structure based on an ordered list. Suppose, you want construct split-list map based on \p gc::HP
   GC and \p MichaelList as ordered list implementation. Your map should map \p int key to \p std::string value. So, you
   beginning your code with the following: \code #include <cds/container/michael_list_hp.h> #include
   <cds/container/split_list_map.h>

    namespace cc = cds::container;
    \endcode
    The inclusion order is important: first, include file for ordered-list implementation (for this example,
   <tt>cds/container/michael_list_hp.h</tt>), then the header for split-list map
   <tt>cds/container/split_list_map.h</tt>.

    Now, you should declare traits for split-list map. The main parts of traits are a hash functor and a comparing
   functor for the ordered list. We use <tt>std::hash<int></tt> as hash functor and <tt>std::less<int></tt> predicate as
   comparing functor.

    The second attention: instead of using \p %MichaelList in \p %SplitListMap traits we use a tag \p
   cds::contaner::michael_list_tag for the Michael's list. The split-list requires significant support from underlying
   ordered list class and it is not good idea to dive you into deep implementation details of split-list and ordered
   list interrelations. The tag paradigm simplifies split-list interface.
    ...
    >  int_string_map;
    \endcode
    In case of option-based declaration with \p split_list::make_traits metafunction the struct \p foo_set_traits is not
   required.
   ...
   inherited from \p container::split_list::traits. There are many other options for deep tuning of the split-list and
    ordered-list containers.
*/
template <class GC, typename Key, typename Value,
...
#endif
          >
class SplitListMap : protected container::SplitListSet<GC, std::pair<Key const, Value>,
                                                       split_list::details::wrap_map_traits<Key, Value, Traits> > {
    //@cond
    typedef container::SplitListSet<GC, std::pair<Key const, Value>,
                                    split_list::details::wrap_map_traits<Key, Value, Traits> >
        base_class;
    //@endcond
  ...
  public:
    ...
    */
    ...
    /**
       ...
       "IterableKVList" as the ordered list see \ref cds_intrusive_item_creating "insert item troubleshooting". \ref
    ...
    */
    ...
    std::pair<bool, bool>
#else
    typename std::enable_if<std::is_same<K, K>::value && !is_iterable_list<ordered_list>::value,
    ...
    update(K &&key, Func func, bool bAllowInsert = true)
    {
        ...
        return base_class::update(
            std::make_pair(key_type(key), mapped_type()),
    ...
    {
        return base_class::upsert(std::make_pair(key_type(std::forward<Q>(key)), mapped_type(std::forward<V>(val))),
                                  bAllowInsert);
    }
    ...
    */
    ...
    {
        return base_class::find(key, [&f](value_type &pair, K const &) {
            f(pair);
        });
    }
