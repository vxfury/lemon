...
struct link_node {
    ...
    typename SyncMonitor::node_injection
        m_SyncMonitorInjection;              ///< @ref cds_sync_monitor "synchronization monitor" injected data
    atomics::atomic<mapped_type *> m_pValue; ///< Value

  public:
    link_node() : m_nHeight(0), m_nVersion(0), m_pParent(nullptr), m_pLeft(nullptr), m_pRight(nullptr)
    {
    ...
    link_node(int nHeight, version_type version, node_type *pParent, node_type *pLeft, node_type *pRight)
        : m_nHeight(nHeight), m_nVersion(version), m_pParent(pParent), m_pLeft(pLeft), m_pRight(pRight)
    {
        m_pValue.store(nullptr, atomics::memory_order_release);
    }
    ...
    {
        BackOff bkoff;
        while (is_shrinking(order)) bkoff();
    }
...
struct node<Key, T *, SyncMonitor> : public link_node<node<Key, T *, SyncMonitor>, T, SyncMonitor> {
  ...
  public:
    //@cond
    template <typename Q>
    node(Q &&key) : base_class(), m_key(std::forward<Q>(key)), m_pNextRemoved(nullptr)
    {
    }

    template <typename Q>
    node(Q &&key, int nHeight, version_type version, node *pParent, node *pLeft, node *pRight)
        : base_class(nHeight, version, pParent, pLeft, pRight), m_key(std::forward<Q>(key)), m_pNextRemoved(nullptr)
    {
    }
    //@endcond
};
...
struct stat {
    ...
    event_counter m_nRelaxedInsertFailed; ///< Count of false creating of data nodes (only if @ref
                                          ///< bronson_avltree::relaxed_insert "relaxed insertion" is enabled)
    ...
    event_counter
        m_nUpdateRootWaitShrinking;  ///< Count of waiting until root shrinking completed duting \p update() call
    ...
    event_counter
        m_nRemoveWaitShrinking; ///< ount of waiting until shrinking completed during \p erase() or \p extract() call
    event_counter m_nRemoveRootWaitShrinking; ///< Count of waiting until root shrinking completed duting \p erase() or
                                              ///< \p extract() call
    ...
    void onFindWaitShrinking()
    {
        ++m_nFindWaitShrinking;
    }
    ...
    void onRelaxedInsertFailed()
    {
        ++m_nRelaxedInsertFailed;
    }
    void onInsertRetry()
    {
        ++m_nInsertRetry;
    }
    void onUpdateWaitShrinking()
    {
        ++m_nUpdateWaitShrinking;
    }
    void onUpdateRetry()
    {
        ++m_nUpdateRetry;
    }
    void onUpdateRootWaitShrinking()
    {
        ++m_nUpdateRootWaitShrinking;
    }
    void onUpdateSuccess()
    {
        ++m_nUpdateSuccess;
    }
    void onUpdateUnlinked()
    {
        ++m_nUpdateUnlinked;
    }
    void onDisposeNode()
    ...
    void onRemoveWaitShrinking()
    {
        ++m_nRemoveWaitShrinking;
    }
    void onRemoveRootWaitShrinking()
    {
        ++m_nRemoveRootWaitShrinking;
    }
    void onMakeRoutingNode()
    {
        ++m_nMakeRoutingNode;
    }
    ...
    void onRotateRightOverLeft()
    {
        ++m_nRightLeftRotation;
    }
    void onRotateLeftOverRight()
    {
        ++m_nLeftRightRotation;
    }

    void onRotateAfterRightRotation()
    {
        ++m_nRotateAfterRightRotation;
    }
    void onRemoveAfterRightRotation()
    {
        ++m_nRemoveAfterRightRotation;
    }
    void onDamageAfterRightRotation()
    {
        ++m_nDamageAfterRightRotation;
    }

    void onRotateAfterLeftRotation()
    {
        ++m_nRotateAfterLeftRotation;
    }
    void onRemoveAfterLeftRotation()
    {
        ++m_nRemoveAfterLeftRotation;
    }
    void onDamageAfterLeftRotation()
    {
        ++m_nDamageAfterLeftRotation;
    }

    void onRotateAfterRLRotation()
    {
        ++m_nRotateAfterRLRotation;
    }
    void onRemoveAfterRLRotation()
    {
        ++m_nRemoveAfterRLRotation;
    }
    void onRotateAfterLRRotation()
    {
        ++m_nRotateAfterLRRotation;
    }
    void onRemoveAfterLRRotation()
    {
        ++m_nRemoveAfterLRRotation;
    }

    void onInsertRebalanceRequired()
    {
        ++m_nInsertRebalanceReq;
    }
    void onRemoveRebalanceRequired()
    {
        ++m_nRemoveRebalanceReq;
    }
    //@endcond
};
...
/**
    ...
    - \ref cds_container_BronsonAVLTreeMap_rcu_ptr "pointer-oriented" - the tree node stores an user-provided pointer to
   value
...
struct traits {
    ...
    */
    ...
    /**
        ...
        Default is \ref cds::intrusive::opt::delete_disposer "cds::container::opt::v::delete_disposer<>" which calls \p
       delete operator.
    */
...
/**
   ...
   value
    ...
    - \p opt::less - specifies binary predicate used for key compare. At least \p %opt::compare or \p %opt::less should
   be defined.
    ...
    - \p opt::item_counter - the type of item counting feature, by default it is disabled (\p
   atomicity::empty_item_counter) To enable it use \p atomicity::item_counter or \p
   ...
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
    ...
    - \p opt::rcu_check_deadlock - a deadlock checking policy for RCU-based tree, default is \p
   opt::v::rcu_throw_deadlock
*/
