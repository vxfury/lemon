...
/**
    ...
    - \p opt::item_counter - the type of item counting feature. Default is \p atomicity::empty_item_counter that is no
   item counting.
   ...
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
    - \p skip_list::random_level_generator - random level generator. Can be \p skip_list::xor_shift, \p skip_list::turbo
   or user-provided one. Default is \p %skip_list::turbo32.
...
class node_allocator {
  protected:
    ...
    typedef typename std::allocator_traits<typename traits::allocator>::template rebind_alloc<unsigned char>
        tower_allocator_type;
    typedef typename std::allocator_traits<typename traits::allocator>::template rebind_alloc<node_type>
        node_allocator_type;
...
struct dummy_node_builder {
    ...
    static intrusive_node_type *make_tower(intrusive_node_type *pNode, RandomGen & /*gen*/)
    {
        return pNode;
    }
    static intrusive_node_type *make_tower(intrusive_node_type *pNode, unsigned int /*nHeight*/)
    {
        return pNode;
    }
...
class iterator {
  ...
  public: // for internal use only!!!
    iterator(intrusive_iterator const &it) : m_It(it) {}
