...
struct make_michael_kvlist {
    ...
    struct node_type : public intrusive::michael_list::node<gc> {
        ...
        node_type(key_type const &key) : m_Data(key, value_type()) {}

        template <typename Q>
        node_type(Q const &key) : m_Data(key_type(key), value_type())
        {
        }
        ...
        node_type(key_type const &key, value_type const &value) : m_Data(key, value) {}

        template <typename R>
        node_type(key_type const &key, R const &value) : m_Data(key, value_type(value))
        {
        }

        template <typename Q>
        node_type(Q const &key, value_type const &value) : m_Data(key_type(key), value)
        {
        }

        template <typename Q, typename R>
        node_type(Q const &key, R const &value) : m_Data(key_type(key), value_type(value))
        {
        }
