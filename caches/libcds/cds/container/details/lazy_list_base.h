...
struct traits {
    ...
    */
    ...
    /**
        If \p sort is \p true, than list maintains items in sorted order, otherwise items are unordered. Default is \p
       true. Note that if \p sort is \p false then lookup operations scan entire list.
    */
...
/**
   ...
   unordered list. No default is provided. If \p equal_to is not specified, \p compare is used, if \p compare is not
   specified, \p less is used.
    ...
    - \p opt::allocator - the allocator used for creating and freeing list's item. Default is \ref CDS_DEFAULT_ALLOCATOR
   macro.
