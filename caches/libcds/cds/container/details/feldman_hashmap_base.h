...
struct traits {
    ...
    /**
        ...
        fixed-sized bit-string, for example, <a href="https://en.wikipedia.org/wiki/Secure_Hash_Algorithm">SHA1,
       SHA2</a>, <a href="https://en.wikipedia.org/wiki/MurmurHash">MurmurHash</a>, <a
       href="https://en.wikipedia.org/wiki/CityHash">CityHash</a> or its successor <a
       href="https://code.google.com/p/farmhash/">FarmHash</a>.
        ...
        @code
            ...
            // We do not specify any comparing predicate (less or compare) so <tt> std::less<std::string> </tt> will be
       used by default struct url_map_traits: public cds::container::multilevl_hashmap::traits
            {
...
struct hash_selector {
    ...
    typedef
        typename std::decay<typename std::remove_reference<decltype(hasher()(std::declval<key_type>()))>::type>::type
            hash_type;
    ...
    struct node_type {
        ...
        node_type(hasher &h, Q &&key, Args &&...args)
            : m_Value(std::move(
                std::make_pair(key_type(std::forward<Q>(key)), std::move(mapped_type(std::forward<Args>(args)...))))),
