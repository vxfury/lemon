...
struct node : public cds::intrusive::ellen_bintree::node<GC> {
    ...
    /// Initializing ctor
    template <typename Q>
    node(Q const &v) : m_Value(v)
    {
    }

    /// Copy constructor
    template <typename... Args>
    node(Args const &...args) : m_Value(args...)
    {
    }
...
struct map_node : public cds::intrusive::ellen_bintree::node<GC> {
    ...
    template <typename K>
    map_node(K const &key) : m_Value(std::make_pair(key_type(key), mapped_type()))
    {
    }

    /// Initializes key and value fields
    template <typename K, typename Q>
    map_node(K const &key, Q const &v) : m_Value(std::make_pair(key_type(key), mapped_type(v)))
    {
    }
};
...
/**
   ...
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
    ...
    - \p ellen_bintree::update_desc_allocator - an allocator of \ref ellen_bintree::update_desc "update descriptors",
        ...
        Therefore, a bounded lock-free container like \p cds::container::VyukovMPMCCycleQueue is good choice for the
   free-list of update descriptors, see \p cds::memory::vyukov_queue_pool free-list implementation. Also notice that
   size of update descriptor is not dependent on the type of data stored in the tree so single free-list object can be
   used for several EllenBinTree-based object.
...
/**
   ...
   be defined.
    - \p opt::item_counter - the type of item counter, default is disabled (\p atomicity::empty_item_counter).
        To enable it use \p atomicity::item_counter or \p atomicity::cache_friendly_item_counter
    - opt::memory_model - C++ memory ordering model. Can be \p opt::v::relaxed_ordering (relaxed memory model, the
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
   ...
   used for several EllenBinTree-based object.
    - \p opt::stat - internal statistics, by default disabled (\p ellen_bintree::empty_stat). To enable
        it use \p ellen_bintree::stat.
    - \p opt::backoff - back-off strategy, by default no strategy is used (\p cds::backoff::empty)
    - \p opt::rcu_check_deadlock - a deadlock checking policy, only for RCU-based tree. Default is \p
...
struct make_ellen_bintree_map {
    ...
    typedef typename std::conditional<std::is_same<typename original_traits::copy_policy, opt::none>::value,
                                      assignment_copy_policy, typename original_traits::copy_policy>::type copy_policy;
