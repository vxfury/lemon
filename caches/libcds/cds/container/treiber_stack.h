...
/**
    ...
    - \p opt::allocator - allocator (like \p std::allocator) used for allocating stack nodes. Default is \ref
   ...
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
    ...
    - \p opt::buffer - an initialized buffer type for elimination array, see \p opt::v::initialized_static_buffer, \p
...
struct make_treiber_stack {
    ...
    struct node_type : public cds::intrusive::treiber_stack::node<gc> {
        ...
        node_type(const value_type &val) : m_value(val) {}

        template <typename... Args>
        node_type(Args &&...args) : m_value(std::forward<Args>(args)...)
        {
        }
    };
...
#endif
{
  ...
  public:
    ...
    typedef
        typename maker::allocator_type allocator_type; ///< Allocator type used for allocating/deallocating the nodes
  ...
  public:
    ...
    */
    TreiberStack(size_t nCollisionCapacity) : base_class(nCollisionCapacity) {}
    ...
    */
    ...
    {
        return pop_with([&val](value_type &src) {
            val = std::move(src);
        });
    }
