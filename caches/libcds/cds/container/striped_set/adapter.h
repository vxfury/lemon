...
<b>Bucket interface</b>
    ...
    <b>Updates \p item</b>
    \code template <typename Q, typename Func> std::pair<bool, bool> update( const Q& val, Func func, bool bAllowInsert
) \endcode The operation performs inserting or changing data.
...
using rational_load_factor_resizing =
    cds::intrusive::striped_set::rational_load_factor_resizing<Numerator, Denominator>;
...
class boost_set_adapter : public striped_set::adapted_container {
  ...
  private:
    typedef typename cds::opt::select<
        ...
        cds::container::striped_set::move_item, move_item_policy<container_type> >::type copy_item;
  ...
  public:
    ...
    bool insert(const Q &val, Func f)
    {
        ...
        if (res.second) f(const_cast<value_type &>(*res.first));
        return res.second;
    }
  ...
  public:
    ...
    bool emplace(Q &&key, Args &&...args)
    {
        std::pair<iterator, bool> res =
            m_Map.emplace(key_type(std::forward<Q>(key)), mapped_type(std::forward<Args>(args)...));
        return res.second;
    }
