...
/** @ingroup cds_nonintrusive_set
    ...
    - \p Traits - set traits, default is \p michael_set::traits.
        Instead of defining \p Traits struct you may use option-based syntax with \p michael_set::make_traits
   metafunction.
    ...
    Some member functions of Michael's hash set accept the key parameter of type \p Q which differs from node type \p
   value_type. It is expected that type \p Q contains full key of node type \p value_type, and if keys of type \p Q and
   \p value_type are equal the hash values of these keys must be equal too.
...
class MichaelHashSet {
  ...
  public:
    ...
    /**
       ...
       (or assertion) "no free guard" may be thrown if the limit of guard count per thread is exceeded.
        ...
        - for \p MichaelList and \p LazyList: iterator guarantees safety even if you delete the item that iterator
       points to because that item is guarded by hazard pointer. However, in case of concurrent deleting operations it
       is no guarantee that you iterate all item in the set. Moreover, a crash is possible when you try to iterate the
  ...
  public:
    ...
    */
    MichaelHashSet(size_t nMaxItemCount, ///< estimation of max item count in the hash set
    ...
    */
    ...
    /**
        ...
        @warning For \ref cds_nonintrusive_MichaelList_gc "MichaelList" and \ref cds_nonintrusive_IterableList_gc
       "IterableList" as the bucket see \ref cds_intrusive_item_creating "insert item troubleshooting".
        @ref cds_nonintrusive_LazyList_gc "LazyList" provides exclusive access to inserted item and does not require any
  ...
  private:
    ...
    typename std::enable_if<!is_iterable_list<List>::value, bool>::type bucket_emplace(Args &&...args)
    {
