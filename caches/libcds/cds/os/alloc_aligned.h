...
class aligned_allocator {
  ...
  public:
    ...
    /**
        ...
        To guarantee the alignment for each element of the array the size of an object of type \p T must be multiple of
       \p nAlign: \code sizeof(T) % nAlign == 0 \endcode
    ...
    */
    pointer allocate(size_type nAlign, size_type nCount)
    {
        ...
        if (!p) CDS_THROW_EXCEPTION(std::bad_alloc());
