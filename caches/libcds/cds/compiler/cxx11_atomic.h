...
struct atomic_integral_bitwise_ops {
  public:
    ...
    {
        unsigned_type cur =
            atomic_ops::atomic_load_explicit(reinterpret_cast<unsigned_type volatile *>(pDest), memory_order_relaxed);
        do {
        } while (!atomic_ops::atomic_compare_exchange_weak_explicit(reinterpret_cast<unsigned_type volatile *>(pDest),
                                                                    &cur, cur & unsigned_type(val), order,
                                                                    memory_order_relaxed));
        return T(cur);
    }
...
struct atomic_generic_ops<T, 1, Primary> {
    ...
    // cas
    static bool atomic_compare_exchange_weak_explicit(T volatile *pDest, T *expected, T desired,
                                                      memory_order mo_success, memory_order mo_fail) noexcept
    {
        assert(expected);
        return platform::cas8_weak(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                   mo_fail);
    }
    static bool atomic_compare_exchange_weak_explicit(T *pDest, T *expected, T desired, memory_order mo_success,
                                                      memory_order mo_fail) noexcept
    {
        assert(expected);
        return platform::cas8_weak(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                   mo_fail);
    ...
    {
        return atomic_compare_exchange_weak_explicit(pDest, expected, desired, memory_order_seq_cst,
                                                     memory_order_relaxed);
    }
    ...
    static bool atomic_compare_exchange_strong_explicit(T volatile *pDest, T *expected, T desired,
                                                        memory_order mo_success, memory_order mo_fail) noexcept
    {
        assert(expected);
        return platform::cas8_strong(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                     mo_fail);
    }
    static bool atomic_compare_exchange_strong_explicit(T *pDest, T *expected, T desired, memory_order mo_success,
                                                        memory_order mo_fail) noexcept
    {
        assert(expected);
        return platform::cas8_strong(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
    ...
    {
        return atomic_compare_exchange_strong_explicit(pDest, expected, desired, memory_order_seq_cst,
                                                       memory_order_relaxed);
    }
...
template <typename T>
struct atomic_integral_ops<T, 1> : atomic_generic_ops<T, 1, T>, atomic_integral_bitwise_ops<T> {
    ...
    static T atomic_fetch_add_explicit(T volatile *pDest, T val, memory_order order) noexcept
    {
        ...
        do {
        } while (!atomic_compare_exchange_weak_explicit(pDest, &cur, cur + val, order, memory_order_relaxed));
        return cur;
#endif
    }
...
struct atomic_generic_ops<T, 2, Primary> {
    ...
    {
        assert(expected);
        return platform::cas16_weak(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                    mo_fail);
    }
    static bool atomic_compare_exchange_weak_explicit(T *pDest, T *expected, T desired, memory_order mo_success,
                                                      memory_order mo_fail) noexcept
    ...
    static bool atomic_compare_exchange_weak(T volatile *pDest, T *expected, T desired) noexcept
    {
        return atomic_compare_exchange_weak_explicit(pDest, expected, primary::val(desired), memory_order_seq_cst,
                                                     memory_order_relaxed);
    }
    static bool atomic_compare_exchange_weak(T *pDest, T *expected, T desired) noexcept
    {
    ...
    {
        assert(expected);
        return platform::cas16_strong(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                      mo_fail);
    }
    static bool atomic_compare_exchange_strong_explicit(T *pDest, T *expected, T desired, memory_order mo_success,
                                                        memory_order mo_fail) noexcept
...
template <typename T>
struct atomic_integral_ops<T, 2> : atomic_generic_ops<T, 2, T>, atomic_integral_bitwise_ops<T> {
    typedef atomic_integral_bitwise_ops<T> bitwise_ops;
...
struct atomic_generic_ops<T, 4, Primary> {
    ...
    {
        assert(expected);
        return platform::cas32_weak(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                    mo_fail);
    }
    static bool atomic_compare_exchange_weak_explicit(T *pDest, T *expected, T desired, memory_order mo_success,
                                                      memory_order mo_fail) noexcept
    ...
    {
        assert(expected);
        return platform::cas32_strong(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                      mo_fail);
    }
    static bool atomic_compare_exchange_strong_explicit(T *pDest, T *expected, T desired, memory_order mo_success,
                                                        memory_order mo_fail) noexcept
...
template <typename T>
struct atomic_integral_ops<T, 4> : atomic_generic_ops<T, 4, T>, atomic_integral_bitwise_ops<T> {
    typedef atomic_integral_bitwise_ops<T> bitwise_ops;
    // fetch_add
    static T atomic_fetch_add_explicit(T volatile *pDest, T val, memory_order order) noexcept
    {
...
struct atomic_generic_ops<T, 8, Primary> {
    ...
    {
        assert(expected);
        return platform::cas64_weak(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                    mo_fail);
    }
    static bool atomic_compare_exchange_weak_explicit(T *pDest, T *expected, T desired, memory_order mo_success,
                                                      memory_order mo_fail) noexcept
    ...
    {
        assert(expected);
        return platform::cas64_strong(primary::ptr(pDest), primary::ref(*expected), primary::val(desired), mo_success,
                                      mo_fail);
    }
    static bool atomic_compare_exchange_strong_explicit(T *pDest, T *expected, T desired, memory_order mo_success,
                                                        memory_order mo_fail) noexcept
...
template <typename T>
struct atomic_integral_ops<T, 8> : atomic_generic_ops<T, 8, T>, atomic_integral_bitwise_ops<T> {
    typedef atomic_integral_bitwise_ops<T> bitwise_ops;
    ...
    static T atomic_fetch_add_explicit(T volatile *pDest, T val, memory_order order) noexcept
    {
        ...
        do {
            ...
            !general_ops::atomic_compare_exchange_weak_explicit(pDest, &cur, cur + val, order, memory_order_relaxed));
        return cur;
#endif
    }
    static T atomic_fetch_add_explicit(T *pDest, T val, memory_order order) noexcept
...
struct atomic_pointer : public atomic_pointer_base<T> {
    ...
    static T *atomic_fetch_add_explicit(T *volatile *pDest, ptrdiff_t val, memory_order order) noexcept
    {
        ...
        do {
            ...
            !base_class::atomic_compare_exchange_weak_explicit(pDest, &cur, cur + val, order, memory_order_relaxed));
        return cur;
#endif
    }
...
struct atomic_pointer<void> : public atomic_pointer_base<void> {
    ...
    static void *atomic_fetch_add_explicit(void *volatile *pDest, ptrdiff_t val, memory_order order) noexcept
    {
        ...
        do {
        } while (!base_class::atomic_compare_exchange_weak_explicit(pDest, &cur, reinterpret_cast<char *>(cur) + val,
...
struct atomic_integral {
  ...
  public:
    ...
    {
        return atomic_ops::atomic_compare_exchange_weak_explicit(&m_val, &expected, desired, success_order,
                                                                 failure_order);
    }
    ...
    bool compare_exchange_strong(T &expected, T desired, memory_order success_order,
                                 memory_order failure_order) volatile noexcept
    {
        return atomic_ops::atomic_compare_exchange_strong_explicit(&m_val, &expected, desired, success_order,
                                                                   failure_order);
    }
    bool compare_exchange_strong(T &expected, T desired, memory_order success_order,
    ...
    bool compare_exchange_weak(T &expected, T desired,
                               memory_order success_order = memory_order_seq_cst) volatile noexcept
    {
    ...
    constexpr atomic_integral(T val) noexcept : m_val(val) {}
...
struct atomic {
  ...
  public:
    ...
    bool compare_exchange_weak(T &expected, T desired, memory_order success_order,
                               memory_order failure_order) volatile noexcept
    {
        return atomic_ops::atomic_compare_exchange_weak_explicit(&m_data, &expected, desired, success_order,
                                                                 failure_order);
    }
    bool compare_exchange_weak(T &expected, T desired, memory_order success_order, memory_order failure_order) noexcept
    {
    ...
    bool compare_exchange_strong(T &expected, T desired, memory_order success_order,
                                 memory_order failure_order) volatile noexcept
    {
        return atomic_ops::atomic_compare_exchange_strong_explicit(&m_data, &expected, desired, success_order,
                                                                   failure_order);
    }
    bool compare_exchange_strong(T &expected, T desired, memory_order success_order,
                                 memory_order failure_order) noexcept
    ...
    atomic() = default;
    constexpr atomic(T val) : m_data(val) {}
...
#define CDS_DECLARE_ATOMIC_INTEGRAL(_type)                          \
    ...
    struct atomic<_type> : public details::atomic_integral<_type> { \
      ...
      public:                                                       \
        ...
        atomic &operator=(const atomic &) = delete;                 \
        atomic &operator=(const atomic &) volatile = delete;        \
        _type operator=(_type val) volatile noexcept                \
        {                                                           \
            return base_class::operator=(val);                      \
        }                                                           \
        _type operator=(_type val) noexcept                         \
        {                                                           \
            return base_class::operator=(val);                      \
        }                                                           \
    };
...
class atomic<T *> {
  ...
  public:
    ...
    bool compare_exchange_weak(T *&expected, T *desired, memory_order success_order,
                               memory_order failure_order) volatile noexcept
    {
        return atomic_ops::atomic_compare_exchange_weak_explicit(&m_ptr, &expected, desired, success_order,
                                                                 failure_order);
    }
    bool compare_exchange_weak(T *&expected, T *desired, memory_order success_order,
                               memory_order failure_order) noexcept
    ...
    bool compare_exchange_strong(T *&expected, T *desired, memory_order success_order,
                                 memory_order failure_order) volatile noexcept
    {
        return atomic_ops::atomic_compare_exchange_strong_explicit(&m_ptr, &expected, desired, success_order,
                                                                   failure_order);
    }
    bool compare_exchange_strong(T *&expected, T *desired, memory_order success_order,
                                 memory_order failure_order) noexcept
    ...
    atomic() = default;
    constexpr atomic(T *val) noexcept : m_ptr(val) {}
...
template <class T>
static inline bool atomic_compare_exchange_weak_explicit(volatile atomic<T> *p, T *expected, T desired,
...
template <class T>
static inline bool atomic_compare_exchange_weak_explicit(atomic<T> *p, T *expected, T desired,
                                                         memory_order success_order,
                                                         memory_order failure_order) noexcept
{
    return p->compare_exchange_weak(*expected, desired, success_order, failure_order);
...
template <class T>
static inline bool atomic_compare_exchange_strong_explicit(volatile atomic<T> *p, T *expected, T desired,
                                                           memory_order success_order,
                                                           memory_order failure_order) noexcept
{
...
template <class T>
static inline bool atomic_compare_exchange_strong_explicit(atomic<T> *p, T *expected, T desired,
                                                           memory_order success_order,
                                                           memory_order failure_order) noexcept
{
    return p->compare_exchange_strong(*expected, desired, success_order, failure_order);
...
typedef struct atomic_flag {
    ...
    {
        assert(order != memory_order_acquire && order != memory_order_acq_rel && order != memory_order_consume);
