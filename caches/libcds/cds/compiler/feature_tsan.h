...
#define CDS_TSAN_ANNOTATE_PUBLISH_MEMORY_RANGE(addr, sz) \
    AnnotatePublishMemoryRange(__FILE__, __LINE__, reinterpret_cast<void *>(addr), sz)
#define CDS_TSAN_ANNOTATE_UNPUBLISH_MEMORY_RANGE(addr, sz) \
    AnnotateUnpublishMemoryRange(__FILE__, __LINE__, reinterpret_cast<void *>(addr), sz)
#endif
...
#define CDS_TSAN_ANNOTATE_MUTEX_ACQUIRED(addr) \
    AnnotateRWLockAcquired(__FILE__, __LINE__, reinterpret_cast<void *>(addr), 1)
// must be called before actual release
#define CDS_TSAN_ANNOTATE_MUTEX_RELEASED(addr) \
    AnnotateRWLockReleased(__FILE__, __LINE__, reinterpret_cast<void *>(addr), 1)
