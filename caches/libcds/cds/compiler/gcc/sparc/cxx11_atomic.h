...
static inline bool atomic_flag_tas(atomic_flag_type volatile *pFlag, memory_order order) noexcept
{
    ...
    fence_before(order);
    __asm__ __volatile__("ldstub    [%[pFlag]], %[fCur] \n\t"
                         : [fCur] "=r"(fCur)
                         : [pFlag] "r"(pFlag)
                         : "memory", "cc");
    fence_after(order);
...
{
    fence_before(order);
    __asm__ __volatile__(CDS_SPARC_MB_REL "stub    %%g0, [%[pFlag]] \n\t" ::[pFlag] "r"(pFlag) : "memory");
    fence_after(order);
}
...
static inline void store32(T volatile *pDest, T src, memory_order order) noexcept
{
    ...
    assert(order == memory_order_relaxed || order == memory_order_release || order == memory_order_seq_cst);
    assert(pDest);
...
{
    static_assert(sizeof(T) == 4, "Illegal size of operand");
    assert(order == memory_order_relaxed || order == memory_order_consume || order == memory_order_acquire
...
template <typename T>
static inline bool cas32_strong(T volatile *pDest, T &expected, T desired, memory_order mo_success,
                                memory_order mo_fail) noexcept
{
    ...
    __asm__ __volatile__("cas [%[pDest]], %[expected], %[desired]"
                         : [desired] "+r"(desired)
...
template <typename T>
static inline bool cas32_weak(T volatile *pDest, T &expected, T desired, memory_order mo_success,
                              memory_order mo_fail) noexcept
{
...
static inline T exchange32(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    do {
    } while (!cas32_strong(pDest, cur, v, order, memory_order_relaxed));
    return cur;
}
...
template <typename T>
static inline bool cas64_strong(T volatile *pDest, T &expected, T desired, memory_order mo_success,
                                memory_order mo_fail) noexcept
{
    static_assert(sizeof(T) == 8, "Illegal size of operand");
    assert(pDest);
    ...
    fence_before(mo_success);
    __asm__ __volatile__("casx [%[pDest]], %[expected], %[desired]"
                         : [desired] "+r"(desired)
                         : [pDest] "r"(pDest), [expected] "r"(expected)
                         : "memory");
...
template <typename T>
static inline bool cas64_weak(T volatile *pDest, T &expected, T desired, memory_order mo_success,
                              memory_order mo_fail) noexcept
{
...
static inline T exchange64(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    do {
    } while (!cas64_strong(pDest, cur, v, order, memory_order_relaxed));
    return cur;
}
...
template <typename T>
static inline bool cas8_strong(T volatile *pDest, T &expected, T desired, memory_order mo_success,
                               memory_order mo_fail) noexcept
{
    static_assert(sizeof(T) == 1, "Illegal size of operand");
    assert(pDest);
    ...
    bool bSuccess;
    for (;;) {
        uExpected.w = uDesired.w = pDest32->w;
        uExpected.c[nCharIdx] = expected;
        uDesired.c[nCharIdx] = desired;

        bSuccess =
            cas32_weak(reinterpret_cast<uint32_t volatile *>(pDest32), uExpected.w, uDesired.w, mo_success, mo_fail);
        if (bSuccess || uExpected.c[nCharIdx] != expected) break;
    }
...
template <typename T>
static inline bool cas8_weak(T volatile *pDest, T &expected, T desired, memory_order mo_success,
                             memory_order mo_fail) noexcept
{
    static_assert(sizeof(T) == 1, "Illegal size of operand");
    assert(pDest);
...
static inline T exchange8(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    do {
    } while (!cas8_strong(pDest, cur, v, order, memory_order_relaxed));
    return cur;
}
...
template <typename T>
static inline bool cas16_strong(T volatile *pDest, T &expected, T desired, memory_order mo_success,
                                memory_order mo_fail) noexcept
{
    static_assert(sizeof(T) == 2, "Illegal size of operand");
    assert(pDest);
    ...
    for (;;) {
        ...
        bSuccess =
            cas32_weak(reinterpret_cast<uint32_t volatile *>(pDest32), uExpected.w, uDesired.w, mo_success, mo_fail);
        if (bSuccess || uExpected.c[nIdx] != expected) break;
    }
...
template <typename T>
static inline bool cas16_weak(T volatile *pDest, T &expected, T desired, memory_order mo_success,
                              memory_order mo_fail) noexcept
{
    static_assert(sizeof(T) == 2, "Illegal size of operand");
    assert(pDest);
...
static inline T exchange16(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    do {
    } while (!cas16_strong(pDest, cur, v, order, memory_order_relaxed));
    return cur;
}
...
template <typename T>
static inline bool cas_ptr_strong(T *volatile *pDest, T *&expected, T *desired, memory_order mo_success,
                                  memory_order mo_fail) noexcept
{
    static_assert(sizeof(T *) == sizeof(void *), "Illegal size of operand");

    return cas64_strong((uint64_t volatile *)pDest, *reinterpret_cast<uint64_t *>(&expected), (uint64_t)desired,
                        mo_success, mo_fail);
}

template <typename T>
static inline bool cas_ptr_weak(T *volatile *pDest, T *&expected, T *desired, memory_order mo_success,
                                memory_order mo_fail) noexcept
{
