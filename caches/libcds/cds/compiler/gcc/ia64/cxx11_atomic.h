...
#define CDS_ITANIUM_ATOMIC_LOAD(n_bytes, n_bits)                                                               \
    ...
    {                                                                                                          \
        ...
        assert(pSrc);                                                                                          \
        T val;                                                                                                 \
        __asm__ __volatile__("ld" #n_bytes ".acq %[val] = [%[pSrc]]  \n\t"                                     \
                             : [val] "=r"(val)                                                                 \
                             : [pSrc] "r"(pSrc)                                                                \
                             : "memory");                                                                      \
        return val;                                                                                            \
...
#define CDS_ITANIUM_ATOMIC_STORE(n_bytes, n_bits)                                                                   \
    ...
    {                                                                                                               \
        ...
        if (order == memory_order_seq_cst) {                                                                        \
            __asm__ __volatile__("st" #n_bytes ".rel [%[pDest]] = %[val] \n\t"                                      \
                                 "mf     \n\t" ::[pDest] "r"(pDest),                                                \
                                 [val] "r"(val)                                                                     \
                                 : "memory");                                                                       \
        } else {                                                                                                    \
...
#define CDS_ITANIUM_ATOMIC_CAS(n_bytes, n_bits)                                                                 \
    template <typename T>                                                                                       \
    static inline bool cas##n_bits##_strong(T volatile *pDest, T &expected, T desired, memory_order mo_success, \
    ...
    {                                                                                                           \
        ...
        switch (mo_success) {                                                                                   \
            ...
            case memory_order_consume:                                                                          \
            case memory_order_acquire:                                                                          \
                __asm__ __volatile__("mov ar.ccv = %[expected] ;;\n\t"                                          \
    ...
    template <typename T>                                                                                       \
    static inline bool cas##n_bits##_weak(T volatile *pDest, T &expected, T desired, memory_order mo_success,   \
                                          memory_order mo_fail) noexcept                                        \
    {                                                                                                           \
...
#define CDS_ITANIUM_ATOMIC_EXCHANGE(n_bytes, n_bits)                                        \
    ...
    {                                                                                       \
        ...
        switch (order) {                                                                    \
            ...
            case memory_order_consume:                                                      \
            case memory_order_acquire:                                                      \
                __asm__ __volatile__("xchg" #n_bytes " %[current] = [%[pDest]], %[val]\n\t" \
                                     : [current] "=r"(current)                              \
                                     : [pDest] "r"(pDest), [val] "r"(val)                   \
                                     : "memory");                                           \
                break;                                                                      \
...
#define CDS_ITANIUM_ATOMIC_FETCH_ADD(n_bytes, n_add)                                             \
    switch (order) {                                                                             \
        ...
        case memory_order_consume:                                                               \
        case memory_order_acquire:                                                               \
            __asm__ __volatile__("fetchadd" #n_bytes ".acq %[cur] = [%[pDest]], " #n_add " \n\t" \
                                 : [cur] "=r"(cur)                                               \
                                 : [pDest] "r"(pDest)                                            \
                                 : "memory");                                                    \
            break;                                                                               \
        case memory_order_release:                                                               \
            __asm__ __volatile__("fetchadd" #n_bytes ".rel %[cur] = [%[pDest]], " #n_add " \n\t" \
                                 : [cur] "=r"(cur)                                               \
                                 : [pDest] "r"(pDest)                                            \
                                 : "memory");                                                    \
            break;                                                                               \
...
static inline T fetch32_add(T volatile *pDest, T val, memory_order order) noexcept
{
    ...
    switch (val) {
        ...
        default:
            ...
            do {
            } while (!cas32_strong(pDest, cur, cur + val, order, memory_order_relaxed));
            break;
    }
    return cur;
}
...
static inline T fetch64_add(T volatile *pDest, T val, memory_order order) noexcept
{
    ...
    switch (val) {
        ...
        default:
            ...
            do {
            } while (!cas64_strong(pDest, cur, cur + val, order, memory_order_relaxed));
            break;
    }
    return cur;
}
...
static inline T *load_ptr(T *volatile const *pSrc, memory_order order) noexcept
{
    ...
    assert(pSrc);
    T *val;
    __asm__ __volatile__("ld8.acq %[val] = [%[pSrc]]  \n\t" : [val] "=r"(val) : [pSrc] "r"(pSrc) : "memory");
    return val;
}
...
static inline void store_ptr(T *volatile *pDest, T *val, memory_order order) noexcept
{
    ...
    if (order == memory_order_seq_cst) {
        __asm__ __volatile__("st8.rel [%[pDest]] = %[val] \n\t"
                             "mf     \n\t" ::[pDest] "r"(pDest),
                             [val] "r"(val)
                             : "memory");
    } else {
        __asm__ __volatile__("st8.rel [%[pDest]] = %[val] \n\t" ::[pDest] "r"(pDest), [val] "r"(val) : "memory");
        fence_after(order);
    }
}
                                  ...
                                  memory_order mo_fail) noexcept
{
    ...
    bool bSuccess = expected == current;
    expected = current;
    if (!bSuccess) fence_after(mo_fail);
    return bSuccess;
}
...
static inline T *exchange_ptr(T *volatile *pDest, T *val, memory_order order) noexcept
{
    ...
    switch (order) {
        ...
        case memory_order_consume:
        case memory_order_acquire:
            __asm__ __volatile__("xchg8 %[current] = [%[pDest]], %[val]\n\t"
                                 : [current] "=r"(current)
                                 : [pDest] "r"(pDest), [val] "r"(val)
                                 : "memory");
            break;
...
template <typename T>
struct atomic_pointer_sizeof {
    enum { value = sizeof(T) };
};
template <>
struct atomic_pointer_sizeof<void> {
    enum { value = 1 };
};
...
static inline T *fetch_ptr_add(T *volatile *pDest, ptrdiff_t val, memory_order order) noexcept
{
    ...
    switch (val) {
        ...
        default:
            ...
            do {
            } while (!cas_ptr_strong(pDest, cur, reinterpret_cast<T *>(reinterpret_cast<uint8_t *>(cur) + val), order,
