                                ...
                                memory_order mo_fail) noexcept
{
    ...
    __asm__ __volatile__("lock ; cmpxchgq %[desired], %[pDest]"
...
static inline T exchange64(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    fence_before(order);
    __asm__ __volatile__("xchgq %[v], %[pDest]" : [v] "+r"(v), [pDest] "+m"(*pDest));
    fence_after(order);
    return v;
}
...
static inline T fetch64_add(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    fence_before(order);
    __asm__ __volatile__("lock ; xaddq %[v], %[pDest]" : [v] "+r"(v), [pDest] "+m"(*pDest));
    fence_after(order);
    return v;
}
