                               ...
                               memory_order mo_fail) noexcept
{
    ...
    __asm__ __volatile__("lock ; cmpxchgb %[desired], %[pDest]"
...
static inline T exchange8(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    fence_before(order);
    __asm__ __volatile__("xchgb %[v], %[pDest]" : [v] "+q"(v), [pDest] "+m"(*pDest));
    fence_after(order);
    return v;
}
...
{
    fence_before(order);
    __asm__ __volatile__("lock ; xaddb %[val], %[pDest]" : [val] "+q"(val), [pDest] "+m"(*pDest));
    fence_after(order);
    return val;
}
...
static inline T exchange16(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    fence_before(order);
    __asm__ __volatile__("xchgw %[v], %[pDest]" : [v] "+q"(v), [pDest] "+m"(*pDest));
    fence_after(order);
    return v;
}
                                ...
                                memory_order mo_fail) noexcept
{
    ...
    T prev = expected;
    fence_before(mo_success);
    __asm__ __volatile__("lock ; cmpxchgw %[desired], %[pDest]"
                         : [prev] "+a"(prev), [pDest] "+m"(*pDest)
                         : [desired] "q"(desired));
    bool success = prev == expected;
    if (success)
...
static inline T fetch16_add(T volatile *pDest, T val, memory_order order) noexcept
{
    ...
    fence_before(order);
    __asm__ __volatile__("lock ; xaddw %[val], %[pDest]" : [val] "+q"(val), [pDest] "+m"(*pDest));
    fence_after(order);
    return val;
}
...
static inline T exchange32(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    fence_before(order);
    __asm__ __volatile__("xchgl %[v], %[pDest]" : [v] "+r"(v), [pDest] "+m"(*pDest));
    fence_after(order);
    return v;
}
                                ...
                                memory_order mo_fail) noexcept
{
    ...
    T prev = expected;
    fence_before(mo_success);
    __asm__ __volatile__("lock ; cmpxchgl %[desired], %[pDest]"
                         : [prev] "+a"(prev), [pDest] "+m"(*pDest)
                         : [desired] "r"(desired));
    bool success = prev == expected;
    if (success)
...
static inline T fetch32_add(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    fence_before(order);
    __asm__ __volatile__("lock ; xaddl %[v], %[pDest]" : [v] "+r"(v), [pDest] "+m"(*pDest));
    fence_after(order);
    return v;
}
