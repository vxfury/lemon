                                ...
                                memory_order mo_fail) noexcept
{
    ...
    __asm__ __volatile__("movl %%ebx, %[ebxStore]\n"
                         "movl %[desiredLo], %%ebx\n"
                         "lock; cmpxchg8b 0(%[pDest])\n"
                         "movl %[ebxStore], %%ebx\n"
                         : [prev] "=A"(prev), [ebxStore] "=m"(ebxStore)
                         : [desiredLo] "D"((int)desired), [desiredHi] "c"((int)(desired >> 32)), [pDest] "S"(pDest),
                           "0"(prev)
                         : "memory");
...
static inline T load64(T volatile const *pSrc, memory_order order) noexcept
{
    ...
    __asm__ __volatile__("movq   (%[pSrc]), %[v]   ;   \n\t" : [v] "=x"(v) : [pSrc] "r"(pSrc) :);
    return v;
}
...
static inline void store64(T volatile *pDest, T val, memory_order order) noexcept
{
    ...
    if (order != memory_order_seq_cst) {
        ...
        __asm__ __volatile__("movq       %[val], (%[pDest])   ;   \n\t"
                             :
                                  ...
                                  memory_order mo_fail) noexcept
{
    ...
    return cas32_strong((uint32_t volatile *)pDest, *reinterpret_cast<uint32_t *>(&expected), (uint32_t)desired,
                        mo_success, mo_fail);
}
