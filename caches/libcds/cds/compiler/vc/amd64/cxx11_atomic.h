                               ...
                               memory_order /*mo_fail*/) noexcept
{
    ...
    for (;;) {
        ...
        if (nPrev == nExpected) return true;
        T nByte;
...
static inline T exchange8(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    T expected = *pDest;
    do {
    } while (!cas8_strong(pDest, expected, v, order, memory_order_relaxed));
    return expected;
#endif
}
#if _MSC_VER >= 1600
...
static inline T exchange16(T volatile *pDest, T v, memory_order order) noexcept
{
    ...
    T expected = *pDest;
    do {
    } while (!cas16_strong(pDest, expected, v, order, memory_order_relaxed));
    return expected;
#endif
}
