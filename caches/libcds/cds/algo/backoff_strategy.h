...
class exponential {
  ...
  public:
    /// Default ctor
    exponential() noexcept : m_nExpCur(traits::lower_bound) {}
    ...
    void operator()() noexcept(noexcept(std::declval<spin_backoff>()()) &&noexcept(std::declval<yield_backoff>()()))
    {
        if (m_nExpCur <= traits::upper_bound) {
            for (size_t n = 0; n < m_nExpCur; ++n) m_bkSpin();
            m_nExpCur *= 2;
        } else
            m_bkYield();
    }
    ...
    template <typename Predicate>
    bool operator()(Predicate pr) noexcept(noexcept(std::declval<Predicate>()()) &&noexcept(
        std::declval<spin_backoff>()()) &&noexcept(std::declval<yield_backoff>()()))
    {
        if (m_nExpCur <= traits::upper_bound) {
            for (size_t n = 0; n < m_nExpCur; ++n) {
                if (m_bkSpin(pr)) return true;
            }
            m_nExpCur *= 2;
        } else
            return m_bkYield(pr);
...
class delay {
  ...
  public:
    ...
    delay() noexcept : timeout(traits::timeout) {}

    /// Initializes timeout from \p nTimeout
    constexpr explicit delay(unsigned int nTimeout) noexcept : timeout(nTimeout) {}
