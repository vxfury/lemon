...
/**
    ...
    record is active (you may use any of \p wait_strategy instead of spinning). If your record is inactive proceed to
   step 5. Once the response is available, reset the request field to null and return the response.</li> <li>If the lock
   is not taken, attempt to acquire it and become a combiner. If you fail, return to spinning in step 2.</li>
...
struct stat {
    ...
    */
    ...
    void onPassiveWaitIteration()
    {
        ++m_nPassiveWaitIteration;
    }
    void onPassiveWaitWakeup()
    {
        ++m_nPassiveWaitWakeup;
    }
    void onInvokeExclusive()
    {
        ++m_nInvokeExclusive;
    }
    void onWakeupByNotifying()
    {
        ++m_nWakeupByNotifying;
    }
    void onPassiveToCombiner()
    {
...
struct traits {
    ...
    typedef cds::algo::flat_combining::wait_strategy::backoff<cds::backoff::delay_of<2>>
        wait_strategy; ///< Wait strategy
    typedef CDS_DEFAULT_ALLOCATOR
        allocator;           ///< Allocator used for TLS data (allocating \p publication_record derivatives)
...
class kernel {
  public:
    ...
    typedef typename wait_strategy::template make_publication_record<PublicationRecord>::type
        publication_record_type; ///< Publication record type

  protected:
    //@cond
    typedef cds::details::Allocator<publication_record_type, allocator>
        cxx11_allocator; ///< internal helper cds::details::Allocator
  ...
  protected:
    ...
    unsigned int const m_nCompactFactor; ///< Publication list compacting factor (the list will be compacted through \p
                                         ///< %m_nCompactFactor combining passes)
  ...
  public:
    ...
    */
    ...
    /// Initializes the object
    kernel(unsigned int nCompactFactor ///< Publication list compacting factor (the list will be compacted through \p
                                       ///< nCompactFactor combining passes)
           ,
    ...
    */
    publication_record_type *acquire_record()
    {
        ...
        if (!pRec) {
            ...
            } while (!m_pAllocatedHead->pNextAllocated.compare_exchange_weak(
                p, pRec, memory_model::memory_order_release, atomics::memory_order_acquire));
  ...
  public:
    ...
    */
    class iterator {
      ...
      protected:
        //@cond
        iterator(publication_record_type *pRec) : m_pRec(pRec)
        {
            skip_inactive();
        }
        ...
        {
            while (m_pRec
                   && (m_pRec->nState.load(memory_model::memory_order_acquire) != active
      ...
      public:
        ...
        iterator() : m_pRec(nullptr) {}

        /// Copy ctor
        iterator(iterator const &src) : m_pRec(src.m_pRec) {}
  ...
  private:
    ...
    void combining(Container &owner)
    {
        ...
        m_Stat.onCombining();
        if ((nCurAge & m_nCompactFactor) == 0) compact_list(nCurAge);
    }
    ...
    void batch_combining(Container &owner)
    {
        ...
        for (unsigned int nPass = 0; nPass < m_nCombinePassCount; ++nPass) owner.fc_process(begin(), end());
    ...
    bool wait_for_combining(publication_record_type *pRec)
    {
        ...
        while (pRec->op(memory_model::memory_order_acquire) != req_Response) {
            ...
            if (m_waitStrategy.wait(*this, *pRec)) m_Stat.onWakeupByNotifying();
    ...
    try_again:
            ...
            switch (p->nState.load(memory_model::memory_order_relaxed)) {
                case active:
                    if (p->nAge.load(memory_model::memory_order_relaxed) + m_nCompactFactor < nCurAge) {
                        ...
                        if (pPrev->pNext.compare_exchange_strong(p, pNext, memory_model::memory_order_acquire,
                ...
                case removed:
                    ...
                    if (cds_likely(pPrev->pNext.compare_exchange_strong(p, pNext, memory_model::memory_order_acquire,
                                                                        atomics::memory_order_relaxed))) {
                        p = pNext;
                        continue;
                    } else {
        ...
        for (publication_record *p = pPrev->pNextAllocated.load(memory_model::memory_order_acquire); p;) {
            if (p->nState.load(memory_model::memory_order_relaxed) == removed) {
                ...
                if (pPrev->pNextAllocated.compare_exchange_strong(p, pNext, memory_model::memory_order_acquire,
                                                                  atomics::memory_order_relaxed)) {
