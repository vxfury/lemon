...
class single_mutex_single_condvar {
  ...
  public:
    ...
    /// Default ctor
    single_mutex_single_condvar() : m_wakeup(false) {}
    ...
    bool wait(FCKernel &fc, PublicationRecord &rec)
    {
        if (fc.get_operation(rec) >= req_Operation) {
            ...
            if (fc.get_operation(rec) >= req_Operation) {
                ...
                bool ret = m_condvar.wait_for(lock, std::chrono::milliseconds(c_nWaitMilliseconds))
...
class single_mutex_multi_condvar {
  ...
  public:
    ...
    /// Default ctor
    single_mutex_multi_condvar() : m_wakeup(false) {}
    ...
    bool wait(FCKernel &fc, PublicationRecord &rec)
    {
        if (fc.get_operation(rec) >= req_Operation) {
            ...
            if (fc.get_operation(rec) >= req_Operation) {
                ...
                bool ret = rec.m_condvar.wait_for(lock, std::chrono::milliseconds(c_nWaitMilliseconds))
                           == std::cv_status::no_timeout;
                m_wakeup = false;
                return ret;
            }
