...
class split_bitstring {
  public:
    ...
    static constexpr size_t const c_bitstring_size =
        BitStringSize ? BitStringSize : sizeof(BitString); ///< size of \p BitString in bytes
  ...
  public:
    ...
    explicit split_bitstring(bitstring const &h)
        : cur_(reinterpret_cast<uint8_t const *>(&h)), offset_(0), first_(cur_), last_(cur_ + c_bitstring_size)
    {
    }
    ...
    */
    uint_type cut(unsigned count)
    {
        ...
        for (unsigned done = 0; done < count;) {
            ...
            if (bits > c_nBitPerByte - offset_) bits = c_nBitPerByte - offset_;
...
class byte_splitter {
  ...
  public:
    ...
    explicit byte_splitter(bitstring const &h)
        : cur_(reinterpret_cast<uint8_t const *>(&h)), first_(cur_), last_(cur_ + c_bitstring_size)
    {
    }
...
class number_splitter {
  ...
  public:
    ...
    explicit number_splitter(int_type n) : number_(n), shift_(0) {}

    /// Initalizes the splitter with nymber \p n and initial bit offset \p initial_offset
    number_splitter(int_type n, size_t initial_offset) : number_(n), shift_(static_cast<unsigned>(initial_offset))
    {
...
#define CDS_SELECT_NUMBER_SPLITTER(num_type)             \
    template <>                                          \
    struct select_splitter<num_type, sizeof(num_type)> { \
        typedef number_splitter<num_type> type;          \
    }
