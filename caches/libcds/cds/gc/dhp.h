...
class not_initialized : public std::runtime_error {
  public:
    //@cond
    not_initialized() : std::runtime_error("Global DHP SMR object is not initialized") {}
    //@endcond
};
  ...
  public:
    ...
    size_t alloc(guard_array<Capacity> &arr)
    {
        for (size_t i = 0; i < Capacity; ++i) {
            if (cds_unlikely(free_head_ == nullptr)) extend();
    ...
    {
        // clear array_
        for (guard *cur = array_, *last = array_ + initial_capacity_; cur < last; ++cur) cur->clear();
    ...
    void init()
    {
        ...
        guard *p = array_;
        for (guard *pEnd = p + initial_capacity_ - 1; p != pEnd; ++p) p->next_ = p + 1;
        p->next_ = nullptr;
        free_head_ = array_;
    }
  ...
  private: // called by basic_smr
    void init()
    {
        if (list_head_ == nullptr) {
            ...
            current_block_ = list_head_ = list_tail_ = block;
    ...
    void fini()
    {
        ...
        current_block_ = list_head_ = list_tail_ = nullptr;
        current_cell_ = nullptr;
    ...
    bool empty() const
    {
        return current_block_ == nullptr || (current_block_ == list_head_ && current_cell_ == current_block_->first());
    }
...
struct stat {
    ...
    void clear()
    {
        guard_allocated = guard_freed = retired_count = free_count = scan_count = help_scan_count = thread_rec_count =
            hp_block_count = retired_block_count = hp_extend_count = retired_extend_count = 0;
    }
};
...
class smr {
  ...
  public:
...
#else
        if (!instance_) CDS_THROW_EXCEPTION(not_initialized());
#endif
        return *instance_;
    }
    ...
    /**
        Dynamic Hazard Pointer SMR is a singleton. If DHP instance is not initialized then the function creates the
       instance. Otherwise it does nothing.
    ...
    */
    ...
    static void Construct(size_t nInitialHazardPtrCount = 16 ///< Initial number of hazard pointer per thread
    )
    {
    ...
    */
    static CDS_EXPORT_API void destruct(bool bDetachAll = false ///< Detach all threads
    );

    // for back-compatibility
    static void Destruct(bool bDetachAll = false ///< Detach all threads
    )
    {
        destruct(bDetachAll);
    }
    ...
    */
    static CDS_EXPORT_API void set_memory_allocator(void *(*alloc_func)(size_t size), void (*free_func)(void *p));
  ...
  private:
    CDS_EXPORT_API explicit smr(size_t nInitialHazardPtrCount);
...
/**  @ingroup cds_garbage_collector
    ...
    %DHP is an adaptive variant of classic \p cds::gc::HP, see @ref cds_garbage_collectors_comparison "Compare HP
   implementation"
...
class DHP {
  public:
    ...
    */
    class Guard {
      public:
        ...
        Guard() noexcept : guard_(dhp::smr::tls()->hazards_.alloc()) {}
        ...
        Guard(Guard &&src) noexcept : guard_(src.guard_)
        {
            src.guard_ = nullptr;
        }
        ...
        */
        ...
        void link()
        {
            if (!guard_) guard_ = dhp::smr::tls()->hazards_.alloc();
        }
    ...
    */
    ...
    class GuardArray {
      ...
      public:
        ...
        */
        ...
        {
            return protect(nIndex, toGuard, [](T p) {
                return p;
            });
        }
    ...
    /**
        ...
        - \p Cast - a functor for converting <tt>GuardedType*</tt> to <tt>ValueType*</tt>. Default is \p void (no
       casting).
        ...
        All set/map container classes from \p libcds declare the typedef for \p %guarded_ptr with appropriate casting
       functor.
    */
    ...
    class guarded_ptr {
      ...
      public:
        ...
        guarded_ptr() noexcept : guard_(nullptr) {}

        //@cond
        explicit guarded_ptr(dhp::guard *g) noexcept : guard_(g) {}

        /// Initializes guarded pointer with \p p
        explicit guarded_ptr(guarded_type *p) noexcept : guard_(nullptr)
        {
            reset(p);
        }
        explicit guarded_ptr(std::nullptr_t) noexcept : guard_(nullptr) {}
        //@endcond

        /// Move ctor
        guarded_ptr(guarded_ptr &&gp) noexcept : guard_(gp.guard_)
        {
            gp.guard_ = nullptr;
        }
        ...
        /// Move ctor
        template <typename GT, typename VT, typename C>
        guarded_ptr(guarded_ptr<GT, VT, C> &&gp) noexcept : guard_(gp.guard_)
        {
            gp.guard_ = nullptr;
        }

        /// Ctor from \p Guard
        explicit guarded_ptr(Guard &&g) noexcept : guard_(g.release()) {}
  ...
  public:
    ...
    */
    explicit DHP(size_t nInitialHazardPtrCount = 16 ///< Initial number of hazard pointer per thread
    )
    {
    ...
    */
    ...
    /**
        ...
        By default, a standard \p new and \p delete operators are used for this.
    */
    static void set_memory_allocator(void *(*alloc_func)(size_t size), ///< \p malloc() function
    ...
    */
    ...
    static void retire(T *p, void (*func)(void *))
    {
        ...
        if (!rec->retired_.push(dhp::retired_ptr(p, func))) dhp::smr::instance().scan(rec);
    }
    ...
    /**
        ...
        \endcode
        Since the functor call can happen at any time after \p retire() call, additional restrictions are imposed to \p
       Disposer type:
    ...
    */
    ...
    {
        if (!dhp::smr::tls()->retired_.push(dhp::retired_ptr(
                p, +[](void *p) {
                    Disposer()(static_cast<T *>(p));
                })))
            scan();
    }
