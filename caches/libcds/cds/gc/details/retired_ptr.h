...
struct retired_ptr {
    ...
    retired_ptr() noexcept : m_p(nullptr), m_funcFree(nullptr) {}

    /// Copy ctor
    retired_ptr(retired_ptr const &rp) noexcept : m_p(rp.m_p), m_funcFree(rp.m_funcFree) {}

    /// Ctor
    retired_ptr(pointer p, free_retired_ptr_func func) noexcept : m_p(p), m_funcFree(func) {}

    /// Typecasting ctor
    template <typename T>
    retired_ptr(T *p, free_retired_ptr_func func) noexcept : m_p(reinterpret_cast<pointer>(p)), m_funcFree(func)
    {
    }
