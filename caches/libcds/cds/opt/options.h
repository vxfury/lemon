...
namespace details
{
enum padding_vs_datasize { padding_datasize_less, padding_datasize_equal, padding_datasize_greater };
...
struct apply_padding {
  ...
  public:
    static constexpr const size_t c_nPadding =
        ...
        : static_cast<unsigned int>(padding) == static_cast<unsigned int>(no_special_padding) ? 0
                                                                                              : padding;

    static_assert((c_nPadding & (c_nPadding - 1)) == 0, "Padding must be a power-of-two number");

    typedef apply_padding_helper<T, c_nPadding, c_nPadding == 0,
                                 sizeof(T) < c_nPadding    ? padding_datasize_less
                                 : sizeof(T) == c_nPadding ? padding_datasize_equal
    ...
    typedef typename std::conditional<std::is_same<typename result::padding_type, void>::value, unsigned int,
                                      typename result::padding_type>::type padding_type;
};
...
/**
    The <b>cds</b> library supports following memory ordering constraints for atomic operations in container
   implementation:
...
struct replace_key_accessor {
    typedef
        typename std::conditional<std::is_same<typename Traits::key_accessor, WhatReplace>::value,
                                  typename opt::key_accessor<ReplaceWith>::template pack<Traits>, Traits>::type type;
};
//@endcond
...
class sequential_item_counter {
  ...
  public:
    sequential_item_counter() : m_nCounter(0) {}
...
struct make_options_impl {
    typedef typename make_options_impl<typename do_pack<OptionList, typename typelist_head<Typelist>::type>::type,
                                       typename typelist_tail<Typelist>::type>::type type;
};
...
struct find_type_traits {
    typedef typename select_default<typename details::find_type_traits_option<Options...>::type, DefaultOptions>::type
        type; ///< Metafunction result
};
...
#define CDS_FIND_OPTION_INTEGRAL_SPECIALIZATION_(_type)      \
    template <template <_type> class What, _type A, _type B> \
    struct compare_option<What<A>, What<B> > {               \
        typedef compare_ok type;                             \
    };

// For user-defined enum types
#define CDS_DECLARE_FIND_OPTION_INTEGRAL_SPECIALIZATION(_type) \
    namespace cds                                              \
    {                                                          \
    namespace opt                                              \
    {                                                          \
    namespace details                                          \
    {                                                          \
    CDS_FIND_OPTION_INTEGRAL_SPECIALIZATION_(_type)            \
    }                                                          \
    }                                                          \
    }
...
struct find_option<What, Opt> {
    typedef typename select_option<typename compare_option<What, Opt>::type, Opt, What>::type type;
};

template <typename What, typename Opt, typename... Options>
struct find_option<What, Opt, Options...> {
    typedef typename select_option<typename compare_option<What, Opt>::type, Opt,
                                   typename find_option<What, Options...>::type>::type type;
};
} // namespace details
//@endcond
