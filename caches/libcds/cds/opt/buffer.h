...
class uninitialized_static_buffer {
  ...
  public:
    ...
    */
    ...
    {
        constexpr_if(c_bExp2) return idx & (capacity() - 1);
        else return idx % capacity();
    }
    ...
    {
        constexpr_if(c_bExp2) return static_cast<size_t>(idx & static_cast<I>(capacity() - 1));
        else return static_cast<size_t>(idx % capacity());
    }
    //@endcond
};
...
class uninitialized_dynamic_buffer {
  ...
  public:
    ...
    */
    uninitialized_dynamic_buffer(size_t nCapacity) : m_nCapacity(c_bExp2 ? beans::ceil2(nCapacity) : nCapacity)
    {
...
class initialized_dynamic_buffer {
  ...
  public:
    ...
    /**
        ...
        of allocating buffer is nearest upper to \p nCapacity power of two.
    */
    initialized_dynamic_buffer(size_t nCapacity) : m_nCapacity(c_bExp2 ? beans::ceil2(nCapacity) : nCapacity)
    {
        assert(m_nCapacity >= 2);
        // Capacity must be power of 2
        assert(!c_bExp2 || (m_nCapacity & (m_nCapacity - 1)) == 0);
