...
struct pool_monitor_traits {
    ...
    struct stat {
        ...
        void onLockContention()
        {
            ++m_nLockContention;
        }
        void onUnlockContention()
        {
            ++m_nUnlockContention;
        }
...
class pool_monitor {
  public:
    ...
    typedef typename std::conditional<std::is_same<BackOff, cds::opt::none>::value, cds::backoff::yield,
                                      BackOff>::type back_off; ///< back-off strategy for spinning
    typedef uint32_t refspin_type;                             ///< Reference counter + spin-lock bit

    /// Internal statistics
    typedef typename std::conditional<Stat, typename pool_monitor_traits::stat<>,
                                      typename pool_monitor_traits::empty_stat>::type internal_stat;
  ...
  public:
    ...
    struct node_injection {
        ...
        //@cond
        node_injection() : m_pLock(nullptr)
        {
    ...
    pool_monitor() : m_Pool(c_nDefaultCapacity) {}

    /// Initializes the pool of \p nPoolCapacity preallocated mutexes
    pool_monitor(size_t nPoolCapacity) : m_Pool(nPoolCapacity ? nPoolCapacity : c_nDefaultCapacity) {}
    ...
    void lock(Node const &p) const
    {
        ...
        if (!p.m_SyncMonitorInjection.m_RefSpin.compare_exchange_weak(cur, cur + c_nRefIncrement + c_nSpinBit,
            ...
            do {
                ...
                bkoff();
                cur &= ~c_nSpinBit;
            } while (!p.m_SyncMonitorInjection.m_RefSpin.compare_exchange_weak(
                cur, cur + c_nRefIncrement + c_nSpinBit, atomics::memory_order_acq_rel, atomics::memory_order_acquire));
        }
    ...
    void unlock(Node const &p) const
    {
        ...
        // try lock spin
        refspin_type cur = p.m_SyncMonitorInjection.m_RefSpin.load(atomics::memory_order_relaxed) & ~c_nSpinBit;
        if (!p.m_SyncMonitorInjection.m_RefSpin.compare_exchange_weak(
                cur, cur | c_nSpinBit, atomics::memory_order_acquire, atomics::memory_order_acquire)) {
            back_off bkoff;
            do {
