...
struct pow2_select_policy {
    ...
    pow2_select_policy(size_t nCapacity) : m_nMask(nCapacity - 1)
    {
        assert(is_capacity_accepted(nCapacity));
    }

    /// Copy constructor
    pow2_select_policy(pow2_select_policy const &src) : m_nMask(src.m_nMask) {}

    /// Move constructor
    pow2_select_policy(pow2_select_policy &&src) : m_nMask(src.m_nMask) {}
...
class lock_array {
  ...
  protected:
    ...
    {
        if (pArr) cxx_allocator().Delete(pArr, nCapacity);
    }

    // Only for internal use!!!
    lock_array() : m_arrLocks(nullptr), m_nCapacity(0) {}

    lock_array(select_cell_policy const &policy) : m_arrLocks(nullptr), m_nCapacity(0), m_SelectCellPolicy(policy) {}
    //@endcond
  ...
  public:
    ...
    */
    lock_array(size_t nCapacity ///< [in] Array size
               )
        : m_arrLocks(nullptr), m_nCapacity(nCapacity)
    {
    ...
    */
    lock_array(size_t nCapacity,                ///< [in] Array size
        ...
        : m_arrLocks(nullptr), m_nCapacity(nCapacity), m_SelectCellPolicy(policy)
    {
    ...
    */
    lock_array(size_t nCapacity,           ///< [in] Array size
        ...
        : m_arrLocks(nullptr), m_nCapacity(nCapacity), m_SelectCellPolicy(std::forward<select_cell_policy>(policy))
    {
        m_arrLocks = create_lock_array(m_nCapacity);
    }
    ...
    */
    ...
    size_t try_lock(Q const &hint)
    {
        size_t nCell = m_SelectCellPolicy(hint, size());
        assert(nCell < size());
        if (m_arrLocks[nCell].try_lock()) return nCell;
    ...
    void lock_all()
    {
        ...
        for (lock_type *pEnd = m_arrLocks + size(); pLock != pEnd; ++pLock) pLock->lock();
    }
    ...
    {
        lock_type *pLock = m_arrLocks;
        for (lock_type *pEnd = m_arrLocks + size(); pLock != pEnd; ++pLock) pLock->unlock();
    }
...
class unique_lock<cds::sync::lock_array<Lock, SelectPolicy, Alloc> > {
  ...
  public:
    ...
    template <typename Q>
    unique_lock(lock_array_type &arrLocks, Q const &hint) : m_arrLocks(arrLocks), m_nLockGuarded(arrLocks.lock(hint))
    {
    }

    /// Locks all from \p arrLocks array
    unique_lock(lock_array_type &arrLocks) : m_arrLocks(arrLocks), m_nLockGuarded(c_nLockAll)
    {
        arrLocks.lock_all();
    }
