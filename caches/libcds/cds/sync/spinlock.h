  ...
  public:
    ...
    */
    bool try_lock() noexcept
    {
        ...
        if (!bCurrent) CDS_TSAN_ANNOTATE_MUTEX_ACQUIRED(this);
#else
        bool bCurrent = m_spin.exchange(true, atomics::memory_order_acquire);
#endif

        CDS_DEBUG_ONLY(if (!bCurrent) { m_dbgOwnerId = OS::get_current_thread_id(); })
        return !bCurrent;
    }
    ...
    */
    ...
    void lock() noexcept(noexcept(backoff_strategy()()))
    {
        ...
        // TATAS algorithm
        while (!try_lock()) {
            while (m_spin.load(atomics::memory_order_acquire)) backoff();
        }
...
class reentrant_spin_lock {
  ...
  private:
    ...
    atomics::atomic<thread_id>
        m_OwnerId; ///< Owner thread id. If spin-lock is not locked it usually equals to \p OS::c_NullThreadId
                   //@endcond
  ...
  private:
    ...
    bool try_acquire() noexcept
    {
        ...
        bool bRet =
            m_spin.compare_exchange_weak(nCurrent, 1, atomics::memory_order_acquire, atomics::memory_order_acquire);

#ifdef CDS_THREAD_SANITIZER_ENABLED
        if (bRet) CDS_TSAN_ANNOTATE_MUTEX_ACQUIRED(this);
#endif
    ...
    void acquire() noexcept(noexcept(backoff_strategy()()))
    {
        ...
        backoff_strategy bkoff;
        while (!try_acquire()) {
            while (m_spin.load(atomics::memory_order_acquire)) bkoff();
        }
    }
    //@endcond
  ...
  public:
    ...
    reentrant_spin_lock() noexcept : m_spin(0), m_OwnerId(OS::c_NullThreadId)
    {
        CDS_TSAN_ANNOTATE_MUTEX_CREATE(this);
    }
    ...
    */
    reentrant_spin_lock(const reentrant_spin_lock<Integral, Backoff> &) noexcept
        : m_spin(0), m_OwnerId(OS::c_NullThreadId)
    {
        CDS_TSAN_ANNOTATE_MUTEX_CREATE(this);
    }

    /// Construct object in specified state
    explicit reentrant_spin_lock(bool bLocked) : m_spin(0), m_OwnerId(OS::c_NullThreadId)
    {
        CDS_TSAN_ANNOTATE_MUTEX_CREATE(this);
        if (bLocked) lock();
    }
    ...
    */
    ...
    bool try_lock() noexcept(noexcept(std::declval<reentrant_spin_lock>().try_acquire()))
    {
        ...
        if (try_taken_lock(tid)) return true;
        if (try_acquire()) {
            take(tid);
            return true;
        }
