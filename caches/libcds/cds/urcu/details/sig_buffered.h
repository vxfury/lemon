...
/**
    ...
    The \p Buffer contains items of \ref cds_urcu_retired_ptr "retired_ptr" type and it should support a queue interface
   with three function:
...
class signal_buffered : public details::sh_singleton<signal_buffered_tag> {
  ...
  protected:
    ...
    signal_buffered(size_t nBufferCapacity, int nSignal = SIGUSR1)
        : base_class(nSignal), m_Buffer(nBufferCapacity), m_nCurEpoch(0), m_nCapacity(nBufferCapacity)
    {
    }
  ...
  public:
    ...
    */
    ...
    {
        if (!singleton_ptr::s_pRCU) singleton_ptr::s_pRCU = new signal_buffered(nBufferCapacity, nSignal);
    }
    ...
    */
    ...
    bool synchronize(epoch_retired_ptr &ep)
    {
        uint64_t nEpoch;
        {
            ...
            if (ep.m_p && m_Buffer.push(ep) && m_Buffer.size() < capacity()) return false;
