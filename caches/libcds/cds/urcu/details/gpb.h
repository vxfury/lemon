...
/**
    ...
    The \p Buffer contains items of \ref cds_urcu_retired_ptr "epoch_retired_ptr" type and it should support a queue
   interface with three function:
...
*/
template <class Buffer = cds::container::VyukovMPMCCycleQueue<epoch_retired_ptr>, class Lock = std::mutex,
...
class general_buffered : public details::gp_singleton<general_buffered_tag> {
  ...
  protected:
    //@cond
    general_buffered(size_t nBufferCapacity) : m_Buffer(nBufferCapacity), m_nCurEpoch(0), m_nCapacity(nBufferCapacity)
    {
    }
  ...
  public:
    ...
    */
    ...
    {
        if (!singleton_ptr::s_pRCU) singleton_ptr::s_pRCU = new general_buffered(nBufferCapacity);
    }
    ...
    static void Destruct(bool bDetachAll = false)
    {
        if (isUsed()) {
            ...
            if (bDetachAll) instance()->m_ThreadList.detach_all();
            delete instance();
  ...
  public:
    ...
    */
    ...
    {
        if (p.m_p) push_buffer(epoch_retired_ptr(p, m_nCurEpoch.load(atomics::memory_order_relaxed)));
    }
    ...
    bool synchronize(epoch_retired_ptr &ep)
    {
        uint64_t nEpoch;
        {
            ...
            if (ep.m_p && m_Buffer.push(ep)) return false;
