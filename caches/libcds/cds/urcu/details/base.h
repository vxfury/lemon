...
/** @ingroup cds_garbage_collector
    ...
    \p synchronize, and corresponding RCU thread-specific interface like \p access_lock, \p access_unlock and \p
   retire_ptr.
    ...
    - \ref cds_urcu_general_buffered_gc "gc<general_buffered>" - general purpose RCU with deferred (buffered)
   reclamation, include file <tt><cds/urcu/general_buffered.h></tt>
...
struct thread_list_record {
    ...
    atomics::atomic<OS::ThreadId> thread_id_{
        cds::OS::c_NullThreadId}; ///< Owner thread id; 0 - the record is free (not owned)

    thread_list_record() = default;

    explicit thread_list_record(OS::ThreadId owner) : thread_id_(owner) {}
  ...
  public:
    ...
    thread_record *alloc()
    {
        ...
        for (pRec = m_pHead.load(atomics::memory_order_acquire); pRec; pRec = pRec->m_list.next_) {
            ...
            if (!pRec->m_list.thread_id_.compare_exchange_strong(thId, curThreadId, atomics::memory_order_acquire,
        ...
        do {
            ...
            CDS_COMPILER_RW_BARRIER;
        } while (!m_pHead.compare_exchange_weak(pOldHead, pRec, atomics::memory_order_release,
...
struct epoch_retired_ptr : public retired_ptr {
    ...
    epoch_retired_ptr(retired_ptr const &rp, uint64_t nEpoch) : retired_ptr(rp), m_nEpoch(nEpoch) {}
};
