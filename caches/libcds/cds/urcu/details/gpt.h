...
/**
    ...
    the RCU \p synchronize() function is called that waits until all reader/updater threads end up their read-side
   critical sections, i.e. until the RCU quiescent state will come. After that the "work ready" message is sent to
   reclamation thread. The reclamation thread frees the buffer. This synchronization cycle may be called in any thread
   that calls \p retire_ptr() function.
...
*/
template <class Buffer = cds::container::VyukovMPSCCycleQueue<epoch_retired_ptr>, class Lock = std::mutex,
          class DisposerThread = dispose_thread<Buffer>, class Backoff = cds::backoff::Default>
class general_threaded : public details::gp_singleton<general_threaded_tag> {
  ...
  protected:
    //@cond
    general_threaded(size_t nBufferCapacity) : m_Buffer(nBufferCapacity), m_nCurEpoch(1), m_nCapacity(nBufferCapacity)
    {
    }
    ...
    */
    ...
    static void Destruct(bool bDetachAll = false)
    {
        if (isUsed()) {
            ...
            if (bDetachAll) pThis->m_ThreadList.detach_all();
  ...
  public:
    ...
    */
    ...
    {
        if (p.m_p) push_buffer(epoch_retired_ptr(p, m_nCurEpoch.load(atomics::memory_order_acquire)));
    }
