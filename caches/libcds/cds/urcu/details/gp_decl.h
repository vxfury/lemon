...
template <>
CDS_EXPORT_API singleton_vtbl *gp_singleton_instance<general_instant_tag>::s_pRCU;
template <>
CDS_EXPORT_API singleton_vtbl *gp_singleton_instance<general_buffered_tag>::s_pRCU;
template <>
CDS_EXPORT_API singleton_vtbl *gp_singleton_instance<general_threaded_tag>::s_pRCU;
#endif
...
#define CDS_GP_RCU_DECLARE_THREAD_GC(tag_)              \
    template <>                                         \
    class thread_gc<tag_> : public gp_thread_gc<tag_> { \
    }
...
class gp_singleton : public singleton_vtbl {
  ...
  protected:
    gp_singleton() : m_nGlobalControl(1) {}
...
#define CDS_GP_RCU_DECLARE_SINGLETON(tag_)                             \
    ...
    class singleton<tag_> {                                            \
      ...
      public:                                                          \
        static bool isUsed()                                           \
        {                                                              \
            return rcu_singleton::isUsed();                            \
        }                                                              \
        static rcu_singleton *instance()                               \
        {                                                              \
            assert(rcu_instance::s_pRCU);                              \
            return static_cast<rcu_singleton *>(rcu_instance::s_pRCU); \
        }                                                              \
        static thread_record *attach_thread()                          \
        {                                                              \
            return instance()->attach_thread();                        \
        }                                                              \
        static void detach_thread(thread_record *pRec)                 \
        {                                                              \
            return instance()->detach_thread(pRec);                    \
        }                                                              \
        static uint32_t global_control_word(atomics::memory_order mo)  \
        {                                                              \
            return instance()->global_control_word(mo);                \
        }                                                              \
    }
