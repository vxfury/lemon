...
template <>
CDS_EXPORT_API singleton_vtbl *sh_singleton_instance<signal_buffered_tag>::s_pRCU;
#endif
...
#define CDS_SH_RCU_DECLARE_THREAD_GC(tag_)              \
    template <>                                         \
    class thread_gc<tag_> : public sh_thread_gc<tag_> { \
    }
...
class sh_singleton : public singleton_vtbl {
  ...
  protected:
    sh_singleton(int nSignal) : m_nGlobalControl(1), m_nSigNo(nSignal)
    {
        set_signal_handler();
    }
