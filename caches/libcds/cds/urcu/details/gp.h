...
{
    if (!threading::Manager::isThreadAttached()) cds::threading::Manager::attachThread();
}
...
inline void gp_thread_gc<RCUtag>::access_lock()
{
    ...
    if ((tmp & rcu_class::c_nNestMask) == 0) {
        pRec->m_nAccessControl.store(
            gp_singleton<RCUtag>::instance()->global_control_word(atomics::memory_order_relaxed),
...
inline void gp_singleton<RCUtag>::flip_and_wait(Backoff &bkoff)
{
    ...
    for (thread_record *pRec = m_ThreadList.head(atomics::memory_order_acquire); pRec; pRec = pRec->m_list.next_) {
        while (pRec->m_list.thread_id_.load(atomics::memory_order_acquire) != nullThreadId
               && check_grace_period(pRec)) {
            bkoff();
            CDS_COMPILER_RW_BARRIER;
        }
        bkoff.reset();
