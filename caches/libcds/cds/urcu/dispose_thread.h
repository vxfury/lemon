...
class dispose_thread {
  ...
  private:
    ...
    class dispose_thread_starter : public thread_type {
      ...
      public:
        dispose_thread_starter(dispose_thread *pThis) : thread_type(thread_func, pThis) {}
    };
    ...
    void execute()
    {
        ...
        while (!bQuit) {
...

            {
                ...
                while ((pBuffer = m_pBuffer.load(atomics::memory_order_relaxed)) == nullptr) m_cvDataReady.wait(lock);
            ...
            if (pBuffer) dispose_buffer(pBuffer, nCurEpoch);
        }
    }
  ...
  public: // methods called from any thread
    //@cond
    dispose_thread() : m_pBuffer(nullptr) {}
    //@endcond
    ...
    {
        {
            ...
            while (!m_bReady) m_cvReady.wait(lock);
