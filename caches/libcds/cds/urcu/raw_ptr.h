...
class raw_ptr {
  ...
  public:
    ...
    /// Move ctor
    raw_ptr(raw_ptr &&p) : m_ptr(p.m_ptr), m_Enum(std::move(p.m_Enum))
    {
        p.m_ptr = nullptr;
    }
    ...
    raw_ptr(value_type *p, reclaimed_enumerator &&e) : m_ptr(p), m_Enum(std::move(e)) {}
    raw_ptr(reclaimed_enumerator &&e) : m_ptr(nullptr), m_Enum(std::move(e)) {}
    //@endcond
...
template <typename ValueType, typename RawPtr, typename Converter>
class raw_ptr_adaptor : private RawPtr {
  ...
  public:
    // Constructs an empty raw pointer
    raw_ptr_adaptor() : intrusive_raw_ptr() {}

    // Move ctor
    raw_ptr_adaptor(intrusive_raw_ptr &&p) : intrusive_raw_ptr(std::move(p)) {}

    // Move ctor
    raw_ptr_adaptor(raw_ptr_adaptor &&p) : intrusive_raw_ptr(std::move(p)) {}
