...
/**
    ...
    The destructor (and \p release() member function) invokes <tt>RCU::retire_ptr< Disposer >()</tt> function to dispose
   the node. For non-intrusive containers from \p cds::container namespace \p Disposer is usually an invocation of node
   deallocator. For intrusive containers the disposer can be empty or it can trigger an event "node can be reused
   safely". In any case, the exempt pointer concept keeps RCU semantics. The destructor or \p release() should be called
   outside the RCU lock of current thread.

    You don't need use this helper class directly. Any RCU-based container typedefs a simplified version of this
   template.
...
*/
template <class RCU, typename NodeType, typename ValueType, typename Disposer,
...
class exempt_ptr {
  ...
  public:
    ...
    exempt_ptr() noexcept : m_pNode(nullptr) {}

    //@cond
    /// Creates exempt pointer for \p pNode. Only for internal use.
    explicit exempt_ptr(node_type *pNode) noexcept : m_pNode(pNode) {}
    explicit exempt_ptr(std::nullptr_t) noexcept : m_pNode(nullptr) {}
    //@endcond

    /// Move ctor
    exempt_ptr(exempt_ptr &&p) noexcept : m_pNode(p.m_pNode)
    {
        p.m_pNode = nullptr;
    }
