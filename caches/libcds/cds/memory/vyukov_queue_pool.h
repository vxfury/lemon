...
class vyukov_queue_pool {
  ...
  public:
    ...
    */
    vyukov_queue_pool(size_t nCapacity = 0) : m_Queue(nCapacity)
    {
        preallocate_pool();
    }
...
class lazy_vyukov_queue_pool {
  ...
  public:
    ...
    lazy_vyukov_queue_pool(size_t nCapacity = 0) : m_Queue(nCapacity) {}

    /// Deallocates all objects from the pool
    ~lazy_vyukov_queue_pool()
    {
        std_allocator a;
        while (!m_Queue.empty()) a.deallocate(m_Queue.pop(), 1);
    }
    ...
    */
    void deallocate(value_type *p, size_t n)
    {
        ...
        if (p) {
            ...
            if (!m_Queue.push(*p)) std_allocator().deallocate(p, 1);
        }
    }
};
...
class bounded_vyukov_queue_pool {
  ...
  protected:
    ...
    void preallocate_pool()
    {
        ...
        for (value_type *p = m_pFirst; p < m_pLast; ++p) CDS_VERIFY(m_Queue.push(*p)); // must be true
    }
  ...
  public:
    ...
    /**
        ...
        See \p cds::intrusive::VyukovMPMCCycleQueue for explanation.
    */
    bounded_vyukov_queue_pool(size_t nCapacity = 0) : m_Queue(nCapacity)
    {
        preallocate_pool();
    }
