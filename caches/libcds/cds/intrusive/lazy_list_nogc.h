...
template <
    ...
    typename Lock = cds::sync::spin, typename Tag = opt::none
#else
    typename Lock, typename Tag
#endif
    >
...
class LazyList<gc::nogc, T, Traits> {
...
#else
    typedef
        typename std::conditional<c_bSort, typename opt::details::make_comparator<value_type, traits>::type,
                                  typename opt::details::make_equal_to<value_type, traits>::type>::type key_comparator;
#endif
  ...
  protected:
    ...
    class auto_lock_position {
      ...
      public:
        auto_lock_position(position &pos) : m_pos(pos)
        {
            pos.lock();
        }
        ~auto_lock_position()
  ...
  protected:
    ...
    {
        if (insert_at(pHead, val)) return iterator(node_traits::to_node_ptr(val));
        return end();
    }
