...
/**
     This option reserves additional space in the hook to store the hash value of the object once it's introduced in the
   container. When this option is used, the unordered container will store the calculated hash value in the hook and
   rehashing operations won't need to recalculate the hash of the value. This option will improve the performance of
   unordered containers when rehashing is frequent or hashing the value is a slow operation
...
/**
    ...
    Template arguments:
    - \p RecursiveLock - the type of recursive mutex. The default is \p std::recursive_mutex. The mutex type should be
   default-constructible. Note that a recursive spin-lock is not suitable for lock striping for performance reason.
...
*/
template <class RecursiveLock = std::recursive_mutex, unsigned int Arity = 2, class Alloc = CDS_DEFAULT_ALLOCATOR,
...
class striping {
  public:
    ...
    typedef cds::sync::lock_array<lock_type, cds::sync::pow2_select_policy, allocator_type>
        lock_array_type; ///< lock array type
    ...
    class lock_array : public lock_array_type {
      public:
        ...
        lock_array(size_t nCapacity)
            : lock_array_type(nCapacity, typename lock_array_type::select_cell_policy(nCapacity))
        {
        }
    };
  ...
  public:
    ...
    class scoped_cell_lock {
      ...
      public:
        ...
        ~scoped_cell_lock()
        {
            for (unsigned int i = 0; i < c_nArity; ++i) m_guard[i]->unlock();
        }
    };
    ...
    class scoped_full_lock {
      ...
      public:
        scoped_full_lock(striping &policy) : m_guard(policy.m_Locks[0])
        {
            policy.m_Stat.onFullLock();
        }

        /// Ctor for scoped_resize_lock - no statistics is incremented
        scoped_full_lock(striping &policy, bool) : m_guard(policy.m_Locks[0]) {}
    };

    class scoped_resize_lock : public scoped_full_lock {
      public:
        scoped_resize_lock(striping &policy) : scoped_full_lock(policy, false)
        {
  ...
  public:
    /// Constructor
    striping(size_t nLockCount ///< The size of lock array. Must be power of two.
    )
    {
...
struct refinable_stat {
    ...
    counter_type
        m_nSecondCellLockFailed; ///< Count of unsuccess obtaining cell lock when another cell is already locked
    ...
    void onCellWaitResizing()
    {
        ++m_nCellLockWaitResizing;
    }
    void onCellArrayChanged()
    {
        ++m_nCellLockArrayChanged;
    }
    void onCellLockFailed()
    {
        ++m_nCellLockFailed;
    }
    void onSecondCellLock()
    {
        ++m_nSecondCellLockCount;
    }
    void onSecondCellLockFailed()
    {
        ++m_nSecondCellLockFailed;
    }
    void onFullLock()
    {
        ++m_nFullLockCount;
    ...
    void onResizeLockIter()
    {
        ++m_nResizeLockIter;
    }
    void onResizeLockArrayChanged()
    {
...
/**
    ...
    - \p Stat - internal statistics type. Note that this template argument is automatically selected by \ref CuckooSet
        class according to its \p opt::stat option.
*/
template <class RecursiveLock = std::recursive_mutex, unsigned int Arity = 2, typename BackOff = cds::backoff::Default,
          class Alloc = CDS_DEFAULT_ALLOCATOR, class Stat = empty_refinable_stat>
class refinable {
  public:
    typedef RecursiveLock lock_type;            ///< lock type
    typedef Alloc allocator_type;               ///< allocator type
  ...
  protected:
    ...
    class lock_array_type : public cds::sync::lock_array<lock_type, lock_selection_policy, allocator_type>,
      ...
      public:
        lock_array_type(size_t nCapacity) : lock_array_base(nCapacity) {}
    };
  ...
  protected:
        ...
        while (true) {
            {
                ...
                for (unsigned int i = 0; i < c_nArity; ++i) pLockArr[i] = m_arrLocks[i];
            ...
            while (true) {
                ...
                if (!(who & 1) || (who >> 1) == (me & c_nOwnerMask)) break;
                bkoff();
            ...
            if (cur_capacity == m_nCapacity.load(atomics::memory_order_acquire)) {
                ...
                for (unsigned int i = 0; i < c_nArity; ++i) parrLock[i]->unlock();

                m_Stat.onCellLockFailed();
            } else
                m_Stat.onCellArrayChanged();

            // clears pLockArr can lead to calling dtor for each item of pLockArr[i] that may be a heavy-weighted
            // operation (each pLockArr[i] is a shared pointer to array of a ton of mutexes) It is better to do this
            // before the next loop iteration where we will use spin-locked assignment to pLockArr However, destructing
            // a lot of mutexes under spin-lock is a bad solution
            for (unsigned int i = 0; i < c_nArity; ++i) pLockArr[i].reset();
        }
    }
    ...
    void acquire_all()
    {
        ...
        while (true) {
            owner_t ownNull = 0;
            if (m_Owner.compare_exchange_strong(ownNull, (me << 1) | 1, atomics::memory_order_acq_rel,
    ...
    void acquire_resize(lock_array_ptr *pOldLocks)
    {
        ...
        while (true) {
            {
                scoped_spinlock sl(m_access);
                for (unsigned int i = 0; i < c_nArity; ++i) pOldLocks[i] = m_arrLocks[i];
                cur_capacity = m_nCapacity.load(atomics::memory_order_acquire);
            }
            ...
            // clears pOldLocks can lead to calling dtor for each item of pOldLocks[i] that may be a heavy-weighted
            // operation (each pOldLocks[i] is a shared pointer to array of a ton of mutexes) It is better to do this
            // before the next loop iteration where we will use spin-locked assignment to pOldLocks However, destructing
            // a lot of mutexes under spin-lock is a bad solution
            for (unsigned int i = 0; i < c_nArity; ++i) pOldLocks[i].reset();
        }
    }
  ...
  public:
    ...
    class scoped_cell_lock {
      ...
      public:
        ...
        ~scoped_cell_lock()
        {
            for (unsigned int i = 0; i < c_nArity; ++i) m_arrLock[i]->unlock();
        }
    };
    ...
    class scoped_full_lock {
      ...
      public:
        scoped_full_lock(refinable &policy) : m_policy(policy)
        {
            policy.acquire_all();
        }
        ~scoped_full_lock()
    ...
    class scoped_resize_lock {
      ...
      public:
        scoped_resize_lock(refinable &policy) : m_policy(policy)
        {
  ...
  public:
    /// Constructor
    refinable(size_t nLockCount ///< The size of lock array. Must be power of two.
              )
        : m_Owner(0), m_nCapacity(nLockCount)
    {
        assert(cds::beans::is_power2(nLockCount));
        for (unsigned int i = 0; i < c_nArity; ++i) m_arrLocks[i] = create_lock_array(nLockCount);
    }
    ...
    void resize(size_t nCapacity)
    {
        ...
        for (unsigned int i = 0; i < c_nArity; ++i) pNew[i] = create_lock_array(nCapacity);

        {
            scoped_spinlock sl(m_access);
            m_nCapacity.store(nCapacity, atomics::memory_order_release);
            for (unsigned int i = 0; i < c_nArity; ++i) m_arrLocks[i] = pNew[i];
        }
...
struct stat {
    ...
    counter_type
        m_nFalseResizeCount; ///< Count of false \p resize() function call (when other thread has been resized the set)
    ...
    void onRelocateCall()
    {
        ++m_nRelocateCallCount;
    }
    void onRelocateRound()
    {
        ++m_nRelocateRoundCount;
    }
    void onFalseRelocateRound()
    {
        ++m_nFalseRelocateCount;
    }
    void onSuccessRelocateRound()
    {
        ++m_nSuccessRelocateCount;
    }
    void onRelocateAboveThresholdRound()
    {
        ++m_nRelocateAboveThresholdCount;
    }
    void onFailedRelocate()
    {
    ...
    void onFalseResizeCall()
    {
        ++m_nFalseResizeCount;
    }
    void onResizeSuccessMove()
    {
        ++m_nResizeSuccessNodeMove;
    }
    void onResizeRelocateCall()
    {
        ++m_nResizeRelocateCall;
    }
    ...
    void onInsertResize()
    {
        ++m_nInsertResizeCount;
    }
    void onInsertRelocate()
    {
        ++m_nInsertRelocateCount;
    }
    void onInsertRelocateFault()
    {
        ++m_nInsertRelocateFault;
    }
    ...
    {
        ++m_nUpdateSuccessCount;
    }
    void onUpdateResize()
    {
        ++m_nUpdateResizeCount;
    }
    void onUpdateRelocate()
    {
        ++m_nUpdateRelocateCount;
    }
    void onUpdateRelocateFault()
    {
        ++m_nUpdateRelocateFault;
    }
    ...
    void onFindWithSuccess()
    {
        ++m_nFindWithSuccess;
    }
    void onFindWithFailed()
    {
        ++m_nFindWithFailed;
    }
    //@endcond
...
/**
    ...
    - \p opt::hash - hash functor tuple, mandatory option. At least, two hash functors should be provided. All hash
   functor should be orthogonal (different): for each <tt> i,j: i != j => h[i](x) != h[j](x) </tt>. The hash functors
   are passed as <tt> std::tuple< H1, H2, ... Hn > </tt>. The number of hash functors specifies the number \p k - the
   count of hash tables in cuckoo hashing.
...
template <typename... Options>
struct make_traits {
    typedef typename cds::opt::make_options<typename cds::opt::find_type_traits<cuckoo::traits, Options...>::type,
                                            Options...>::type type; ///< Result of metafunction
};
...
class bucket_entry<Node, cuckoo::list> {
  ...
  public:
    class iterator {
      ...
      public:
        ...
        iterator(node_type *p) : pNode(p) {}
        iterator(iterator const &it) : pNode(it.pNode) {}
  ...
  public:
    bucket_entry() : pHead(nullptr), nSize(0)
    {
        static_assert((std::is_same<typename node_type::probeset_type, probeset_type>::value),
...
class bucket_entry<Node, cuckoo::vector<Capacity>> {
  ...
  protected:
    ...
    void shift_up(unsigned int nFrom)
    {
        ...
        if (nFrom < m_nSize) std::copy_backward(m_arrNode + nFrom, m_arrNode + m_nSize, m_arrNode + m_nSize + 1);
    }
  ...
  public:
    class iterator {
      ...
      public:
        iterator() : pArr(nullptr) {}
        iterator(node_type **p) : pArr(p) {}
        iterator(iterator const &it) : pArr(it.pArr) {}
...
struct contains<NodeTraits, true> {
    ...
    static bool find(BucketEntry &probeset, Position &pos, unsigned int /*nTable*/, size_t /*nHash*/, Q const &val,
                     Compare cmp)
    {
        // Ordered version
...
struct contains<NodeTraits, false> {
    ...
    static bool find(BucketEntry &probeset, Position &pos, unsigned int nTable, size_t nHash, Q const &val, EqualTo eq)
    {
        ...
        for (bucket_iterator it = probeset.begin(), itEnd = probeset.end(); it != itEnd; ++it) {
            if (hash_ops<node_type, node_type::hash_array_size>::equal_to(*it, nTable, nHash)
                && eq(*NodeTraits::to_value_ptr(*it), val)) {
                pos.itFound = it;
                pos.itPrev = itPrev;
                return true;
            }
...
/** @ingroup cds_intrusive_map
    ...
    <b>About Cuckoo hashing</b>
        ...
        <a href="https://en.wikipedia.org/wiki/Cuckoo_hashing">Cuckoo hashing</a> is a hashing algorithm in which a
   newly added item displaces any earlier item occupying the same slot. For brevity, a table is a k-entry array of
   items. For a hash set of size N = 2k we use a two-entry array of tables, and two independent hash functions, <tt> h0,
   h1: KeyRange -> 0,...,k-1</tt> mapping the set of possible keys to entries in he array. To test whether a value \p x
   is in the set, <tt>find(x)</tt> tests whether either <tt>table[0][h0(x)]</tt> or <tt>table[1][h1(x)]</tt> is equal to
   \p x. Similarly, <tt>erase(x)</tt>checks whether \p x is in either <tt>table[0][h0(x)]</tt> or
   <tt>table[1][h1(x)]</tt>, ad removes it if found.
...
class CuckooSet {
  public:
    ...
    typedef typename traits::mutex_policy
        original_mutex_policy; ///< Concurrent access policy, see \p cuckoo::traits::mutex_policy

    //@cond
    typedef typename original_mutex_policy::template rebind_statistics<typename std::conditional<
        std::is_same<stat, cuckoo::empty_stat>::value, typename original_mutex_policy::empty_stat,
        typename original_mutex_policy::real_stat>::type>::other mutex_policy;
    //@endcond
    ...
    */
    ...
    static size_t const c_nArity =
        hash::size; ///< the arity of cuckoo hashing: the number of hash functors provided; minimum 2.
  ...
  protected:
    ...
    using predicate_wrapper =
        typename std::conditional<c_isSorted, cds::opt::details::make_comparator_from_less<Predicate>, Predicate>::type;
  ...
  protected:
    ...
    {
        static_assert((c_nArity == mutex_policy::c_nArity),
                      "The count of hash functors must be equal to mutex_policy arity");
    }
    ...
    void allocate_bucket_tables(size_t nSize)
    {
        ...
        for (unsigned int i = 0; i < c_nArity; ++i) m_BucketTable[i] = alloc.NewArray(nSize);
    }
    ...
    unsigned int contains(position *arrPos, size_t *arrHash, Q const &val, Predicate pred)
    {
        ...
        for (unsigned int i = 0; i < c_nArity; ++i) {
            ...
            if (contains_action::find(probeset, arrPos[i], i, arrHash[i], val, pred)) return i;
        }
    ...
    bool relocate(unsigned int nTable, size_t *arrGoalHash)
    {
        ...
        for (unsigned int nRound = 0; nRound < c_nRelocateLimit; ++nRound) {
            ...
            while (true) {
                ...
                if (!guard2.locked()) continue; // try one more time
  ...
  public:
    ...
    */
    CuckooSet(size_t nInitialSize ///< Initial set size; if 0 - use default initial size \p c_nDefaultInitialSize
              ,
              unsigned int nProbesetSize ///< probe set size
              ,
              unsigned int nProbesetThreshold = 0 ///< probe set threshold, <tt>nProbesetThreshold < nProbesetSize</tt>.
                                                  ///< If 0, <tt>nProbesetThreshold = nProbesetSize - 1</tt>
              )
    ...
    */
    CuckooSet(hash_tuple_type const &h ///< hash functor tuple of type <tt>std::tuple<H1, H2, ... Hn></tt> where <tt> n
                                       ///< == \ref c_nArity </tt>
              )
        : m_nProbesetSize(calc_probeset_size(0)),
          m_nProbesetThreshold(m_nProbesetSize - 1),
    ...
    */
    CuckooSet(size_t nInitialSize ///< Initial set size; if 0 - use default initial size \p c_nDefaultInitialSize
              ...
              ,
              unsigned int nProbesetThreshold ///< probe set threshold, <tt>nProbesetThreshold < nProbesetSize</tt>. If
                                              ///< 0, <tt>nProbesetThreshold = nProbesetSize - 1</tt>
              ,
              hash_tuple_type const &h ///< hash functor tuple of type <tt>std::tuple<H1, H2, ... Hn></tt> where <tt> n
                                       ///< == \ref c_nArity </tt>
              )
        : m_nProbesetSize(calc_probeset_size(nProbesetSize)),
          m_nProbesetThreshold(nProbesetThreshold ? nProbesetThreshold : m_nProbesetSize - 1),
    ...
    /**
        The probe set size and threshold are set as default, see \p CuckooSet()
    */
    CuckooSet(hash_tuple_type &&h ///< hash functor tuple of type <tt>std::tuple<H1, H2, ... Hn></tt> where <tt> n ==
                                  ///< \ref c_nArity </tt>
              )
        : m_nProbesetSize(calc_probeset_size(0)),
    ...
    */
    CuckooSet(size_t nInitialSize ///< Initial set size; if 0 - use default initial size \p c_nDefaultInitialSize
              ...
              unsigned int nProbesetThreshold ///< probe set threshold, <tt>nProbesetThreshold < nProbesetSize</tt>. If
                                              ///< 0, <tt>nProbesetThreshold = nProbesetSize - 1</tt>
              ,
              hash_tuple_type &&h ///< hash functor tuple of type <tt>std::tuple<H1, H2, ... Hn></tt> where <tt> n ==
                                  ///< \ref c_nArity </tt>
              )
        : m_nProbesetSize(calc_probeset_size(nProbesetSize)),
          m_nProbesetThreshold(nProbesetThreshold ? nProbesetThreshold : m_nProbesetSize - 1),
  ...
  public:
    ...
    */
    ...
    /**
        ...
        If you use unordered cuckoo set, then \p Predicate should have the interface and semantics like \p
       std::equal_to. \p Predicate must imply the same element order as the comparator used for building the set.
    */
    template <typename Q, typename Predicate, typename Func>
    ...
    */
    ...
    /**
        ...
        If you use ordered cuckoo set, then \p Predicate should have the interface and semantics like \p std::less.
        If you use unordered cuckoo set, then \p Predicate should have the interface and semantics like \p
       std::equal_to. \p pred must imply the same element order as the comparator used for building the set.
    */
    template <typename Q, typename Predicate, typename Func>
    ...
    void clear_and_dispose(Disposer oDisposer)
    {
        ...
        for (unsigned int i = 0; i < c_nArity; ++i) {
            ...
            for (; pEntry != pEnd; ++pEntry) {
                pEntry->clear([&oDisposer](node_type *pNode) {
                    oDisposer(node_traits::to_value_ptr(pNode));
                });
            }
        }
        m_ItemCounter.reset();
