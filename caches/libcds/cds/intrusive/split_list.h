...
/** @ingroup cds_intrusive_map
    ...
    - \p OrderedList - ordered list implementation used as a bucket for hash set, for example, \p MichaelList, \p
   LazyList. The intrusive ordered list implementation specifies the type \p T stored in the split-list set, the
   comparison functor for the type \p T and other features specific for the ordered list.
    - \p Traits - split-list traits, default is \p split_list::traits.
        Instead of defining \p Traits struct you can use option-based syntax provided by \p split_list::make_traits
   metafunction.
...
class SplitListSet {
  ...
  public:
    ...
    typedef
        typename traits::bit_reversal bit_reversal; ///< Bit reversal algorithm, see \p split_list::traits::bit_reversal
  ...
  protected:
    ...
    typedef typename split_list::details::bucket_table_selector<
        traits::dynamic_bucket_table, gc, typename ordered_list_adapter::aux_node,
        opt::allocator<typename traits::allocator>, opt::memory_model<memory_model>,
        opt::free_list<typename traits::free_list> >::type bucket_table;
  ...
  protected:
    ...
    class ordered_list_wrapper : public ordered_list {
      ...
      public:
        ...
        template <typename Q>
        typename std::enable_if<std::is_same<Q, value_type>::value && is_iterable_list<ordered_list>::value,
        ...
        typename std::enable_if<std::is_same<Iterator, typename ordered_list::iterator>::value
                                    && is_iterable_list<ordered_list>::value,
                                bool>::type
        ...
        typename std::enable_if<std::is_same<Q, Q>::value && is_iterable_list<ordered_list>::value,
                                typename base_class::iterator>::type
  ...
  protected:
    ...
    class iterator_type : public split_list::details::iterator_type<node_traits, ordered_list, IsConst> {
      ...
      public:
        iterator_type() : iterator_base_class() {}

        iterator_type(iterator_type const &src) : iterator_base_class(src) {}

        // This ctor should be protected...
        iterator_type(list_iterator itCur, list_iterator itEnd) : iterator_base_class(itCur, itEnd) {}
    };
    //@endcond
  ...
  public:
    ...
    /**
        ...
        - iterator cannot be moved across thread boundary because it may contain GC's guard that is thread-private GC
       data.
  ...
  public:
    ...
    */
    SplitListSet() : m_nBucketCountLog2(1), m_nMaxItemCount(max_item_count(2, m_Buckets.load_factor()))
    {
        init();
    }
    ...
    ~SplitListSet()
    {
        ...
        m_List.destroy([](node_type *pNode) -> bool {
    ...
    */
    ...
    /**
        ...
        <b>for \p MichaelList, \p LazyList</b>
            ...
            @warning For \ref cds_intrusive_MichaelList_hp "MichaelList" as the bucket see \ref
       cds_intrusive_item_creating "insert item troubleshooting". \ref cds_intrusive_LazyList_hp "LazyList" provides
       exclusive access to inserted item and does not require any node-level synchronization.
  ...
  protected:
    ...
    aux_node_type *get_bucket(size_t nHash)
    {
        ...
        if (pHead == nullptr) pHead = init_bucket(nBucket);
    ...
    void inc_item_count()
    {
        ...
        if (++m_ItemCounter <= nMaxCount) return;
    ...
    bool find_(Q &val, Compare cmp, Func f)
    {
        ...
        return m_Stat.onFind(
            m_List.find_at(pHead, sv, cmp, [&f](value_type &item, split_list::details::search_value_type<Q> &v) {
                f(item, v.val);
            }));
    }
