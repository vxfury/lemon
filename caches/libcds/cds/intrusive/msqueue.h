...
/**
    ...
    - \p opt::hook - hook used. Possible hooks are: \p msqueue::base_hook, \p msqueue::member_hook, \p
   msqueue::traits_hook. If the option is not specified, \p %msqueue::base_hook<> is used.
   ...
   counting disabled) To enable item counting use \p cds::atomicity::item_counter
    - \p opt::stat - the type to gather internal statistics.
        Possible statistics types are: \p msqueue::stat, \p msqueue::empty_stat, user-provided class that supports \p
   %msqueue::stat interface. Default is \p %msqueue::empty_stat (internal statistics disabled).
...
/** @ingroup cds_intrusive_queue
    ...
    - [1998] Maged Michael, Michael Scott "Simple, fast, and practical non-blocking and blocking concurrent queue
   algorithms"

    Template arguments:
    - \p GC - garbage collector type: \p gc::HP, \p gc::DHP
    - \p T - type of value to be stored in the queue. A value of type \p T must be derived from \p msqueue::node for \p
   msqueue::base_hook, or it should have a member of type \p %msqueue::node for \p msqueue::member_hook, or it should be
   convertible to \p %msqueue::node for \p msqueue::traits_hook.
    ...
    The Michael & Scott's queue algo has a key feature: even if the queue is empty it contains one item that is "dummy"
   one from the standpoint of the algo. See \p dequeue() function for explanation.
...
class MSQueue {
  ...
  protected:
    ...
    bool do_dequeue(dequeue_result &res)
    {
        ...
        node_type *h;
        while (true) {
            h = res.guards.protect(0, m_pHead, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            });
            pNext = res.guards.protect(1, h->m_pNext, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            });
            if (m_pHead.load(memory_model::memory_order_acquire) != h) continue;
            ...
            if (m_pHead.compare_exchange_strong(h, pNext, memory_model::memory_order_acquire,
                                                atomics::memory_order_relaxed))
                break;
    ...
    void dispose_node(node_type *p)
    {
        ...
        if (p != &m_Dummy) gc::template retire<disposer_thunk>(node_traits::to_value_ptr(p));
    }
    //@endcond

  public:
    /// Initializes empty queue
    MSQueue() : m_pHead(&m_Dummy), m_pTail(&m_Dummy) {}
    ...
    */
    bool enqueue(value_type &val)
    {
        ...
        node_type *t;
        while (true) {
            t = guard.protect(m_pTail, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            });
        ...
        if (!m_pTail.compare_exchange_strong(t, pNew, memory_model::memory_order_release,
                                             atomics::memory_order_relaxed))
  ...
  Top ->|      Item 1      |  -> Dispose Item 1        |      Item 2      | <- Top
    ...
    */
    ...
    {
        typename gc::Guard guard;
        node_type *p = guard.protect(m_pHead, [](node_type *pNode) -> value_type * {
