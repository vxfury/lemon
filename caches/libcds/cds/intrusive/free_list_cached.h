...
class CachedFreeList {
  ...
  public:
    ...
    CachedFreeList()
    {
        for (auto &i : m_cache) i.store(nullptr, atomics::memory_order_relaxed);
    }
    ...
    */
    ...
    void put(node *pNode)
    {
        ...
        node *expect = nullptr;
        if (m_cache[get_hash()].compare_exchange_weak(expect, pNode, atomics::memory_order_release,
    ...
    node *get()
    {
        ...
        for (auto &item : m_cache) {
            ...
            if (p
                && item.compare_exchange_weak(p, nullptr, atomics::memory_order_acquire, atomics::memory_order_relaxed))
                return p;
        }
