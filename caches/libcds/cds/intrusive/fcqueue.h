...
struct traits : public cds::algo::flat_combining::traits {
    typedef cds::intrusive::opt::v::empty_disposer
        disposer;            ///< Disposer to erase removed elements. Used only in \p FCQueue::clear() function
...
#endif
{
  public:
    ...
    typedef typename traits::disposer
        disposer;                       ///< The disposer functor. The disposer is used only in \ref clear() function
  ...
  public:
    ...
    FCQueue(unsigned int nCompactFactor ///< Flat combining: publication list compacting factor
            ,
    ...
    */
    bool enqueue(value_type &val)
    {
        ...
        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_enq, pRec, *this);
        else m_FlatCombining.combine(op_enq, pRec, *this);
    ...
    */
    value_type *dequeue()
    {
        ...
        constexpr_if(c_bEliminationEnabled) m_FlatCombining.batch_combine(op_deq, pRec, *this);
        else m_FlatCombining.combine(op_deq, pRec, *this);
    ...
    */
    void clear(bool bDispose = false)
    {
        ...
        else m_FlatCombining.combine(bDispose ? op_clear_and_dispose : op_clear, pRec, *this);
    ...
    */
    ...
    {
        auto &queue = m_Queue;
        m_FlatCombining.invoke_exclusive([&queue, &f]() {
            f(queue);
        });
    }
    ...
    */
    bool empty() const
    {
        bool bRet = false;
        auto const &queue = m_Queue;
        m_FlatCombining.invoke_exclusive([&queue, &bRet]() {
            bRet = queue.empty();
        });
        return bRet;
    }
