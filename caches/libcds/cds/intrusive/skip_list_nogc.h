...
class node<cds::gc::nogc, Tag> {
  ...
  protected:
    ...
    atomic_ptr *m_arrNext;  ///< Array of next items for levels 1 .. m_nHeight - 1. For node at level 0 \p m_arrNext is
                            ///< \p nullptr
  ...
  public:
    ...
    {
        return m_pNext.load(atomics::memory_order_relaxed) == nullptr && m_arrNext == nullptr && m_nHeight <= 1;
    }
};
} // namespace skip_list
...
class iterator<cds::gc::nogc, NodeTraits, BackOff, IsConst> {
  ...
  public: // for internal use only!!!
    iterator(node_type &refHead) : m_pNode(refHead[0].load(atomics::memory_order_relaxed)) {}
  ...
  public:
    ...
    iterator &operator++()
    {
        if (m_pNode) m_pNode = m_pNode->next(0).load(atomics::memory_order_relaxed);
        return *this;
    }
  ...
  protected:
    ...
    bool insert_at_position(value_type &val, node_type *pNode, position &pos, Func f)
    {
...

        {
            ...
            if (!pos.pPrev[0]->next(0).compare_exchange_strong(p, pNode, memory_model::memory_order_release,
        ...
        for (unsigned int nLevel = 1; nLevel < nHeight; ++nLevel) {
            ...
            while (true) {
                ...
                if (pNode->next(nLevel).compare_exchange_strong(p, q, memory_model::memory_order_release,
                                                                memory_model::memory_order_relaxed)) {
                    p = q;
                    if (pos.pPrev[nLevel]->next(nLevel).compare_exchange_strong(
                            q, pNode, memory_model::memory_order_release, memory_model::memory_order_relaxed))
                        break;
                }
    ...
    void increase_height(unsigned int nHeight)
    {
        ...
        while (nCur < nHeight
               && !m_nHeight.compare_exchange_weak(nCur, nHeight, memory_model::memory_order_acquire,
  ...
  public:
    ...
    */
    ...
    value_type *contains(Q const &key) const
    {
        ...
        if (pNode) return node_traits::to_value_ptr(pNode);
        return nullptr;
    }
    //@cond
    template <typename Q>
    ...
    */
    ...
    value_type *contains(Q const &key, Less pred) const
    {
        CDS_UNUSED(pred);
        node_type *pNode =
            find_with_(key, cds::opt::details::make_comparator_from_less<Less>(), [](value_type &, Q const &) {
            });
        if (pNode) return node_traits::to_value_ptr(pNode);
        return nullptr;
    }
