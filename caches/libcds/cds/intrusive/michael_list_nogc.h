...
class MichaelList<gc::nogc, T, Traits> {
  ...
  protected:
    ...
    static bool link_node(node_type *pNode, position &pos)
    {
        ...
        if (cds_likely(pos.pPrev->compare_exchange_strong(pos.pCur, pNode, memory_model::memory_order_release,
  ...
  public:
    ...
    */
    ...
    void clear(Disposer disp)
    {
        ...
        do {
        } while (cds_unlikely(!m_pHead.compare_exchange_weak(pHead, nullptr, memory_model::memory_order_relaxed)));
  ...
  protected:
    ...
    {
        if (insert_at(refHead, val)) return iterator(node_traits::to_node_ptr(val));
        return end();
    }
