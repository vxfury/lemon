...
struct base_node<cds::urcu::gc<RCU> > : public basic_node {
    ...
    explicit base_node(bool bInternal) : basic_node(bInternal), m_pNextRetired(nullptr) {}
};
...
class EllenBinTree<cds::urcu::gc<RCU>, Key, T, Traits> {
  ...
  public:
    ...
    static constexpr const bool c_bExtractLockExternal =
        false; ///< Group of \p extract_xxx functions do not require external locking
    ...
    class retired_list {
      ...
      private:
        class forward_iterator {
          ...
          public:
            forward_iterator(retired_list const &l) : m_pUpdate(l.pUpdateHead), m_pNode(l.pNodeHead) {}

            forward_iterator() : m_pUpdate(nullptr), m_pNode(nullptr) {}
            ...
            cds::urcu::retired_ptr operator*()
            {
                ...
                if (m_pNode) {
                    if (m_pNode->is_leaf()) {
                        return cds::urcu::retired_ptr(
                            reinterpret_cast<void *>(node_traits::to_value_ptr(static_cast<leaf_node *>(m_pNode))),
                            free_leaf_node_void);
                    } else {
            ...
            void operator++()
            {
                ...
                if (m_pNode) m_pNode = m_pNode->m_pNextRetired;
            }
      ...
      public:
        retired_list() : pUpdateHead(nullptr), pNodeHead(nullptr) {}
  ...
  public:
    ...
    */
    void clear()
    {
        for (exempt_ptr ep = extract_min(); !ep.empty(); ep = extract_min()) ep.release();
    }
  ...
  protected:
    ...
    bool help_delete(update_desc *pOp, retired_list &rl)
    {
        ...
        if (pOp->dInfo.pParent->m_pUpdate.compare_exchange_strong(pUpdate, pMark, memory_model::memory_order_acquire,
                                                                  atomics::memory_order_relaxed)) {
            help_marked(pOp);
            retire_node(pOp->dInfo.pParent, rl);
            // For extract operations the leaf should NOT be disposed
            if (pOp->dInfo.bDisposeLeaf) retire_node(pOp->dInfo.pLeaf, rl);
    ...
    void help_marked(update_desc *pOp)
    {
        ...
        if (pOp->dInfo.bRightParent) {
            pOp->dInfo.pGrandParent->m_pRight.compare_exchange_strong(
                p, pOp->dInfo.pParent->get_child(!pOp->dInfo.bRightLeaf, memory_model::memory_order_acquire),
                memory_model::memory_order_release, atomics::memory_order_relaxed);
        } else {
            pOp->dInfo.pGrandParent->m_pLeft.compare_exchange_strong(
                p, pOp->dInfo.pParent->get_child(!pOp->dInfo.bRightLeaf, memory_model::memory_order_acquire),
                memory_model::memory_order_release, atomics::memory_order_relaxed);
        }
...

        {
            ...
            for (;;) {
                if (!search(res, val, cmp) || !eq(val, *res.pLeaf)) {
                    if (pOp) retire_update_desc(pOp, updRetire, false);
                ...
                if (res.updGrandParent.bits() != update_desc::Clean)
                    ...
                    if (check_delete_precondition(res)) {
                        ...
                        if (res.pGrandParent->m_pUpdate.compare_exchange_strong(
                                updGP, update_ptr(pOp, update_desc::DFlag), memory_model::memory_order_acq_rel,
    ...
    bool try_insert(value_type &val, internal_node *pNewInternal, search_result &res, retired_list &updRetire)
    {
        ...
        // check search result
        if (static_cast<leaf_node *>(res.pParent->get_child(res.bRightLeaf, memory_model::memory_order_relaxed))
            == res.pLeaf) {
