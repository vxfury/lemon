  ...
  private:
    class adapted_container {
      public:
        ...
        typedef typename opt::value<typename opt::find_option<
            opt::buffer<opt::v::initialized_static_buffer<cds::any_type, 256> >, Options...>::type>::buffer
            initial_buffer_type;
      ...
      private:
        ...
        struct equal_from_compare {
            Compare &m_cmp;
            equal_from_compare(Compare &cmp) : m_cmp(cmp) {}

            equal_from_compare(equal_from_compare const &src) : m_cmp(src.m_cmp) {}
      ...
      public:
        adapted_container() : m_Set(typename container_type::bucket_traits(m_Buckets.buffer(), m_Buckets.capacity())) {}
        ...
        std::pair<bool, bool> update(value_type &val, Func f, bool bAllowInsert)
        {
            if (bAllowInsert) {
                ...
                if (it == m_Set.end()) return std::make_pair(false, false);
                f(false, *it, val);
        ...
        bool unlink(value_type &val)
        {
            ...
            if (it == m_Set.end() || &(*it) != &val) return false;
            m_Set.erase(it);
            return true;
        }
        ...
        value_type *erase(Q const &key, Func f)
        {
            ...
            if (it == m_Set.end()) return nullptr;
            value_type &val = *it;
            f(val);
            m_Set.erase(it);
            return &val;
...
template <typename T, typename O1, typename O2, typename O3, typename O4, typename O5, typename O6, typename O7,
          typename O8, typename O9, typename O10, typename... Options>
class adapt<boost::intrusive::unordered_set<T, O1, O2, O3, O4, O5, O6, O7, O8, O9, O10>, Options...>
    : public details::adapt_boost_unordered_set<
          boost::intrusive::unordered_set<T, O1, O2, O3, O4, O5, O6, O7, O8, O9, O10>, Options...> {
};
#else
