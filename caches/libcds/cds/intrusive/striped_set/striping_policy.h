...
class striping {
  ...
  public:
    ...
    class scoped_cell_lock {
      ...
      public:
        scoped_cell_lock(striping &policy, size_t nHash) : m_guard(policy.m_Locks, nHash) {}
    };
    ...
    class scoped_full_lock {
      ...
      public:
        scoped_full_lock(striping &policy) : m_guard(policy.m_Locks) {}
    };

    class scoped_resize_lock : public scoped_full_lock {
      public:
        scoped_resize_lock(striping &policy) : scoped_full_lock(policy) {}
...
/**
    ...
    - \p Alloc - allocator type used for lock array memory allocation. Default is \p CDS_DEFAULT_ALLOCATOR.
*/
template <class RecursiveLock = std::recursive_mutex, typename BackOff = cds::backoff::yield,
...
class refinable {
  ...
  protected:
    ...
    lock_type &acquire(size_t nHash)
    {
        ...
        while (true) {
            ...
            who = m_Owner.load(atomics::memory_order_acquire);
            if ((!(who & 1) || (who >> 1) == (me & c_nOwnerMask)) && m_arrLocks == pLocks) return lock;
            lock.unlock();
        }
    }
    ...
    lock_array_ptr acquire_all()
    {
        ...
        while (true) {
            ...
            who = m_Owner.load(atomics::memory_order_acquire);
            if ((!(who & 1) || (who >> 1) == (me & c_nOwnerMask)) && m_arrLocks == pLocks) return pLocks;
    ...
    bool acquire_resize()
    {
        ...
        for (unsigned int nAttempts = 0; nAttempts < 32; ++nAttempts) {
            owner_t ownNull = 0;
            if (m_Owner.compare_exchange_strong(ownNull, (me << 1) | 1, atomics::memory_order_acquire,
  ...
  public:
    ...
    class scoped_cell_lock {
      ...
      public:
        scoped_cell_lock(refinable &policy, size_t nHash) : m_guard(policy.acquire(nHash), std::adopt_lock_t()) {}
    };
    ...
    class scoped_resize_lock {
      ...
      public:
        ...
        ~scoped_resize_lock()
        {
            if (m_bSucceess) m_Policy.release_resize();
        }
  ...
  public:
    /// Constructor
    refinable(size_t nLockCount ///< Initial size of lock array. Must be power of two.
              )
