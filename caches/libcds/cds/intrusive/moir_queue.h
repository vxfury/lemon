...
class MoirQueue : public MSQueue<GC, T, Traits> {
  ...
  protected:
    ...
    bool do_dequeue(dequeue_result &res)
    {
        ...
        node_type *h;
        while (true) {
            h = res.guards.protect(0, base_class::m_pHead, [](node_type *p) -> value_type * {
            ...
            if (base_class::m_pHead.compare_exchange_strong(h, pNext, memory_model::memory_order_release,
                                                            atomics::memory_order_relaxed)) {
                node_type *t = base_class::m_pTail.load(memory_model::memory_order_acquire);
                if (h == t)
                    base_class::m_pTail.compare_exchange_strong(t, pNext, memory_model::memory_order_release,
                                                                atomics::memory_order_relaxed);
                break;
            }
