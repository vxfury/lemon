...
struct operation : public cds::algo::elimination::operation_desc {
    ...
    operation() : pVal(nullptr), nStatus(0 /*op_free*/) {}
};
//@endcond
...
struct traits {
    ...
    /// Hook, possible types are \p treiber_stack::base_hook, \p treiber_stack::member_hook, \p
    /// treiber_stack::traits_hook
    typedef treiber_stack::base_hook<> hook;

    /// The functor used for dispose removed items. Default is \p opt::v::empty_disposer. This option is used only in \p
    /// TreiberStack::clear() function
...
/**
    ...
    - \p opt::hook - hook used. Possible hooks are: \p treiber_stack::base_hook, \p treiber_stack::member_hook, \p
   treiber_stack::traits_hook. If the option is not specified, \p %treiber_stack::base_hook<> is used.
   ...
   is used only in \p TreiberStack::clear function.
   ...
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
    ...
    - \p opt::buffer - a buffer type for elimination array, see \p opt::v::initialized_static_buffer, \p
   opt::v::initialized_dynamic_buffer. The buffer can be any size: \p Exp2 template parameter of those classes can be \p
   false. The size should be selected empirically for your application and hardware, there are no common rules for that.
        Default is <tt> %opt::v::initialized_static_buffer< any_type, 4 > </tt>.
    - \p opt::random_engine - a random engine to generate a random position in elimination array.
        Default is \p opt::v::c_rand.
    - \p opt::elimination_storage - a storage class for elmination records. Default is \p
   cds::algo::elimination::storage.
...
class elimination_backoff<true, T, Traits> {
    ...
    typedef typename Traits::buffer::template rebind<
        typename cds::details::type_padding<collision_array_record, cds::c_nCacheLineSize>::type>::other
        collision_array;
    ...
    struct elimination_data {
        ...
        elimination_data(size_t nCollisionCapacity) : collisions(nCollisionCapacity) {}
    };

    elimination_data m_Elimination;

    enum operation_status { op_free = 0, op_waiting = 1, op_collided = 2 };
  ...
  public:
    ...
    elimination_backoff(size_t nCollisionCapacity) : m_Elimination(nCollisionCapacity)
    {
        m_Elimination.collisions.zeroize();
    }
    ...
    bool backoff(operation_desc &op, Stat &stat)
    {
        ...
        bkoff([&op]() noexcept -> bool {
            return op.nStatus.load(atomics::memory_order_acquire) != op_waiting;
        });

        {
            slot_scoped_lock l(slot.lock);
            if (slot.pRec == myRec) slot.pRec = nullptr;
        }
...
/** @ingroup cds_intrusive_stack
    ...
    - R. K. Treiber. Systems programming: Coping with parallelism. Technical Report RJ 5118, IBM Almaden Research
   Center, April 1986.
    ...
    @note Be careful when you want destroy an item popped, see \ref cds_intrusive_item_destroying "Destroying items of
...
class TreiberStack {
  ...
  protected:
    ...
    static_assert(!enable_elimination
                      || std::is_same<typename elimination_random_engine::result_type, unsigned int>::value,
  ...
  public:
    ...
    */
    TreiberStack(size_t nCollisionCapacity) : m_Top(nullptr), m_Backoff(nCollisionCapacity) {}
    ...
    */
    bool push(value_type &val)
    {
        ...
        while (true) {
            ...
            if (m_Top.compare_exchange_weak(t, pNew, memory_model::memory_order_release,
            ...
            if (bkoff.backoff(op, m_stat)) return true;
        }
    }
    ...
    */
    value_type *pop()
    {
        ...
        while (true) {
            node_type *t = guard.protect(m_Top, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            });
            if (t == nullptr) return nullptr; // stack is empty

            node_type *pNext = t->m_pNext.load(memory_model::memory_order_relaxed);
            if (m_Top.compare_exchange_weak(t, pNext, memory_model::memory_order_acquire,
    ...
    */
    void clear()
    {
        ...
        while (true) {
            ...
            if (m_Top.compare_exchange_weak(pTop, nullptr, memory_model::memory_order_acquire,
                                            atomics::memory_order_relaxed)) {
                m_ItemCounter.reset();
                break;
            }
