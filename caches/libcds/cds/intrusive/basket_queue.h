...
struct node {
    ...
    typedef typename gc::template atomic_marked_ptr<marked_ptr>
        atomic_marked_ptr; ///< atomic marked pointer specific for GC
...
struct stat {
    ...
    counter_type m_TryAddBasket; ///< Count of attemps adding new item to a basket (only or BasketQueue, for other queue
                                 ///< this metric is not used)
    counter_type m_AddBasketCount; ///< Count of events "Enqueue a new item into basket" (only or BasketQueue, for other
                                   ///< queue this metric is not used)
...
struct traits {
    ...
    /// The functor used for dispose removed items. Default is \p opt::v::empty_disposer. This option is used for
    /// dequeuing
...
/**
    ...
    - \p opt::hook - hook used. Possible hooks are: \p basket_queue::base_hook, \p basket_queue::member_hook, \p
   basket_queue::traits_hook. If the option is not specified, \p %basket_queue::base_hook<> is used.
    - \p opt::back_off - back-off strategy used, default is \p cds::backoff::empty.
    - \p opt::disposer - the functor used for dispose removed items. Default is \p opt::v::empty_disposer. This option
   is used when dequeuing.
    - \p opt::link_checker - the type of node's link fields checking. Default is \p opt::debug_check_link
    - \p opt::item_counter - the type of item counting feature. Default is \p cds::atomicity::empty_item_counter (item
   counting disabled) To enable item counting use \p cds::atomicity::item_counter
    - \p opt::stat - the type to gather internal statistics.
        Possible statistics types are: \p basket_queue::stat, \p basket_queue::empty_stat, user-provided class that
   supports \p %basket_queue::stat interface. Default is \p %basket_queue::empty_stat (internal statistics disabled).
    - \p opt::padding - padding for internal critical atomic data. Default is \p opt::cache_line_padding
    - \p opt::memory_model - C++ memory ordering model. Can be \p opt::v::relaxed_ordering (relaxed memory model, the
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
...
#else
    typedef
        typename cds::opt::make_options<typename cds::opt::find_type_traits<traits, Options...>::type, Options...>::type
            type;
#endif
};
} // namespace basket_queue
...
/** @ingroup cds_intrusive_queue
    ...
    Like \p MSQueue, the Baskets queue algo has a key feature: even if the queue is empty it contains one item that is
   "dummy" one from the standpoint of the algo. See \p dequeue() function doc for explanation.
...
class BasketQueue {
  public:
    ...
    typedef
        typename single_link::get_link_checker<node_type, traits::link_checker>::type link_checker; ///< link checker
  ...
  protected:
    ...
    bool do_dequeue(dequeue_result &res, bool bDeque)
    {
        ...
        while (true) {
            h = res.guards.protect(0, m_pHead, [](marked_ptr p) -> value_type * {
                return node_traits::to_value_ptr(p.ptr());
            });
            t = res.guards.protect(1, m_pTail, [](marked_ptr p) -> value_type * {
                return node_traits::to_value_ptr(p.ptr());
            });
            pNext = res.guards.protect(2, h->m_pNext, [](marked_ptr p) -> value_type * {
                return node_traits::to_value_ptr(p.ptr());
            });
            ...
            if (h == m_pHead.load(memory_model::memory_order_acquire)) {
                if (h.ptr() == t.ptr()) {
                    ...
                    {
                        typename gc::Guard g;
                        while (pNext->m_pNext.load(memory_model::memory_order_relaxed).ptr()
                               && m_pTail.load(memory_model::memory_order_relaxed) == t) {
                            pNext = g.protect(pNext->m_pNext, [](marked_ptr p) -> value_type * {
                                return node_traits::to_value_ptr(p.ptr());
                            });
                            res.guards.copy(2, g);
                        }
                    ...
                    m_pTail.compare_exchange_weak(t, marked_ptr(pNext.ptr()), memory_model::memory_order_acquire,
                    ...
                    while (pNext.ptr() && pNext.bits() && iter.ptr() != t.ptr()
                           && m_pHead.load(memory_model::memory_order_relaxed) == h) {
                        iter = pNext;
                        g.assign(res.guards.template get<value_type>(2));
                        pNext = res.guards.protect(2, pNext->m_pNext, [](marked_ptr p) -> value_type * {
                            return node_traits::to_value_ptr(p.ptr());
                        });
                        ++hops;
                    }

                    if (m_pHead.load(memory_model::memory_order_relaxed) != h) continue;
                    ...
                    if (iter.ptr() == t.ptr())
                        ...
                        if (iter->m_pNext.compare_exchange_weak(pNext, marked_ptr(pNext.ptr(), 1),
                                                                memory_model::memory_order_acquire,
                                                                atomics::memory_order_relaxed)) {
                            if (hops >= m_nMaxHops) free_chain(h, pNext);
                            break;
                        }
                    } else
                        return true;
            ...
            if (bDeque) m_Stat.onDequeueRace();
            bkoff();
        }
    ...
    void free_chain(marked_ptr head, marked_ptr newHead)
    {
        ...
        if (m_pHead.compare_exchange_strong(head, marked_ptr(newHead.ptr()), memory_model::memory_order_release,
                                            atomics::memory_order_relaxed)) {
            typename gc::template GuardArray<2> guards;
            guards.assign(0, node_traits::to_value_ptr(head.ptr()));
            while (head.ptr() != newHead.ptr()) {
                marked_ptr pNext = guards.protect(1, head->m_pNext, [](marked_ptr p) -> value_type * {
                    return node_traits::to_value_ptr(p.ptr());
                });
                assert(pNext.bits() != 0);
  ...
  public:
    ...
    BasketQueue() : m_pHead(&m_Dummy), m_pTail(&m_Dummy), m_nMaxHops(3) {}
    ...
    */
    bool enqueue(value_type &val)
    {
        ...
        marked_ptr t;
        while (true) {
            t = guard.protect(m_pTail, [](marked_ptr p) -> value_type * {
                return node_traits::to_value_ptr(p.ptr());
            });
            ...
            if (pNext.ptr() == nullptr) {
                ...
                if (t->m_pNext.compare_exchange_weak(pNext, marked_ptr(pNew), memory_model::memory_order_release,
                                                     atomics::memory_order_relaxed)) {
                    if (!m_pTail.compare_exchange_strong(t, marked_ptr(pNew), memory_model::memory_order_release,
                                                         atomics::memory_order_relaxed))
                ...
                // Reread tail next
            try_again:
                pNext = gNext.protect(t->m_pNext, [](marked_ptr p) -> value_type * {
                    return node_traits::to_value_ptr(p.ptr());
                });

                // add to the basket
                if (m_pTail.load(memory_model::memory_order_relaxed) == t
                    && t->m_pNext.load(memory_model::memory_order_relaxed) == pNext && !pNext.bits()) {
                    bkoff();
                ...
                while ((p = pNext->m_pNext.load(memory_model::memory_order_acquire)).ptr() != nullptr) {
                    ...
                    if (pNext->m_pNext.load(memory_model::memory_order_relaxed) != p) continue;
                    pNext = p;
                    g.assign(0, g.template get<value_type>(1));
                }
                if (!bTailOk
                    || !m_pTail.compare_exchange_weak(t, marked_ptr(pNext.ptr()), memory_model::memory_order_release,
                                                      atomics::memory_order_relaxed))
                    m_Stat.onAdvanceTailFailed();
    ...
    */
    value_type *dequeue()
    {
        ...
        if (do_dequeue(res, true)) return node_traits::to_value_ptr(*res.pNext);
        return nullptr;
    }
    ...
    */
    ...
    /**
        The value returned depends on \p Traits (see basket_queue::traits::item_counter). For \p
       atomicity::empty_item_counter, this function always returns 0.
