...
class node<cds::urcu::gc<RCU>, Tag> {
  ...
  protected:
    ...
    atomic_marked_ptr *m_arrNext; ///< Array of next items for levels 1 .. m_nHeight - 1. For node at level 0 \p
                                  ///< m_arrNext is \p nullptr
    atomics::atomic<unsigned> m_nUnlink; ///< Unlink helper

  public:
    /// Constructs a node of height 1 (a bottom-list node)
    node() : m_pNext(nullptr), m_pDelChain(nullptr), m_nHeight(1), m_arrNext(nullptr)
    {
    ...
    {
        return m_pNext == atomic_marked_ptr() && m_arrNext == nullptr && m_nHeight <= 1;
    }
...
class iterator<cds::urcu::gc<RCU>, NodeTraits, BackOff, IsConst> {
  ...
  public: // for internal use only!!!
    iterator(node_type &refHead) : m_pNode(nullptr)
    {
        back_off bkoff;
  ...
  public:
    ...
    iterator(iterator const &s) : m_pNode(s.m_pNode) {}
...
/** @ingroup cds_intrusive_map
    ...
    <b>Template arguments</b>:
        ...
        - \p T - type to be stored in the list. The type must be based on \p skip_list::node (for \p
   skip_list::base_hook) or it must have a member of type \p skip_list::node (for \p skip_list::member_hook).
    ...
    \endcode
    Note, the iterator object returned by \ref end, \p cend member functions points to \p nullptr and should not be
   dereferenced.
...
class SkipListSet<cds::urcu::gc<RCU>, T, Traits> {
  public:
    ...
    typedef implementation_defined
        key_comparator; ///< key comparison functor based on \p Traits::compare and \p Traits::less
#else
    ...
    /**
        The max height is specified by \ref skip_list::random_level_generator "random level generator" constant \p
       m_nUpperBound but it should be no more than 32 (\ref skip_list::c_nHeightLimit).
    */
    static unsigned int const c_nMaxHeight =
        std::conditional<(random_level_generator::c_nUpperBound <= skip_list::c_nHeightLimit),
                         std::integral_constant<unsigned int, random_level_generator::c_nUpperBound>,
                         std::integral_constant<unsigned int, skip_list::c_nHeightLimit> >::type::value;
  ...
  protected:
    //@cond
    typedef skip_list::details::intrusive_node_builder<node_type, atomic_node_ptr, allocator_type>
        intrusive_node_builder;

    typedef
        typename std::conditional<std::is_same<typename traits::internal_node_builder, cds::opt::none>::value,
                                  intrusive_node_builder, typename traits::internal_node_builder>::type node_builder;
  ...
  public:
    using exempt_ptr =
        cds::urcu::exempt_ptr<gc, value_type, value_type, node_disposer, void>; ///< pointer to extracted node
  ...
  public:
    /// Default constructor
    SkipListSet() : m_Head(c_nMaxHeight), m_nHeight(c_nMinHeight), m_pDeferredDelChain(nullptr)
    {
  ...
  public:
    ...
    std::pair<bool, bool> update(value_type &val, Func func, bool bInsert = true)
    {
...

        {
            ...
            while (true) {
                ...
                if (!insert_at_position(val, pNode, pos, [&func](value_type &item) {
                        func(true, item, item);
                    })) {
                    m_Stat.onInsertRetry();
                    continue;
    ...
    */
    ...
    /**
        The function searches an item with minimal key, unlinks it, and returns \ref cds::urcu::exempt_ptr "exempt_ptr"
       pointer to the item. If the skip-list is empty the function returns an empty \p exempt_ptr.
    ...
    */
    ...
    /**
        The function searches an item with maximal key, unlinks it, and returns \ref cds::urcu::exempt_ptr "exempt_ptr"
       pointer to the item. If the skip-list is empty the function returns an empty \p exempt_ptr.
    ...
    */
    ...
    raw_ptr get(Q const &key)
    {
        ...
        if (do_find_with(
                key, key_comparator(),
                [&pFound](value_type &found, Q const &) {
                    pFound = &found;
                },
                pos))
    ...
    */
    ...
    raw_ptr get_with(Q const &key, Less pred)
    {
        ...
        position pos;
        if (do_find_with(
                key, cds::opt::details::make_comparator_from_less<Less>(),
                [&pFound](value_type &found, Q const &) {
                    pFound = &found;
                },
                pos)) {
  ...
  protected:
    ...
    void help_remove(int nLevel, node_type *pPred, marked_node_ptr pCur, marked_node_ptr pSucc, position &pos)
    {
        ...
        if (pCur->is_upper_level(nLevel)
            && pPred->next(nLevel).compare_exchange_strong(
                p, marked_node_ptr(pSucc.ptr()), memory_model::memory_order_release, atomics::memory_order_relaxed)) {
    ...
    retry:
        ...
        for (int nLevel = static_cast<int>(c_nMaxHeight - 1); nLevel >= 0; --nLevel) {
            while (true) {
                ...
                if (pPred->next(nLevel).load(memory_model::memory_order_acquire).all() != pCur.ptr()) goto retry;
    ...
    bool insert_at_position(value_type &val, node_type *pNode, position &pos, Func f)
    {
        ...
        // Insert at level 0
        {
            ...
            if (!pos.pPrev[0]->next(0).compare_exchange_strong(
                    p, marked_node_ptr(pNode), memory_model::memory_order_release, atomics::memory_order_relaxed))
                return false;
        ...
        for (unsigned int nLevel = 1; nLevel < nHeight; ++nLevel) {
            ...
            while (true) {
                ...
                // Set pNode->next
                // pNode->next must be null but can have a "logical deleted" flag if another thread is removing pNode
                // right now
                if (!pNode->next(nLevel).compare_exchange_strong(p, pSucc, memory_model::memory_order_acq_rel,
    ...
    bool try_remove_at(node_type *pDel, position &pos, Func f, bool bExtract)
    {
        ...
        for (unsigned int nLevel = pDel->height() - 1; nLevel > 0; --nLevel) {
            ...
            if (pSucc.bits() == 0) {
                bkoff.reset();
                while (!pDel->next(nLevel).compare_exchange_weak(
                    pSucc, pSucc | nMask, memory_model::memory_order_release, atomics::memory_order_acquire)) {
                    if (pSucc.bits() == 0) {
                        bkoff();
                        m_Stat.onMarkFailed();
        ...
        while (true) {
            if (pDel->next(0).compare_exchange_strong(p, p | nMask, memory_model::memory_order_release,
                                                      atomics::memory_order_acquire)) {
                ...
                for (int nLevel = static_cast<int>(pDel->height() - 1); nLevel >= 0; --nLevel) {
                    ...
                    if (pos.pPrev[nLevel]->next(nLevel).compare_exchange_strong(p, marked_node_ptr(pSucc.ptr()),
                                                                                memory_model::memory_order_acq_rel,
                                                                                atomics::memory_order_relaxed)) {
                        pDel->level_unlinked();
                    } else {
                        // Make slow erase
    ...
    enum finsd_fastpath_result { find_fastpath_found, find_fastpath_not_found, find_fastpath_abort };
    ...
    try_again:
        pPred = m_Head.head();
        for (int nLevel = static_cast<int>(m_nHeight.load(memory_model::memory_order_relaxed) - 1); nLevel >= 0;
             --nLevel) {
            pCur = pPred->next(nLevel).load(memory_model::memory_order_acquire);
    ...
    void increase_height(unsigned int nHeight)
    {
        ...
        if (nCur < nHeight)
            m_nHeight.compare_exchange_strong(nCur, nHeight, memory_model::memory_order_release,
                                              atomics::memory_order_relaxed);
    }
