...
struct traits {
    ...
    /// Hook, possible types are \p optimistic_queue::base_hook, \p optimistic_queue::member_hook, \p
    /// optimistic_queue::traits_hook
...
/**
    ...
    - \p opt::hook - hook used. Possible hooks are: \p optimistic_queue::base_hook, \p optimistic_queue::member_hook, \p
   optimistic_queue::traits_hook. If the option is not specified, \p %optimistic_queue::base_hook<> is used.
...
/** @ingroup cds_intrusive_queue
    ...
    - \p T - type of value to be stored in the queue. A value of type \p T must be derived from \p
   optimistic_queue::node for \p optimistic_queue::base_hook, or it should have a member of type \p
   %optimistic_queue::node for \p optimistic_queue::member_hook, or it should be convertible to \p
   %optimistic_queue::node for \p optimistic_queue::traits_hook.
    ...
    The optimistic queue algo has a key feature: even if the queue is empty it contains one item that is "dummy" one
   from the standpoint of the algo. See \p dequeue() function for explanation.
...
class OptimisticQueue {
  public:
    ...
    typedef typename optimistic_queue::get_link_checker<node_type, traits::link_checker>::type
  ...
  protected:
    ...
    bool do_dequeue(dequeue_result &res)
    {
        ...
        while (true) { // Try till success or empty
            pHead = res.guards.protect(0, m_pHead, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            });
            pTail = res.guards.protect(1, m_pTail, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            });
            assert(pHead != nullptr);
            pFirstNodePrev = res.guards.protect(2, pHead->m_pPrev, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            });
            ...
            if (pHead == m_pHead.load(memory_model::memory_order_acquire)) {
                if (pTail != pHead) {
                    ...
                    if (m_pHead.compare_exchange_weak(pHead, pFirstNodePrev, memory_model::memory_order_acquire,
    ...
    void fix_list(node_type *pTail, node_type *pHead)
    {
        ...
        while (pCurNode != pHead) { // While not at head
            pCurNodeNext = guards.protect(0, pCurNode->m_pNext, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            });
            if (pHead != m_pHead.load(memory_model::memory_order_acquire)) break;
  ...
  public:
    ...
    OptimisticQueue() : m_pTail(&m_Dummy), m_pHead(&m_Dummy) {}
    ...
    bool enqueue(value_type &val)
    {
        ...
        guards.assign(1, &val);
        while (true) {
            node_type *pTail = guards.protect(0, m_pTail, [](node_type *p) -> value_type * {
                return node_traits::to_value_ptr(p);
            }); // Read the tail
            pNew->m_pNext.store(pTail, memory_model::memory_order_relaxed);
            if (m_pTail.compare_exchange_strong(pTail, pNew, memory_model::memory_order_release,
                                                atomics::memory_order_acquire)) { // Try to CAS the tail
