...
struct stat {
    ...
    void onPushPopulated()
    {
        ++m_nPushPopulated;
    }
    void onPushContended()
    {
        ++m_nPushContended;
    }
    void onPop()
    ...
    void onCreateSegmentReq()
    {
        ++m_nCreateSegmentReq;
    }
    void onDeleteSegmentReq()
    {
        ++m_nDeleteSegmentReq;
    }
    void onSegmentCreated()
    {
        ++m_nSegmentCreated;
    }
...
struct traits {
    ...
    */
    ...
    /// Memory model, default is opt::v::relaxed_ordering. See cds::opt::memory_model for the full list of possible
    /// types
...
/**
    ...
    - \p opt::stat - internal statistics, possible type: \p segmented_queue::stat, \p segmented_queue::empty_stat (the
   default)
...
class SegmentedQueue {
  public:
    ...
    typedef typename traits::disposer
        disposer; ///< value disposer, called only in \p clear() when the element to be dequeued
    typedef typename traits::allocator allocator;       ///< Allocator maintaining the segments
    typedef typename traits::memory_model memory_model; ///< Memory ordering. See cds::opt::memory_model option
    typedef
        typename traits::item_counter item_counter; ///< Item counting policy, see cds::opt::item_counter option setter
    typedef typename traits::stat stat;             ///< Internal statistics policy
    typedef
        typename traits::lock_type lock_type; ///< Type of mutex for maintaining an internal list of allocated segments.
    typedef typename traits::permutation_generator
        permutation_generator; ///< Random permutation generator for sequence [0, quasi-factor)
  ...
  protected:
    ...
    struct segment : public boost::intrusive::slist_base_hook<> {
        ...
        explicit segment(size_t nCellCount)
            ...
            : cells(reinterpret_cast<cell *>(this + 1)), version(0)
        {
            init(nCellCount);
        }
    ...
    typedef typename opt::details::alignment_setter<atomics::atomic<segment *>, traits::alignment>::type
        aligned_segment_ptr;
    //@endcond
    ...
    class segment_list {
      ...
      public:
        segment_list(size_t nQuasiFactor, stat &st)
            : m_pHead(nullptr), m_pTail(nullptr), m_nQuasiFactor(nQuasiFactor), m_Stat(st)
        {
        ...
        bool populated(segment const &s) const
        {
            ...
            for (cell const *pCell = s.cells; pCell < pLastCell; ++pCell) {
                if (!pCell->data.load(memory_model::memory_order_relaxed).all()) return false;
            }
            return true;
        }
        bool exhausted(segment const &s) const
        {
            ...
            cell const *pLastCell = s.cells + quasi_factor();
            for (cell const *pCell = s.cells; pCell < pLastCell; ++pCell) {
                if (!pCell->data.load(memory_model::memory_order_relaxed).bits()) return false;
            }
            return true;
        }
#endif
        ...
        segment *create_tail(segment *pTail, typename gc::Guard &guard)
        {
            ...
            if (m_List.empty()) m_pHead.store(pNew, memory_model::memory_order_release);
  ...
  public:
    ...
    SegmentedQueue(size_t nQuasiFactor ///< Quasi factor. If it is not a power of 2 it is rounded up to nearest power
                                       ///< of 2. Minimum is 2.
                   )
            ...
            do {
                ...
                if (!item.ptr())
                    ...
                    if (!item.bits()) {
                        ...
                        if (pHeadSegment->cells[i].data.compare_exchange_strong(
                                item, item | 1, memory_model::memory_order_acquire, atomics::memory_order_relaxed)) {
                            --m_ItemCounter;
                            m_Stat.onPop();
