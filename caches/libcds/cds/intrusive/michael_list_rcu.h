...
struct node<cds::urcu::gc<RCU>, Tag> {
    ...
    constexpr node() noexcept : m_pNext(nullptr), m_pDelChain(nullptr) {}
};
} // namespace michael_list
//@endcond
...
class MichaelList<cds::urcu::gc<RCU>, T, Traits> {
  ...
  protected:
    //@cond
    enum erase_node_mask { erase_mask = 1, extract_mask = 3 };
    ...
    struct position {
        ...
        position(atomic_node_ptr &head) : refHead(head), pDelChain(nullptr) {}
  ...
  public:
    using exempt_ptr =
        cds::urcu::exempt_ptr<gc, value_type, value_type, clear_and_dispose, void>; ///< pointer to extracted node
  ...
  protected:
    ...
    class iterator_type {
        ...
        void next()
        {
            if (m_pNode) {
                node_type *p =
                    node_traits::to_node_ptr(*m_pNode)->m_pNext.load(memory_model::memory_order_relaxed).ptr();
      ...
      public:
        ...
        iterator_type(const iterator_type &src) : m_pNode(src.m_pNode) {}
  ...
  public:
    ...
    */
    ...
    /**
        ...
        The function can throw \ref cds_urcu_rcu_deadlock "cds::urcu::rcu_deadlock" exception if a deadlock is detected
       and the deadlock checking policy is \p opt::v::rcu_throw_deadlock.
    */
    template <typename Q>
            ...
            for (;;) {
                {
                    ...
                    if (cds_unlikely(!pHead->m_pNext.compare_exchange_weak(
                            pNext, pNext | 1, memory_model::memory_order_acquire, memory_model::memory_order_relaxed)))
                        continue;
                    if (cds_unlikely(!m_pHead.compare_exchange_weak(pHead, marked_node_ptr(pNext.ptr()),
  ...
  protected:
    ...
    bool link_node(node_type *pNode, position &pos)
    {
        ...
        if (cds_likely(pos.pPrev->compare_exchange_strong(p, marked_node_ptr(pNode), memory_model::memory_order_release,
    ...
    bool unlink_node(position &pos, erase_node_mask nMask)
    {
        ...
        if (cds_likely(pos.pCur->m_pNext.compare_exchange_strong(next, next | nMask, memory_model::memory_order_release,
                                                                 atomics::memory_order_relaxed))) {
            ...
            if (cds_likely(pos.pPrev->compare_exchange_strong(cur, marked_node_ptr(pos.pNext),
                                                              ...
                                                              atomics::memory_order_relaxed))) {
                if (nMask == erase_mask) link_to_remove_chain(pos, pos.pCur);
            } else {
                // Slow path
    ...
    {
        rcu_lock l;
        if (insert_at_locked(refHead, val)) return iterator(node_traits::to_node_ptr(val));
        return end();
    }
    ...
    try_again:
        ...
        while (true) {
            ...
            if (pNext.bits()) {
                ...
                if (cds_likely(pPrev->compare_exchange_weak(pCur, marked_node_ptr(pNext.ptr()),
                                                            memory_model::memory_order_acquire,
                                                            atomics::memory_order_relaxed))) {
                    if (pNext.bits() == erase_mask) link_to_remove_chain(pos, pCur.ptr());
