...
/** @ingroup cds_intrusive_map
   ...
   that hash value.
    ...
    @note Before including <tt><cds/intrusive/feldman_hashset_rcu.h></tt> you should include appropriate RCU header
...
class FeldmanHashSet<cds::urcu::gc<RCU>, T, Traits> : protected feldman_hashset::multilevel_array<T, Traits> {
  ...
  public:
    ...
    static constexpr const bool c_bExtractLockExternal =
        false; ///< Group of \p extract_xxx functions does not require external locking
  ...
  public:
    ...
    */
    ...
    bool erase(hash_type const &hash, Func f)
    {
        ...
        {
            rcu_lock rcuLock;
            p = do_erase(hash, [](value_type const &) -> bool {
                return true;
            });
        }
    ...
    /**
        ...
        Example:
        \code
        typedef cds::intrusive::FeldmanHashSet< cds::urcu::gc< cds::urcu::general_buffered<> >, foo, my_traits >
       set_type; set_type theSet;
        // ...
  ...
  protected:
    ...
    class iterator_base {
      ...
      public:
        iterator_base() noexcept : m_pNode(nullptr), m_idx(0), m_pValue(nullptr), m_set(nullptr) {}

        iterator_base(iterator_base const &rhs) noexcept
            : m_pNode(rhs.m_pNode), m_idx(rhs.m_idx), m_pValue(rhs.m_pValue), m_set(rhs.m_set)
        {
        }
      ...
      protected:
        iterator_base(FeldmanHashSet const &set, array_node *pNode, size_t idx, bool)
            : m_pNode(pNode), m_idx(idx), m_pValue(nullptr), m_set(&set)
        {
        }
  ...
  public:
    ...
    typedef implementation_defined
        iterator; ///< @ref cds_intrusive_FeldmanHashSet_rcu_iterators "bidirectional iterator" type
    typedef implementation_defined
        const_iterator; ///< @ref cds_intrusive_FeldmanHashSet_rcu_iterators "bidirectional const iterator" type
    typedef implementation_defined
        reverse_iterator; ///< @ref cds_intrusive_FeldmanHashSet_rcu_iterators "bidirectional reverse iterator" type
    typedef implementation_defined const_reverse_iterator; ///< @ref cds_intrusive_FeldmanHashSet_rcu_iterators
  ...
  protected:
            ...
            if (pos.pArr->nodes[pos.nSlot].load(memory_model::memory_order_acquire) == slot) {
                if (slot.ptr()) {
                    if (cmp(hash, hash_accessor()(*slot.ptr())) == 0) {
                        ...
                        if (pos.pArr->nodes[pos.nSlot].compare_exchange_strong(slot, node_ptr(&val),
                    ...
                    if (bInsert) {
                        node_ptr pNull;
                        if (pos.pArr->nodes[pos.nSlot].compare_exchange_strong(pNull, node_ptr(&val),
                                                                               memory_model::memory_order_release,
                                                                               atomics::memory_order_relaxed)) {
                            // the new data node has been inserted
                            f(val, nullptr);
    ...
    update_existing_done:
        if (pOld) gc::template retire_ptr<disposer>(pOld);
        return std::make_pair(true, false);
    }
