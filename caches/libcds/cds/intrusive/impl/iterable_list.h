...
#endif
{
  public:
    ...
    struct rebind_traits {
        typedef IterableList<gc, value_type, typename cds::opt::make_options<traits, Options...>::type> type;
    };
  ...
  protected:
    ...
    class iterator_type {
      ...
      protected:
        ...
        void next()
        {
            for (node_type *p = m_pNode->next.load(memory_model::memory_order_relaxed); p != m_pNode;
                 p = p->next.load(memory_model::memory_order_relaxed)) {
                m_pNode = p;
                if (m_Guard
                        .protect(p->data,
                                 [](marked_data_ptr ptr) {
        ...
        explicit iterator_type(node_type *pNode) : m_pNode(pNode)
        {
            if (!m_Guard
                     .protect(pNode->data,
                              [](marked_data_ptr p) {
        ...
        iterator_type(node_type *pNode, value_type *pVal) : m_pNode(pNode)
        {
            if (m_pNode) {
                assert(pVal != nullptr);
                m_Guard.assign(pVal);
      ...
      public:
        ...
        iterator_type() : m_pNode(nullptr) {}

        iterator_type(iterator_type const &src) : m_pNode(src.m_pNode)
        {
  ...
  public:
    ...
    /**
        ...
        - to protect the value, the iterator contains a GC-specific guard.
          For some GC (like as \p gc::HP), a guard is a limited resource per thread, so an exception (or assertion) "no
       free guard" may be thrown if the limit of guard count per thread is exceeded.
        - The iterator cannot be moved across thread boundary since it contains thread-private GC's guard.
        - Iterator is thread-safe: even if the element the iterator points to is removed, the iterator stays valid
       because it contains the guard keeping the value from to be recycled.
        ...
        \code
            if ( it1 == it2 )
                assert( &(*it1) == &(*it2));
        \endcode
        can throw assertion. The point is that the iterator stores the value of element which can be modified later by
       other thread. The guard inside the iterator prevents recycling that value so the iterator's value remains valid
       even after changing. Other iterator may observe modified value of the element.
    */
  ...
  public:
    ...
    explicit IterableList(Stat &st) : m_Stat(st)
    {
        init_list();
    }
    //@endcond
    ...
    */
    bool erase_at(iterator const &iter)
    {
        ...
        if (iter.m_pNode->data.compare_exchange_strong(val, marked_data_ptr(), memory_model::memory_order_acquire,
    ...
    */
    ...
    void clear()
    {
        position pos;
        pos.pPrev = nullptr;
        for (pos.pCur = m_Head.next.load(memory_model::memory_order_relaxed); pos.pCur != pos.pPrev;
             pos.pCur = pos.pCur->next.load(memory_model::memory_order_relaxed)) {
            while (true) {
                pos.pFound = pos.guard
                                 .protect(pos.pCur->data,
                                          [](marked_data_ptr p) {
                                              return p.ptr();
                                          })
                                 .ptr();
    ...
    /**
        ...
        Thus, if you need to use \p %empty() you should provide appropriate (non-empty) \p
       iterable_list::traits::item_counter feature.
    */
    bool empty() const
    {
        return size() == 0;
    ...
    /**
        The value returned depends on item counter provided by \p iterable_list::traits::item_counter. For \p
  ...
  protected:
    ...
    bool insert_aux_node(node_type *pHead, node_type *pNode)
    {
        ...
        while (true) {
            if (inserting_search(pHead, *pNode->data.load(memory_model::memory_order_relaxed).ptr(), pos,
                                 key_comparator())) {
  ...
  private:
    ...
    bool link_data(value_type *pVal, insert_position &pos, node_type *pHead)
    {
        ...
        if (!pos.pCur->data.compare_exchange_strong(valCur, valCur | 1, memory_model::memory_order_acquire,
                                                    atomics::memory_order_relaxed)) {
        ...
        if (!pos.pPrev->data.compare_exchange_strong(valPrev, valPrev | 1, memory_model::memory_order_acquire,
                                                     atomics::memory_order_relaxed)) {
        ...
        if (pos.pPrev != pos.pHead && pos.pPrevVal == nullptr) {
            ...
            valPrev |= 1;
            bool result = pos.pPrev->data.compare_exchange_strong(
                valPrev, marked_data_ptr(pVal), memory_model::memory_order_release, atomics::memory_order_relaxed);
            ...
            bool result = pos.pPrev->next.compare_exchange_strong(pos.pCur, pNode, memory_model::memory_order_release,
                                                                  atomics::memory_order_relaxed);
    ...
    static bool unlink_data(position &pos)
    {
        ...
        if (pos.pCur->data.compare_exchange_strong(val, marked_data_ptr(), memory_model::memory_order_acquire,
                                                   atomics::memory_order_relaxed)) {
            retire_data(pos.pFound);
            return true;
        }
    ...
    node_type *find_prev(node_type const *pHead, Q const &val) const
    {
        ...
        while (true) {
            ...
            if (pVal && cmp(*pVal, val) >= 0) return pPrev;
