...
/** @ingroup cds_intrusive_map
    ...
    - <tt><cds/intrusive/ellen_bintree_rcu.h></tt> - for RCU (see \ref cds_intrusive_EllenBinTree_rcu "RCU-based
   EllenBinTree")
    ...
    \p Traits::less, \p Traits::compare and other predicates using with member fuctions should accept at least
   parameters of type \p T and \p Key in any combination. For example, for \p Foo struct with \p std::string key field
   the appropiate \p less functor is: \code struct Foo: public cds::intrusive::ellen_bintree::node< ... >
    {
        std::string m_strKey;
        ...
    };
...
class EllenBinTree {
  ...
  protected:
    ...
    struct search_result {
        ...
        search_result()
            : pGrandParent(nullptr), pParent(nullptr), pLeaf(nullptr), bRightLeaf(false), bRightParent(false)
        {
        }
    };
    //@endcond
  ...
  public:
    ...
    bool insert(value_type &val, Func f)
    {
        ...
        for (;;) {
            ...
            if (res.updGrandParent.bits() == update_desc::Clean && res.updParent.bits() == update_desc::Clean) {
                if (!pNewInternal.get()) pNewInternal.reset(alloc_internal_node());
    ...
    */
    bool unlink(value_type &val)
    {
        return erase_(
            val, node_compare(),
            [](value_type const &v, leaf_node const &n) -> bool {
    ...
    */
    ...
    {
        CDS_UNUSED(pred);
        typedef ellen_bintree::details::compare<key_type, value_type, opt::details::make_comparator_from_less<Less>,
                                                node_traits>
            compare_functor;
    ...
    */
    void unsafe_clear()
    {
        while (true) {
            ...
            pGrandParent->m_pLeft.store(pParent->m_pRight.load(memory_model::memory_order_relaxed),
  ...
  protected:
    ...
    bool check_consistency(internal_node const *pRoot) const
    {
        ...
        if (node_compare()(*pLeft, *pRoot) < 0 && node_compare()(*pRoot, *pRight) <= 0
            && node_compare()(*pLeft, *pRight) < 0) {
            bool bRet = true;
            if (pLeft->is_internal()) bRet = check_consistency(static_cast<internal_node *>(pLeft));
            assert(bRet);

            if (bRet && pRight->is_internal()) bRet = bRet && check_consistency(static_cast<internal_node *>(pRight));
            assert(bRet);
    ...
    {
    retry:
        tree_node *p = bRight ? res.guards.protect(search_result::Guard_Leaf, pParent->m_pRight,
                                                   [](tree_node *pn) -> internal_node * {
                                                       return static_cast<internal_node *>(pn);
                                                   })
        ...
        tree_node *pVal = bRight
                              ? res.guards.protect(search_result::Guard_temporary, pParent->m_pRight,
                                                   [](tree_node *pn) -> value_type * {
                                                       return node_traits::to_value_ptr(static_cast<leaf_node *>(pn));
                                                   })
    ...
    {
        return res.guards.protect(search_result::Guard_updParent, src, [](update_ptr p) -> update_desc * {
            return p.ptr();
        });
    }
    ...
    retry:
        ...
        if (pLeaf->infinite_key()) return false;
    ...
    */
    ...
    void help_insert(update_desc *pOp)
    {
        ...
        if (pOp->iInfo.bRightLeaf) {
            CDS_VERIFY(pOp->iInfo.pParent->m_pRight.compare_exchange_strong(
                pLeaf, static_cast<tree_node *>(pOp->iInfo.pNew), memory_model::memory_order_release,
                atomics::memory_order_relaxed));
        } else {
            CDS_VERIFY(pOp->iInfo.pParent->m_pLeft.compare_exchange_strong(
                pLeaf, static_cast<tree_node *>(pOp->iInfo.pNew), memory_model::memory_order_release,
                atomics::memory_order_relaxed));
        }
    ...
    bool check_delete_precondition(search_result &res) const
    {
        ...
        return static_cast<internal_node *>(
                   res.pGrandParent->get_child(res.bRightParent, memory_model::memory_order_relaxed))
                   == res.pParent
               && static_cast<leaf_node *>(res.pParent->get_child(res.bRightLeaf, memory_model::memory_order_relaxed))
                      == res.pLeaf;
    }
    ...
    bool help_delete(update_desc *pOp)
    {
        ...
        if (pOp->dInfo.pParent->m_pUpdate.compare_exchange_strong(pUpdate, pMark, // *
            ...
            if (pOp->dInfo.pGrandParent->m_pUpdate.compare_exchange_strong(
                    pDel, pOp->dInfo.pGrandParent->null_update_desc(), memory_model::memory_order_release,
                    atomics::memory_order_relaxed)) {
                retire_update_desc(pOp);
            }
            return false;
    ...
    {
        tree_node *pSibling = guard.protect(sibling, [](tree_node *p) -> internal_node * {
            return static_cast<internal_node *>(p);
        });
        if (pSibling->is_leaf()) guard.assign(node_traits::to_value_ptr(static_cast<leaf_node *>(pSibling)));
        return pSibling;
    }
    ...
    void help_marked(update_desc *pOp)
    {
        ...
        tree_node *pOpposite =
            protect_sibling(guard, pOp->dInfo.bRightLeaf ? pOp->dInfo.pParent->m_pLeft : pOp->dInfo.pParent->m_pRight);

        if (pOp->dInfo.bRightParent) {
            CDS_VERIFY(pOp->dInfo.pGrandParent->m_pRight.compare_exchange_strong(
                pParent, pOpposite, memory_model::memory_order_release, atomics::memory_order_relaxed));
        } else {
            CDS_VERIFY(pOp->dInfo.pGrandParent->m_pLeft.compare_exchange_strong(
                pParent, pOpposite, memory_model::memory_order_release, atomics::memory_order_relaxed));
        }

        update_ptr upd(pOp, update_desc::DFlag);
        CDS_VERIFY(pOp->dInfo.pGrandParent->m_pUpdate.compare_exchange_strong(
            upd, pOp->dInfo.pGrandParent->null_update_desc(), memory_model::memory_order_release,
            atomics::memory_order_relaxed));
    }
    ...
    bool erase_(Q const &val, Compare cmp, Equal eq, Func f)
    {
        ...
        for (;;) {
            if (!search(res, val, cmp) || !eq(val, *res.pLeaf)) {
                if (pOp) retire_update_desc(pOp);
                m_Stat.onEraseFailed();
                return false;
            }

            if (res.updGrandParent.bits() == update_desc::Clean && res.updParent.bits() == update_desc::Clean) {
                if (!pOp) pOp = alloc_update_desc();
