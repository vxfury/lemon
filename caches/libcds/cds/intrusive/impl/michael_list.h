...
/** @ingroup cds_intrusive_list
   ...
   michael_list::node).
    - \p T - type to be stored in the list. The type must be based on \p michael_list::node (for \p
   michael_list::base_hook) or it must have a member of type \p michael_list::node (for \p michael_list::member_hook).
   ...
   hook option ,cds::intrusive::opt::compare< my_compare >     // item comparator option
            >::type
        >     option_based_list;
        \endcode
...
class MichaelList {
  public:
    ...
    typedef
        typename michael_list::get_link_checker<node_type, traits::link_checker>::type link_checker; ///< link checker
    ...
    struct rebind_traits {
        typedef MichaelList<gc, value_type, typename cds::opt::make_options<traits, Options...>::type> type;
    };
  ...
  protected:
    ...
    struct position {
        ...
        enum { guard_prev_item, guard_current_item, guard_next_item };
    };
  ...
  protected:
    ...
    static bool link_node(node_type *pNode, position &pos)
    {
        ...
        if (cds_likely(pos.pPrev->compare_exchange_strong(
                cur, marked_node_ptr(pNode), memory_model::memory_order_release, atomics::memory_order_relaxed)))
            return true;
    ...
    static bool unlink_node(position &pos)
    {
        ...
        if (cds_likely(pos.pCur->m_pNext.compare_exchange_strong(next, marked_node_ptr(pos.pNext, 1),
                                                                 memory_model::memory_order_release,
                                                                 atomics::memory_order_relaxed))) {
            // physical deletion may be performed by search function if it detects that a node is logically deleted
            // (marked) CAS may be successful here or in other thread that searching something
            marked_node_ptr cur(pos.pCur);
            if (cds_likely(pos.pPrev->compare_exchange_strong(cur, marked_node_ptr(pos.pNext),
      ...
      protected:
        ...
        iterator_type(atomic_node_ptr const &pNode)
        {
            for (;;) {
                ...
                if (cds_likely(p == pNode.load(memory_model::memory_order_acquire))) break;
            }
        }
  ...
  public:
    ...
    explicit MichaelList(Stat &st) : m_pHead(nullptr), m_Stat(st)
    {
    }
    //@endcond
    ...
    */
    void clear()
    {
        ...
        while (true) {
            ...
            if (head.ptr()) guard.assign(node_traits::to_value_ptr(*head.ptr()));
            if (cds_likely(m_pHead.load(memory_model::memory_order_acquire) == head)) {
                if (head.ptr() == nullptr) break;
  ...
  protected:
    ...
    try_again:
        ...
        pCur = pos.guards.protect(position::guard_current_item, *pPrev, [](marked_node_ptr p) -> value_type * {
        ...
        while (true) {
            ...
            pNext = pos.guards.protect(position::guard_next_item, pCur->m_pNext, [](marked_node_ptr p) -> value_type * {
                return node_traits::to_value_ptr(p.ptr());
            });
            ...
            if (pNext.bits() == 1) {
                ...
                if (cds_unlikely(pPrev->compare_exchange_strong(cur, marked_node_ptr(pNext.ptr()),
                                                                memory_model::memory_order_acquire,
                                                                atomics::memory_order_relaxed))) {
