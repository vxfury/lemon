...
/** @ingroup cds_intrusive_map
    ...
    which, in combination with <b>perfect hashing</b>, means that each element has a unique final, as well as current,
   position. It is important to note that the perfect hash function required by our hash map is trivial to realize as
    ...
    The multi-level array differs from a tree in that each position on the tree could hold an array of nodes or a single
   node. A position that holds a single node is a \p dataNode which holds the hash value of a key and the value that is
   associated with that key; it is a simple struct holding two variables. A \p dataNode in the multi-level array could
   be marked. A \p markedDataNode refers to a pointer to a \p dataNode that has been bitmarked at the least significant
   bit (LSB) of the pointer to the node. This signifies that this \p dataNode is contended. An expansion must occur at
   this node; any thread that sees this \p markedDataNode will try to replace it with an \p arrayNode; which is a
   position that holds an array of nodes. The pointer to an \p arrayNode is differentiated from that of a pointer to a
   \p dataNode by a bitmark on the second-least significant bit.

    \p %FeldmanHashSet multi-level array is similar to a tree in that we keep a pointer to the root, which is a memory
   array called \p head. The length of the \p head memory array is unique, whereas every other \p arrayNode has a
   uniform length; a normal \p arrayNode has a fixed power-of-two length equal to the binary logarithm of a variable
   called \p arrayLength. The maximum depth of the tree, \p maxDepth, is the maximum number of pointers that must be
   followed to reach any node. We define \p currentDepth as the number of memory arrays that we need to traverse to
   reach the \p arrayNode on which we need to operate; this is initially one, because of \p head.

    That approach to the structure of the hash set uses an extensible hashing scheme; <b> the hash value is treated as a
   bit string</b> and rehash incrementally.
    ...
    - all keys must be fixed-size. It means that you cannot use \p std::string as a key for \p %FeldmanHashSet.
      Instead, for the strings you should use well-known hashing algorithms like <a
   href="https://en.wikipedia.org/wiki/Secure_Hash_Algorithm">SHA1, SHA2</a>, <a
   href="https://en.wikipedia.org/wiki/MurmurHash">MurmurHash</a>, <a
   href="https://en.wikipedia.org/wiki/CityHash">CityHash</a> or its successor <a
   href="https://code.google.com/p/farmhash/">FarmHash</a> and so on, which converts variable-length strings to
   fixed-length bit-strings, and use that hash as a key in \p %FeldmanHashSet.
    - \p %FeldmanHashSet uses a perfect hashing. It means that if two different keys, for example, of type \p
   std::string, have identical hash then you cannot insert both that keys in the set. \p %FeldmanHashSet does not
   maintain the key, it maintains its fixed-size hash value.
    ...
    - \p Traits - type traits, the structure based on \p feldman_hashset::traits or result of \p
   feldman_hashset::make_traits metafunction. \p Traits is the mandatory argument because it has one mandatory type - an
   @ref feldman_hashset::traits::hash_accessor "accessor" to hash value of \p T. The set algorithm does not calculate
   that hash value.
    ...
    - <tt><cds/intrusive/feldman_hashset_rcu.h></tt> for \ref cds_intrusive_FeldmanHashSet_rcu "RCU type". RCU
   specialization has a slightly different interface.
*/
...
class FeldmanHashSet : protected feldman_hashset::multilevel_array<T, Traits> {
  ...
  public:
    ...
    typedef typename base_class::hash_comparator
        hash_comparator; ///< hash compare functor based on \p traits::compare and \p traits::less options
  ...
  protected:
    ...
    class iterator_base {
      ...
      public:
        iterator_base() noexcept : m_pNode(nullptr), m_idx(0), m_set(nullptr) {}

        iterator_base(iterator_base const &rhs) noexcept : m_pNode(rhs.m_pNode), m_idx(rhs.m_idx), m_set(rhs.m_set)
        {
      ...
      protected:
        iterator_base(FeldmanHashSet const &set, array_node *pNode, size_t idx, bool)
            : m_pNode(pNode), m_idx(idx), m_set(&set)
        {
        }
            ...
            for (;;) {
                if (idx < nodeSize) {
                    ...
                    if (slot.bits() == base_class::flag_array_node) {
                        ...
                        if (slot.ptr()) {
                            // data node
                            if (m_guard.protect(pNode->nodes[idx],
                                                [](node_ptr p) -> value_type * {
                                                    return p.ptr();
                                                })
                                == slot) {
                                m_pNode = pNode;
    ...
    class bidirectional_iterator : protected iterator_base {
      ...
      public:
        ...
        typedef
            typename std::conditional<IsConst, value_type const &, value_type &>::type value_ref; ///< Value reference

      public:
        bidirectional_iterator() noexcept {}

        bidirectional_iterator(bidirectional_iterator const &rhs) noexcept : iterator_base(rhs) {}
      ...
      protected:
        ...
        bidirectional_iterator(FeldmanHashSet &set, array_node *pNode, size_t idx) : iterator_base(set, pNode, idx) {}
    };
    ...
    class reverse_bidirectional_iterator : public iterator_base {
      ...
      public:
        reverse_bidirectional_iterator() noexcept : iterator_base() {}

        reverse_bidirectional_iterator(reverse_bidirectional_iterator const &rhs) noexcept : iterator_base(rhs) {}
  ...
  public:
    ...
    typedef implementation_defined
        iterator; ///< @ref cds_intrusive_FeldmanHashSet_iterators "bidirectional iterator" type
    typedef implementation_defined
        const_iterator; ///< @ref cds_intrusive_FeldmanHashSet_iterators "bidirectional const iterator" type
    typedef implementation_defined
        reverse_iterator; ///< @ref cds_intrusive_FeldmanHashSet_iterators "bidirectional reverse iterator" type
    typedef implementation_defined const_reverse_iterator; ///< @ref cds_intrusive_FeldmanHashSet_iterators
  ...
  public:
    ...
    */
    FeldmanHashSet(size_t head_bits = 8, size_t array_bits = 4) : base_class(head_bits, array_bits) {}
    ...
    */
    ...
    bool insert(value_type &val, Func f)
    {
        ...
        while (true) {
            ...
            if (guards.protect(0, pos.pArr->nodes[pos.nSlot],
                               [](node_ptr p) -> value_type * {
                                   return p.ptr();
                               })
                != slot) {
                ...
                node_ptr pNull;
                if (pos.pArr->nodes[pos.nSlot].compare_exchange_strong(
                        pNull, node_ptr(&val), memory_model::memory_order_release, atomics::memory_order_relaxed)) {
    ...
    */
    bool unlink(value_type const &val)
    {
        ...
        auto pred = [&val](value_type const &item) -> bool {
            return &item == &val;
        };
    ...
    */
    ...
    {
        typename gc::Guard guard;
        value_type *p = do_erase(hash, guard, [](value_type const &) -> bool {
            return true;
        });
    ...
    */
    ...
    {
        typename gc::Guard guard;
        if (do_erase(hash, guard, [](value_type const &) -> bool {
                return true;
            }))
    ...
    */
    ...
    {
        typename gc::Guard guard;
        if (search(hash, guard)) return guarded_ptr(std::move(guard));
        return guarded_ptr();
    }
  ...
  public:
    ...
    /** @anchor cds_intrusive_FeldmanHashSet_iterators
        ...
        It is guaranteed that the iterators will remain valid even if another thread deletes the node the iterator
       points to: Hazard Pointer embedded into the iterator object protects the node from physical reclamation.
        ...
        - helper member function \p release() that clears internal hazard pointer.
            After \p release() the iterator points to \p nullptr but it still remain valid: further iterating is
       possible.
    ...
    */
    ...
    /// Returns an iterator to the element following the last element of the set. This element acts as a placeholder;
    ...
    /// Returns a const iterator to the element following the last element of the set. This element acts as a
            ...
            while (true) {
                ...
                if (slot.bits() == base_class::flag_array_node) {
                    ...
                    while ((slot = pArr->load(memory_model::memory_order_acquire)).bits()
                           == base_class::flag_array_converting) {
                        bkoff();
                    ...
                    // data node
                    if (pArr->compare_exchange_strong(slot, node_ptr(), memory_model::memory_order_acquire,
  ...
  protected:
    ...
    value_type *search(hash_type const &hash, typename gc::Guard &guard)
    {
        ...
        while (true) {
            ...
            // protect data node by hazard pointer
            if (guard.protect(pos.pArr->nodes[pos.nSlot],
                              [](node_ptr p) -> value_type * {
                                  return p.ptr();
                              })
                != slot) {
        ...
        while (true) {
            ...
            if (guard.protect(pos.pArr->nodes[pos.nSlot],
                ...
                if (cmp(hash, hash_accessor()(*slot.ptr())) == 0 && pred(*slot.ptr())) {
                    ...
                    if (pos.pArr->nodes[pos.nSlot].compare_exchange_strong(slot, node_ptr(nullptr),
    ...
    {
        if (iter.m_set != this) return false;
        if (iter.m_pNode == head()) {
            if (iter.m_idx >= head_size()) return false;
        ...
        for (;;) {
            ...
            if (slot.bits() == 0 && slot.ptr() == iter.pointer()) {
                if (iter.m_pNode->nodes[iter.m_idx].compare_exchange_strong(
                        slot, node_ptr(nullptr), memory_model::memory_order_acquire, atomics::memory_order_relaxed)) {
        ...
        while (true) {
            ...
            if (guards.protect(0, pos.pArr->nodes[pos.nSlot],
                ...
                if (cmp(hash, hash_accessor()(*slot.ptr())) == 0) {
                    ...
                    if (pos.pArr->nodes[pos.nSlot].compare_exchange_strong(
                            slot, node_ptr(&val), memory_model::memory_order_release, atomics::memory_order_relaxed)) {
