...
/** @ingroup cds_intrusive_map
    ...
    <b>Template arguments</b>:
        - \p GC - Garbage collector used. Note the \p GC must be the same as the GC used for item type \p T, see \p
   skip_list::node.
   ...
   istead of \p Traits template argument.
    ...
    \endcode
    Note, the iterator object returned by \p end(), \p cend() member functions points to \p nullptr and should not be
   dereferenced.
...
class SkipListSet {
  public:
    ...
    typedef implementation_defined
        key_comparator; ///< key comparison functor based on opt::compare and opt::less option setter.
#else
  ...
  public:
    ...
    /**
       ...
       m_nUpperBound but it should be no more than 32 (\p skip_list::c_nHeightLimit).
    */
    ...
    static size_t const c_nHazardPtrCount =
        c_nMaxHeight * 2 + 3; ///< Count of hazard pointer required for the skip-list
  ...
  public:
    ...
    */
    SkipListSet() : m_Head(c_nMaxHeight), m_nHeight(c_nMinHeight)
    {
  ...
  public:
    ...
    /**
       ...
       (or assertion) "no free guard" may be thrown if the limit of guard count per thread is exceeded.
        ...
        - Iterator ensures thread-safety even if you delete the item the iterator points to. However, in case of
       concurrent deleting operations there is no guarantee that you iterate all item in the list. Moreover, a crash is
       possible when you try to iterate the next element that has been deleted by concurrent thread.
  ...
  protected:
    ...
    bool insert_at_position(value_type &val, node_type *pNode, position &pos, Func f)
    {
        ...
        for (unsigned int nLevel = 1; nLevel < nHeight; ++nLevel) {
            ...
            while (true) {
                ...
                if (!pNode->next(nLevel).compare_exchange_strong(p, pSucc, memory_model::memory_order_release,
    ...
    bool try_remove_at(node_type *pDel, position &pos, Func f)
    {
        ...
        for (unsigned int nLevel = pDel->height() - 1; nLevel > 0; --nLevel) {
            ...
            if (pSucc.bits() == 0) {
                bkoff.reset();
                while (!(pDel->next(nLevel).compare_exchange_weak(pSucc, pSucc | 1, memory_model::memory_order_release,
                                                                  atomics::memory_order_acquire)
                         || pSucc.bits() != 0)) {
                    bkoff();
                    m_Stat.onMarkFailed();
        ...
        while (true) {
            if (pDel->next(0).compare_exchange_strong(p, p | 1, memory_model::memory_order_release,
                                                      atomics::memory_order_acquire)) {
    ...
    {
        guarded_ptr gp;
        if (find_with_(val, cmp, [&gp](value_type &found, Q const &) {
                gp.reset(&found);
            }))
            return gp;
        return guarded_ptr();
    ...
    void increase_height(unsigned int nHeight)
    {
        ...
        if (nCur < nHeight)
            m_nHeight.compare_exchange_strong(nCur, nHeight, memory_model::memory_order_relaxed,
                                              atomics::memory_order_relaxed);
    }
