...
/** @ingroup cds_intrusive_list
    ...
    Template arguments:
    - \p GC - Garbage collector used. Note the \p GC must be the same as the GC used for item type \p T (see
   lazy_list::node).
    ...
    - \p Traits - type traits. See lazy_list::traits for explanation.
        It is possible to declare option-based list with cds::intrusive::lazy_list::make_traits metafunction instead of
   \p Traits template argument. For example, the following traits-based declaration of \p gc::HP lazy list \code
        ...
        typedef cds::intrusive::LazyList< cds::gc::HP, item,
            typename cds::intrusive::lazy_list::make_traits<
                cds::intrusive::opt::hook< cds::intrusive::lazy_list::base_hook< cds::opt::gc< cds::gc::HP > > >    //
...
class LazyList {
  public:
    ...
    typedef
        typename traits::memory_model memory_model; ///< C++ memory ordering (see \p lazy_list::traits::memory_model)
    ...
    struct rebind_traits {
        typedef LazyList<gc, value_type, typename cds::opt::make_options<traits, Options...>::type> type;
    };
  ...
  protected:
    ...
    struct position {
        ...
        enum { guard_prev_item, guard_current_item };
  ...
  protected:
    ...
    void unlink_node(node_type *pPred, node_type *pCur, node_type *pHead)
    {
        ...
        pCur->m_pNext.store(marked_node_ptr(pHead, 1),
                            memory_model::memory_order_release); // logical removal + back-link for search
      ...
      protected:
        ...
        void next()
        {
            ...
            if (m_pNode) {
                ...
                if (pCur->m_pNext.load(memory_model::memory_order_relaxed).ptr()
                    != nullptr) { // if pCur is not tail node
                    node_type *pNext;
                    do {
        ...
        void skip_deleted()
        {
            if (m_pNode != nullptr) {
                ...
                while (pNode->is_marked()) {
                    ...
                    if (p == pNode->m_pNext.load(memory_model::memory_order_relaxed).ptr()) pNode = p;
                }
                if (pNode != node_traits::to_node_ptr(m_pNode)) m_pNode = m_Guard.assign(g.template get<value_type>());
            }
        }
  ...
  public:
    ...
    /**
       ...
       increment operator. For some GC (\p gc::HP), a guard is limited resource per thread, so an exception (or
       assertion) "no free guard" may be thrown if a limit of guard count per thread is exceeded.
        - The iterator cannot be moved across thread boundary since it contains GC's guard that is thread-private GC
       data.
        - Iterator ensures thread-safety even if you delete the item that iterator points to. However, in case of
       concurrent deleting operations it is no guarantee that you iterate all item in the list. Moreover, a crash is
  ...
  public:
    ...
    explicit LazyList(Stat &st) : m_Stat(st)
    {
        m_Head.m_pNext.store(marked_node_ptr(&m_Tail), memory_model::memory_order_relaxed);
    }
    //@endcond
  ...
  protected:
    ...
    bool find_at(node_type *pHead, Q &val, Compare cmp, Func f)
    {
        ...
        if (pos.pCur != &m_Tail) {
            ...
            if (!pos.pCur->is_marked() && cmp(*node_traits::to_value_ptr(*pos.pCur), val) == 0) {
  ...
  protected:
    ...
    void search(node_type *pHead, const Q &key, position &pos, Compare cmp)
    {
        ...
        while (pCur.ptr() != pTail) {
            if (pCur.ptr() != pHead) {
                if (cmp(*node_traits::to_value_ptr(*pCur.ptr()), key) >= 0) break;
            }

            pos.guards.copy(position::guard_prev_item, position::guard_current_item);
            pPrev = pCur;

            pCur = pos.guards.protect(position::guard_current_item, pPrev->m_pNext, [](marked_node_ptr p) {
                return node_traits::to_value_ptr(p.ptr());
            });
            assert(pCur.ptr() != nullptr);
            if (pCur.bits()) pPrev = pCur = pHead;
        }
    ...
    {
        return !pPred->is_marked() && !pCur->is_marked()
