...
struct stat {
    ...
    event_counter m_nItemMovedTop;  ///< Count of events when \p push() encountered that inserted item was moved to top
                                    ///< by a concurrent \p pop()
    event_counter m_nItemMovedUp;   ///< Count of events when \p push() encountered that inserted item was moved upwards
                                    ///< by a concurrent \p pop()
    ...
    {
        ++m_nPushHeapifySwapCount;
    }
    void onPopHeapifySwap()
    {
        ++m_nPopHeapifySwapCount;
    }
...
/**
    \p Options:
    - \p opt::buffer - the buffer type for heap array. Possible type are: \p opt::v::initialized_static_buffer, \p
   opt::v::initialized_dynamic_buffer. Default is \p %opt::v::initialized_dynamic_buffer. You may specify any type of
   value for the buffer since at instantiation time the \p buffer::rebind member metafunction is called to change the
   type of values stored in the buffer.
    ...
    - \p opt::stat - internal statistics. Available types: \p mspriority_queue::stat, \p mspriority_queue::empty_stat
...
class MSPriorityQueue : public cds::bounded_container {
  public:
    ...
    typedef implementation_defined
        key_comparator; ///< priority comparing functor based on opt::compare and opt::less option setter.
#else
  ...
  protected:
    ...
    enum tag_value { Available = -1, Empty = 0 };
    //@endcond
    ...
    struct node {
        ...
        /// Creates empty node
        node() : m_pVal(nullptr), m_nTag(tag_type(Empty)) {}
  ...
  public:
    ...
    */
    MSPriorityQueue(size_t nCapacity) : m_Heap(nCapacity) {}
  ...
  protected:
    ...
    void heapify_after_push(counter_type i, tag_type curId)
    {
        ...
        if (i == 1) {
            node &refItem = m_Heap[i];
            refItem.lock();
            if (refItem.m_nTag == curId) refItem.m_nTag = tag_type(Available);
            refItem.unlock();
        }
    }
