...
struct hash_node {
    ...
    explicit hash_node(size_t nHash) : m_nHash(nHash)
    {
        assert(is_dummy());
    }
...
struct node : public OrderedListNode, public hash_node {
    ...
    /// Initializes dummy node with \p nHash value
    explicit node(size_t nHash) : hash_node(nHash)
    {
        assert(is_dummy());
    }
...
struct stat {
    ...
    void onHeadNodeAllocated()
    {
        ++m_nHeadNodeAllocated;
    }
    void onHeadNodeFreed()
    {
        ++m_nHeadNodeFreed;
    }
    void onNewBucket()
    ...
    void onRecursiveInitBucket()
    {
        ++m_nInitBucketRecursive;
    }
    void onBucketInitContenton()
    {
        ++m_nInitBucketContention;
    }
    void onBusyWaitBucketInit()
    {
        ++m_nBusyWaitBucketInit;
    }
    void onBucketsExhausted()
    {
        ++m_nBucketsExhausted;
    }
    //@endcond
...
struct empty_stat {
    ...
    bool onFind(bool bSuccess) const
    {
        return bSuccess;
    }
...
struct traits {
    ...
    */
    ...
    /**
        ...
        for example, based on x86 SIMD/AVX instruction set, see <a
       href="http://stackoverflow.com/questions/746171/best-algorithm-for-bit-reversal-from-msb-lsb-to-lsb-msb-in-c">here</a>
    */
    ...
    /**
        ...
        Default is \p cds::atomicity::item_counter; to avoid false sharing you may use \p
       atomicity::cache_friendly_item_counter
    */
...
/**
    ...
    - \p opt::memory_model - memory model used. Possible types are \p opt::v::sequential_consistent, \p
   opt::v::relaxed_ordering
    - \p opt::free_list - free-list implementation; default is \p TaggedFreeList if the processor supports double-with
   CAS otherwise \p FreeList.
*/
...
class static_bucket_table {
  ...
  public:
    ...
    static_bucket_table() : m_nLoadFactor(1), m_nCapacity(512 * 1024), m_nAuxNodeAllocated(0)
    {
        allocate_table();
    }

    /// Creates the table with specified size rounded up to nearest power-of-two
    static_bucket_table(size_t nItemCount, ///< Max expected item count in split-ordered list
    ...
    aux_node_type *alloc_aux_node()
    {
        ...
        if (pFree) return static_cast<aux_node_type *>(pFree);
...
class expandable_bucket_table {
  ...
  protected:
    ...
    struct aux_node_segment {
        ...
        aux_node_segment() : next_segment(nullptr)
        {
  ...
  public:
    /// Constructs bucket table for 512K buckets. Load factor is 1.
    expandable_bucket_table() : m_metrics(calc_metrics(512 * 1024, 1))
    {
        init();
    }

    /// Creates the table with specified capacity rounded up to nearest power-of-two
    expandable_bucket_table(size_t nItemCount, ///< Max expected item count in split-ordered list
                            size_t nLoadFactor ///< Load factor
                            )
    ...
    ~expandable_bucket_table()
    {
        ...
        for (size_t i = 0; i < m_metrics.nSegmentCount; ++i) {
            ...
            if (pEntry != nullptr) destroy_segment(pEntry);
        }
    ...
    aux_node_type *bucket(size_t nBucket) const
    {
        ...
        if (pSegment == nullptr) return nullptr; // uninitialized bucket
    ...
    void bucket(size_t nBucket, aux_node_type *pNode)
    {
        ...
        if (segment.load(memory_model::memory_order_relaxed) == nullptr) {
            ...
            if (!segment.compare_exchange_strong(pNull, pNewSegment, memory_model::memory_order_release,
                                                 atomics::memory_order_relaxed))
                destroy_segment(pNewSegment);
        }

        assert(segment.load(atomics::memory_order_relaxed)[nBucket & (m_metrics.nSegmentSize - 1)].load(
                   atomics::memory_order_relaxed)
               == nullptr);
        segment.load(memory_model::memory_order_acquire)[nBucket & (m_metrics.nSegmentSize - 1)].store(
            pNode, memory_model::memory_order_release);
    }
    ...
    aux_node_type *alloc_aux_node()
    {
        ...
        for (;;) {
            ...
            if (m_auxNodeList.compare_exchange_strong(aux_segment, new_aux_segment, memory_model::memory_order_release,
  ...
  protected:
    ...
    metrics calc_metrics(size_t nItemCount, size_t nLoadFactor)
    {
        ...
        if (nBucketCount <= 2) {
            ...
            m.nSegmentCount = m.nSegmentSize = ((size_t)1) << (nBucketCount / 2);
            if (nBucketCount & 1) m.nSegmentSize *= 2;
            if (m.nSegmentCount * m.nSegmentSize * m.nLoadFactor < nItemCount) m.nSegmentSize *= 2;
        }
    ...
    {
        raw_allocator().deallocate(reinterpret_cast<char *>(p),
                                   sizeof(aux_node_segment) + sizeof(aux_node_type) * m_metrics.nSegmentSize);
    }
...
struct search_value_type {
    ...
    search_value_type(Q &v, size_t h) : val(v), nHash(h) {}
};
...
class ordered_list_adapter<OrderedList, Traits, false> {
    ...
    struct key_compare {
        ...
        {
            splitlist_node_type const *n1 =
                static_cast<splitlist_node_type const *>(native_node_traits::to_node_ptr(v1));
            splitlist_node_type const *n2 =
                static_cast<splitlist_node_type const *>(native_node_traits::to_node_ptr(v2));
            if (n1->m_nHash != n2->m_nHash) return n1->m_nHash < n2->m_nHash ? -1 : 1;
        ...
        int operator()(value_type const &v, search_value_type<Q> const &q) const
        {
            ...
            if (n->m_nHash != q.nHash) return n->m_nHash < q.nHash ? -1 : 1;
    ...
    struct wrapped_disposer {
        void operator()(value_type *v)
        {
            ...
            if (!p->is_dummy()) native_disposer()(v);
        }
    };
  ...
  public:
    ...
    struct make_compare_from_less : public cds::opt::details::make_comparator_from_less<Less> {
        ...
        {
            splitlist_node_type const *n = static_cast<splitlist_node_type const *>(native_node_traits::to_node_ptr(v));
            if (n->m_nHash != q.nHash) return q.nHash < n->m_nHash ? -1 : 1;
        ...
        {
            splitlist_node_type const *n1 =
                static_cast<splitlist_node_type const *>(native_node_traits::to_node_ptr(lhs));
            splitlist_node_type const *n2 =
                static_cast<splitlist_node_type const *>(native_node_traits::to_node_ptr(rhs));
            if (n1->m_nHash != n2->m_nHash) return n1->m_nHash < n2->m_nHash ? -1 : 1;
    ...
    typedef
        typename native_ordered_list::template rebind_traits<opt::compare<key_compare>, opt::disposer<wrapped_disposer>,
                                                             opt::boundary_node_type<splitlist_node_type> >::type
            result;
};
...
class ordered_list_adapter<OrderedList, Traits, true> {
    ...
    struct wrapped_disposer {
        void operator()(value_type *v)
        {
            if (!static_cast<hash_node *>(v)->is_dummy()) native_disposer()(v);
        }
    };
  ...
  public:
    ...
    struct aux_node : public native_ordered_list::node_type, public hash_node {
        aux_node()
        {
            ...
            list_node_type::data.store(
                typename list_node_type::marked_data_ptr(static_cast<value_type *>(static_cast<hash_node *>(this))),
    ...
    typedef
        typename native_ordered_list::template rebind_traits<opt::compare<key_compare>, opt::disposer<wrapped_disposer>,
                                                             opt::boundary_node_type<aux_node> >::type result;
};
...
class iterator_type {
  ...
  public:
    ...
    iterator_type(iterator_type const &src) : m_itCur(src.m_itCur), m_itEnd(src.m_itEnd) {}

    // This ctor should be protected...
    iterator_type(list_iterator itCur, list_iterator itEnd) : m_itCur(itCur), m_itEnd(itEnd)
    {
        // skip dummy nodes
        while (m_itCur != m_itEnd && node_traits::to_node_ptr(*m_itCur)->is_dummy()) ++m_itCur;
    }
