...
struct traits {
    ...
    */
    ...
    /**
        ...
        Default is \p atomicity::item_counter; to avoid false sharing you may use \p
...
{
    if (nLoadFactor == 0) nLoadFactor = 1;
    if (nMaxItemCount == 0) nMaxItemCount = 4;
...
class iterator {
  ...
  protected:
    ...
    void next()
    {
        if (m_pCurBucket < m_pEndBucket) {
            if (++m_itList != m_pCurBucket->end()) return;
  ...
  public:
    iterator() : m_pCurBucket(nullptr), m_itList(), m_pEndBucket(nullptr) {}

    iterator(list_iterator const &it, bucket_ptr pFirst, bucket_ptr pLast)
        : m_pCurBucket(pFirst), m_itList(it), m_pEndBucket(pLast)
    {
        if (it == pFirst->end()) next();
    }

    iterator(iterator const &src)
        : m_pCurBucket(src.m_pCurBucket), m_itList(src.m_itList), m_pEndBucket(src.m_pEndBucket)
    {
    }
