...
struct wrapped_stat {
    ...
    wrapped_stat(stat_type &st) : m_stat(st) {}
...
struct traits {
    ...
    */
    ...
    /// Item counting feature; by default, disabled. Use \p cds::atomicity::item_counter or \p
    /// atomicity::cache_friendly_item_counter to enable item counting
...
/**
    ...
    - \p opt::hook - hook used. Possible values are: \p lazy_list::base_hook, \p lazy_list::member_hook, \p
   lazy_list::traits_hook. If the option is not specified, \p %lazy_list::base_hook and \p gc::HP is used.
    - \p opt::compare - key comparison functor. No default functor is provided.
        If the option is not specified, the \p opt::less is used.
    - \p opt::less - specifies binary predicate used for key comparison. Default is \p std::less<T>.
    - \p opt::equal_to - specifies binary functor for comparing keys for equality. This option is applicable only for
   unordered list. If \p equal_to is not specified, \p compare is used, \p compare is not specified, \p less is used.
   ...
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
    - \p opt::rcu_check_deadlock - a deadlock checking policy for \ref cds_intrusive_MichaelList_rcu "RCU-based
   MichaelList" Default is \p opt::v::rcu_throw_deadlock
*/
template <typename... Options>
struct make_traits {
