...
class node {
  ...
  public:
    node() : m_pNext(nullptr), m_nHeight(1), m_arrNext(nullptr)
    {
...
/**
    ...
    - <tt>random_generator()</tt> - the constructor of generator object initialises the generator instance (its internal
   state).
    - <tt>unsigned int operator()()</tt> - the main generating function. Returns random level from range <tt>[0 ..
   c_nUpperBound - 1]</tt>
...
class xor_shift {
  ...
  public:
    /// The upper bound of generator's return value. The generator produces random number in range
    /// <tt>[0..c_nUpperBound)</tt>
...
struct stat {
    ...
    void onFindFastSuccess()
    {
        ++m_nFindFastSuccess;
    }
    void onFindFastFailed()
    {
        ++m_nFindFastFailed;
    }
    void onFindSlowSuccess()
    {
        ++m_nFindSlowSuccess;
    }
    void onFindSlowFailed()
    {
        ++m_nFindSlowFailed;
    }
    void onEraseWhileFind()
    {
        ++m_nEraseWhileFind;
    }
    void onExtractWhileFind()
    {
        ++m_nExtractWhileFind;
    }
    void onRenewInsertPosition()
    {
        ++m_nRenewInsertPosition;
    }
    void onLogicDeleteWhileInsert()
    {
        ++m_nLogicDeleteWhileInsert;
    }
    void onRemoveWhileInsert()
    {
        ++m_nRemoveWhileInsert;
    }
    void onFastErase()
    {
        ++m_nFastErase;
...
/**
    \p Options are:
    - \p opt::hook - hook used. Possible values are: \p skip_list::base_hook, \p skip_list::member_hook, \p
   skip_list::traits_hook. If the option is not specified, <tt>skip_list::base_hook<></tt> and \p gc::HP is used.
    - \p opt::compare - key comparison functor. No default functor is provided.
        If the option is not specified, the \p opt::less is used.
    - \p opt::less - specifies binary predicate used for key comparison. Default is \p std::less<T>.
    - \p opt::disposer - the functor used for dispose removed items. Default is \p opt::v::empty_disposer. Due the
   nature of GC schema the disposer may be called asynchronously.
    - \p opt::item_counter - the type of item counting feature. Default is disabled, i.e. \p
   ...
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
    ...
    - \p opt::allocator - although the skip-list is an intrusive container,
        an allocator should be provided to maintain variable randomly-calculated height of the node
        since the node can contain up to 32 next pointers. The allocator option is used to allocate an array of next
   pointers for nodes which height is more than 1. Default is \ref CDS_DEFAULT_ALLOCATOR.
...
struct intrusive_node_builder {
    ...
    {
        if (nHeight > 1) pNode->make_tower(nHeight, tower_allocator().NewArray(nHeight - 1, nullptr));
        return pNode;
    }
    ...
    static void dispose_tower(node_type *pNode)
    {
        ...
        if (nHeight > 1) tower_allocator().Delete(pNode->release_tower(), nHeight);
    }
