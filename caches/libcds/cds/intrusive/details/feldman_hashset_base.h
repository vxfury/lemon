...
struct stat {
    ...
    void onExpandNodeSuccess()
    {
        ++m_nExpandNodeSuccess;
    }
    void onExpandNodeFailed()
    {
        ++m_nExpandNodeFailed;
    }
    void onSlotChanged()
    ...
    void onSlotConverting()
    {
        ++m_nSlotConverting;
    }
    void onArrayNodeCreated()
    {
        ++m_nArrayNodeCount;
    }
    void height(size_t h)
    {
        if (m_nHeight < h) m_nHeight = h;
    }
    //@endcond
};
...
struct traits {
    ...
    */
    ...
    /**
        If \p %less and \p %compare are not specified, \p memcmp() -like @ref bitwise_compare "bit-wise hash comparator"
       is used because the hash value is treated as fixed-sized bit-string.
    */
    ...
    /**
        ...
        Default is \p atomicity::item_counter. To avoid false sharing you can aldo use \p
...
/**
    ...
    - \p opt::disposer - the functor used for disposing removed data node. Default is \p opt::v::empty_disposer. Due the
   nature of GC schema the disposer may be called asynchronously.
    - \p opt::item_counter - the type of item counting feature.
         The item counting is an important part of \p FeldmanHashSet algorithm:
         the \p empty() member function depends on correct item counting.
         Therefore, \p atomicity::empty_item_counter is not allowed as a type of the option.
         Default is \p atomicity::item_counter. To avoid false sharing you can use or \p
   atomicity::cache_friendly_item_counter
   ...
   default) or \p opt::v::sequential_consistent (sequentially consisnent memory model).
    ...
    - \p opt::rcu_check_deadlock - a deadlock checking policy for \ref cds_intrusive_FeldmanHashSet_rcu "RCU-based
   FeldmanHashSet" Default is \p opt::v::rcu_throw_deadlock
*/
...
struct level_statistics {
    ...
    //@cond
    level_statistics() : array_node_count(0), data_cell_count(0), array_cell_count(0), empty_cell_count(0) {}
    //@endcond
};
...
struct metrics {
    ...
    static metrics make(size_t head_bits, size_t array_bits, size_t hash_size)
    {
        ...
        if (array_bits < 2) array_bits = 2;
        if (head_bits < 4) head_bits = 4;
        if (head_bits > hash_bits) head_bits = hash_bits;
        if ((hash_bits - head_bits) % array_bits != 0) head_bits += (hash_bits - head_bits) % array_bits;
...
class multilevel_array {
  public:
    ...
    typedef typename std::decay<
        typename std::remove_reference<decltype(hash_accessor()(std::declval<value_type>()))>::type>::type hash_type;
    ...
    typedef typename cds::opt::details::make_comparator_from<
        hash_type, traits, feldman_hashset::bitwise_compare<hash_type> >::type hash_comparator;

    /// The size of hash_type in bytes, see \p traits::hash_size for explanation
    static constexpr size_t const c_hash_size =
        traits::hash_size == 0 ? sizeof(hash_type) : static_cast<size_t>(traits::hash_size);

    typedef typename std::conditional<std::is_same<typename traits::hash_splitter, cds::opt::none>::value,
                                      typename cds::algo::select_splitter<hash_type, c_hash_size>::type,
                                      typename traits::hash_splitter>::type hash_splitter;
  ...
  protected:
    ...
    struct array_node {
        ...
        array_node(array_node *parent, size_t idx) : pParent(parent), idxParent(idx) {}
    ...
    struct traverse_data {
        ...
        traverse_data(hash_type const &hash, multilevel_array &arr) : splitter(hash)
        {
            reset(arr);
        }
  ...
  protected:
    ...
    {
        array_node *pNode = cxx_array_node_allocator().NewBlock(
            sizeof(array_node) + sizeof(atomic_node_ptr) * (nSize - 1), pParent, idxParent);
    ...
    union converter {
        ...
        converter(value_type *p) : pData(p) {}

        converter(array_node *p) : pArr(p) {}
    };
    ...
    void gather_level_statistics(std::vector<feldman_hashset::level_statistics> &stat, size_t nLevel, array_node *pArr,
                                 size_t nSize) const
    {
        if (stat.size() <= nLevel) {
  ...
  private:
    bool expand_slot(array_node *pParent, size_t idxParent, node_ptr current, size_t nOffset)
    {
        ...
        if (!slot.compare_exchange_strong(cur, cur | flag_array_converting, memory_model::memory_order_release,
        ...
        typename hash_splitter::uint_type idx = hash_splitter(hash_accessor()(*current.ptr()), nOffset)
                                                    .cut(static_cast<unsigned>(m_Metrics.array_node_size_log));
        pArr->nodes[idx].store(current, memory_model::memory_order_release);

        cur = cur | flag_array_converting;
        CDS_VERIFY(slot.compare_exchange_strong(cur, node_ptr(to_node(pArr), flag_array_node),
