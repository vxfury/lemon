...
struct node;
template <class GC, typename Key>
...
struct update_desc {
    ...
    enum { Clean = 0, DFlag = 1, IFlag = 2, Mark = 3 };
    ...
    update_desc() : pNextRetire(nullptr) {}
    //@endcond
};
...
struct base_node : public basic_node {
    ...
    /// Constructs leaf (bIntrenal == false) or internal (bInternal == true) node
    explicit base_node(bool bInternal) : base_class(bInternal) {}
};
//@endcond
...
#endif
{
    ...
    /// Default ctor
    internal_node() : base_class(true), m_pLeft(nullptr), m_pRight(nullptr), m_pUpdate(update_ptr())
    {
    ...
    {
        return update_ptr(reinterpret_cast<update_desc_type *>(
            ((m_nEmptyUpdate.fetch_add(1, atomics::memory_order_relaxed) + 1) << 2) & 0xFFFF));
    }
...
struct stat {
    ...
    {
        ++m_nInternalNodeCreated;
    }
    void onInternalNodeDeleted()
    {
        ++m_nInternalNodeDeleted;
    }
    void onUpdateDescCreated()
    {
        ++m_nUpdateDescCreated;
    }
    void onUpdateDescDeleted()
    {
        ++m_nUpdateDescDeleted;
    }
...
/**
    \p Options are:
    - \p opt::hook - hook used. Possible values are: \p ellen_bintree::base_hook, \p ellen_bintree::member_hook, \p
   ellen_bintree::traits_hook. If the option is not specified, <tt>ellen_bintree::base_hook<></tt> is used.
    - \p ellen_bintree::key_extractor - key extracting functor, mandatory option. The functor has the following
   prototype: \code struct key_extractor { void operator ()( Key& dest, T const& src );
            };
        \endcode
   ...
   be defined.
    - \p opt::disposer - the functor used for dispose removed nodes. Default is \p opt::v::empty_disposer. Due the
   nature of GC schema the disposer may be called asynchronously. The disposer is used only for leaf nodes.
...
struct compare {
    ...
    {
        if (n1.infinite_key() != n2.infinite_key()) return n1.infinite_key() - n2.infinite_key();
    ...
    {
        if (n.infinite_key()) return 1;
        if (n.is_leaf()) return operator()(node_traits::to_leaf_node(n), v);
    ...
    {
        if (i.is_leaf()) return operator()(static_cast<LeafNode const &>(i), n);
