...
/** @ingroup cds_intrusive_list
    ...
    - \p Traits - type traits. See \p lazy_list::traits for explanation.
        It is possible to declare option-based list with \p %cds::intrusive::lazy_list::make_traits metafunction instead
   of \p Traits template argument.
...
class LazyList<cds::urcu::gc<RCU>, T, Traits> {
  public:
    ...
    typedef implementation_defined
        key_comparator; ///< key compare functor based on opt::compare and opt::less option setter.
#else
    ...
    static constexpr const bool c_bExtractLockExternal =
        true; ///< Group of \p extract_xxx functions require external locking
    ...
    class iterator_type {
      ...
      protected:
        ...
        void next()
        {
            ...
            if (pNext != nullptr) m_pNode = node_traits::to_value_ptr(pNext);
        }
        ...
        void skip_deleted()
        {
            if (m_pNode != nullptr) {
                ...
                while (pNode->is_marked()) pNode = pNode->m_pNext.load(memory_model::memory_order_acquire).ptr();

                if (pNode != node_traits::to_node_ptr(m_pNode)) m_pNode = node_traits::to_value_ptr(pNode);
            }
        }
  ...
  public:
    ...
    */
    ...
    /**
        ...
        The function can throw \ref cds_urcu_rcu_deadlock "cds::urcu::rcu_deadlock" exception if deadlock is encountered
       and deadlock checking policy is \p opt::v::rcu_throw_deadlock.
    */
    template <typename Q>
  ...
  protected:
    ...
    void unlink_node(node_type *pPred, node_type *pCur, node_type *pHead)
    {
        ...
        pCur->m_pNext.store(marked_node_ptr(pHead, 1),
                            memory_model::memory_order_relaxed); // logical deletion + back-link for search
    ...
    {
        rcu_lock l;
        if (insert_at_locked(pHead, val)) return iterator(node_traits::to_node_ptr(val));
        return end();
    }
