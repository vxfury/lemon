...
/** @ingroup cds_intrusive_freelist
    ...
    The algorithm is taken from <a
   href="http://moodycamel.com/blog/2014/solving-the-aba-problem-for-lock-free-free-lists">this article</a>. The algo
   does not require any SMR like Hazard Pointer to prevent ABA problem.
    ...
    */
    ...
    node *get()
    {
        ...
        while (head != nullptr) {
            ...
            if (cds_unlikely((refs & c_RefsMask) == 0
                             || !head->m_freeListRefs.compare_exchange_strong(
                                 refs, refs + 1, atomics::memory_order_acquire, atomics::memory_order_relaxed))) {
            ...
            if (cds_likely(m_Head.compare_exchange_strong(head, next, atomics::memory_order_acquire,
            ...
            if (refs == c_ShouldBeOnFreeList + 1) add_knowing_refcount_is_zero(prevHead);
        }
  ...
  private:
    ...
    void add_knowing_refcount_is_zero(node *pNode)
    {
        ...
        while (true) {
            ...
            if (cds_unlikely(!m_Head.compare_exchange_strong(head, pNode, atomics::memory_order_release,
                                                             atomics::memory_order_relaxed))) {
