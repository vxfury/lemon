...
/**
    ...
    equal to another type alignment. For example, the following declaration produces an error in Microsoft Visual Studio
   2008 compiler: \code typedef double  my_double; typedef __declspec(align( __alignof(my_double))) int   aligned_int;
    \endcode
...
//@cond none
#define CDS_ALIGNED_TYPE_impl(nAlign)              \
    template <typename T>                          \
    struct aligned_type<T, nAlign> {               \
        typedef CDS_TYPE_ALIGNMENT(nAlign) T type; \
    }
