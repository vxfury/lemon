...
class Allocator : public std::conditional<std::is_same<T, typename Alloc::value_type>::value, Alloc,
                                          typename std::allocator_traits<Alloc>::template rebind_alloc<T>>::type {
  public:
    /// Underlying allocator type
    typedef
        typename std::conditional<std::is_same<T, typename Alloc::value_type>::value, Alloc,
                                  typename std::allocator_traits<Alloc>::template rebind_alloc<T>>::type allocator_type;
    ...
    value_type *NewArray(size_t nCount)
    {
        ...
        for (size_t i = 0; i < nCount; ++i) Construct(p + i);
        return p;
    }
    ...
    */
    ...
    {
        value_type *p = allocator_traits::allocate(*this, nCount, nullptr);
        for (size_t i = 0; i < nCount; ++i) Construct(p + i, src);
        return p;
    }
    ...
    */
    ...
    {
        for (size_t i = 0; i < nCount; ++i) allocator_traits::destroy(*this, p + i);
