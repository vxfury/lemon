...
class config_file {
  ...
  public:
    ...
    config const &operator[](const std::string &testName) const
    {
        ...
        if (it != m_cfg.end()) return it->second;
        return m_emptyCfg;
    }
};
...
void config_file::load(const char *fileName)
{
    ...
    while (!s.eof()) {
        ...
        if (!pszEq) continue;
        if (pszEq == pszStr) continue;

        pszEnd = pszEq;
        while (pszStr <= --pszEnd && (*pszEnd == ' ' || *pszEnd == '\t' || *pszEnd == '\n' || *pszEnd == '\r'))
            ;

        if (pszEnd <= pszStr) continue;
        pszEnd[1] = 0;
...
void init_config(int argc, char **argv)
{
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        char *eq = strchr(arg, '=');
        if (eq) {
            if (strncmp(arg, "--cfg", eq - arg) == 0) cfg_file = eq + 1;
        }
    }
    ...
    if (!cfg_file || *cfg_file == 0) cfg_file = default_cfg_file;
