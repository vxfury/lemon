...
class Map_Iter_Del3 : public cds_test::stress_fixture {
  ...
  protected:
    ...
    class Inserter : public cds_test::thread {
      ...
      public:
        Inserter(cds_test::thread_pool &pool, Map &map) : base_class(pool, inserter_thread), m_Map(map)
        {
            init_data();
        }

        Inserter(Inserter &src) : base_class(src), m_Map(src.m_Map)
        {
            init_data();
        }
    ...
    class Deleter : public cds_test::thread {
      ...
      public:
        Deleter(cds_test::thread_pool &pool, Map &map) : base_class(pool, deleter_thread), m_Map(map)
        {
            init_data();
        }

        Deleter(Deleter &src) : base_class(src), m_Map(src.m_Map)
        {
            init_data();
        }
    ...
    class Extractor : public cds_test::thread {
      ...
      public:
        Extractor(cds_test::thread_pool &pool, Map &map) : base_class(pool, extractor_thread), m_Map(map)
        {
            init_data();
        }

        Extractor(Extractor &src) : base_class(src), m_Map(src.m_Map)
        {
            init_data();
        }
    ...
    class Observer : public cds_test::thread {
      ...
      public:
        Observer(cds_test::thread_pool &pool, Map &map) : base_class(pool, find_thread), m_Map(map) {}

        Observer(Observer &src) : base_class(src), m_Map(src.m_Map) {}
  ...
  protected:
    ...
    void do_test(Map &testMap)
    {
        ...
        pool.add(new delete_thread(pool, testMap),
                 s_nDelThreadCount ? s_nDelThreadCount : cds::OS::topology::processor_count());
        if (s_nFindThreadCount) pool.add(new observer_thread(pool, testMap), s_nFindThreadCount);

        propout() << std::make_pair("insert_thread_count", s_nInsThreadCount)
                  << std::make_pair("delete_thread_count", s_nDelThreadCount)
                  << std::make_pair("find_thread_count", s_nFindThreadCount) << std::make_pair("map_size", s_nMapSize)
    ...
    void do_test_extract(Map &testMap)
    {
        ...
        cds_test::thread_pool &pool = get_pool();
        pool.add(new insert_thread(pool, testMap), s_nInsThreadCount);
        if (s_nDelThreadCount) pool.add(new delete_thread(pool, testMap), s_nDelThreadCount);
        if (s_nExtractThreadCount) pool.add(new extract_thread(pool, testMap), s_nExtractThreadCount);
        if (s_nFindThreadCount) pool.add(new observer_thread(pool, testMap), s_nFindThreadCount);
...
class Map_Iter_Del3_LF : public Map_Iter_Del3, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Map>
    void run_test()
    {
