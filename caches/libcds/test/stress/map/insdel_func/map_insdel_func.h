...
class Map_InsDel_func : public cds_test::stress_fixture {
  public:
    ...
    struct value_type {
        ...
        value_type()
            : nKey(0), nData(0), nUpdateCall(0), bInitialized(false), threadId(cds::OS::get_current_thread_id())
        {
        }
  ...
  protected:
    enum { insert_thread, delete_thread, update_thread };
    ...
    class Inserter : public cds_test::thread {
        ...
        struct insert_functor {
            ...
            insert_functor() : nTestFunctorRef(0) {}
      ...
      public:
        Inserter(cds_test::thread_pool &pool, Map &map) : base_class(pool, insert_thread), m_Map(map) {}
        ...
        virtual void test()
        {
            ...
            if (id() & 1) {
                ...
                for (size_t nPass = 0; nPass < nPassCount; ++nPass) {
                    for (key_array::const_reverse_iterator it = s_arrKeys.rbegin(), itEnd = s_arrKeys.rend();
                         it != itEnd; ++it) {
                        if (rMap.insert_with(*it, std::ref(func)))
                            ++m_nInsertSuccess;
                        else
    ...
    class Updater : public cds_test::thread {
      ...
      public:
        Updater(cds_test::thread_pool &pool, Map &map) : base_class(pool, update_thread), m_Map(map) {}

        Updater(Updater &src) : base_class(src), m_Map(src.m_Map) {}
    ...
    class Deleter : public cds_test::thread {
        ...
        struct value_container {
            ...
            value_container() : nKeyExpected(), nSuccessItem(0), nFailedItem(0) {}
        };
      ...
      public:
        Deleter(cds_test::thread_pool &pool, Map &map) : base_class(pool, delete_thread), m_Map(map) {}
        ...
        virtual void test()
        {
            ...
            if (id() & 1) {
                for (size_t nPass = 0; nPass < nPassCount; ++nPass) {
                    for (key_array::const_iterator it = s_arrKeys.cbegin(), itEnd = s_arrKeys.cend(); it != itEnd;
                         ++it) {
                ...
                for (size_t nPass = 0; nPass < nPassCount; ++nPass) {
                    for (key_array::const_reverse_iterator it = s_arrKeys.crbegin(), itEnd = s_arrKeys.crend();
                         it != itEnd; ++it) {
                        func.m_cnt.nKeyExpected = *it;
                        if (rMap.erase(*it, std::ref(func)))
                            ++m_nDeleteSuccess;
  ...
  protected:
    ...
    void do_test(Map &testMap)
    {
        ...
        if (s_nUpdateThreadCount) pool.add(new updater(pool, testMap), s_nUpdateThreadCount);
        ...
        propout() << std::make_pair("insert_thread_count", s_nInsertThreadCount)
                  ...
                  << std::make_pair("pass_count", s_nThreadPassCount) << std::make_pair("map_size", s_nMapSize);
        ...
        propout() << std::make_pair("insert_success", nInsertSuccess) << std::make_pair("insert_failed", nInsertFailed)
        ...
        for (size_t nItem = 0; nItem < s_nMapSize; ++nItem) testMap.erase(nItem);
...
class Map_InsDel_func_LF : public Map_InsDel_func, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Set>
    void run_test()
    {
