...
class Map_InsDel_item_int : public cds_test::stress_fixture {
  ...
  protected:
    enum { insert_thread, delete_thread };
    ...
    class Inserter : public cds_test::thread {
        ...
        struct update_func {
            ...
            {
                if (bNew) item.second = item.first;
            }
            ...
            // for FeldmanHashMap
            void operator()(std::pair<key_type const, value_type> &item,
                            std::pair<key_type const, value_type> *pOld) const
            {
                if (!pOld) item.second = item.first;
            }
        };
  ...
  protected:
    ...
    void do_test(Map &testMap)
    {
        ...
        // Fill the map
        {
            ...
            for (size_t i = 0; i < s_nMapSize; ++i) v.push_back(i);
            shuffle(v.begin(), v.end());
            for (auto i : v) EXPECT_TRUE(testMap.insert(i, i));
        }
        ...
        propout() << std::make_pair("insert_thread_count", s_nInsertThreadCount)
                  ...
                  << std::make_pair("map_size", s_nMapSize) << std::make_pair("goal_item", s_nGoalItem)
        ...
        for (size_t i = 0; i < s_nMapSize; ++i) EXPECT_TRUE(testMap.contains(i)) << "key=" << i;
...
class Map_InsDel_item_int_LF : public Map_InsDel_item_int, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Set>
    void run_test()
    {
