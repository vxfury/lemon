...
template <typename Key, typename Value, typename Lock,
          class Alloc = typename std::allocator_traits<CDS_DEFAULT_ALLOCATOR>::template rebind_alloc<
...
class StdMap : public std::map<Key, Value, std::less<Key>, Alloc> {
  ...
  public:
    ...
    {
        scoped_lock al(m_lock);
        std::pair<typename base_class::iterator, bool> pRet =
            base_class::insert(typename base_class::value_type(key, Value()));
        if (pRet.second) {
...
template <typename Key, typename Value, typename Lock,
          class Alloc = typename std::allocator_traits<CDS_DEFAULT_ALLOCATOR>::template rebind_alloc<
              std::pair<Key const, Value> > >
class StdHashMap : public std::unordered_map<Key, Value, std::hash<Key>, std::equal_to<Key>, Alloc> {
  public:
    Lock m_lock;
    typedef std::unique_lock<Lock> scoped_lock;
    typedef std::unordered_map<Key, Value, std::hash<Key>, std::equal_to<Key>, Alloc> base_class;
