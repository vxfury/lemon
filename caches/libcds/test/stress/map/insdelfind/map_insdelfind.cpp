...
void Map_InsDelFind::SetUpTestCase()
{
    ...
    if (s_nThreadCount == 0) s_nThreadCount = std::min(16u, std::thread::hardware_concurrency() * 2);
    ...
    if (s_nInsertPercentage >= 100) s_nInsertPercentage = 99;

    s_nDeletePercentage = cfg.get_uint("DeletePercentage", s_nDeletePercentage);
    if (s_nDeletePercentage >= 100) s_nDeletePercentage = 99;
    ...
    if (s_nDuration < 5) s_nDuration = 5;
    ...
    if (pFirst < pLast) std::fill(pFirst, pLast, do_find);
