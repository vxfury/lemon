...
class Map_InsDelFind : public cds_test::stress_fixture {
  ...
  protected:
    ...
    class Worker : public cds_test::thread {
      ...
      public:
        Worker(cds_test::thread_pool &pool, Map &map) : base_class(pool), m_Map(map) {}

        Worker(Worker &src) : base_class(src), m_Map(src.m_Map) {}
  ...
  protected:
    ...
    void do_test(Map &testMap)
    {
        ...
        // fill map - only odd number
        {
            ...
            for (size_t i = 0; i < s_nMapSize; ++i) arr.push_back(i * 2 + 1);
            shuffle(arr.begin(), arr.end());
            for (size_t i = 0; i < s_nMapSize; ++i) testMap.insert(arr[i], arr[i]);
        }
        ...
        propout() << std::make_pair("thread_count", s_nThreadCount)
                  ...
                  << std::make_pair("delete_percentage", s_nDeletePercentage) << std::make_pair("map_size", s_nMapSize);
...
class Map_InsDelFind_LF : public Map_InsDelFind, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Map>
    void run_test()
    {
