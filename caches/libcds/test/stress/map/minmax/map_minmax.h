  ...
  protected:
    ...
    class Inserter : public cds_test::thread {
        ...
        void init_data()
        {
            ...
            for (key_type i = keyMin + 10; i >= keyMin; --i) m_arr.push_back(i);
            for (key_type i = keyMax - 10; i <= keyMax; ++i) m_arr.push_back(i);
      ...
      public:
        ...
        virtual void test()
        {
            ...
            do {
                gp = rMap.extract_min();
                if (gp) {
                    if (gp->first == keyMin) ++m_nDeleteMin;
                } else
                    ++m_nDeleteMinFailed;

                gp = rMap.extract_max();
                if (gp) {
                    if (gp->first == keyMax) ++m_nDeleteMax;
                } else
                    ++m_nDeleteMaxFailed;
      ...
      public:
        ...
        virtual void test()
        {
            ...
            do {
                ...
                if (res.second) {
                    if (res.first == keyMin) ++m_nDeleteMin;
                } else
                    ++m_nDeleteMinFailed;

                res = rMap.extract_max_key();
                if (res.second) {
                    if (res.first == keyMax) ++m_nDeleteMax;
                } else
                    ++m_nDeleteMaxFailed;
            } while (fixture.m_nInsThreadCount.load(atomics::memory_order_acquire) != 0);
  ...
  protected:
    ...
    void do_test(Map &testMap)
    {
...

        {
            ...
            for (int i = 0; i < static_cast<int>(s_nMapSize); ++i) arr[i] = i;
            ;
            shuffle(arr.begin(), arr.end());

            for (key_type key : arr) testMap.insert(key, key);
        }
        ...
        propout() << std::make_pair("insert_thread_count", s_nInsThreadCount)
                  ...
                  << std::make_pair("map_size", s_nMapSize) << std::make_pair("pass_count", s_nPassCount);
        ...
        propout() << std::make_pair("insert_min", nInsertMinSuccess + 1)
                  << std::make_pair("insert_min_double", nInsertMinFailed)
                  << std::make_pair("insert_max", nInsertMaxSuccess + 1)
                  << std::make_pair("insert_max_double", nInsertMaxFailed) << std::make_pair("extract_min", nDeleteMin)
                  << std::make_pair("extract_min_failed", nDeleteMinFailed) << std::make_pair("extract_max", nDeleteMax)
