...
class Map_find_string : public cds_test::stress_fixture {
  ...
  private:
    ...
    class Worker : public cds_test::thread {
      ...
      public:
        Worker(cds_test::thread_pool &pool, Map &map) : base_class(pool), m_Map(map), m_KeyExists(), m_KeyNotExists() {}

        Worker(Worker &src) : base_class(src), m_Map(src.m_Map), m_KeyExists(), m_KeyNotExists() {}
  ...
  protected:
    ...
    void test(Map &testMap)
    {
        ...
        propout() << std::make_pair("thread_count", s_nThreadCount) << std::make_pair("map_size", s_nMapSize)
                  << std::make_pair("percent_exist", s_nPercentExists) << std::make_pair("pass_count", s_nPassCount);
        ...
        propout() << std::make_pair("exist_found", nExistSuccess)
...
class Map_find_string_LF : public Map_find_string, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Map>
    void run_test()
    {
