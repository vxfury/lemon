...
struct key_thread {
    ...
    key_thread(size_t key, size_t threadNo) : nKey(static_cast<uint32_t>(key)), nThread(static_cast<uint16_t>(threadNo))
    {
    }

    key_thread() : nKey(), nThread() {}
};
...
struct cmp<key_thread> {
    ...
    {
        if (k1.nKey < k2.nKey) return -1;
        if (k1.nKey > k2.nKey) return 1;
        if (k1.nThread < k2.nThread) return -1;
        if (k1.nThread > k2.nThread) return 1;
        return 0;
    }
    int operator()(key_thread const &k1, size_t k2) const
    {
        if (k1.nKey < k2) return -1;
        if (k1.nKey > k2) return 1;
        return 0;
    }
    int operator()(size_t k1, key_thread const &k2) const
    {
        if (k1 < k2.nKey) return -1;
        if (k1 > k2.nKey) return 1;
        return 0;
    }
};
...
struct less<set::key_thread> {
    ...
    {
        if (k1.nKey <= k2.nKey) return k1.nKey < k2.nKey || k1.nThread < k2.nThread;
        return false;
    }
};
...
class Set_Del3 : public cds_test::stress_fixture {
  ...
  protected:
    ...
    enum { inserter_thread, deleter_thread, extractor_thread, find_thread };
    ...
    class Inserter : public cds_test::thread {
        ...
        void init_data()
        {
            prepare_array(m_arr, [](size_t) -> bool {
                return true;
            });
      ...
      public:
        Inserter(cds_test::thread_pool &pool, Set &set) : base_class(pool, inserter_thread), m_Set(set)
        {
            init_data();
        }
    ...
    class Deleter : public cds_test::thread {
        ...
        void init_data()
        {
            prepare_array(m_arr, [](size_t el) -> bool {
                return (el & 3) != 0;
            });
        }
      ...
      public:
        Deleter(cds_test::thread_pool &pool, Set &set) : base_class(pool, deleter_thread), m_Set(set)
        {
            init_data();
        }
        Deleter(Deleter &src) : base_class(src), m_Set(src.m_Set)
        {
            init_data();
        }
    ...
    class Extractor : public cds_test::thread {
      ...
      public:
        Extractor(cds_test::thread_pool &pool, Set &set) : base_class(pool, extractor_thread), m_Set(set)
        {
            init_data();
        }

        Extractor(Extractor &src) : base_class(src), m_Set(src.m_Set)
        {
            init_data();
        }
    ...
    class Observer : public cds_test::thread {
      ...
      public:
        Observer(cds_test::thread_pool &pool, Set &set) : base_class(pool, find_thread), m_Set(set) {}

        Observer(Observer &src) : base_class(src), m_Set(src.m_Set) {}
  ...
  protected:
    ...
    void do_test_with(Set &testSet)
    {
        ...
        pool.add(new delete_thread(pool, testSet),
                 s_nDelThreadCount ? s_nDelThreadCount : cds::OS::topology::processor_count());
        if (s_nFindThreadCount) pool.add(new observer_thread(pool, testSet), s_nFindThreadCount);

        propout() << std::make_pair("insert_thread_count", s_nInsThreadCount)
                  << std::make_pair("delete_thread_count", s_nDelThreadCount)
                  << std::make_pair("find_thread_count", s_nFindThreadCount) << std::make_pair("set_size", s_nSetSize)
        ...
        propout() << std::make_pair("insert_init_success", nInsertInitSuccess)
                  << std::make_pair("insert_init_failed", nInsertInitFailed)
                  << std::make_pair("insert_success", nInsertSuccess) << std::make_pair("insert_failed", nInsertFailed)
                  << std::make_pair("delete_success", nDeleteSuccess) << std::make_pair("delete_failed", nDeleteFailed)
    ...
    void do_test_extract_with(Set &testSet)
    {
        ...
        cds_test::thread_pool &pool = get_pool();
        pool.add(new insert_thread(pool, testSet), s_nInsThreadCount);
        if (s_nDelThreadCount) pool.add(new delete_thread(pool, testSet), s_nDelThreadCount);
        if (s_nExtractThreadCount) pool.add(new extract_thread(pool, testSet), s_nExtractThreadCount);
        if (s_nFindThreadCount) pool.add(new observer_thread(pool, testSet), s_nFindThreadCount);
...
class Set_Del3_LF : public Set_Del3, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Set>
    void run_test()
    {
