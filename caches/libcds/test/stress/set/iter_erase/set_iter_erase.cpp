...
void Set_Iter_Del3::SetUpTestCase()
{
    ...
    if (s_nInsThreadCount == 0) s_nInsThreadCount = 1;
    ...
    if (s_nInsertPassCount == 0) s_nInsertPassCount = 1000;

    s_nFeldmanSet_HeadBits = cfg.get_size_t("FeldmanMapHeadBits", s_nFeldmanSet_HeadBits);
    if (s_nFeldmanSet_HeadBits == 0) s_nFeldmanSet_HeadBits = 8;

    s_nFeldmanSet_ArrayBits = cfg.get_size_t("FeldmanMapArrayBits", s_nFeldmanSet_ArrayBits);
    if (s_nFeldmanSet_ArrayBits == 0) s_nFeldmanSet_ArrayBits = 8;

    m_arrData.resize(s_nSetSize);
    for (size_t i = 0; i < s_nSetSize; ++i) m_arrData[i] = i;
