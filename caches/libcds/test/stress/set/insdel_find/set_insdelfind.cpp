...
void Set_InsDelFind::SetUpTestCase()
{
    ...
    if (s_nThreadCount == 0) s_nThreadCount = 2;
    ...
    if (s_nInsertPercentage == 0 || s_nInsertPercentage > 90) s_nInsertPercentage = 5;

    s_nDeletePercentage = cfg.get_uint("DeletePercentage", s_nDeletePercentage);
    if (s_nDeletePercentage == 0 || s_nDeletePercentage > 90) s_nDeletePercentage = 5;

    s_nDuration = cfg.get_uint("Duration", s_nDuration);
    if (s_nDuration == 0) s_nDuration = 5;
