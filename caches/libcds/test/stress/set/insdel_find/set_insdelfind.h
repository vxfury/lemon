...
class Set_InsDelFind : public cds_test::stress_fixture {
  ...
  protected:
    ...
    class Worker : public cds_test::thread {
      ...
      public:
        Worker(cds_test::thread_pool &pool, Set &set) : base_class(pool), m_Set(set) {}

        Worker(Worker &src) : base_class(src), m_Set(src.m_Set) {}
  ...
  protected:
    ...
    void do_test(Set &testSet)
    {
        ...
        // fill map - only odd number
        {
            ...
            for (size_t i = 0; i < s_nSetSize; ++i) pInitArr[i] = i * 2 + 1;
            shuffle(pInitArr, pEnd);
            for (size_t *p = pInitArr; p < pEnd; ++p) testSet.insert(typename Set::value_type(*p, *p));
            delete[] pInitArr;
        }
        ...
        propout() << std::make_pair("thread_count", s_nThreadCount) << std::make_pair("set_size", s_nSetSize)
        ...
        propout() << std::make_pair("insert_success", nInsertSuccess) << std::make_pair("insert_failed", nInsertFailed)
                  ...
                  << std::make_pair("find_success", nFindSuccess) << std::make_pair("find_failed", nFindFailed);

        {
            ASSERT_TRUE(std::chrono::duration_cast<std::chrono::seconds>(duration).count() > 0);
            size_t nTotalOps =
                nInsertSuccess + nInsertFailed + nDeleteSuccess + nDeleteFailed + nFindSuccess + nFindFailed;
            propout() << std::make_pair("avg_speed",
                                        nTotalOps / std::chrono::duration_cast<std::chrono::seconds>(duration).count());
        }
...
class Set_InsDelFind_LF : public Set_InsDelFind, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Set>
    void run_test()
    {
