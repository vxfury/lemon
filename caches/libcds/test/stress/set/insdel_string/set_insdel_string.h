...
class Set_InsDel_string : public cds_test::stress_fixture {
  ...
  private:
    ...
    enum { insert_thread, delete_thread, extract_thread };
  ...
  protected:
    ...
    void do_test(Set &testSet)
    {
        ...
        propout() << std::make_pair("insert_success", nInsertSuccess)
                  ...
                  << std::make_pair("delete_failed", nDeleteFailed) << std::make_pair("final_set_size", testSet.size());

        // testSet.clear();
        for (auto const &str : m_arrString) testSet.erase(str);
...
class Set_InsDel_string_LF : public Set_InsDel_string, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Set>
    void run_test()
    {
