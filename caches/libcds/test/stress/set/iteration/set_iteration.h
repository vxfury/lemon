...
class Set_Iteration : public cds_test::stress_fixture {
  public:
    ...
    struct value_type {
        ...
        explicit value_type(size_t v) : val(v), hash(0) {}
    };

  private:
    enum { insert_thread, delete_thread, extract_thread, iterator_thread };
    ...
    class Extractor : public cds_test::thread {
      ...
      public:
        Extractor(cds_test::thread_pool &pool, Set &set) : base_class(pool, extract_thread), m_Set(set) {}
    ...
    class Iterator : public cds_test::thread {
      ...
      public:
        Iterator(cds_test::thread_pool &pool, Set &set) : base_class(pool, iterator_thread), m_Set(set) {}

        Iterator(Iterator &src) : base_class(src), m_Set(src.m_Set) {}
  ...
  protected:
    ...
    void do_test_extract(Set &testSet)
    {
        ...
        propout() << std::make_pair("insert_success", nInsertSuccess)
                  ...
                  << std::make_pair("insert_failed", nInsertFailed) << std::make_pair("delete_failed", nDeleteFailed)
...
class Set_Iteration_LF : public Set_Iteration, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Set>
    void run_test()
    {
