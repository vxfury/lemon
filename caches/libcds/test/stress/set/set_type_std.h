...
template <typename Value, typename Hash, typename Less, typename EqualTo, typename Lock,
          class Alloc = typename std::allocator_traits<CDS_DEFAULT_ALLOCATOR>::template rebind_alloc<Value> >
class StdHashSet : public std::unordered_set<Value, Hash, EqualTo, Alloc> {
  public:
    Lock m_lock;
    typedef std::unique_lock<Lock> scoped_lock;
    typedef std::unordered_set<Value, Hash, EqualTo, Alloc> base_class;
