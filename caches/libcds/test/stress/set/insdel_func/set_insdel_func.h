...
class Set_InsDel_func : public cds_test::stress_fixture {
  ...
  public:
    ...
    struct value {
        ...
        value() : nKey(0), nData(0), nUpdateCall(0), bInitialized(false), threadId(cds::OS::get_current_thread_id()) {}
    ...
    enum { insert_thread, update_thread, delete_thread };
    ...
    class Inserter : public cds_test::thread {
      ...
      public:
        Inserter(cds_test::thread_pool &pool, Set &set) : base_class(pool, insert_thread), m_Set(set) {}
    ...
    class Updater : public cds_test::thread {
      ...
      public:
        Updater(cds_test::thread_pool &pool, Set &set) : base_class(pool, update_thread), m_Set(set) {}

        Updater(Updater &src) : base_class(src), m_Set(src.m_Set) {}
    ...
    class Deleter : public cds_test::thread {
      ...
      public:
        Deleter(cds_test::thread_pool &pool, Set &set) : base_class(pool, delete_thread), m_Set(set) {}
  ...
  protected:
    ...
    void run_test(Set &testSet)
    {
        ...
        for (size_t i = 0; i < s_nSetSize; ++i) m_pKeyArr[i] = i;
        ...
        propout() << std::make_pair("insert_thread_count", s_nInsertThreadCount)
                  ...
                  << std::make_pair("thread_pass_count", s_nThreadPassCount) << std::make_pair("set_size", s_nSetSize);
        ...
        propout() << std::make_pair("insert_success", nInsertSuccess)
                  << std::make_pair("delete_success", nDeleteSuccess) << std::make_pair("insert_failed", nInsertFailed)
                  << std::make_pair("delete_failed", nDeleteFailed) << std::make_pair("update_created", nUpdateCreated)
                  << std::make_pair("update_modified", nUpdateModified)
                  << std::make_pair("update_failed", nUpdateFailed) << std::make_pair("final_set_size", testSet.size());
        ...
        // testSet.clear();
        for (size_t *p = m_pKeyFirst; p != m_pKeyLast; ++p) testSet.erase(*p);
    ...
    void run_test2()
    {
        ...
        for (auto it = s.begin(); it != s.end(); ++it) std::cout << "key=" << it->key << std::endl;
    }
};

class Set_InsDel_func_LF : public Set_InsDel_func, public ::testing::WithParamInterface<size_t> {
  public:
    template <class Set>
    void run_test()
    {
