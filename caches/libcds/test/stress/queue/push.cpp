    ...
    class Producer : public cds_test::thread {
      ...
      public:
        Producer(cds_test::thread_pool &pool, Queue &queue)
            : base_class(pool), m_Queue(queue), m_nStartItem(0), m_nEndItem(0), m_nPushError(0)
        {
        }

        Producer(Producer &src) : base_class(src), m_Queue(src.m_Queue), m_nStartItem(0), m_nEndItem(0), m_nPushError(0)
        {
        }
        ...
        virtual void test()
        {
            for (size_t nItem = m_nStartItem; nItem < m_nEndItem; ++nItem) {
                if (!m_Queue.push(nItem)) ++m_nPushError;
            }
        }
...
CDSSTRESS_MSQueue(queue_push) CDSSTRESS_MoirQueue(queue_push) CDSSTRESS_BasketQueue(queue_push)
    CDSSTRESS_OptimsticQueue(queue_push) CDSSTRESS_FCQueue(queue_push) CDSSTRESS_FCDeque(queue_push)
        CDSSTRESS_RWQueue(queue_push) CDSSTRESS_StdQueue(queue_push)
...
#undef CDSSTRESS_Queue_F
    ...
    class segmented_queue_push : public queue_push,
...
INSTANTIATE_TEST_CASE_P(SQ, segmented_queue_push, ::testing::ValuesIn(segmented_queue_push::get_test_parameters()),
