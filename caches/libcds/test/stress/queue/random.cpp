...
class queue_random : public cds_test::stress_fixture {
    ...
    class Strain : public cds_test::thread {
      ...
      public:
        Strain(cds_test::thread_pool &pool, Queue &q, size_t nPushCount, size_t nSpread = 0)
            : base_class(pool), m_Queue(q), m_nSpread(nSpread), m_nTotalPushCount(nPushCount)
        {
        }

        Strain(Strain &src)
            : base_class(src), m_Queue(src.m_Queue), m_nSpread(src.m_nSpread), m_nTotalPushCount(src.m_nTotalPushCount)
        {
        }
        ...
        virtual void test()
        {
            ...
            while (!m_Queue.empty() || s_nProducerCount.load(std::memory_order_relaxed) != 0) pop(nThreadCount);
        }
                ...
                if (node.nThread < nThreadCount) {
                    ...
                    if (m_nSpread) {
                        if (m_arrLastRead[node.nThread] > node.nNo) {
                            if (m_arrLastRead[node.nThread] - node.nNo > m_nSpread) ++m_nRepeatValue;
  ...
  protected:
    ...
    void analyze(Queue &q)
    {
        ...
        for (size_t i = 0; i < s_nThreadCount; ++i) EXPECT_EQ(arrPushCount[i], nThreadPushCount) << "thread=" << i;
    }
...
CDSSTRESS_MSQueue(queue_random) CDSSTRESS_MoirQueue(queue_random) CDSSTRESS_BasketQueue(queue_random)
    CDSSTRESS_OptimsticQueue(queue_random) CDSSTRESS_FCQueue(queue_random) CDSSTRESS_FCDeque(queue_random)
        CDSSTRESS_RWQueue(queue_random) CDSSTRESS_StdQueue(queue_random)
...
#undef CDSSTRESS_Queue_F
    ...
    class segmented_queue_random : public queue_random,
...
INSTANTIATE_TEST_CASE_P(SQ, segmented_queue_random, ::testing::ValuesIn(segmented_queue_random::get_test_parameters()),
                        get_test_parameter_name);
#else
    INSTANTIATE_TEST_CASE_P(SQ, segmented_queue_random,
