...
class queue_pop : public cds_test::stress_fixture {
    ...
    class Consumer : public cds_test::thread {
      ...
      public:
        Consumer(cds_test::thread_pool &pool, Queue &queue)
            : base_class(pool), m_Queue(queue), m_arr(new uint8_t[s_nQueueSize]), m_nPopCount(0)
        {
        }

        Consumer(Consumer &src)
            : base_class(src), m_Queue(src.m_Queue), m_arr(new uint8_t[s_nQueueSize]), m_nPopCount(0)
        {
        }
  ...
  public:
    static void SetUpTestCase()
    {
        ...
        if (s_nQueueSize == 0) s_nQueueSize = 1000;
    }
  ...
  protected:
    ...
    void analyze(Queue &q)
    {
        ...
        for (size_t i = 0; i < pool.size(); ++i) {
            ...
            for (size_t j = 0; j < s_nQueueSize; ++j) arr[j] += thread.m_arr[j];
    ...
    void test(Queue &q)
    {
        ...
        for (size_t i = 0; i < s_nQueueSize; ++i) q.push(i);
...
CDSSTRESS_MSQueue(queue_pop) CDSSTRESS_MoirQueue(queue_pop) CDSSTRESS_BasketQueue(queue_pop)
    CDSSTRESS_OptimsticQueue(queue_pop) CDSSTRESS_FCQueue(queue_pop) CDSSTRESS_FCDeque(queue_pop)
        CDSSTRESS_RWQueue(queue_pop) CDSSTRESS_StdQueue(queue_pop)
...
#undef CDSSTRESS_Queue_F
    ...
    class segmented_queue_pop : public queue_pop,
...
INSTANTIATE_TEST_CASE_P(SQ, segmented_queue_pop, ::testing::ValuesIn(segmented_queue_pop::get_test_parameters()),
