...
class queue_push_pop : public cds_test::stress_fixture {
    ...
    class Producer : public cds_test::thread {
      ...
      public:
        Producer(cds_test::thread_pool &pool, Queue &queue, size_t nPushCount)
            : base_class(pool, producer_thread), m_Queue(queue), m_nPushFailed(0), m_nPushCount(nPushCount)
        {
        }

        Producer(Producer &src)
            : base_class(src), m_Queue(src.m_Queue), m_nPushFailed(0), m_nPushCount(src.m_nPushCount)
        {
        }
    ...
    class Consumer : public cds_test::thread {
      ...
      private:
        void initPoppedData()
        {
            ...
            for (size_t i = 0; i < nProducerCount; ++i) m_WriterData[i].reserve(m_nPushPerProducer);
        }
    ...
    void analyze(Queue &q, size_t /*nLeftOffset*/ = 0, size_t nRightOffset = 0)
    {
        ...
        for (size_t i = 0; i < pool.size(); ++i) {
            ...
            if (thr.type() == consumer_thread) {
                ...
                size_t nPopped = 0;
                for (size_t n = 0; n < s_nProducerThreadCount; ++n) nPopped += consumer.m_WriterData[n].size();
        ...
        EXPECT_EQ(nTotalPops + nPostTestPops, s_nQueueSize)
            << "nTotalPops=" << nTotalPops << ", nPostTestPops=" << nPostTestPops;
        EXPECT_TRUE(q.empty());
  ...
  public:
    static void SetUpTestCase()
    {
        ...
        if (s_nConsumerThreadCount == 0u) s_nConsumerThreadCount = 1;
        if (s_nProducerThreadCount == 0u) s_nProducerThreadCount = 1;
        if (s_nQueueSize == 0u) s_nQueueSize = 1000;
        if (s_nHeavyValueSize == 0) s_nHeavyValueSize = 1;
...
CDSSTRESS_MSQueue(simple_queue_push_pop) CDSSTRESS_MoirQueue(simple_queue_push_pop)
    CDSSTRESS_BasketQueue(simple_queue_push_pop) CDSSTRESS_OptimsticQueue(simple_queue_push_pop)
        CDSSTRESS_FCQueue(simple_queue_push_pop) CDSSTRESS_FCDeque(simple_queue_push_pop)
            CDSSTRESS_FCDeque_HeavyValue(fc_with_heavy_value) CDSSTRESS_RWQueue(simple_queue_push_pop)
...
#undef CDSSTRESS_Queue_F
    ...
    class segmented_queue_push_pop : public queue_push_pop<>,
...
INSTANTIATE_TEST_CASE_P(SQ, segmented_queue_push_pop,
