...
class spsc_buffer : public cds_test::stress_fixture {
  protected:
    ...
    class Producer : public cds_test::thread {
      ...
      public:
        Producer(cds_test::thread_pool &pool, Queue &queue) : base_class(pool, producer_thread), m_Queue(queue) {}
    ...
    class Consumer : public cds_test::thread {
      ...
      public:
        Consumer(cds_test::thread_pool &pool, Queue &queue) : base_class(pool, consumer_thread), m_Queue(queue) {}
        ...
        virtual void test()
        {
            ...
            while (true) {
                ...
                if (buf.first) {
                    ...
                    if (!m_Queue.pop_front()) ++m_nPopFrontFailed;
                } else {
                    ++m_nPopEmpty;
  ...
  protected:
    ...
    void test_queue(Queue &q)
    {
        ...
        propout() << std::make_pair("buffer_size", s_nBufferSize) << std::make_pair("push_count", s_nPushCount);
        ...
        propout() << std::make_pair("producer_push_length", producer->m_nPushed)
  ...
  public:
    static void SetUpTestCase()
    {
        ...
        if (s_nBufferSize < 1024 * 64) s_nBufferSize = 1024 * 64;
        if (s_nPushCount == 0u) s_nPushCount = 1024;
    }
};
...
CDSSTRESS_WeakRingBuffer_void(spsc_buffer) CDSSTRESS_WeakRingBuffer_void(spsc_buffer_slow_producer)
