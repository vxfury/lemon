...
class intrusive_queue_push_pop : public cds_test::stress_fixture {
  ...
  protected:
    ...
    class Producer : public cds_test::thread {
      ...
      public:
        Producer(cds_test::thread_pool &pool, Queue &q) : base_class(pool, producer_thread), m_Queue(q) {}
    ...
    class Consumer : public cds_test::thread {
      ...
      private:
        void initPoppedData()
        {
            ...
            for (size_t i = 0; i < nWriterCount; ++i) m_WriterData[i].reserve(nWriterPushCount);
        }

      public:
        Consumer(cds_test::thread_pool &pool, Queue &q) : base_class(pool, consumer_thread), m_Queue(q)
        {
            initPoppedData();
        }
        ...
        virtual void test()
        {
            ...
            while (true) {
                ...
                if (p) {
                    ...
                    ++m_nPopEmpty;
                    if (s_nProducerCount.load(atomics::memory_order_acquire) == 0 && m_Queue.empty()) break;
                }
            }
        }
    };
    ...
    class value_array {
      ...
      public:
        value_array(size_t nSize) : m_pArr(new T[nSize]) {}
  ...
  public:
    static void SetUpTestCase()
    {
        ...
        if (s_nReaderThreadCount == 0u) s_nReaderThreadCount = 1;
        if (s_nWriterThreadCount == 0u) s_nWriterThreadCount = 1;
  ...
  protected:
    ...
    void analyze(Queue &testQueue, size_t /*nLeftOffset*/, size_t nRightOffset)
    {
        ...
        while (testQueue.pop()) ++nPostTestPops;
        ...
        for (size_t i = 0; i < pool.size(); ++i) {
            ...
            if (thr.type() == consumer_thread) {
                ...
                size_t nPopped = 0;
                for (size_t n = 0; n < s_nWriterThreadCount; ++n) nPopped += consumer.m_WriterData[n].size();
        ...
        propout() << std::make_pair("success_pop", nTotalPops) << std::make_pair("empty_pop", nPopFalse)
        ...
        for (size_t nWriter = 0; nWriter < s_nWriterThreadCount; ++nWriter) {
            ...
            for (size_t nReader = 0; nReader < arrReaders.size(); ++nReader) {
                ...
                if (it != itEnd) {
                    ...
                    for (++it; it != itEnd; ++it) {
                        EXPECT_LT(*itPrev, *it + nRightOffset) << "Reader " << nReader << ", Writer " << nWriter
                                                               << ": prev=" << *itPrev << ", cur=" << *it;
                        itPrev = it;
                    }
                }

                for (it = arrReaders[nReader]->m_WriterData[nWriter].begin(); it != itEnd; ++it) arrData.push_back(*it);
            }
            std::sort(arrData.begin(), arrData.end());
            for (size_t i = 1; i < arrData.size(); ++i) {
                if (arrData[i - 1] + 1 != arrData[i]) {
                    EXPECT_EQ(arrData[i - 1] + 1, arrData[i])
                        << "Writer " << nWriter << ": [" << (i - 1) << "]=" << arrData[i - 1] << ", [" << i
                        << "]=" << arrData[i];
                }
            }
    ...
    void test(Queue &q, value_array<typename Queue::value_type> &arrValue, size_t nLeftOffset, size_t nRightOffset)
    {
        ...
        // Check that all values have been dequeued
        {
            ...
            for (typename Queue::value_type *it = pValStart; it != pEnd; ++it) {
                if (it->nConsumer == c_nBadConsumer) ++nBadConsumerCount;
            }
...
class intrusive_segmented_queue_push_pop : public intrusive_queue_push_pop,
  ...
  public:
    static std::vector<size_t> get_test_parameters()
    {
        ...
        if (bIterative && quasi_factor > 4) {
            for (size_t qf = 4; qf <= quasi_factor; qf *= 2) args.push_back(qf);
        } else {
            if (quasi_factor > 2)
...
INSTANTIATE_TEST_CASE_P(SQ, intrusive_segmented_queue_push_pop,
