...
class pqueue_pop : public cds_test::stress_fixture {
  ...
  protected:
    ...
    class Consumer : public cds_test::thread {
      ...
      public:
        Consumer(cds_test::thread_pool &pool, PQueue &queue) : base_class(pool), m_Queue(queue) {}
            ...
            if (m_Queue.pop(val)) {
                ...
                while (m_Queue.pop(val)) {
                    ...
                    if (val.key > nPrevKey) {
                        ...
                        if (prevPopFailed) m_arrFailedPops.back().next_key = val.key;
                        prevPopFailed = false;
                    }
                    if (nPrevKey > val.key) nPrevKey = val.key;
                }
  ...
  protected:
    ...
    void test(PQueue &q)
    {
        ...
        // push
        {
            ...
            for (size_t i = 0; i < s_nQueueSize; ++i) arr.push_back(i);
            shuffle(arr.begin(), arr.end());

            size_t nPushError = 0;
            typedef typename PQueue::value_type value_type;
            for (auto it = arr.begin(); it != arr.end(); ++it) {
                if (!q.push(value_type(*it))) ++nPushError;
            }
            ...
            for (size_t i = 0; i < pool.size(); ++i) {
                ...
                if (!cons.m_arrFailedPops.empty()) {
                    ...
                    for (size_t k = 0; k < cons.m_arrFailedPops.size(); ++k) {
                        std::cerr << "\n    "
                                  << "prev_key=" << cons.m_arrFailedPops[k].prev_key
                                  << " popped_key=" << cons.m_arrFailedPops[k].popped_key;
...
CDSSTRESS_MSPriorityQueue(pqueue_pop, MSPriorityQueue_dyn_less) CDSSTRESS_MSPriorityQueue(pqueue_pop,
...
#define CDSSTRESS_PriorityQueue(fixture_t, pqueue_t)                       \
    ...
    {                                                                      \
        ...
        CDSSTRESS_PriorityQueue(pqueue_pop, FCPQueue_vector) CDSSTRESS_PriorityQueue(pqueue_pop, FCPQueue_vector_stat)
            CDSSTRESS_PriorityQueue(pqueue_pop, FCPQueue_deque) CDSSTRESS_PriorityQueue(pqueue_pop, FCPQueue_deque_stat)
                                                ...
                                                CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_DHP_max)
                                                    CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_DHP_max_stat)
                                                        CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_DHP_min)
                                                            CDSSTRESS_PriorityQueue(pqueue_pop,
    ...
    CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_RCU_gpb_max) CDSSTRESS_PriorityQueue(pqueue_pop,
                                                                                          EllenBinTree_RCU_gpb_max_stat)
        CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_RCU_gpb_min) CDSSTRESS_PriorityQueue(
            pqueue_pop, EllenBinTree_RCU_gpb_min_stat) CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_RCU_gpt_max)
            CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_RCU_gpt_max_stat) CDSSTRESS_PriorityQueue(
                pqueue_pop, EllenBinTree_RCU_gpt_min) CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_RCU_gpt_min_stat)
#ifdef CDS_URCU_SIGNAL_HANDLING_ENABLED
                CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_RCU_shb_max)
                    CDSSTRESS_PriorityQueue(pqueue_pop, EllenBinTree_RCU_shb_max_stat) CDSSTRESS_PriorityQueue(
                        pqueue_pop, EllenBinTree_RCU_shb_min) CDSSTRESS_PriorityQueue(pqueue_pop,
                                                                                      EllenBinTree_RCU_shb_min_stat)
#endif

                        CDSSTRESS_PriorityQueue(pqueue_pop, SkipList32_HP_max) CDSSTRESS_PriorityQueue(
                            pqueue_pop, SkipList32_HP_max_stat) CDSSTRESS_PriorityQueue(pqueue_pop, SkipList32_HP_min)
                            CDSSTRESS_PriorityQueue(pqueue_pop, SkipList32_HP_min_stat) CDSSTRESS_PriorityQueue(
                                pqueue_pop, SkipList32_DHP_max) CDSSTRESS_PriorityQueue(pqueue_pop,
                                                                                        SkipList32_DHP_max_stat)
                                CDSSTRESS_PriorityQueue(pqueue_pop, SkipList32_DHP_min) CDSSTRESS_PriorityQueue(
                                    pqueue_pop, SkipList32_DHP_min_stat) CDSSTRESS_PriorityQueue(pqueue_pop,
                                                                                                 SkipList32_RCU_gpi_max)
                                    CDSSTRESS_PriorityQueue(pqueue_pop, SkipList32_RCU_gpi_min) CDSSTRESS_PriorityQueue(
                                        pqueue_pop, SkipList32_RCU_gpb_max)
