...
class put_get : public cds_test::stress_fixture {
  protected:
    ...
    class Worker : public cds_test::thread {
      ...
      public:
        Worker(cds_test::thread_pool &pool, FreeList &s) : base_class(pool), m_FreeList(s) {}

        Worker(Worker &src) : base_class(src), m_FreeList(src.m_FreeList) {}
        ...
        virtual void test()
        {
            ...
            for (size_t pass = 0; pass < s_nPassCount; ++pass) {
                ...
                for (size_t i = 0; i < n; ++i) m_FreeList.put(arr[i]);
            }
        }
    };
  ...
  public:
    static void SetUpTestCase()
    {
        ...
        if (s_nPassCount == 0) s_nPassCount = 1000;
    }
  ...
  protected:
    ...
    void test(FreeList &list)
    {
        ...
        for (auto &i : arr) list.put(&i);

        pool.add(new Worker<FreeList>(pool, list), s_nThreadCount);

        propout() << std::make_pair("work_thread", s_nThreadCount) << std::make_pair("pass_count", s_nPassCount);
        ...
        // analyze result
        size_t nTotal = 0;
        for (auto const &i : arr) nTotal += i.counter;
