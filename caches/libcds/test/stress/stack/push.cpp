...
class stack_push : public cds_test::stress_fixture {
    ...
    class Producer : public cds_test::thread {
      ...
      public:
        Producer(cds_test::thread_pool &pool, Stack &stack)
            : base_class(pool), m_stack(stack), m_nStartItem(0), m_nEndItem(0), m_nPushError(0)
        {
        }

        Producer(Producer &src) : base_class(src), m_stack(src.m_stack), m_nStartItem(0), m_nEndItem(0), m_nPushError(0)
        {
        }
        ...
        virtual void test()
        {
            ...
            for (v.nNo = m_nStartItem; v.nNo < m_nEndItem; ++v.nNo) {
                if (!m_stack.push(v)) ++m_nPushError;
            }
        }
  ...
  protected:
    ...
    void test(Stack &stack)
    {
        ...
        propout() << std::make_pair("thread_count", s_nThreadCount) << std::make_pair("push_count", s_nStackSize);
    ...
    void analyze(Stack &testStack)
    {
        ...
        propout() << std::make_pair("pop_duration", std::chrono::duration_cast<std::chrono::milliseconds>(
...
CDSSTRESS_TreiberStack(stack_push) CDSSTRESS_EliminationStack(stack_push) CDSSTRESS_FCStack(stack_push)
    CDSSTRESS_FCDeque(stack_push) CDSSTRESS_StdStack(stack_push)
