...
class intrusive_fcstack_push_pop : public cds_test::intrusive_stack_push_pop,
  ...
  public:
    ...
    static std::string get_test_parameter_name(testing::TestParamInfo<fc_param> const &p)
    {
        if (p.param.nCombinePassCount) {
            std::stringstream ss;
            ss << "compact_factor" << p.param.nCompactFactor << "__combine_pass_count" << p.param.nCombinePassCount;
            return ss.str();
        } else {
    ...
    void test()
    {
        ...
        if (s_bFCIterative) {
            ...
            if (arg.nCombinePassCount) {
                propout() << std::make_pair("compact_factor", arg.nCompactFactor)
...
#ifdef CDSTEST_GTEST_INSTANTIATE_TEST_CASE_P_HAS_4TH_ARG
INSTANTIATE_TEST_CASE_P(FC, intrusive_fcstack_push_pop,
