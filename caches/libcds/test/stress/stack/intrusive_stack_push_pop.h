...
class intrusive_stack_push_pop : public cds_test::stress_fixture {
    ...
    class Producer : public cds_test::thread {
      ...
      public:
        Producer(cds_test::thread_pool &pool, Stack &s)
            : base_class(pool, producer_thread), m_Stack(s), m_nPushError(0), m_pStart(nullptr), m_pEnd(nullptr)
        {
        }

        Producer(Producer &src)
            : base_class(src), m_Stack(src.m_Stack), m_nPushError(0), m_pStart(nullptr), m_pEnd(nullptr)
        {
        }
    ...
    class Consumer : public cds_test::thread {
      ...
      public:
        Consumer(cds_test::thread_pool &pool, Stack &s)
            : base_class(pool, consumer_thread), m_Stack(s), m_nPopCount(0), m_nPopEmpty(0), m_nDirtyPop(0)
        {
        }
  ...
  protected:
    ...
    void analyze(Stack & /*stack*/)
    {
        ...
        for (size_t threadNo = 0; threadNo < pool.size(); ++threadNo) {
            ...
            if (thread.type() == producer_thread) {
                ...
                for (size_t i = 0; i < sizeof(arrVal) / sizeof(arrVal[0]); ++i) arrVal[i] += producer.m_arrPush[i];
            } else {
                ...
                for (size_t i = 0; i < sizeof(arrVal) / sizeof(arrVal[0]); ++i) arrVal[i] -= consumer.m_arrPop[i];
            }
        }
        ...
        propout() << std::make_pair("push_count", s_nStackSize) << std::make_pair("push_error", nPushError)
                  << std::make_pair("pop_count", nPopCount) << std::make_pair("pop_empty", nPopEmpty)
    ...
    void do_test(Stack &stack, value_array<typename Stack::value_type> &arrValue)
    {
        ...
        {
            for (typename Stack::value_type *it = pValStart; it != pValEnd; ++it) it->nConsumer = c_nBadConsumer;
...

        {
            ...
            for (typename Stack::value_type *it = pValStart; it != pEnd; ++it) EXPECT_NE(it->nConsumer, nBadConsumer);
        }
