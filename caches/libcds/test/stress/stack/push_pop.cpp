...
class stack_push_pop : public cds_test::stress_fixture {
  protected:
    enum thread_type { producer_thread, consumer_thread };
    ...
    class Producer : public cds_test::thread {
      ...
      public:
        Producer(cds_test::thread_pool &pool, Stack &stack, size_t push_count)
            : base_class(pool, producer_thread), m_stack(stack), m_nItemCount(push_count), m_nPushError(0)
        {
        }

        Producer(Producer &src) : base_class(src), m_stack(src.m_stack), m_nItemCount(src.m_nItemCount), m_nPushError(0)
        {
        }
    ...
    class Consumer : public cds_test::thread {
      ...
      public:
        Consumer(cds_test::thread_pool &pool, Stack &stack)
            : base_class(pool, consumer_thread), m_stack(stack), m_nPopCount(0), m_nPopEmpty(0), m_nDirtyPop(0)
        {
        }

        Consumer(Consumer &src) : base_class(src), m_stack(src.m_stack), m_nPopCount(0), m_nPopEmpty(0), m_nDirtyPop(0)
        {
        }
  ...
  protected:
    static void SetUpTestCase()
    {
        ...
        if (s_nPushThreadCount == 0) s_nPushThreadCount = 1;
        if (s_nPopThreadCount == 0) s_nPopThreadCount = 1;
    }
    ...
    void analyze(Stack & /*stack*/)
    {
        ...
        for (size_t threadNo = 0; threadNo < pool.size(); ++threadNo) {
            ...
            if (thread.type() == producer_thread) {
                ...
                for (size_t i = 0; i < c_nValArraySize; ++i) arrVal[i] += producer.m_arrPush[i];
            } else {
                ...
                for (size_t i = 0; i < c_nValArraySize; ++i) arrVal[i] -= consumer.m_arrPop[i];
            }
        }
        ...
        propout() << std::make_pair("push_count", s_nStackSize) << std::make_pair("push_error", nPushError)
                  << std::make_pair("pop_empty", nPopEmpty) << std::make_pair("dirty_pop", nDirtyPop);
    }
};

CDSSTRESS_TreiberStack(stack_push_pop) CDSSTRESS_EliminationStack(stack_push_pop) CDSSTRESS_FCStack(stack_push_pop)
    CDSSTRESS_FCDeque(stack_push_pop) CDSSTRESS_StdStack(stack_push_pop)
