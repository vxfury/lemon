...
#define CDS_HPSTAT_OUT(stat, fld) std::make_pair("dhp_" + property_stream::stat_prefix() + "." #fld, stat.fld)
    return o << CDS_HPSTAT_OUT(s, guard_allocated) << CDS_HPSTAT_OUT(s, guard_freed) << CDS_HPSTAT_OUT(s, retired_count)
             ...
             << CDS_HPSTAT_OUT(s, thread_rec_count) << CDS_HPSTAT_OUT(s, hp_block_count)
             << CDS_HPSTAT_OUT(s, retired_block_count) << CDS_HPSTAT_OUT(s, hp_extend_count)
...
#define CDS_HPSTAT_OUT(stat, fld) "\t" << #fld << "=" << stat.fld << "\n"
    return o << "DHP post-mortem statistics:\n"
