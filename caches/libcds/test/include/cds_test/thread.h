...
class thread_pool {
  ...
  public:
    explicit thread_pool(::testing::Test &fx) : m_fixture(fx), m_bTimeElapsed(false) {}
    ...
    std::chrono::milliseconds run(std::chrono::seconds duration)
    {
        ...
        for (auto w : m_workers) threads.emplace_back(&thread::run, w);
        ...
        if (duration != std::chrono::seconds::zero()) {
            for (;;) {
                ...
                if (time_now >= expected_end) break;
    ...
    thread &get(size_t idx) const
    {
        return *m_workers.at(idx);
    }
...
inline thread::thread(thread_pool &master, int type /*= 0*/) : m_pool(master), m_type(type), m_id(master.get_next_id())
{
}

inline thread::thread(thread const &sample) : m_pool(sample.m_pool), m_type(sample.m_type), m_id(m_pool.get_next_id())
{
}
