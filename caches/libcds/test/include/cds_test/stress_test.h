...
class config {
  public:
    std::string get(const char *pszParamName, const char *pszDefVal = NULL) const
    {
        ...
        if (it == m_Cfg.end()) return std::string(pszDefVal); // param not found -> returns default value
        return it->second;
    }
    ...
    int get_int(const char *pszParamName, int defVal = 0) const
    {
        ...
        if (it == m_Cfg.end()) return defVal; // param not found -> returns default value
    ...
    bool get_bool(const char *pszParamName, bool defVal = false) const
    {
        ...
        cfg_map::const_iterator it = m_Cfg.find(strParamName);
        if (it == m_Cfg.end()) return defVal; // param not found -> returns default value
        return !(it->second.empty() || it->second == "0" || it->second == "false" || it->second == "no");
    }
...
struct stat_prefix {
    ...
    stat_prefix() : prefix_(nullptr) {}

    stat_prefix(char const *prefix) : prefix_(prefix) {}
};
...
class stress_fixture : public fixture {
  ...
  protected:
    stress_fixture() : m_thread_pool(*this) {}
