...
class has_set_array_size {
    ...
    template <typename C, void (C::*)(size_t)>
    class selector {
    };

    template <typename C>
    static select_small test(selector<C, &C::set_array> *);
    template <typename C>
    static select_big test(...);
...
struct heavy_value {
    ...
    explicit heavy_value(int new_value = 0) : value(new_value), nNo(0), nWriterNo(0){};

    heavy_value(heavy_value const &other) : value(other.value), nNo(other.nNo), nWriterNo(other.nWriterNo)
    {
