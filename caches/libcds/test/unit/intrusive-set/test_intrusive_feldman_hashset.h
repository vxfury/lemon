...
class intrusive_feldman_hashset : public fixture {
  public:
    ...
    struct int_item : public stat {
        ...
        explicit int_item(int key) : nKey(key), nVal(key) {}

        int_item(int key, int val) : nKey(key), nVal(val) {}

        int_item(int_item const &v) : stat(), nKey(v.nKey), nVal(v.nVal) {}
    ...
    struct int_item2 : public key_val, public stat {
        ...
        int_item2(int_item2 const &v) : key_val(v), stat() {}
    };
        ...
        for (auto idx : indices) {
            ...
            switch (i.key() % 3) {
                ...
                case 2:
                    ...
                    ASSERT_TRUE(s.erase(v.key(), [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    ASSERT_FALSE(s.erase(v.key(), [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    break;
