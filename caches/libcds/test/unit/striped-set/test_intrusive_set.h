  ...
  protected:
    ...
    void test_(Set &s, std::vector<typename Set::value_type> &data)
    {
        ...
        for (auto idx : indices) {
            ...
            updResult = s.update(
                i,
                [](bool /*bNew*/, value_type &, value_type &) {
                    ASSERT_TRUE(false);
                },
                false);
        ...
        for (auto idx : indices) {
            ...
            switch (i.key() % 6) {
                ...
                case 4:
                    ...
                    ASSERT_TRUE(s.erase(v, [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    ASSERT_FALSE(s.erase(v, [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    break;
                case 5:
                    EXPECT_EQ(i.nEraseCount, 0u);
                    ASSERT_TRUE(s.erase_with(other_item(i.key()), other_predicate(), [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    ASSERT_FALSE(s.erase_with(other_item(i.key()), other_predicate(), [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    break;
