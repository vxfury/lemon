...
class intrusive_tree : public fixture {
  public:
    ...
    struct base_int_item : public Node,
                           public stat

    {
        ...
        explicit base_int_item(int key) : nKey(key), nVal(key) {}

        base_int_item(int key, int val) : nKey(key), nVal(val) {}

        base_int_item(base_int_item const &v) : Node(), stat(), nKey(v.nKey), nVal(v.nVal) {}
    ...
    struct member_int_item : public stat {
        ...
        explicit member_int_item(int key) : nKey(key), nVal(key) {}

        member_int_item(int key, int val) : nKey(key), nVal(val) {}

        member_int_item(member_int_item const &v) : stat(), nKey(v.nKey), nVal(v.nVal) {}
  ...
  protected:
    ...
    void test(Tree &t)
    {
        ...
        for (auto idx : indices) {
            ...
            switch (i.key() % 3) {
                case 0:
                    ...
                    updResult = t.update(
                        i,
                        [](bool bNew, value_type &val, value_type &arg) {
                            EXPECT_FALSE(bNew);
                            EXPECT_EQ(&val, &arg);
                        },
                        false);
                ...
                case 1:
                    ...
                    ASSERT_TRUE(t.insert(i, [](value_type &v) {
                        ++v.nUpdateNewCount;
                    }));
                    EXPECT_EQ(i.nUpdateNewCount, 1u);
                    ASSERT_FALSE(t.insert(i, [](value_type &v) {
                        ++v.nUpdateNewCount;
                    }));
                    EXPECT_EQ(i.nUpdateNewCount, 1u);
                    i.nUpdateNewCount = 0;
            ...
            ASSERT_TRUE(t.find(i.nKey, [](value_type &v, int) {
                ++v.nFindCount;
            }));
            EXPECT_EQ(i.nFindCount, 1u);
            ASSERT_TRUE(t.find_with(other_item(i.key()), other_less(), [](value_type &v, other_item const &) {
                ++v.nFindCount;
            }));
            EXPECT_EQ(i.nFindCount, 2u);
            ASSERT_TRUE(t.find(i, [](value_type &v, value_type &) {
                ++v.nFindCount;
            }));
        ...
        std::for_each(data.begin(), data.end(), [](value_type &v) {
            v.clear_stat();
        });
        ...
        for (auto idx : indices) {
            ...
            switch (i.key() % 6) {
                ...
                case 4:
                    ...
                    ASSERT_TRUE(t.erase(v, [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    ASSERT_FALSE(t.erase(v, [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    break;
                case 5:
                    EXPECT_EQ(i.nEraseCount, 0u);
                    ASSERT_TRUE(t.erase_with(other_item(i.key()), other_less(), [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    ASSERT_FALSE(t.erase_with(other_item(i.key()), other_less(), [](value_type &val) {
                        ++val.nEraseCount;
                    }));
                    EXPECT_EQ(i.nEraseCount, 1u);
                    break;
