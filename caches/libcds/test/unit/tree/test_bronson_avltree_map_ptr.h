...
class bronson_avltree_map_ptr : public cds_test::tree_map_fixture {
  ...
  protected:
    ...
    void test(Map &m)
    {
        ...
        for (auto const &i : arrKeys) {
            ...
            switch (i.nKey % 8) {
                ...
                case 7:
                    ...
                    ASSERT_FALSE(m.erase_with(other_item(i.nKey), other_less(),
                                              [](key_type const & /*key*/, mapped_type & /*v*/) {
                                                  EXPECT_TRUE(false);
                                              }));
                    break;
            }
            ...
            ASSERT_FALSE(
                m.find_with(other_item(i.nKey), other_less(), [](key_type const & /*key*/, mapped_type & /*val*/) {
                    EXPECT_TRUE(false);
                }));
        }
        ASSERT_TRUE(m.empty());
        ...
        xp = m.extract_min();
        EXPECT_TRUE(!xp);
        xp = m.extract_min([](key_type const &) {
            EXPECT_FALSE(true);
        });
        EXPECT_TRUE(!xp);
        xp = m.extract_max();
        EXPECT_TRUE(!xp);
        xp = m.extract_max([](key_type const &) {
            EXPECT_FALSE(true);
        });
        EXPECT_TRUE(!xp);
        {
...
TYPED_TEST_P(BronsonAVLTreeMapPtr, compare)
{
    ...
    typedef cc::BronsonAVLTreeMap<rcu_type, key_type, value_type *,
                                  typename cc::bronson_avltree::make_traits<
                                      ...
                                      cds::intrusive::opt::disposer<bronson_avltree_map_ptr::mock_disposer>>::type>
        map_type;
...
REGISTER_TYPED_TEST_CASE_P(BronsonAVLTreeMapPtr, compare, less, cmpmix, stat, item_counting, relaxed_insert, seq_cst,
