...
class bronson_avltree_map : public cds_test::tree_map_fixture {
  ...
  protected:
    ...
    void test(Map &m)
    {
        ...
        for (auto const &i : arrKeys) {
            ...
            switch (i.nKey % 16) {
                ...
                case 9:
                    updResult = m.update(
                        i.nKey,
                        [](bool, key_type const & /*key*/, value_type & /*val*/) {
                            EXPECT_TRUE(false);
                        },
                        false);
        ...
        for (auto const &i : arrKeys) {
            ...
            switch (i.nKey % 8) {
                ...
                case 7:
                    ...
                    ASSERT_FALSE(m.erase_with(other_item(i.nKey), other_less(),
                                              [](key_type const & /*key*/, value_type & /*val*/) {
                                                  EXPECT_TRUE(false);
                                              }));
                    break;
            }
            ...
            ASSERT_FALSE(
                m.find_with(other_item(i.nKey), other_less(), [](key_type const & /*key*/, value_type & /*val*/) {
                    EXPECT_TRUE(false);
                }));
        }
        ASSERT_TRUE(m.empty());
        ...
        for (auto const &i : arrKeys) ASSERT_TRUE(m.insert(i, arrVals[i.nKey]));
        ASSERT_FALSE(m.empty());
        ASSERT_CONTAINER_SIZE(m, kkSize);
...
TYPED_TEST_P(BronsonAVLTreeMap, compare)
{
    ...
    typedef cc::BronsonAVLTreeMap<
        rcu_type, key_type, value_type,
        typename cc::bronson_avltree::make_traits<cds::opt::compare<typename TestFixture::cmp>>::type>
        map_type;
...
TYPED_TEST_P(BronsonAVLTreeMap, less)
{
    ...
    typedef cc::BronsonAVLTreeMap<
        rcu_type, key_type, value_type,
        typename cc::bronson_avltree::make_traits<cds::opt::less<typename TestFixture::less>>::type>
        map_type;
...
TYPED_TEST_P(BronsonAVLTreeMap, cmpmix)
{
    ...
    typedef cc::BronsonAVLTreeMap<
        rcu_type, key_type, value_type,
        typename cc::bronson_avltree::make_traits<cds::opt::less<typename TestFixture::less>,
                                                  cds::opt::compare<typename TestFixture::cmp>>::type>
        map_type;
...
REGISTER_TYPED_TEST_CASE_P(BronsonAVLTreeMap, compare, less, cmpmix, stat, item_counting, relaxed_insert, seq_cst,
                           sync_monitor, lazy_sync_monitor, rcu_check_deadlock, rcu_no_check_deadlock);
