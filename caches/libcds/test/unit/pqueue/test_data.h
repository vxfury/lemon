...
class PQueueTest : public ::cds_test::fixture {
  public:
    ...
    struct value_type {
        ...
        value_type(value_type const &kv) : k(kv.k), v(kv.v) {}

        value_type(key_type key) : k(key), v(key) {}

        value_type(key_type key, int val) : k(key), v(val) {}

        value_type(std::pair<key_type, int> const &p) : k(p.first), v(p.second) {}
    };
    ...
    enum { c_nMinValue = -123, c_nCapacity = 1024 };
  ...
  protected:
    ...
    class data_array {
      ...
      public:
        data_array(size_t nSize) : pFirst(new T[nSize]), pLast(pFirst.get() + nSize)
        {
            key_type i = c_nMinValue;
            for (T *p = pFirst.get(); p != pLast; ++p, ++i) p->k = p->v = i;
