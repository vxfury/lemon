...
class cxx11_atomic_func : public ::testing::Test {
  protected:
    ...
    void do_test_atomic_type(Atomic &a, atomics::memory_order order)
    {
        ...
        for (size_t nByte = 0; nByte < sizeof(Integral); ++nByte) {
            ...
            EXPECT_TRUE(
                atomics::atomic_compare_exchange_weak_explicit(&a, &expected, n, order, atomics::memory_order_relaxed));
            EXPECT_EQ(expected, prev);
            EXPECT_FALSE(
                atomics::atomic_compare_exchange_weak_explicit(&a, &expected, n, order, atomics::memory_order_relaxed));
            EXPECT_EQ(expected, n);
        ...
        for (size_t nByte = 0; nByte < sizeof(Integral); ++nByte) {
            ...
            EXPECT_TRUE(atomics::atomic_compare_exchange_strong_explicit(&a, &expected, n, order,
                                                                         atomics::memory_order_relaxed));
            EXPECT_EQ(expected, prev);
            EXPECT_FALSE(atomics::atomic_compare_exchange_strong_explicit(&a, &expected, n, order,
                                                                          atomics::memory_order_relaxed));
            EXPECT_EQ(expected, n);
    ...
    void do_test_atomic_bool(AtomicBool &a, atomics::memory_order order)
    {
        ...
        bool expected = false;
        EXPECT_TRUE(
            atomics::atomic_compare_exchange_weak_explicit(&a, &expected, true, order, atomics::memory_order_relaxed));
        EXPECT_FALSE(expected);
        EXPECT_FALSE(
            atomics::atomic_compare_exchange_weak_explicit(&a, &expected, false, order, atomics::memory_order_relaxed));
        EXPECT_TRUE(expected);
        EXPECT_TRUE(atomics::atomic_load_explicit(&a, oLoad));
        ...
        expected = false;
        EXPECT_TRUE(atomics::atomic_compare_exchange_strong_explicit(&a, &expected, true, order,
                                                                     atomics::memory_order_relaxed));
        EXPECT_FALSE(expected);
        EXPECT_FALSE(atomics::atomic_compare_exchange_strong_explicit(&a, &expected, false, order,
                                                                      atomics::memory_order_relaxed));
        EXPECT_TRUE(expected);
    ...
    void test_atomic_pointer_for_(Atomic &a, Integral *arr, Integral aSize, atomics::memory_order order)
    {
        ...
        p = arr;
        EXPECT_TRUE(
            atomics::atomic_compare_exchange_weak_explicit(&a, &p, arr + 5, order, atomics::memory_order_relaxed));
        EXPECT_EQ(p, arr + 0);
        EXPECT_EQ(*p, 1);
        EXPECT_FALSE(
            atomics::atomic_compare_exchange_weak_explicit(&a, &p, arr + 3, order, atomics::memory_order_relaxed));
        EXPECT_EQ(p, arr + 5);
        EXPECT_EQ(*p, 6);

        EXPECT_TRUE(
            atomics::atomic_compare_exchange_strong_explicit(&a, &p, arr + 3, order, atomics::memory_order_relaxed));
        EXPECT_EQ(p, arr + 5);
        EXPECT_EQ(*p, 6);
        EXPECT_FALSE(
            atomics::atomic_compare_exchange_strong_explicit(&a, &p, arr + 5, order, atomics::memory_order_relaxed));
        EXPECT_EQ(p, arr + 3);
        EXPECT_EQ(*p, 4);
    ...
    void do_test_atomic_pointer_void_(Atomic &a, char *arr, char aSize, atomics::memory_order order)
    {
        ...
        p = arr;
        EXPECT_TRUE(atomics::atomic_compare_exchange_weak_explicit(&a, (void **)&p, (void *)(arr + 5), order,
                                                                   atomics::memory_order_relaxed));
        EXPECT_EQ(p, arr + 0);
        EXPECT_EQ(*p, 1);
        EXPECT_FALSE(atomics::atomic_compare_exchange_weak_explicit(&a, (void **)&p, (void *)(arr + 3), order,
                                                                    atomics::memory_order_relaxed));
        EXPECT_EQ(p, arr + 5);
        EXPECT_EQ(*p, 6);

        EXPECT_TRUE(atomics::atomic_compare_exchange_strong_explicit(&a, (void **)&p, (void *)(arr + 3), order,
                                                                     atomics::memory_order_relaxed));
        EXPECT_EQ(p, arr + 5);
        EXPECT_EQ(*p, 6);
        EXPECT_FALSE(atomics::atomic_compare_exchange_strong_explicit(&a, (void **)&p, (void *)(arr + 5), order,
                                                                      atomics::memory_order_relaxed));
        EXPECT_EQ(p, arr + 3);
        EXPECT_EQ(*p, 4);
  ...
  public:
    void test_atomic_flag()
    {
        ...
        for (size_t i = 0; i < sizeof(flags) / sizeof(flags[0]); ++i) do_test_atomic_flag(flags[i]);
    }
