...
class intrusive_list_common : public fixture {
  public:
    ...
    struct base_item : public Node {
        ...
        base_item(int key, int val) : nKey(key), nVal(val), s() {}

        base_item(const base_item &v) : nKey(v.nKey), nVal(v.nVal), s() {}
    ...
    struct member_item {
        ...
        member_item(int key, int val) : nKey(key), nVal(val), s() {}

        member_item(const member_item &v) : nKey(v.nKey), nVal(v.nVal), s() {}
  ...
  protected:
    ...
    void test_common(List &l)
    {
        ...
        for (auto const &i : arr) EXPECT_EQ(i.s.nDisposeCount, 1);
