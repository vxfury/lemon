...
class intrusive_iterable_list : public fixture {
  public:
    struct stat {
        ...
        stat() : nDisposeCount(0), nUpdateExistsCall(0), nUpdateNewCall(0), nFindCall(0), nEraseCall(0), nInsertCall(0)
        {
        }
    ...
    struct item_type {
        ...
        item_type(int key, int val) : nKey(key), nVal(val), s() {}

        item_type(const item_type &v) : nKey(v.nKey), nVal(v.nVal), s() {}
  ...
  protected:
    ...
    void test_common(List &l)
    {
        ...
        for (auto &i : arr) {
            ...
            EXPECT_FALSE(l.find(i.nKey, [](value_type &item, int) {
                ++item.s.nFindCall;
            }));
            EXPECT_TRUE(l.find(i.nKey) == l.end());
            EXPECT_EQ(i.s.nFindCall, 0);
            EXPECT_FALSE(l.find_with(other_item(i.nKey), other_less(), [](value_type &item, other_item const &) {
                ++item.s.nFindCall;
            }));
            EXPECT_EQ(i.s.nFindCall, 0);
            ...
            switch (i.nKey % 4) {
                ...
                case 1:
                    ...
                    EXPECT_TRUE(l.insert(i, [](value_type &v) {
                        ++v.s.nInsertCall;
                    }));
                    EXPECT_EQ(i.s.nInsertCall, 1);
                    break;
                case 2: {
                    std::pair<bool, bool> ret = l.update(
                        i,
                        [](value_type &v, value_type *old) {
            ...
            EXPECT_TRUE(l.find(i.nKey, [](value_type &item, int) {
                ++item.s.nFindCall;
            }));
            EXPECT_EQ(i.s.nFindCall, 1);
            EXPECT_TRUE(l.find(i, [](value_type &item, value_type const &) {
                ++item.s.nFindCall;
            }));
            EXPECT_EQ(i.s.nFindCall, 2);
            EXPECT_TRUE(l.find_with(other_item(i.nKey), other_less(), [](value_type &item, other_item const &) {
                ++item.s.nFindCall;
            }));
        ...
        for (auto const &i : arr) EXPECT_EQ(i.s.nDisposeCount, 2);
        for (auto const &i : arr2) EXPECT_EQ(i.s.nDisposeCount, 1);
        ...
        for (auto const &i : arr) {
            ...
            if (i.nKey & 1) {
                ...
                EXPECT_TRUE(l.erase(i.nKey, [](value_type &item) {
                    ++item.s.nEraseCall;
                }));
                EXPECT_FALSE(l.erase(i.nKey, [](value_type &item) {
                    ++item.s.nEraseCall;
                }));
            }
        ...
        for (auto const &i : arr) EXPECT_EQ(i.s.nDisposeCount, 3);

        // clear test
        for (auto &i : arr) EXPECT_TRUE(l.insert(i));
