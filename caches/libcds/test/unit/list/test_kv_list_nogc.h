...
class kv_list_nogc : public fixture {
  public:
    struct key_type {
        ...
        key_type() = delete;
        explicit key_type(int n) : key(n) {}

        key_type(key_type const &s) : key(s.key) {}

        key_type(key_type &&s) : key(s.key)
        {
            s.key = 0;
        }
    };
  ...
  protected:
    ...
    void test_ordered_iterator(List &l)
    {
        ...
        for (auto &i : arr) EXPECT_TRUE(l.insert(i.key, i.val) != l.end());
