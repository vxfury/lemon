...
class kv_iterable_list : public fixture {
  public:
    struct key_type {
        ...
        key_type(key_type &&s) : nKey(s.nKey)
        {
            s.nKey = 0;
        }
    ...
    struct value_type {
        ...
        explicit value_type(int n) : val(n) {}
    };
  ...
  protected:
    ...
    void test_common(List &l)
    {
        ...
        for (auto const &i : arr) {
            ...
            switch (i.key % 6) {
                ...
                case 4: {
                    auto pair = l.update(
                        i.key,
                        [](list_value_type &, list_value_type *) {
                            ASSERT_TRUE(false);
                        },
                        false);
            ...
            auto pair = l.update(
                i.key,
                [](list_value_type &n, list_value_type *old) {
        ...
        // clear test
        for (auto &i : arr) EXPECT_TRUE(l.insert(i.key, i.val));
