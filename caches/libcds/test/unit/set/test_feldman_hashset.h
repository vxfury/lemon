...
class feldman_hashset : public fixture {
  public:
    ...
    struct other_item {
        ...
        explicit other_item(int k) : nKey(k) {}
    ...
    struct int_item : public stat {
        ...
        int_item() : nKey(0), nVal(0) {}

        explicit int_item(int k) : nKey(k), nVal(k * 2) {}

        template <typename Q>
        explicit int_item(Q const &src) : nKey(src.key()), nVal(0)
        {
        }

        int_item(int_item const &src) : nKey(src.nKey), nVal(src.nVal), strVal(src.strVal) {}

        int_item(int_item &&src) : nKey(src.nKey), nVal(src.nVal), strVal(std::move(src.strVal)) {}

        int_item(int k, std::string &&s) : nKey(k), nVal(k * 2), strVal(std::move(s)) {}

        explicit int_item(other_item const &s) : nKey(s.key()), nVal(s.key() * 2) {}
    ...
    struct key_val {
        ...
        key_val(int key) : nKey(key), nVal(key) {}

        key_val(int key, int val) : nKey(key), nVal(val) {}

        key_val(key_val const &v) : nKey(v.nKey), nVal(v.nVal) {}
    ...
    struct int_item2 : public key_val, public stat {
        ...
        explicit int_item2(int key) : key_val(key) {}

        int_item2(int key, int val) : key_val(key, val) {}

        int_item2(int_item2 const &v) : key_val(v), stat(), strVal(v.strVal) {}

        int_item2(int_item2 &&v) : key_val(v), stat(), strVal(std::move(v.strVal)) {}

        int_item2(int k, std::string &&s) : key_val(k, k * 2), strVal(std::move(s)) {}

        explicit int_item2(other_item const &s) : key_val(s.key(), s.key() * 2) {}
    };
    ...
    struct simple_item_counter {
        ...
        simple_item_counter() : m_nCount(0) {}
        ...
        for (auto idx : indices) {
            ...
            switch (idx % 8) {
                case 0:
                    ...
                    updResult = s.update(
                        i,
                        [](value_type &val, value_type *prev) {
