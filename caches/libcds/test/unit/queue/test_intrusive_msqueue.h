...
class intrusive_msqueue : public ::testing::Test {
  protected:
    ...
    struct base_hook_item : public Base {
        ...
        base_hook_item(base_hook_item const &s) : nVal(s.nVal), nDisposeCount(s.nDisposeCount) {}
    };
    ...
    struct member_hook_item {
        ...
        member_hook_item(member_hook_item const &s) : nVal(s.nVal), nDisposeCount(s.nDisposeCount) {}
    };
    ...
    void test(Queue &q, Data &arr)
    {
        ...
        value_type *pv;
        for (size_t i = 0; i < nSize; ++i) arr[i].nVal = static_cast<int>(i);
