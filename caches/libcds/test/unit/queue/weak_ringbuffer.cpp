  ...
  public:
    ...
    void test_array(Queue &q)
    {
...

        {
            ...
            for (unsigned pass = 0; pass < 3; ++pass) {
                ...
                for (size_t i = 0; i < nSize; i += nArrSize) {
                    for (size_t k = 0; k < nArrSize; ++k) el[k] = static_cast<value_type>(i + k);
...

        {
            ...
            auto func_push = [](value_type &dest, size_t src) {
                dest = static_cast<value_type>(src * 10);
            };

            for (unsigned pass = 0; pass < 3; ++pass) {
                for (size_t i = 0; i < nSize; i += nArrSize) {
                    for (size_t k = 0; k < nArrSize; ++k) el[k] = i + k;
                ...
                auto func_pop = [](size_t &dest, value_type src) {
                    dest = static_cast<size_t>(src / 10);
                };
                size_t expected = 0;
    ...
    void test_varsize_buffer(Queue &q)
    {
        ...
        while (total_push < capacity * 4) {
            ...
            for (size_t i = 0; i < pair.second; ++i) ASSERT_EQ(*reinterpret_cast<uint8_t *>(pair.first), chfill);
