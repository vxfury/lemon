...
class intrusive_segmented_queue : public ::testing::Test {
  protected:
    struct item {
        ...
        item() : nValue(0), nDisposeCount(0), nDispose2Count(0) {}

        item(int nVal) : nValue(nVal), nDisposeCount(0), nDispose2Count(0) {}
    };

    struct big_item : public item {
        big_item() {}

        big_item(int nVal) : item(nVal) {}
    ...
    void test(Queue &q, Data &val)
    {
        ...
        val.resize(100);
        for (size_t i = 0; i < val.size(); ++i) val[i].nValue = static_cast<int>(i);
        ...
        while (!q.empty()) {
            ...
            EXPECT_TRUE(nMin <= pVal->nValue && pVal->nValue <= nMax)
                << nMin << " <= " << pVal->nValue << " <= " << nMax;
        ...
        // clear
        for (size_t i = 0; i < val.size(); ++i) EXPECT_TRUE(q.push(val[i]));
