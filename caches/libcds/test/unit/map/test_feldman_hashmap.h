...
class feldman_hashmap : public map_fixture {
  public:
    ...
    struct key_type2 {
        ...
        explicit key_type2(int n) : nKey(n), subkey(static_cast<uint16_t>(n)) {}

        explicit key_type2(size_t n) : nKey(static_cast<int>(n)), subkey(static_cast<uint16_t>(n)) {}

        explicit key_type2(std::string const &str) : nKey(std::stoi(str)), subkey(static_cast<uint16_t>(nKey)) {}

        key_type2(key_type2 const &s) : nKey(s.nKey), subkey(s.subkey) {}
    };
