...
capo::random<> rand__{static_cast<int>(min_sz), static_cast<int>(max_sz)};
...
void do_counting()
{
    for (int i = 1; !is_quit__.load(std::memory_order_acquire); ++i) {
        ...
        if (i % 10) continue;
        i = 0;
        std::cout << speed_of(size_counter__.exchange(0, std::memory_order_relaxed)) << std::endl;
    }
}

void do_send()
{
    std::cout << __func__ << ": start [" << str_of_size(min_sz) << " - " << str_of_size(max_sz) << "]...\n";
