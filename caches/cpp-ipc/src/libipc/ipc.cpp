...
struct msg_t : msg_t<0, AlignSize> {
        ...
        : msg_t<0, AlignSize>{cc_id, id, remain, (data == nullptr) || (size == 0)}
    {
        if (this->storage_) {
            if (data != nullptr) {
                // copy storage-id
                *reinterpret_cast<ipc::storage_id_t *>(&data_) = *static_cast<ipc::storage_id_t const *>(data);
            }
        } else
...
IPC_CONSTEXPR_ std::size_t calc_chunk_size(std::size_t size) noexcept
{
    return ipc::make_align(
        ...
        align_chunk_size(ipc::make_align(alignof(std::max_align_t), sizeof(std::atomic<ipc::circ::cc_t>)) + size));
}
...
bool sub_rc(ipc::wr<Rp, Rc, ipc::trans::unicast>, std::atomic<ipc::circ::cc_t> & /*conns*/,
            ipc::circ::cc_t /*curr_conns*/, ipc::circ::cc_t /*conn_id*/) noexcept
{
    return true;
}

template <ipc::relat Rp, ipc::relat Rc>
bool sub_rc(ipc::wr<Rp, Rc, ipc::trans::broadcast>, std::atomic<ipc::circ::cc_t> &conns, ipc::circ::cc_t curr_conns,
...
bool clear_message(void *p)
{
    ...
    if (msg->storage_) {
        ...
        release_storage(*reinterpret_cast<ipc::storage_id_t *>(&msg->data_), static_cast<std::size_t>(r_size));
    }
    return true;
}
...
struct queue_generator {
    ...
    struct conn_info_t : conn_info_head {
        ...
        conn_info_t(char const *name)
            : conn_info_head{name},
              que_{("__QU_CONN__" + ipc::to_string(DataSize) + "__" + ipc::to_string(AlignSize) + "__" + name).c_str()}
        {
        }
    ...
    static bool send(F &&gen_push, ipc::handle_t h, void const *data, std::size_t size)
    {
        ...
        if (size > ipc::large_msg_limit) {
            ...
            if (buf != nullptr) {
                ...
                return try_push(static_cast<std::int32_t>(size) - static_cast<std::int32_t>(ipc::data_length),
                                &(dat.first), 0);
            }
        ...
        if (remain > 0) {
            if (!try_push(remain - static_cast<std::int32_t>(ipc::data_length),
                          static_cast<ipc::byte_t const *>(data) + offset, static_cast<std::size_t>(remain))) {
                return false;
            }
        }
        return true;
            ...
            [tm](auto info, auto que, auto msg_id) {
                return [tm, info, que, msg_id](std::int32_t remain, void const *data, std::size_t size) {
                    if (!wait_for(
                        ...
                        if (!que->force_push(clear_message<typename queue_t::value_t>, info->cc_id_, msg_id, remain,
                                             data, size)) {
                            return false;
                        }
                    }
            ...
            if (msg.storage_) {
                ...
                if (buf != nullptr) {
                    struct recycle_t {
                        ...
                        recycle_t{buf_id, que->elems()->connections(std::memory_order_relaxed), que->connected_id()});
                    if (r_info == nullptr) {
                        ...
                        return ipc::buff_t{buf, msg_size,
                                           [](void *p_info, std::size_t size) {
                                               ...
                                               recycle_storage<flag_t>(r_info->storage_id, size, r_info->curr_conns,
                                                                       r_info->conn_id);
                                           },
                                           r_info};
                    }
