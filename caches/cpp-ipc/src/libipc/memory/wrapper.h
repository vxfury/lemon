...
class limited_recycler<AllocP, true> {
  ...
  public:
    void try_recover(alloc_policy &alc)
    {
        ...
        take_first_do([&alc](alloc_policy &first) {
            alc.swap(first);
        });
    }
...
class default_recycler : public limited_recycler<AllocP> {
    ...
    void try_fill(A &alc)
    {
        ...
        this->take_first_do([&alc](alloc_policy &first) {
            alc.take(std::move(first));
        });
    }
...
template <typename AllocP, template <typename> class RecyclerP = default_recycler>
class async_wrapper {
  public:
    using alloc_policy = AllocP;
  ...
  private:
    ...
    class alloc_proxy : public AllocP {
      ...
      public:
        ...
        template <typename... P>
        alloc_proxy(async_wrapper *w, P &&...pars) : AllocP(std::forward<P>(pars)...), w_(w)
        {
            assert(w_ != nullptr);
...
class sync_wrapper {
  ...
  public:
    template <typename... P>
    sync_wrapper(P &&...pars) : alloc_(std::forward<P>(pars)...)
    {
    }
...
struct default_mapping_policy {
    enum : std::size_t { base_size = BaseSize, iter_size = IterSize, classes_size = 64 };
...
template <typename FixedAlloc, typename DefaultAlloc = mem::static_alloc, typename MappingP = default_mapping_policy<>>
class variable_wrapper {
  ...
  public:
    ...
    {
        MappingP::foreach (
            [](std::size_t id, falc_t *in, falc_t *ot) {
    ...
    void *alloc(std::size_t size)
    {
        return MappingP::classify(
            [](std::size_t id, std::size_t size, falc_t *a) {
    ...
    {
        MappingP::classify(
            [](std::size_t id, std::size_t size, void *p, falc_t *a) {
