...
class scope_alloc_base {
  protected:
    ...
    enum : std::size_t { aligned_block_size = aligned(sizeof(block_t), alignof(std::max_align_t)) };
...
class scope_alloc : public detail::scope_alloc_base {
  ...
  public:
    ...
    scope_alloc(scope_alloc &&rhs)
    {
        swap(rhs);
    }
    scope_alloc &operator=(scope_alloc rhs)
    {
        swap(rhs);
        return (*this);
    }
...
struct fixed_expand_policy {
    enum : std::size_t { base_size = BaseSize, limit_size = LimitSize };
...
template <std::size_t BlockSize, typename AllocP = scope_alloc<>, typename ExpandP = fixed_expand_policy<>>
class fixed_alloc : public detail::fixed_alloc<AllocP, ExpandP> {
  public:
    using base_t = detail::fixed_alloc<AllocP, ExpandP>;

    enum : std::size_t { block_size = ipc::detail::max<std::size_t>(BlockSize, sizeof(void *)) };

  public:
    explicit fixed_alloc(std::size_t init_expand) : base_t(block_size, init_expand) {}

    fixed_alloc() : fixed_alloc(1) {}

    fixed_alloc(fixed_alloc &&rhs) : base_t(std::move(rhs)) {}
...
class variable_alloc : public detail::variable_alloc_base {
  public:
    ...
    enum : std::size_t { aligned_chunk_size = detail::aligned(ChunkSize, alignof(std::max_align_t)) };
  ...
  public:
    ...
    variable_alloc(variable_alloc &&rhs)
    {
        swap(rhs);
    }
    variable_alloc &operator=(variable_alloc rhs)
    {
        swap(rhs);
        return (*this);
    }
