...
class mutex {
    ...
    struct curr_prog {
        struct shm_data {
            ...
            shm_data(init arg) : shm{arg.name, arg.size}, ref{0} {}
        };
    ...
    pthread_mutex_t *acquire_mutex(char const *name)
    {
        ...
        if (it == info.mutex_handles.end()) {
            it = curr_prog::get()
                     .mutex_handles.emplace(name, curr_prog::shm_data::init{name, sizeof(pthread_mutex_t)})
                     .first;
        }
        shm_ = &it->second.shm;
        ref_ = &it->second.ref;
  ...
  public:
    ...
    bool open(char const *name) noexcept
    {
        ...
        IPC_UNUSED_ auto guard_mutex_attr = guard([&mutex_attr] {
    ...
    bool lock(std::uint64_t tm) noexcept
    {
        ...
        for (;;) {
            ...
            int eno = (tm == invalid_value) ? ::pthread_mutex_lock(mutex_) : ::pthread_mutex_timedlock(mutex_, &ts);
            switch (eno) {
                case 0:
                    return true;
                case ETIMEDOUT:
