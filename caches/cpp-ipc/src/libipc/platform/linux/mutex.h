  ...
  public:
    bool lock(std::uint64_t tm) noexcept
    {
        ...
        for (;;) {
            ...
            int eno = A0_SYSERR((tm == invalid_value) ? a0_mtx_lock(native()) : a0_mtx_timedlock(native(), {ts}));
            switch (eno) {
                case 0:
                    return true;
                case ETIMEDOUT:
...
class mutex {
    ...
    void acquire_mutex(char const *name)
    {
        ...
        if (it == info.mutex_handles.end()) {
            it = curr_prog::get().mutex_handles.emplace(name, curr_prog::shm_data::init{name}).first;
        }
