...
class scope_guard {
  ...
  public:
    template <typename D>
    scope_guard(D &&destructor) : destructor_(std::forward<D>(destructor)), dismiss_(false)
    {
    }

    scope_guard(scope_guard &&rhs) : destructor_(std::move(rhs.destructor_)), dismiss_(true) /* dismiss rhs */
    {
...
{
    return scope_guard<std::decay_t<D>>{std::forward<D>(destructor)};
}
