 ...
 * @param fake_tso Omit generation of explicit barrier code to avoid unnecesary instructions in tso scenario (e.g.
 * simple microcontrollers/single core) @param cacheline_size Size of the cache line, to insert appropriate padding in
 * between indexes and buffer @param index_t Type of array indexing type. Serves also as placeholder for future
 * implementations.
 */
...
size_t RingBufferSPSC<T, buffer_size, fake_tso, cacheline_size, index_t>::writeBuff(const T *buff, size_t count)
{
    index_t available = 0;
    index_t tmp_head = head.load(std::memory_order_relaxed);
                                                                                    ...
                                                                                    void (*execute_data_callback)())
{
    ...
    while (written < count) {
        ...
        while (to_write--) data_buff[tmp_head++ & buffer_mask] = buff[written++];

        std::atomic_signal_fence(std::memory_order_release);
        head.store(tmp_head, index_release_barrier);

        if (execute_data_callback != nullptr) execute_data_callback();
...
template <typename T, size_t buffer_size, bool fake_tso, size_t cacheline_size, typename index_t>
size_t RingBufferSPSC<T, buffer_size, fake_tso, cacheline_size, index_t>::readBuff(T *buff, size_t count)
{
    index_t available = 0;
    index_t tmp_tail = tail.load(std::memory_order_relaxed);
                                                                                   ...
                                                                                   void (*execute_data_callback)())
{
    ...
    while (read < count) {
        ...
        while (to_read--) buff[read++] = data_buff[tmp_tail++ & buffer_mask];
...
class LockFreeRingBufferTrivialMovable {
  ...
  public:
    LockFreeRingBufferTrivialMovable(std::size_t size)
        : pack{{size ? (2ul << lzcnt64(size)) - 1 : 0ul}, {size ? Alloc::Allocate(2ull << lzcnt64(size)) : nullptr}},
          reserver{0},
          last{0},
          first{0}
    {
    ...
    bool enqueue(const Ty &value) noexcept
    {
        ...
        do {
            ...
            if (last.load() == incremented) return false;
        } while (!reserver.compare_exchange_weak(candidate, incremented));

        auto reserved = candidate;
        pack.data[reserved] = value;

        for (const auto saved_reserved = reserved; !first.compare_exchange_weak(reserved, incremented);
             reserved = saved_reserved) {
        }
    ...
    bool dequeue(Ty &value) noexcept
    {
        ...
        do {
            if (first.load() == currentLast) return false;
...
template <class Ty, typename Alloc = std::allocator<Ty>>
class LockFreeRingBufferNonTrivialMovable : public LockFreeRingBufferTrivialMovable<Ty> {
  private:
  ...
  protected:
    using MyBase = LockFreeRingBufferTrivialMovable<Ty>;

  public:
    LockFreeRingBufferNonTrivialMovable(std::size_t size) LockFreeRingBufferNonTrivialMovable(std::size_t size)
        : MyBase(size), lastReserver{0}
    {
    }
    ...
    bool dequeue(Ty &value) noexcept(std::is_nothrow_move_assignable<Ty>::value)
    {
        ...
        do {
            if (MyBase::first.load() == candidate) return false;
            incremented = (candidate + 1) & mask;
        ...
        for (const auto saved_reserved = reserved; !MyBase::last.compare_exchange_weak(reserved, incremented);
             reserved = saved_reserved) {
        }
...
using LockFreeRingBuffer =
    typename std::conditional<std::is_trivially_move_assignable<Ty>::value, LockFreeRingBufferTrivialMovable<Ty, Alloc>,
                              LockFreeRingBufferNonTrivialMovable<Ty, Alloc>>::type;

static_assert((sizeof(LockFreeRingBuffer<void *>) & (cacheline_size - 1)) == 0,
              "The LockFreeRingBuffer has unexpected size!");
} // namespace caches
