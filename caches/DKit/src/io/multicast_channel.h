...
struct multicast_channel {
  ...
  public:
     ...
     */
    multicast_channel() : endpoint(), id('\0'), primary(false) {}
     ...
     */
    multicast_channel(const std::string &aURL) : endpoint(), id('\0'), primary(false)
    {
    ...
    multicast_channel(const std::string &aURL, char anID, bool aPrimary = true)
        : endpoint(), id(anID), primary(aPrimary)
    {
        // set the endpoint from the given URL
        setEndpoint(aURL);
    }
     ...
     * around.
     */
    multicast_channel(const multicast_channel &anOther) : endpoint(), id('\0'), primary(false)
    {
    ...
    std::string getURL() const
    {
        ...
        bzero(url, 80);
        if (!empty()) {
            snprintf(url, 79, "udp://%s:%d", endpoint.address().to_string().c_str(), endpoint.port());
        }
        return url;
    }
