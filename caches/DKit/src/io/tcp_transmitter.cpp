/**
 * tcp_transmitter.cpp - this file implements a simple TCP transmitter
 *                       that is configured with a channel and takes
 *                       TCP datagrams that it receives from sources and sends
 ...
 * what's needed.
 */
thread_map tcp_transmitter::_threads;
// this is a simple sinlock that makes sure we're thread-safe
spinlock tcp_transmitter::_threads_mutex;
/**
 ...
 */
pool<datagram *, 16, dkit::sp_mc> tcp_transmitter::_pool;
...
tcp_transmitter::tcp_transmitter()
    : detail::basic_tcp_transmitter<datagram *>(),
 ...
 */
tcp_transmitter::tcp_transmitter(const channel &aChannel, uint32_t aXmitBuffSize)
    : detail::basic_tcp_transmitter<datagram *>(),
 ...
 */
tcp_transmitter::tcp_transmitter(const tcp_transmitter &anOther, const channel &aChannel)
    : detail::basic_tcp_transmitter<datagram *>(),
      _xmit_buff_size(DEFAULT_XMIT_BUFFER_SIZE),
      _channel(aChannel),
 ...
 * around.
 */
tcp_transmitter::tcp_transmitter(const tcp_transmitter &anOther)
    : detail::basic_tcp_transmitter<datagram *>(),
      _xmit_buff_size(DEFAULT_XMIT_BUFFER_SIZE),
      _channel(),
      _service(),
 ...
 */
tcp_transmitter &tcp_transmitter::operator=(const tcp_transmitter &anOther)
{
    /**
    ...
    if (this != &anOther) {
        ...
        detail::basic_tcp_transmitter<datagram *>::operator=(anOther);
 ...
 */
const channel &tcp_transmitter::getChannel() const
{
    return _channel;
}
 ...
 */
std::string tcp_transmitter::getURL() const
{
 ...
 */
void tcp_transmitter::shareService(const tcp_transmitter &anOther)
{
 ...
 */
void tcp_transmitter::shareService(const io_svc_ptr &aService)
{
 ...
 */
bool tcp_transmitter::init(const channel &aChannel, uint32_t aXmitBuffSize)
{
 ...
 * TCP channel.
 */
bool tcp_transmitter::onMessage(const datagram *aDatagram)
{
 ...
 * this guy.
 */
std::string tcp_transmitter::toString() const
{
    std::ostringstream msg;
    msg << "[tcp_transmitter on:" << _channel << ((_socket != NULL) ? " open" : "") << "]";
    return msg.str();
}
 ...
 * well.
 */
size_t tcp_transmitter::hash() const
{
    // get the hash code of the super class first...
    size_t ans = detail::basic_tcp_transmitter<datagram *>::hash();
 ...
 */
bool tcp_transmitter::operator==(const tcp_transmitter &anOther) const
{
    bool equals = false;
    if ((this == &anOther)
        || (detail::basic_tcp_transmitter<datagram *>::operator==(anOther)
 ...
 * sending process.
 */
bool tcp_transmitter::init_nl(const channel &aChannel, uint32_t aXmitBuffSize)
{
    bool error = false;
    bool allDone = false;
 ...
 * defined, but not initialized and not sending.
 */
void tcp_transmitter::shutdown_nl()
{
 ...
 */
bool tcp_transmitter::asyncSend(const datagram *aDatagram)
{
 ...
 * keep the datagram around through the duration of the async send.
 */
bool tcp_transmitter::asyncSend_nl(const datagram *aDatagram)
{
    bool error = false;
    ...
    // now we need to send out this datagram to the TCP channel
    if (!error) {
        _socket->async_write_some(buffer(dg->what, dg->size),
                                  boost::bind(&tcp_transmitter::asyncSendComplete, this, dg, placeholders::error,
 ...
 */
void tcp_transmitter::asyncSendComplete(datagram *aPayload, const boost::system::error_code &anError,
 ...
 */
bool tcp_transmitter::verifyProcessing(const io_svc_ptr &aService)
{
    bool error = false;
    bool allDone = false;
    ...
    if (!error && !allDone) {
        ...
        _threads[aService].thread = boost::thread(&tcp_transmitter::process, aService);
    }
 ...
 */
void tcp_transmitter::process(io_svc_ptr &aService)
{
    /**
 ...
 */
bool tcp_transmitter::isRunning(const io_svc_ptr &aService)
{
    bool running = false;
 ...
 */
void tcp_transmitter::terminate(const io_svc_ptr &aService)
{
    // lock up the map for this check
    spinlock::scoped_lock lock(_threads_mutex);
    thread_map::iterator it = _threads.find(aService);
 ...
 */
bool tcp_transmitter::incr_use_count(const io_svc_ptr &aService)
{
 ...
 * be done.
 */
bool tcp_transmitter::decr_use_count(const io_svc_ptr &aService)
{
    // lock up the map for this operation
    spinlock::scoped_lock lock(_threads_mutex);
 ...
 */
uint32_t tcp_transmitter::use_count(const io_svc_ptr &aService)
{
    // lock up the map for this operation
    spinlock::scoped_lock lock(_threads_mutex);
