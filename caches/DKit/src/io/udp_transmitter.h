/**
 * udp_transmitter.h - this file defines a simple UDP multicast transmitter
 ...
 *                     UDP receivers.
 */
#ifndef __DKIT_IO_UDP_TRANSMITTER_H
#define __DKIT_IO_UDP_TRANSMITTER_H
 ...
 * UDP transmitters, and it's all done with boost shared_ptr. We will
 * make one - if we don't have one to use, but if we are given another
 * UDP transmitter, we'll copy it's smart pointer for the io_service and
...
{
/**
 * In order to make the UDP transmitter a simple object to use, we need to
 ...
 *
 * This class will then be subclassed by the "real" UDP transmitter class
...
{
template <class T>
class basic_udp_transmitter : public sink<T> {
  public:
 ...
 */
class udp_transmitter : public detail::basic_udp_transmitter<datagram *> {
  public:
    /*******************************************************************
     *
     ...
     */
    udp_transmitter(const multicast_channel &aChannel, uint32_t aXmitBuffSize = DEFAULT_XMIT_BUFFER_SIZE);
    /**
     * This version of the constructor takes a single multicast channel
     ...
     */
    udp_transmitter(const udp_transmitter &anOther, const multicast_channel &aChannel);
    /**
     * This is the standard copy constructor and needs to be in every
     * class to make sure that we don't have too many things running
     * around.
     */
    udp_transmitter(const udp_transmitter &anOther);
     ...
     */
    udp_transmitter &operator=(const udp_transmitter &anOther);
     ...
     */
    void shareService(const udp_transmitter &anOther);
    /**
     ...
     */
    bool init(const multicast_channel &aChannel, uint32_t aXmitBuffSize = DEFAULT_XMIT_BUFFER_SIZE);
    /**
     ...
     */
    bool operator==(const udp_transmitter &anOther) const;
    /**
  ...
  protected:
     ...
     * sending process.
     */
    bool init_nl(const multicast_channel &aChannel, uint32_t aXmitBuffSize = DEFAULT_XMIT_BUFFER_SIZE);
    /**
     ...
     */
    void asyncSendComplete(datagram *aPayload, const boost::system::error_code &anError, size_t aBytesReceived);
  ...
  private:
    ...
    /**
     * This is the multicast channel that we will be sending on. It's
     ...
     * existing UDP transmitter, then it will grab the shared_ptr, and
...
#endif // __DKIT_IO_UDP_TRANSMITTER_H
