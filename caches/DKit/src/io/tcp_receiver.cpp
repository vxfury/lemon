/**
 * tcp_receiver.cpp - this file implements a simpleP TCP receiver that
 *                    is configured with a channel and sends
 *                    downstream tcp datagrams that it receives from the
 ...
 * what's needed.
 */
thread_map tcp_receiver::_threads;
// this is a simple sinlock that makes sure we're thread-safe
spinlock tcp_receiver::_threads_mutex;
/**
 ...
 * in case.
 */
pool<datagram *, 16, dkit::sp_sc> tcp_receiver::_pool;
...
tcp_receiver::tcp_receiver()
    : source<datagram *>(), _rcv_buff_size(DEFAULT_RCV_BUFFER_SIZE), _channel(), _service(), _socket(NULL), _mutex()
{
}
 ...
 */
tcp_receiver::tcp_receiver(const channel &aChannel, uint32_t aRcvBuffSize)
    : source<datagram *>(), _rcv_buff_size(aRcvBuffSize), _channel(aChannel), _service(), _socket(NULL), _mutex()
{
}
 ...
 * the datagram.
 */
tcp_receiver::tcp_receiver(const tcp_receiver &anOther, const channel &aChannel)
    : source<datagram *>(),
 ...
 * around.
 */
tcp_receiver::tcp_receiver(const tcp_receiver &anOther)
    : source<datagram *>(), _rcv_buff_size(DEFAULT_RCV_BUFFER_SIZE), _channel(), _service(), _socket(NULL), _mutex()
{
 ...
 */
tcp_receiver &tcp_receiver::operator=(const tcp_receiver &anOther)
{
    /**
 ...
 * process.
 */
const channel &tcp_receiver::getChannel() const
{
    return _channel;
}
 ...
 */
std::string tcp_receiver::getURL() const
{
 ...
 */
void tcp_receiver::shareService(const tcp_receiver &anOther)
{
 ...
 */
void tcp_receiver::shareService(const io_svc_ptr &aService)
{
 ...
 */
bool tcp_receiver::listen(const channel &aChannel, uint32_t aRcvBuffSize)
{
 ...
 */
bool tcp_receiver::isListening() const
{
    bool listening = true;
 ...
 * this guy.
 */
std::string tcp_receiver::toString() const
{
    std::ostringstream msg;
    msg << "[tcp_receiver on:" << _channel << ((_socket != NULL) ? " open" : "") << (isListening() ? " listening" : "")
        << "]";
    return msg.str();
}
 ...
 * well.
 */
size_t tcp_receiver::hash() const
{
 ...
 */
bool tcp_receiver::operator==(const tcp_receiver &anOther) const
{
    bool equals = false;
    if ((this == &anOther)
        || (source<datagram *>::operator==(anOther) && (_rcv_buff_size == anOther._rcv_buff_size)
            && (_channel == anOther._channel) && (_service == anOther._service)
 ...
 */
bool tcp_receiver::listen_nl(const channel &aChannel, uint32_t aRcvBuffSize)
{
    bool error = false;
    ...
    // if we're OK, then fire up the TCP listening socket listening
 ...
 * TCP multicast channel, and the thread is running listening to
 * datagrams arriving at the socket and shipping them downstream.
 */
bool tcp_receiver::isListening_nl()
{
    bool listening = true;
    // first, see if we have a socket, and if it's open
    if (listening) {
 ...
 */
bool tcp_receiver::startAsyncRead()
{
 ...
 * where we can start the process of a read.
 */
bool tcp_receiver::startAsyncRead_nl()
{
    bool error = false;
    ...
    if (!error) {
        _socket->async_receive(buffer(dg->what, dg->capacity),
                               boost::bind(&tcp_receiver::handleAsyncRead, this, dg, placeholders::error,
 ...
 */
void tcp_receiver::handleAsyncRead(datagram *aPayload, const boost::system::error_code &anError, size_t aBytesReceived)
{
    bool error = false;
    bool bail = false;
 ...
 */
bool tcp_receiver::verifyProcessing(const io_svc_ptr &aService, bool addWork)
{
    bool error = false;
    bool allDone = false;
    ...
    if (!error && !allDone) {
        ...
        _threads[aService].thread = boost::thread(&tcp_receiver::process, aService, addWork);
    }
 ...
 */
void tcp_receiver::process(io_svc_ptr &aService, bool addWork)
{
    /**
 ...
 */
bool tcp_receiver::isRunning(const io_svc_ptr &aService)
{
    bool running = false;
 ...
 */
void tcp_receiver::terminate(const io_svc_ptr &aService)
{
    // lock up the map for this check
    spinlock::scoped_lock lock(_threads_mutex);
    thread_map::iterator it = _threads.find(aService);
 ...
 */
bool tcp_receiver::incr_use_count(const io_svc_ptr &aService)
{
 ...
 * be done.
 */
bool tcp_receiver::decr_use_count(const io_svc_ptr &aService)
{
    // lock up the map for this operation
    spinlock::scoped_lock lock(_threads_mutex);
 ...
 */
uint32_t tcp_receiver::use_count(const io_svc_ptr &aService)
{
    // lock up the map for this operation
    spinlock::scoped_lock lock(_threads_mutex);
