 ...
 */
    ...
    static inline uint64_t usecStamp()
    {
        ...
        timespec ts;
        clock_gettime(CLOCK_REALTIME, &ts);
        now = ((uint64_t)ts.tv_sec * 1000000LL + (uint64_t)ts.tv_nsec / 1000LL);
#endif
        return now;
    }
    ...
    static inline std::string formatTimestamp(uint64_t aTimestamp, bool inclUSec = false)
    {
        ...
        char buff[128];
        if (inclUSec) {
            snprintf(buff, 127, "%4d-%02d-%02d %02d:%02d:%02d.%06d", (when.tm_year + 1900), (when.tm_mon + 1),
                     when.tm_mday, when.tm_hour, when.tm_min, when.tm_sec, usec);
        } else {
            snprintf(buff, 127, "%4d-%02d-%02d %02d:%02d:%02d", (when.tm_year + 1900), (when.tm_mon + 1), when.tm_mday,
    ...
    static inline std::string formatDate(uint64_t aTimestamp)
    {
        ...
        // now build up the string based on all this data
        char buff[80];
        snprintf(buff, 79, "%4d-%02d-%02d", (when.tm_year + 1900), (when.tm_mon + 1), when.tm_mday);
        // return what we have created.
        return buff;
    }
    ...
    static inline std::string formatTime(uint64_t aTimestamp, bool inclUSec = false)
    {
        ...
        char buff[80];
        if (inclUSec) {
            snprintf(buff, 79, "%02d:%02d:%02d.%06d", when.tm_hour, when.tm_min, when.tm_sec, usec);
        } else {
            snprintf(buff, 79, "%02d:%02d:%02d", when.tm_hour, when.tm_min, when.tm_sec);
        }
        // return what we have created.
        return buff;
    }
