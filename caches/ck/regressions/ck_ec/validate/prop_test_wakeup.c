...
static void wait32(const struct ck_ec_wait_state *state, const uint32_t *addr, uint32_t expected,
...
static void wait64(const struct ck_ec_wait_state *state, const uint64_t *addr, uint64_t expected,
                   const struct timespec *deadline)
{
    (void)addr;
    (void)expected;
