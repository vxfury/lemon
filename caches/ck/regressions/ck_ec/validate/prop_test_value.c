...
static const struct ck_ec_mode modes[] = {
    {.single_producer = true, .ops = &test_ops},
    {.single_producer = false, .ops = &test_ops},
};
...
static void wait32(const struct ck_ec_wait_state *wait_state, const uint32_t *addr, uint32_t expected,
...
static void wait64(const struct ck_ec_wait_state *wait_state, const uint64_t *addr, uint64_t expected,
                   const struct timespec *deadline)
{
    (void)addr;
    (void)expected;
...
static inline int test_value(const struct example *example)
{
    for (size_t i = 0; i < 2; i++) {
        ...
        assert(ck_ec32_value(&ec) == ((value0 + value1) & INT32_MAX));


        ck_ec32_inc(&ec, mode);
        assert(ck_ec32_value(&ec) == ((value0 + value1 + 1) & INT32_MAX));
    }
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        assert(ck_ec64_value(&ec) == ((value0 + value1) & INT64_MAX));

        ck_ec64_inc(&ec, mode);
        assert(ck_ec64_value(&ec) == ((value0 + value1 + 1) & INT64_MAX));
    }
#endif /* CK_F_EC64 */
