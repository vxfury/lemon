...
static void wait32(const struct ck_ec_wait_state *, const uint32_t *, uint32_t, const struct timespec *);
static void wake64(const struct ck_ec_ops *, const uint64_t *);
static void wait64(const struct ck_ec_wait_state *, const uint64_t *, uint64_t, const struct timespec *);

static const struct ck_ec_ops test_ops = {
    .gettime = gettime, .wait32 = wait32, .wait64 = wait64, .wake32 = wake32, .wake64 = wake64};
...
static void wait32(const struct ck_ec_wait_state *state, const uint32_t *address, uint32_t expected,
                   const struct timespec *deadline)
{
    assert(state->ops == &test_ops);
    syscall(SYS_futex, address, FUTEX_WAIT_BITSET, expected, deadline, NULL, FUTEX_BITSET_MATCH_ANY, 0);
    return;
}

static void wait64(const struct ck_ec_wait_state *state, const uint64_t *address, uint64_t expected,
                   const struct timespec *deadline)
{
    const void *low_half;
...
#error "__BYTE_ORDER__ must be defined."
    ...
    syscall(SYS_futex, low_half, FUTEX_WAIT_BITSET, (uint32_t)expected, deadline, NULL, FUTEX_BITSET_MATCH_ANY, 0);
    return;
}
...
{
    assert(ops == &test_ops);
    syscall(SYS_futex, address, FUTEX_WAKE, INT_MAX,
...
#error "__BYTE_ORDER__ must be defined."
    ...
    syscall(SYS_futex, low_half, FUTEX_WAKE, INT_MAX,
            /* ignored arguments */ NULL, NULL, 0);
    return;
}
#endif /* __linux__ */

static const struct ck_ec_mode sp = {.ops = &test_ops, .single_producer = true};

static const struct ck_ec_mode mp = {.ops = &test_ops, .single_producer = false};
...
static void test_deadline(void)
{
    ...
    {
        const struct timespec timeout = {.tv_sec = 1, .tv_nsec = 1000};
        const struct timespec no_timeout = {.tv_sec = 0};
        struct timespec now;
    ...
    {
        const struct timespec timeout = {.tv_sec = TIME_MAX - 1, .tv_nsec = 1000};
...
static int pred(const struct ck_ec_wait_state *state, struct timespec *deadline)
{
    double initial_ts = state->start.tv_sec + 1e-9 * state->start.tv_nsec;
...
#ifdef CK_F_EC64
static int pred2(const struct ck_ec_wait_state *state, struct timespec *deadline)
{
    double initial_ts = state->start.tv_sec + 1e-9 * state->start.tv_nsec;
    int *count = state->data;
...
int main(int argc, char **argv)
{
    ...
    if (test_ops.gettime == NULL || test_ops.wake32 == NULL || test_ops.wait32 == NULL) {
