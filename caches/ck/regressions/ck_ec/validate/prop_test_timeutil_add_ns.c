...
static const struct example examples[] = {
    {{42, 100}, 1}, {{42, 100}, 2 * NSEC_MAX}, {{42, NSEC_MAX}, NSEC_MAX}, {{TIME_MAX - 1, 1000}, 2 * NSEC_MAX}};

static inline int test_timespec_add_ns(const struct example *example)
{
    struct timespec ts = {.tv_sec = example->ts.tv_sec, .tv_nsec = example->ts.tv_nsec};
    ...
    dword_t nanos = (dword_t)ts.tv_sec * (NSEC_MAX + 1) + ts.tv_nsec;
