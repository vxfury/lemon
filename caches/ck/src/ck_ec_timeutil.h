 ...
 * overflow.
 */
CK_CC_UNUSED static uint32_t wait_time_scale(uint32_t nsec, uint32_t multiplier, unsigned int shift)
{
 ...
 *
 */
CK_CC_UNUSED static struct timespec timespec_add_ns(const struct timespec ts, uint32_t ns)
{
    struct timespec ret = {.tv_sec = TIME_MAX, .tv_nsec = NSEC_MAX};
    time_t sec;
    uint32_t sum_ns;
 ...
 */
CK_CC_UNUSED static struct timespec timespec_add(const struct timespec ts, const struct timespec inc)
{
...
CK_CC_UNUSED static int timespec_cmp(const struct timespec x, const struct timespec y)
{
 ...
 */
CK_CC_UNUSED static bool check_deadline(struct timespec *now, const struct ck_ec_ops *ops,
