...
static const struct timespec final_wait_time = {.tv_sec = 1};
...
{
    return ck_ec32_wait_pred_slow(ec, ops, old_value, NULL, NULL, deadline);
}
...
int ck_ec64_wait_slow(struct ck_ec64 *ec, const struct ck_ec_ops *ops, uint64_t old_value,
                      const struct timespec *deadline)
{
    return ck_ec64_wait_pred_slow(ec, ops, old_value, NULL, NULL, deadline);
}
#endif
 ...
 * values.
 */
static int exponential_backoff(struct ck_ec_wait_state *wait_state,
                               bool (*sleep)(const void *sleep_state, const struct ck_ec_wait_state *wait_state,
                               ...
                               const struct timespec *deadline)
{
    ...
    const uint32_t scale_factor = (ops->wait_scale_factor != 0) ? ops->wait_scale_factor : DEFAULT_WAIT_SCALE_FACTOR;
    const uint32_t shift_count = (ops->wait_shift_count != 0) ? ops->wait_shift_count : DEFAULT_WAIT_SHIFT_COUNT;
    uint32_t wait_ns = (ops->initial_wait_ns != 0) ? ops->initial_wait_ns : DEFAULT_INITIAL_WAIT_NS;
    bool first = true;
    ...
    for (;;) {
        ...
        if (timespec_cmp(now, stop_backoff) >= 0) {
            ...
            do {
                partial_deadline = timespec_add_ns(begin, wait_ns);
                wait_ns = wait_time_scale(wait_ns, scale_factor, shift_count);
        ...
        if (sleep(sleep_state, wait_state, ((partial_deadline.tv_sec == TIME_MAX) ? NULL : &partial_deadline))
            == true) {
            return 0;
        }
    }
...
#define DEF_WAIT_EASY(W)                                                                                            \
    static uint##W##_t ck_ec##W##_wait_easy(struct ck_ec##W *ec, const struct ck_ec_ops *ops, uint##W##_t expected) \
    {                                                                                                               \
        uint##W##_t current = ck_pr_load_##W(&ec->counter);                                                         \
        size_t n = (ops->busy_loop_iter != 0) ? ops->busy_loop_iter : DEFAULT_BUSY_LOOP_ITER;                       \
                                                                                                                    \
        for (size_t i = 0; i < n && current == expected; i++) {                                                     \
            ck_pr_stall();                                                                                          \
...
#define DEF_UPGRADE(W)                                                                                               \
    static bool ck_ec##W##_upgrade(struct ck_ec##W *ec, uint##W##_t current, uint##W##_t unflagged,                  \
                                   uint##W##_t flagged)                                                              \
    {                                                                                                                \
        uint##W##_t old_word;                                                                                        \
                                                                                                                     \
         ...
         */                                                                                                          \
        return (ck_pr_cas_##W##_value(&ec->counter, unflagged, flagged, &old_word) == false && old_word != flagged); \
    }
 ...
 * forever.
 */
static bool ck_ec32_wait_slow_once(const void *vstate, const struct ck_ec_wait_state *wait_state,
                                   const struct timespec *partial_deadline)
{
    ...
    wait_state->ops->wait32(wait_state, &ec->counter, flagged_word, partial_deadline);
    return ck_pr_load_32(&ec->counter) != flagged_word;
}

#ifdef CK_F_EC64
static bool ck_ec64_wait_slow_once(const void *vstate, const struct ck_ec_wait_state *wait_state,
                                   const struct timespec *partial_deadline)
{
    ...
    wait_state->ops->wait64(wait_state, &ec->counter, flagged_word, partial_deadline);
 ...
 * every early exit.
 */
#define WAIT_SLOW_BODY(W, ec, ops, pred, data, deadline_ptr, old_value, unflagged, flagged)           \
    do {                                                                                              \
        struct ck_ec_wait_state wait_state = {.ops = ops, .data = data};                              \
        const struct ck_ec##W##_slow_path_state state = {.ec = ec, .flagged_word = flagged};          \
        const struct timespec deadline = canonical_deadline(deadline_ptr);                            \
                                                                                                      \
        ...
        for (;;) {                                                                                    \
             ...
             */                                                                                       \
            if (CK_CC_LIKELY(ck_ec##W##_upgrade(ec, current, unflagged, flagged) == true)) {          \
                ck_pr_fence_acquire();                                                                \
                return 0;                                                                             \
            }                                                                                         \
                                                                                                      \
            ...
            if (CK_CC_LIKELY(current != flagged_word)) {                                              \
                ...
                return 0;                                                                             \
                                                                                                      ...
                                                                                                      \
            r = exponential_backoff(&wait_state, ck_ec##W##_wait_slow_once, &state, pred, &deadline); \
            if (r != 0) {                                                                             \
                return r;                                                                             \
            }                                                                                         \
                                                                                                      \
                           ...
                           const struct timespec *deadline_ptr)
{
    ...
    WAIT_SLOW_BODY(32, ec, ops, pred, data, deadline_ptr, old_value, unflagged_word, flagged_word);
}
                           ...
                           const struct timespec *deadline_ptr)
{
    ...
    WAIT_SLOW_BODY(64, ec, ops, pred, data, deadline_ptr, old_value, unflagged_word, flagged_word);
}
#endif
