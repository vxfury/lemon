...
static inline struct cds_lfq_node_rcu *make_dummy(struct cds_lfq_queue_rcu *q, struct cds_lfq_node_rcu *next)
{
    ...
    dummy = (struct cds_lfq_node_rcu_dummy *)malloc(sizeof(struct cds_lfq_node_rcu_dummy));
...
{
    struct cds_lfq_node_rcu_dummy *dummy = caa_container_of(head, struct cds_lfq_node_rcu_dummy, head);
    free(dummy);
}
...
static inline void _cds_lfq_init_rcu(struct cds_lfq_queue_rcu *q,
 ...
 */
static inline int _cds_lfq_destroy_rcu(struct cds_lfq_queue_rcu *q)
{
    struct cds_lfq_node_rcu *head;

    head = rcu_dereference(q->head);
    if (!(head->dummy && head->next == NULL)) return -EPERM; /* not empty */
    free_dummy(head);
    return 0;
}
...
static inline struct cds_lfq_node_rcu *_cds_lfq_dequeue_rcu(struct cds_lfq_queue_rcu *q)
{
    for (;;) {
        ...
        if (head->dummy && next == NULL) return NULL; /* empty */
        /*
        ...
        if (uatomic_cmpxchg(&q->head, head, next) != head) continue; /* Concurrently pushed. */
        if (head->dummy) {
