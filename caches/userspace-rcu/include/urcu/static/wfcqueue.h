 ...
 * cds_wfcq_init().
 */
static inline void _cds_wfcq_destroy(struct cds_wfcq_head *head, struct cds_wfcq_tail *tail __attribute__((unused)))
{
 ...
 */
static inline bool _cds_wfcq_empty(cds_wfcq_head_ptr_t u_head, struct cds_wfcq_tail *tail)
{
     ...
     */
    return CMM_LOAD_SHARED(head->node.next) == NULL && CMM_LOAD_SHARED(tail->p) == &head->node;
}
...
static inline bool ___cds_wfcq_append(cds_wfcq_head_ptr_t u_head, struct cds_wfcq_tail *tail,
                                      struct cds_wfcq_node *new_head, struct cds_wfcq_node *new_tail)
{
    struct __cds_wfcq_head *head = u_head._h;
 ...
 */
static inline bool _cds_wfcq_enqueue(cds_wfcq_head_ptr_t head, struct cds_wfcq_tail *tail,
 ...
 */
static inline bool ___cds_wfcq_busy_wait(int *attempt, int blocking)
{
 ...
 */
static inline struct cds_wfcq_node *___cds_wfcq_node_sync_next(struct cds_wfcq_node *node, int blocking)
{
    ...
    while ((next = CMM_LOAD_SHARED(node->next)) == NULL) {
        if (___cds_wfcq_busy_wait(&attempt, blocking)) return CDS_WFCQ_WOULDBLOCK;
    }

    return next;
}

static inline struct cds_wfcq_node *___cds_wfcq_first(cds_wfcq_head_ptr_t u_head, struct cds_wfcq_tail *tail,
                                                      int blocking)
{
    struct __cds_wfcq_head *head = u_head._h;
    struct cds_wfcq_node *node;

    if (_cds_wfcq_empty(__cds_wfcq_head_cast(head), tail)) return NULL;
 ...
 */
static inline struct cds_wfcq_node *___cds_wfcq_first_blocking(cds_wfcq_head_ptr_t head, struct cds_wfcq_tail *tail)
{
 ...
 * it needs to block.
 */
static inline struct cds_wfcq_node *___cds_wfcq_first_nonblocking(cds_wfcq_head_ptr_t head, struct cds_wfcq_tail *tail)
{
    return ___cds_wfcq_first(head, tail, 0);
}

static inline struct cds_wfcq_node *___cds_wfcq_next(cds_wfcq_head_ptr_t head __attribute__((unused)),
                                                     struct cds_wfcq_tail *tail, struct cds_wfcq_node *node,
                                                     int blocking)
{
    struct cds_wfcq_node *next;
    ...
    if ((next = CMM_LOAD_SHARED(node->next)) == NULL) {
        ...
        cmm_smp_rmb();
        if (CMM_LOAD_SHARED(tail->p) == node) return NULL;
 ...
 * otherwise.
 */
static inline struct cds_wfcq_node *___cds_wfcq_next_blocking(cds_wfcq_head_ptr_t head, struct cds_wfcq_tail *tail,
                                                              struct cds_wfcq_node *node)
{
 ...
 * it needs to block.
 */
static inline struct cds_wfcq_node *___cds_wfcq_next_nonblocking(cds_wfcq_head_ptr_t head, struct cds_wfcq_tail *tail,
                                                                 struct cds_wfcq_node *node)
{
    return ___cds_wfcq_next(head, tail, node, 0);
}

static inline struct cds_wfcq_node *___cds_wfcq_dequeue_with_state(cds_wfcq_head_ptr_t u_head,
                                                                   struct cds_wfcq_tail *tail, int *state, int blocking)
{
    struct __cds_wfcq_head *head = u_head._h;
    ...
    if ((next = CMM_LOAD_SHARED(node->next)) == NULL) {
        ...
        if (uatomic_cmpxchg(&tail->p, node, &head->node) == node) {
            if (state) *state |= CDS_WFCQ_STATE_LAST;
            return node;
        }
        next = ___cds_wfcq_node_sync_next(node, blocking);
        /*
 ...
 * caller.
 */
static inline struct cds_wfcq_node *___cds_wfcq_dequeue_with_state_blocking(cds_wfcq_head_ptr_t head,
 ...
 * state.
 */
static inline struct cds_wfcq_node *___cds_wfcq_dequeue_blocking(cds_wfcq_head_ptr_t head, struct cds_wfcq_tail *tail)
{
 ...
 */
static inline struct cds_wfcq_node *___cds_wfcq_dequeue_with_state_nonblocking(cds_wfcq_head_ptr_t head,
                                                                               struct cds_wfcq_tail *tail, int *state)
{
 ...
 * state.
 */
static inline struct cds_wfcq_node *___cds_wfcq_dequeue_nonblocking(cds_wfcq_head_ptr_t head,
 ...
 * dest queue.
 */
static inline enum cds_wfcq_ret ___cds_wfcq_splice(cds_wfcq_head_ptr_t u_dest_q_head, struct cds_wfcq_tail *dest_q_tail,
                                                   cds_wfcq_head_ptr_t u_src_q_head, struct cds_wfcq_tail *src_q_tail,
                                                   int blocking)
{
     ...
     */
    if (_cds_wfcq_empty(__cds_wfcq_head_cast(src_q_head), src_q_tail)) return CDS_WFCQ_RET_SRC_EMPTY;
    ...
    for (;;) {
        ...
        if (head) break; /* non-empty */
        if (CMM_LOAD_SHARED(src_q_tail->p) == &src_q_head->node) return CDS_WFCQ_RET_SRC_EMPTY;
        if (___cds_wfcq_busy_wait(&attempt, blocking)) return CDS_WFCQ_RET_WOULDBLOCK;
    }
     ...
     */
    if (___cds_wfcq_append(__cds_wfcq_head_cast(dest_q_head), dest_q_tail, head, tail))
 ...
 */
static inline enum cds_wfcq_ret ___cds_wfcq_splice_blocking(cds_wfcq_head_ptr_t dest_q_head,
                                                            struct cds_wfcq_tail *dest_q_tail,
                                                            cds_wfcq_head_ptr_t src_q_head,
                                                            struct cds_wfcq_tail *src_q_tail)
{
    return ___cds_wfcq_splice(dest_q_head, dest_q_tail, src_q_head, src_q_tail, 1);
}
 ...
 */
static inline enum cds_wfcq_ret ___cds_wfcq_splice_nonblocking(cds_wfcq_head_ptr_t dest_q_head,
                                                               struct cds_wfcq_tail *dest_q_tail,
                                                               cds_wfcq_head_ptr_t src_q_head,
                                                               struct cds_wfcq_tail *src_q_tail)
{
    return ___cds_wfcq_splice(dest_q_head, dest_q_tail, src_q_head, src_q_tail, 0);
}
 ...
 * It is valid to reuse and free a dequeued node immediately.
 */
static inline struct cds_wfcq_node *_cds_wfcq_dequeue_with_state_blocking(struct cds_wfcq_head *head,
                                                                          struct cds_wfcq_tail *tail, int *state)
{
    struct cds_wfcq_node *retval;

    _cds_wfcq_dequeue_lock(head, tail);
    retval = ___cds_wfcq_dequeue_with_state_blocking(cds_wfcq_head_cast(head), tail, state);
 ...
 * dest queue. Never returns CDS_WFCQ_RET_WOULDBLOCK.
 */
static inline enum cds_wfcq_ret _cds_wfcq_splice_blocking(struct cds_wfcq_head *dest_q_head,
                                                          struct cds_wfcq_tail *dest_q_tail,
                                                          ...
                                                          struct cds_wfcq_tail *src_q_tail)
{
    ...
    ret = ___cds_wfcq_splice_blocking(cds_wfcq_head_cast(dest_q_head), dest_q_tail, cds_wfcq_head_cast(src_q_head),
                                      src_q_tail);
