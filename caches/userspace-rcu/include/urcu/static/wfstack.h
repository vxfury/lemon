 ...
 * cds_wfs_init().
 */
static inline void _cds_wfs_destroy(struct cds_wfs_stack *s)
{
 ...
 */
static inline int _cds_wfs_push(cds_wfs_stack_ptr_t u_stack, struct cds_wfs_node *node)
{
    struct __cds_wfs_stack *s = u_stack._s;
 ...
 */
static inline struct cds_wfs_node *___cds_wfs_node_sync_next(struct cds_wfs_node *node, int blocking)
{
    ...
    while ((next = CMM_LOAD_SHARED(node->next)) == NULL) {
        if (!blocking) return CDS_WFS_WOULDBLOCK;
...
static inline struct cds_wfs_node *___cds_wfs_pop(cds_wfs_stack_ptr_t u_stack, int *state, int blocking)
{
    ...
    for (;;) {
        ...
        if (uatomic_cmpxchg(&s->head, head, new_head) == head) {
            if (state && ___cds_wfs_end(new_head)) *state |= CDS_WFS_STATE_LAST;
            return &head->node;
        }
        if (!blocking) {
            return CDS_WFS_WOULDBLOCK;
 ...
 */
static inline struct cds_wfs_node *___cds_wfs_pop_with_state_blocking(cds_wfs_stack_ptr_t u_stack, int *state)
{
    return ___cds_wfs_pop(u_stack, state, 1);
}

static inline struct cds_wfs_node *___cds_wfs_pop_blocking(cds_wfs_stack_ptr_t u_stack)
{
 ...
 * "state" saves state flags atomically sampled with pop operation.
 */
static inline struct cds_wfs_node *___cds_wfs_pop_with_state_nonblocking(cds_wfs_stack_ptr_t u_stack, int *state)
{
 ...
 * it needs to block.
 */
static inline struct cds_wfs_node *___cds_wfs_pop_nonblocking(cds_wfs_stack_ptr_t u_stack)
{
 ...
 *    and __cds_wfs_pop_all(). (multi-provider/single-consumer scheme).
 */
static inline struct cds_wfs_head *___cds_wfs_pop_all(cds_wfs_stack_ptr_t u_stack)
{
    struct __cds_wfs_stack *s = u_stack._s;
    ...
    if (___cds_wfs_end(head)) return NULL;
    return head;
}
 ...
 */
static inline struct cds_wfs_node *_cds_wfs_pop_with_state_blocking(struct cds_wfs_stack *s, int *state)
{
...
static inline struct cds_wfs_node *___cds_wfs_next(struct cds_wfs_node *node, int blocking)
{
    struct cds_wfs_node *next;
     ...
     */
    if (___cds_wfs_end(next)) return NULL;
    return next;
}
