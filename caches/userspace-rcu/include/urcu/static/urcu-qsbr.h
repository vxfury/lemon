...
static inline void urcu_qsbr_wake_up_gp(void)
{
    if (caa_unlikely(_CMM_LOAD_SHARED(URCU_TLS(urcu_qsbr_reader).waiting))) {
        ...
        cmm_smp_mb();
        if (uatomic_read(&urcu_qsbr_gp.futex) != -1) return;
         ...
         * exposed).
         */
        (void)futex_noasync(&urcu_qsbr_gp.futex, FUTEX_WAKE, 1, NULL, NULL, 0);
    }
}
...
static inline enum urcu_state urcu_qsbr_reader_state(unsigned long *ctr)
{
    ...
    if (!v) return URCU_READER_INACTIVE;
    if (v == urcu_qsbr_gp.ctr) return URCU_READER_ACTIVE_CURRENT;
...
static inline void _urcu_qsbr_quiescent_state(void)
{
    ...
    if ((gp_ctr = CMM_LOAD_SHARED(urcu_qsbr_gp.ctr)) == URCU_TLS(urcu_qsbr_reader).ctr) return;
