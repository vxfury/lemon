...
#define DECLARE_URCU_TLS(type, name) URCU_TLS_STORAGE_CLASS type name

#define DEFINE_URCU_TLS(type, name) URCU_TLS_STORAGE_CLASS type name

#define DEFINE_URCU_TLS_INIT(type, name, init) URCU_TLS_STORAGE_CLASS type name = (init)
...
#define DECLARE_URCU_TLS_1(type, name) type *__tls_access_##name(void)
#define DECLARE_URCU_TLS(type, name)   DECLARE_URCU_TLS_1(type, name)
...
#define DEFINE_URCU_TLS_INIT_1(type, name, do_init)                          \
    ...
    {                                                                        \
        ...
        if (!__tls_##name.init_done) {                                       \
            ...
            if (!__tls_##name.init_done) {                                   \
                (void)pthread_key_create(&__tls_##name.key, free);           \
        ...
        if (caa_unlikely(__tls_p == NULL)) {                                 \
            ...
            do_init(void) pthread_setspecific(__tls_##name.key, __tls_p);    \
        }                                                                    \
        return __tls_p;                                                      \
    }

#define _URCU_TLS_INIT(init) *__tls_p = (init);

#define DEFINE_URCU_TLS_INIT(type, name, init) DEFINE_URCU_TLS_INIT_1(type, name, _URCU_TLS_INIT(init))

#define DEFINE_URCU_TLS(type, name) DEFINE_URCU_TLS_INIT_1(type, name, /* empty */)
