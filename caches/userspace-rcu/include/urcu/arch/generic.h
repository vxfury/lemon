...
static inline caa_cycles_t caa_get_cycles(void)
{
    ...
    if (caa_unlikely(!clock_service)) {
        if (host_get_clock_service(mach_host_self(), SYSTEM_CLOCK, &clock_service)) return -1ULL;
    }
    if (caa_unlikely(clock_get_time(clock_service, &ts))) return -1ULL;
...
static inline caa_cycles_t caa_get_cycles(void)
{
    ...
    if (caa_unlikely(clock_gettime(CLOCK_MONOTONIC, &ts))) return -1ULL;
    return ((uint64_t)ts.tv_sec * 1000000000ULL) + ts.tv_nsec;
}
