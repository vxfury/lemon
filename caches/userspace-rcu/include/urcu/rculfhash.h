 ...
 */
static inline void cds_lfht_node_init(struct cds_lfht_node *node __attribute__((unused))) {}
 ...
 */
extern void cds_lfht_node_init_deleted(struct cds_lfht_node *node);
...
struct cds_lfht_mm_type {
    struct cds_lfht *(*alloc_cds_lfht)(unsigned long min_nr_alloc_buckets, unsigned long max_nr_buckets);
 ...
 */
extern struct cds_lfht *_cds_lfht_new(unsigned long init_size, unsigned long min_nr_alloc_buckets,
 ...
 * initialized)
 */
static inline struct cds_lfht *cds_lfht_new_flavor(unsigned long init_size, unsigned long min_nr_alloc_buckets,
                                                   unsigned long max_nr_buckets, int flags,
                                                   const struct rcu_flavor_struct *flavor, pthread_attr_t *attr)
{
    return _cds_lfht_new(init_size, min_nr_alloc_buckets, max_nr_buckets, flags, NULL, flavor, attr);
}
 ...
 * initialized)
 */
static inline struct cds_lfht *cds_lfht_new(unsigned long init_size, unsigned long min_nr_alloc_buckets,
                                            unsigned long max_nr_buckets, int flags, pthread_attr_t *attr)
{
    return _cds_lfht_new(init_size, min_nr_alloc_buckets, max_nr_buckets, flags, NULL, &rcu_flavor, attr);
}
#endif /* URCU_API_MAP */
 ...
 * cds_lfht_destroy.
 */
extern int cds_lfht_destroy(struct cds_lfht *ht, pthread_attr_t **attr);
 ...
 */
extern void cds_lfht_count_nodes(struct cds_lfht *ht, long *split_count_before, unsigned long *count,
 ...
 */
extern void cds_lfht_lookup(struct cds_lfht *ht, unsigned long hash, cds_lfht_match_fct match, const void *key,
 ...
 * This function acts as a rcu_dereference() to read the node pointer.
 */
extern void cds_lfht_next_duplicate(struct cds_lfht *ht, cds_lfht_match_fct match, const void *key,
                                    struct cds_lfht_iter *iter);
 ...
 * atomic commit.
 */
extern void cds_lfht_add(struct cds_lfht *ht, unsigned long hash, struct cds_lfht_node *node);
 ...
 * barrier.
 */
extern struct cds_lfht_node *cds_lfht_add_unique(struct cds_lfht *ht, unsigned long hash, cds_lfht_match_fct match,
 ...
 * atomic commit.
 */
extern struct cds_lfht_node *cds_lfht_add_replace(struct cds_lfht *ht, unsigned long hash, cds_lfht_match_fct match,
                                                  const void *key, struct cds_lfht_node *node);
 ...
 * any memory barrier.
 */
extern int cds_lfht_replace(struct cds_lfht *ht, struct cds_lfht_iter *old_iter, unsigned long hash,
                            cds_lfht_match_fct match, const void *key, struct cds_lfht_node *new_node);
 ...
 * any memory barrier.
 */
extern int cds_lfht_del(struct cds_lfht *ht, struct cds_lfht_node *node);
 ...
 */
extern int cds_lfht_is_node_deleted(const struct cds_lfht_node *node);
 ...
 * section.
 */
extern void cds_lfht_resize(struct cds_lfht *ht, unsigned long new_size);
...
#define cds_lfht_for_each(ht, iter, node)                                             \
    for (cds_lfht_first(ht, iter), node = cds_lfht_iter_get_node(iter); node != NULL; \
         cds_lfht_next(ht, iter), node = cds_lfht_iter_get_node(iter))

#define cds_lfht_for_each_duplicate(ht, hash, match, key, iter, node)                                    \
    for (cds_lfht_lookup(ht, hash, match, key, iter), node = cds_lfht_iter_get_node(iter); node != NULL; \
         cds_lfht_next_duplicate(ht, match, key, iter), node = cds_lfht_iter_get_node(iter))

#define cds_lfht_for_each_entry(ht, iter, pos, member)                                                               \
    for (cds_lfht_first(ht, iter), pos = caa_container_of(cds_lfht_iter_get_node(iter), __typeof__(*(pos)), member); \
         cds_lfht_iter_get_node(iter) != NULL;                                                                       \
         cds_lfht_next(ht, iter), pos = caa_container_of(cds_lfht_iter_get_node(iter), __typeof__(*(pos)), member))

#define cds_lfht_for_each_entry_duplicate(ht, hash, match, key, iter, pos, member)            \
    for (cds_lfht_lookup(ht, hash, match, key, iter),                                         \
         pos = caa_container_of(cds_lfht_iter_get_node(iter), __typeof__(*(pos)), member);    \
         cds_lfht_iter_get_node(iter) != NULL; cds_lfht_next_duplicate(ht, match, key, iter), \
         pos = caa_container_of(cds_lfht_iter_get_node(iter), __typeof__(*(pos)), member))
