 ...
 */
extern bool cds_lfs_push(cds_lfs_stack_ptr_t s, struct cds_lfs_node *node);
 ...
 */
#define cds_lfs_for_each(__head, __node) for (__node = &__head->node; __node != NULL; __node = __node->next)
...
#define cds_lfs_for_each_safe(__head, __node, __n)                                     \
    for (__node = &__head->node, __n = (__node ? __node->next : NULL); __node != NULL; \
...
template <typename T>
static inline bool cds_lfs_push(T s, struct cds_lfs_node *node)
{
    return cds_lfs_push(cds_lfs_stack_cast(s), node);
}

template <typename T>
static inline struct cds_lfs_node *__cds_lfs_pop(T s)
{
    return __cds_lfs_pop(cds_lfs_stack_cast(s));
}

template <typename T>
static inline struct cds_lfs_head *__cds_lfs_pop_all(T s)
{
