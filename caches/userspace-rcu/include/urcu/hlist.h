...
static inline void cds_hlist_add_head(struct cds_hlist_node *newp, struct cds_hlist_head *head)
{
    if (head->next) head->next->prev = newp;
    newp->next = head->next;
...
#define cds_hlist_for_each(pos, head) for (pos = (head)->next; (pos) != NULL; pos = (pos)->next)

#define cds_hlist_for_each_safe(pos, p, head) \
    for (pos = (head)->next; ((pos) != NULL) && (p = (pos)->next, 1); pos = (p))
...
#define cds_hlist_for_each_entry(entry, pos, head, member)                                              \
    for (pos = (head)->next, entry = cds_hlist_entry(pos, __typeof__(*(entry)), member); (pos) != NULL; \
         pos = (pos)->next, entry = cds_hlist_entry(pos, __typeof__(*(entry)), member))

#define cds_hlist_for_each_entry_safe(entry, pos, p, head, member)                       \
    for (pos = (head)->next, entry = cds_hlist_entry(pos, __typeof__(*(entry)), member); \
         ((pos) != NULL) && (p = (pos)->next, 1); pos = (p), entry = cds_hlist_entry(pos, __typeof__(*entry), member))

#define cds_hlist_for_each_entry_2(entry, head, member)                                             \
    for (entry = cds_hlist_entry_safe((head)->next, __typeof__(*(entry)), member); (entry) != NULL; \
...
#define cds_hlist_for_each_entry_safe_2(entry, e, head, member)                                                    \
    for (entry = cds_hlist_entry_safe((head)->next, __typeof__(*(entry)), member);                                 \
         ((entry) != NULL) && (e = (cds_hlist_entry_safe((entry)->member.next, __typeof__(*(entry)), member)), 1); \
         entry = e)
