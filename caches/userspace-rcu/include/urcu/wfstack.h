...
#define __cds_wfs_pop_with_state_nonblocking ___cds_wfs_pop_with_state_nonblocking
 ...
 */
extern struct cds_wfs_node *cds_wfs_pop_with_state_blocking(struct cds_wfs_stack *s, int *state);
 ...
 * empty into state (CDS_WFS_STATE_LAST).
 */
extern struct cds_wfs_node *__cds_wfs_pop_with_state_blocking(cds_wfs_stack_ptr_t u_stack, int *state);
 ...
 * empty into state (CDS_WFS_STATE_LAST).
 */
extern struct cds_wfs_node *__cds_wfs_pop_with_state_nonblocking(cds_wfs_stack_ptr_t u_stack, int *state);
...
#define cds_wfs_for_each_blocking(head, node) \
    for (node = cds_wfs_first(head); node != NULL; node = cds_wfs_next_blocking(node))
...
#define cds_wfs_for_each_blocking_safe(head, node, n)                                               \
    for (node = cds_wfs_first(head), n = (node ? cds_wfs_next_blocking(node) : NULL); node != NULL; \
...
template <typename T>
static inline int cds_wfs_push(T s, struct cds_wfs_node *node)
{
    return cds_wfs_push(cds_wfs_stack_cast(s), node);
}

template <typename T>
static inline struct cds_wfs_node *__cds_wfs_pop_blocking(T s)
{
    return __cds_wfs_pop_blocking(cds_wfs_stack_cast(s));
}

template <typename T>
static inline struct cds_wfs_node *__cds_wfs_pop_with_state_blocking(T s, int *state)
{
    return __cds_wfs_pop_with_state_blocking(cds_wfs_stack_cast(s), state);
}

template <typename T>
static inline struct cds_wfs_node *__cds_wfs_pop_nonblocking(T s)
...
template <typename T>
static inline struct cds_wfs_node *__cds_wfs_pop_with_state_nonblocking(T s, int *state)
{
    return __cds_wfs_pop_with_state_nonblocking(cds_wfs_stack_cast(s), state);
}

template <typename T>
static inline struct cds_wfs_head *__cds_wfs_pop_all(T s)
{
