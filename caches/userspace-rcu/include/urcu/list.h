...
static inline void cds_list_del_init(struct cds_list_head *elem)
{
    cds_list_del(elem);
...
static inline void cds_list_replace(struct cds_list_head *old, struct cds_list_head *_new)
{
    _new->next = old->next;
    _new->prev = old->prev;
    _new->prev->next = _new;
...
#define cds_list_for_each_entry(pos, head, member)                                                 \
    for (pos = cds_list_entry((head)->next, __typeof__(*(pos)), member); &(pos)->member != (head); \
         pos = cds_list_entry((pos)->member.next, __typeof__(*(pos)), member))

#define cds_list_for_each_entry_reverse(pos, head, member)                                         \
    for (pos = cds_list_entry((head)->prev, __typeof__(*(pos)), member); &(pos)->member != (head); \
...
#define cds_list_for_each_entry_safe(pos, p, head, member)                  \
    for (pos = cds_list_entry((head)->next, __typeof__(*(pos)), member),    \
        p = cds_list_entry((pos)->member.next, __typeof__(*(pos)), member); \
         &(pos)->member != (head); pos = (p), p = cds_list_entry((pos)->member.next, __typeof__(*(pos)), member))
...
#define cds_list_for_each_entry_safe_from(pos, p, head, member)                                        \
    for (p = cds_list_entry((pos)->member.next, __typeof__(*(pos)), member); &(pos)->member != (head); \
...
static inline void cds_list_replace_init(struct cds_list_head *old, struct cds_list_head *_new)
{
