...
#ifdef __linux__
static void *mremap_wrapper(void *old_address, size_t old_size, size_t new_size, int flags)
{
 ...
 * This is not generic.
 */
static void *mremap_wrapper(void *old_address __attribute__((unused)), size_t old_size __attribute__((unused)),
                            size_t new_size __attribute__((unused)), int flags)
{
...
#define ARENA_INIT_ALLOC   sizeof(struct registry_chunk) + INIT_NR_THREADS * sizeof(struct urcu_bp_reader)
...
static void __attribute__((constructor)) _urcu_bp_init(void);
static void __attribute__((destructor)) urcu_bp_exit(void);
...
static void smp_mb_master(void)
{
    if (caa_likely(urcu_bp_has_sys_membarrier)) {
        if (membarrier(MEMBARRIER_CMD_PRIVATE_EXPEDITED, 0)) urcu_die(errno);
    } else {
        cmm_smp_mb();
    }
}
 ...
 */
static void wait_for_readers(struct cds_list_head *input_readers, struct cds_list_head *cur_snap_readers,
                             struct cds_list_head *qsreaders)
{
     ...
     */
    for (;;) {
        if (wait_loops < RCU_QS_ACTIVE_ATTEMPTS) wait_loops++;
...
void urcu_bp_synchronize_rcu(void)
{
    ...
    if (cds_list_empty(&registry)) goto out;
...
static void expand_arena(struct registry_arena *arena)
{
    ...
    if (cds_list_empty(&arena->chunk_list)) {
        urcu_posix_assert(ARENA_INIT_ALLOC >= sizeof(struct registry_chunk) + sizeof(struct rcu_reader));
        new_chunk_len = ARENA_INIT_ALLOC;
        new_chunk = (struct registry_chunk *)mmap(NULL, new_chunk_len, PROT_READ | PROT_WRITE,
                                                  MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
        if (new_chunk == MAP_FAILED) abort();
        memset(new_chunk, 0, new_chunk_len);
        new_chunk->data_len = new_chunk_len - sizeof(struct registry_chunk);
        cds_list_add_tail(&new_chunk->node, &arena->chunk_list);
        return; /* We're done. */
    }

    /* Try expanding last chunk. */
    last_chunk = cds_list_entry(arena->chunk_list.prev, struct registry_chunk, node);
    old_chunk_len = last_chunk->data_len + sizeof(struct registry_chunk);
    new_chunk_len = old_chunk_len << 1;

    /* Don't allow memory mapping to move, just expand. */
    new_chunk = mremap_wrapper(last_chunk, old_chunk_len, new_chunk_len, 0);
    if (new_chunk != MAP_FAILED) {
        /* Should not have moved. */
        urcu_posix_assert(new_chunk == last_chunk);
        memset((char *)last_chunk + old_chunk_len, 0, new_chunk_len - old_chunk_len);
        last_chunk->data_len = new_chunk_len - sizeof(struct registry_chunk);
        return; /* We're done. */
    }

    /* Remap did not succeed, we need to add a new chunk. */
    new_chunk =
        (struct registry_chunk *)mmap(NULL, new_chunk_len, PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
    if (new_chunk == MAP_FAILED) abort();
    memset(new_chunk, 0, new_chunk_len);
    new_chunk->data_len = new_chunk_len - sizeof(struct registry_chunk);
    cds_list_add_tail(&new_chunk->node, &arena->chunk_list);
...
static struct rcu_reader *arena_alloc(struct registry_arena *arena)
{
...
retry:
    ...
    {
        if (chunk->data_len - chunk->used < len) continue;
        /* Find spot */
        for (rcu_reader_reg = (struct rcu_reader *)&chunk->data[0];
             rcu_reader_reg < (struct rcu_reader *)&chunk->data[chunk->data_len]; rcu_reader_reg++) {
...
static void cleanup_thread(struct registry_chunk *chunk, struct rcu_reader *rcu_reader_reg)
{
...
static struct registry_chunk *find_chunk(struct rcu_reader *rcu_reader_reg)
{
    struct registry_chunk *chunk;

    cds_list_for_each_entry(chunk, &registry_arena.chunk_list, node)
    {
        if (rcu_reader_reg < (struct rcu_reader *)&chunk->data[0]) continue;
        if (rcu_reader_reg >= (struct rcu_reader *)&chunk->data[chunk->data_len]) continue;
        return chunk;
    }
    return NULL;
}
...
/* Called with signals off and mutex locked */
static void remove_thread(struct rcu_reader *rcu_reader_reg)
{
...
void urcu_bp_register(void)
{
     ...
     */
    if (URCU_TLS(urcu_bp_reader)) goto end;
...
{
    if (caa_unlikely(!URCU_TLS(urcu_bp_reader))) urcu_bp_register(); /* If not yet registered. */
}

/* Disable signals, take mutex, remove from registry */
static void urcu_bp_unregister(struct rcu_reader *rcu_reader_reg)
{
    sigset_t newmask, oldmask;
    int ret;
...
static void urcu_bp_sys_membarrier_status(bool available)
{
    if (!available) abort();
}
#else
...
static void urcu_bp_sys_membarrier_init(void)
{
    ...
    if (mask >= 0) {
        if (mask & MEMBARRIER_CMD_PRIVATE_EXPEDITED) {
            if (membarrier(MEMBARRIER_CMD_REGISTER_PRIVATE_EXPEDITED, 0)) urcu_die(errno);
            available = true;
        }
    }
...
static void _urcu_bp_init(void)
{
...
static void urcu_bp_exit(void)
{
    ...
    if (!--urcu_bp_refcount) {
        ...
        cds_list_for_each_entry_safe(chunk, tmp, &registry_arena.chunk_list, node)
        {
            munmap((void *)chunk, chunk->data_len + sizeof(struct registry_chunk));
        }
...
static void urcu_bp_prune_registry(void)
{
    ...
    cds_list_for_each_entry(chunk, &registry_arena.chunk_list, node)
    {
        for (rcu_reader_reg = (struct urcu_bp_reader *)&chunk->data[0];
             rcu_reader_reg < (struct urcu_bp_reader *)&chunk->data[chunk->data_len]; rcu_reader_reg++) {
            if (!rcu_reader_reg->alloc) continue;
            if (rcu_reader_reg->tid == pthread_self()) continue;
