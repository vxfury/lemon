...
static int set_thread_cpu_affinity(struct urcu_workqueue *workqueue)
{
    ...
    if (workqueue->cpu_affinity < 0) return 0;
    if (++workqueue->loop_count & SET_AFFINITY_CHECK_PERIOD_MASK) return 0;
    if (urcu_sched_getcpu() == workqueue->cpu_affinity) return 0;
...
static void futex_wake_up(int32_t *futex)
{
    ...
    if (caa_unlikely(uatomic_read(futex) == -1)) {
        uatomic_set(futex, 0);
        if (futex_async(futex, FUTEX_WAKE, 1, NULL, NULL, 0) < 0) urcu_die(errno);
    }
}
...
static void *workqueue_thread(void *arg)
{
    ...
    if (set_thread_cpu_affinity(workqueue)) urcu_die(errno);

    if (workqueue->initialize_worker_fct) workqueue->initialize_worker_fct(workqueue, workqueue->priv);
    ...
    for (;;) {
        ...
        if (uatomic_read(&workqueue->flags) & URCU_WORKQUEUE_PAUSE) {
             ...
             */
            if (workqueue->worker_before_pause_fct) workqueue->worker_before_pause_fct(workqueue, workqueue->priv);
            cmm_smp_mb__before_uatomic_or();
            uatomic_or(&workqueue->flags, URCU_WORKQUEUE_PAUSED);
            while ((uatomic_read(&workqueue->flags) & URCU_WORKQUEUE_PAUSE) != 0) (void)poll(NULL, 0, 1);
            uatomic_and(&workqueue->flags, ~URCU_WORKQUEUE_PAUSED);
            cmm_smp_mb__after_uatomic_and();
            if (workqueue->worker_after_resume_fct) workqueue->worker_after_resume_fct(workqueue, workqueue->priv);
        }

        cds_wfcq_init(&cbs_tmp_head, &cbs_tmp_tail);
        splice_ret =
            __cds_wfcq_splice_blocking(&cbs_tmp_head, &cbs_tmp_tail, &workqueue->cbs_head, &workqueue->cbs_tail);
        ...
        if (splice_ret != CDS_WFCQ_RET_SRC_EMPTY) {
            if (workqueue->grace_period_fct) workqueue->grace_period_fct(workqueue, workqueue->priv);
            cbcount = 0;
            __cds_wfcq_for_each_blocking_safe(&cbs_tmp_head, &cbs_tmp_tail, cbs, cbs_tmp_n)
            {
                struct urcu_work *uwp;

                uwp = caa_container_of(cbs, struct urcu_work, next);
                uwp->func(uwp);
                cbcount++;
            }
            uatomic_sub(&workqueue->qlen, cbcount);
        }
        if (uatomic_read(&workqueue->flags) & URCU_WORKQUEUE_STOP) break;
        if (workqueue->worker_before_wait_fct) workqueue->worker_before_wait_fct(workqueue, workqueue->priv);
        if (!rt) {
            if (cds_wfcq_empty(&workqueue->cbs_head, &workqueue->cbs_tail)) {
        ...
        if (workqueue->worker_after_wake_up_fct) workqueue->worker_after_wake_up_fct(workqueue, workqueue->priv);
    }
    if (!rt) {
        /*
    ...
    if (workqueue->finalize_worker_fct) workqueue->finalize_worker_fct(workqueue, workqueue->priv);
    return NULL;
}

struct urcu_workqueue *urcu_workqueue_create(
    unsigned long flags, int cpu_affinity, void *priv,
    void (*grace_period_fct)(struct urcu_workqueue *workqueue, void *priv),
    void (*initialize_worker_fct)(struct urcu_workqueue *workqueue, void *priv),
    void (*finalize_worker_fct)(struct urcu_workqueue *workqueue, void *priv),
    void (*worker_before_wait_fct)(struct urcu_workqueue *workqueue, void *priv),
    ...
    void (*worker_after_resume_fct)(struct urcu_workqueue *workqueue, void *priv))
{
    ...
    if (workqueue == NULL) urcu_die(errno);
...
{
    if (!(_CMM_LOAD_SHARED(workqueue->flags) & URCU_WORKQUEUE_RT)) futex_wake_up(&workqueue->futex);
}
...
void urcu_workqueue_queue_work(struct urcu_workqueue *workqueue, struct urcu_work *work,
...
static void _urcu_workqueue_wait_complete(struct urcu_work *work)
{
    ...
    if (!uatomic_sub_return(&completion->barrier_count, 1)) futex_wake_up(&completion->futex);
...
void urcu_workqueue_queue_completion(struct urcu_workqueue *workqueue, struct urcu_workqueue_completion *completion)
{
...
void urcu_workqueue_pause_worker(struct urcu_workqueue *workqueue)
{
    ...
    while ((uatomic_read(&workqueue->flags) & URCU_WORKQUEUE_PAUSED) == 0) (void)poll(NULL, 0, 1);
}
