...
static int set_thread_cpu_affinity(struct call_rcu_data *crdp)
{
    cpu_set_t mask;
    int ret;

    if (crdp->cpu_affinity < 0) return 0;
    if (++crdp->gp_count & SET_AFFINITY_CHECK_PERIOD_MASK) return 0;
...
#else
static int set_thread_cpu_affinity(struct call_rcu_data *crdp __attribute__((unused)))
{
    return 0;
}
#endif
...
static void call_rcu_wake_up(struct call_rcu_data *crdp)
{
    ...
    if (caa_unlikely(uatomic_read(&crdp->futex) == -1)) {
        ...
        if (futex_async(&crdp->futex, FUTEX_WAKE, 1, NULL, NULL, 0) < 0) urcu_die(errno);
    }
}
...
static void call_rcu_completion_wake_up(struct call_rcu_completion *completion)
{
    ...
    if (caa_unlikely(uatomic_read(&completion->futex) == -1)) {
        ...
        if (futex_async(&completion->futex, FUTEX_WAKE, 1, NULL, NULL, 0) < 0) urcu_die(errno);
    }
}
...
static void *call_rcu_thread(void *arg)
{
    ...
    if (set_thread_cpu_affinity(crdp)) urcu_die(errno);
    ...
    for (;;) {
        ...
        if (uatomic_read(&crdp->flags) & URCU_CALL_RCU_PAUSE) {
            ...
            while ((uatomic_read(&crdp->flags) & URCU_CALL_RCU_PAUSE) != 0) (void)poll(NULL, 0, 1);
        ...
        splice_ret = __cds_wfcq_splice_blocking(&cbs_tmp_head, &cbs_tmp_tail, &crdp->cbs_head, &crdp->cbs_tail);
        ...
        if (uatomic_read(&crdp->flags) & URCU_CALL_RCU_STOP) break;
        rcu_thread_offline();
        if (!rt) {
...
struct call_rcu_data *get_cpu_call_rcu_data(int cpu)
{
    ...
    if (pcpu_crdp == NULL) return NULL;
    if (!warned && cpus_array_len > 0 && (cpu < 0 || cpus_array_len <= cpu)) {
        fprintf(stderr, "[error] liburcu: get CPU # out of range\n");
        warned = 1;
    }
    if (cpu < 0 || cpus_array_len <= cpu) return NULL;
...
static struct call_rcu_data *__create_call_rcu_data(unsigned long flags, int cpu_affinity)
{
    struct call_rcu_data *crdp;
...
{
    if (default_call_rcu_data != NULL) return rcu_dereference(default_call_rcu_data);
    call_rcu_lock(&call_rcu_mutex);
...
struct call_rcu_data *get_call_rcu_data(void)
{
    ...
    if (URCU_TLS(thread_call_rcu_data) != NULL) return URCU_TLS(thread_call_rcu_data);
...
{
    if (!(_CMM_LOAD_SHARED(crdp->flags) & URCU_CALL_RCU_RT)) call_rcu_wake_up(crdp);
}

static void _call_rcu(struct rcu_head *head, void (*func)(struct rcu_head *head), struct call_rcu_data *crdp)
{
...
void call_rcu_data_free(struct call_rcu_data *crdp)
{
    ...
    if ((uatomic_read(&crdp->flags) & URCU_CALL_RCU_STOPPED) == 0) {
        ...
        wake_call_rcu_thread(crdp);
        while ((uatomic_read(&crdp->flags) & URCU_CALL_RCU_STOPPED) == 0) (void)poll(NULL, 0, 1);
    }
    call_rcu_lock(&call_rcu_mutex);
    if (!cds_wfcq_empty(&crdp->cbs_head, &crdp->cbs_tail)) {
        call_rcu_unlock(&call_rcu_mutex);
        ...
        call_rcu_lock(&call_rcu_mutex);
        __cds_wfcq_splice_blocking(&default_call_rcu_data->cbs_head, &default_call_rcu_data->cbs_tail, &crdp->cbs_head,
                                   &crdp->cbs_tail);
        uatomic_add(&default_call_rcu_data->qlen, uatomic_read(&crdp->qlen));
...
void rcu_barrier(void)
{
    ...
    if (was_online) rcu_thread_offline();
    /*
    ...
    if (!completion) urcu_die(errno);

    call_rcu_lock(&call_rcu_mutex);
    cds_list_for_each_entry(crdp, &call_rcu_data_list, list) count++;
    ...
    for (;;) {
        ...
        cmm_smp_mb();
        if (!uatomic_read(&completion->barrier_count)) break;
...
online:
    if (was_online) rcu_thread_online();
}
...
void call_rcu_before_fork(void)
{
    ...
    if (atfork) atfork->before_fork(atfork->priv);
    ...
    cds_list_for_each_entry(crdp, &call_rcu_data_list, list)
    {
        while ((uatomic_read(&crdp->flags) & URCU_CALL_RCU_PAUSED) == 0) (void)poll(NULL, 0, 1);
    }
}
...
void call_rcu_after_fork_parent(void)
{
    ...
    cds_list_for_each_entry(crdp, &call_rcu_data_list, list) uatomic_and(&crdp->flags, ~URCU_CALL_RCU_PAUSE);
    cds_list_for_each_entry(crdp, &call_rcu_data_list, list)
    {
        while ((uatomic_read(&crdp->flags) & URCU_CALL_RCU_PAUSED) != 0) (void)poll(NULL, 0, 1);
    }
    atfork = registered_rculfhash_atfork;
    if (atfork) atfork->after_fork_parent(atfork->priv);
    call_rcu_unlock(&call_rcu_mutex);
}
...
void call_rcu_after_fork_child(void)
{
    ...
    atfork = registered_rculfhash_atfork;
    if (atfork) atfork->after_fork_child(atfork->priv);

    /* Do nothing when call_rcu() has not been used */
    if (cds_list_empty(&call_rcu_data_list)) return;
    ...
    {
        if (crdp == default_call_rcu_data) continue;
...
{
    call_rcu_lock(&call_rcu_mutex);
    if (registered_rculfhash_atfork_refcount++) goto end;
