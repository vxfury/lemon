...
static inline int _get_max_cpuid_from_sysfs(const char *path)
{
    ...
    cpudir = opendir(path);
    if (cpudir == NULL) goto end;

    /*
     * Iterate on all directories named "cpu" followed by an integer.
     */
    while ((entry = readdir(cpudir))) {
        if (entry->d_type == DT_DIR && strncmp(entry->d_name, "cpu", 3) == 0) {
            char *endptr;
            long cpu_id;

            cpu_id = strtol(entry->d_name + 3, &endptr, 10);
            if ((cpu_id < LONG_MAX) && (endptr != entry->d_name + 3) && (*endptr == '\0')) {
                if (cpu_id > max_cpuid) max_cpuid = cpu_id;
            }
        }
    }

    if (closedir(cpudir)) perror("closedir");

    /*
     * If the max CPU id is out of bound, set it to -1 so it results in a
     * CPU num of 0.
     */
    if (max_cpuid < 0 || max_cpuid > INT_MAX) max_cpuid = -1;
...
static inline int get_cpu_mask_from_sysfs(char *buf, size_t max_bytes, const char *path)
{
    ...
    do {
        bytes_read = read(fd, buf + total_bytes_read, max_bytes - total_bytes_read);
    ...
    if (total_bytes_read > INT_MAX) goto end;
...
{
    return get_cpu_mask_from_sysfs(buf, max_bytes, "/sys/devices/system/cpu/possible");
}
...
static inline int get_max_cpuid_from_mask(const char *pmask, size_t len)
{
    ...
    if ((&pmask[i] != endptr) && (cpu_index < INT_MAX)) return (int)cpu_index;
