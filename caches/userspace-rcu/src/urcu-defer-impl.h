...
#define DQ_SET_FCT_BIT(x)   (x = (void *)((unsigned long)(x) | DQ_FCT_BIT))
#define DQ_CLEAR_FCT_BIT(x) (x = (void *)((unsigned long)(x) & ~DQ_FCT_BIT))
...
static void wake_up_defer(void)
{
    if (caa_unlikely(uatomic_read(&defer_thread_futex) == -1)) {
        ...
        if (futex_noasync(&defer_thread_futex, FUTEX_WAKE, 1, NULL, NULL, 0) < 0) urcu_die(errno);
    }
}
 ...
 */
static void rcu_defer_barrier_queue(struct defer_queue *queue, unsigned long head)
{
    unsigned long i;
    void (*fct)(void *p);
    void *p;
...
static void _rcu_defer_barrier_thread(void)
{
    ...
    if (caa_unlikely(!num_items)) return;
    synchronize_rcu();
...
void rcu_defer_barrier(void)
{
    ...
    if (cds_list_empty(&registry_defer)) return;
    ...
    synchronize_rcu();
    cds_list_for_each_entry(index, &registry_defer, list) rcu_defer_barrier_queue(index, index->last_head);
end:
    mutex_unlock(&rcu_defer_mutex);
}
...
static void _defer_rcu(void (*fct)(void *p), void *p)
{
     ...
     */
    if (caa_unlikely(URCU_TLS(defer_queue).last_fct_in != fct || DQ_IS_FCT_BIT(p) || p == DQ_FCT_MARK)) {
        URCU_TLS(defer_queue).last_fct_in = fct;
        if (caa_unlikely(DQ_IS_FCT_BIT(fct) || fct == DQ_FCT_MARK)) {
            _CMM_STORE_SHARED(URCU_TLS(defer_queue).q[head++ & DEFER_QUEUE_MASK], DQ_FCT_MARK);
            _CMM_STORE_SHARED(URCU_TLS(defer_queue).q[head++ & DEFER_QUEUE_MASK], fct);
        } else {
            DQ_SET_FCT_BIT(fct);
            _CMM_STORE_SHARED(URCU_TLS(defer_queue).q[head++ & DEFER_QUEUE_MASK], fct);
        }
...
int rcu_defer_register_thread(void)
{
    ...
    if (!URCU_TLS(defer_queue).q) return -ENOMEM;
    ...
    if (was_empty) start_defer_thread();
