...
#define DEFINE_URCU_WAIT_NODE(name, state) struct urcu_wait_node name = URCU_WAIT_NODE_INIT(name, state)

#define DECLARE_URCU_WAIT_NODE(name) struct urcu_wait_node name
...
#define DECLARE_URCU_WAIT_QUEUE(name) struct urcu_wait_queue name

#define DEFINE_URCU_WAIT_QUEUE(name) struct urcu_wait_queue name = URCU_WAIT_QUEUE_HEAD_INIT(name)
 ...
 */
static inline bool urcu_wait_add(struct urcu_wait_queue *queue, struct urcu_wait_node *node)
{
...
static inline void urcu_wait_node_init(struct urcu_wait_node *node, enum urcu_wait_state state)
{
...
static inline void urcu_adaptative_wake_up(struct urcu_wait_node *wait)
{
    ...
    if (!(uatomic_read(&wait->state) & URCU_WAIT_RUNNING)) {
        if (futex_noasync(&wait->state, FUTEX_WAKE, 1, NULL, NULL, 0) < 0) urcu_die(errno);
    }
...
static inline void urcu_adaptative_busy_wait(struct urcu_wait_node *wait)
{
    ...
    for (i = 0; i < URCU_WAIT_ATTEMPTS; i++) {
        if (uatomic_read(&wait->state) != URCU_WAIT_WAITING) goto skip_futex_wait;
        caa_cpu_relax();
    }
...
skip_futex_wait:
     ...
     */
    for (i = 0; i < URCU_WAIT_ATTEMPTS; i++) {
        if (uatomic_read(&wait->state) & URCU_WAIT_TEARDOWN) break;
        caa_cpu_relax();
    }
    while (!(uatomic_read(&wait->state) & URCU_WAIT_TEARDOWN)) poll(NULL, 0, 10);
...
static inline void urcu_wake_all_waiters(struct urcu_waiters *waiters)
{
    ...
    {
        struct urcu_wait_node *wait_node = caa_container_of(iter, struct urcu_wait_node, node);

        /* Don't wake already running threads */
        if (wait_node->state & URCU_WAIT_RUNNING) continue;
