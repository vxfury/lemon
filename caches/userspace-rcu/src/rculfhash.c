...
struct partition_resize_work {
    ...
    void (*fct)(struct cds_lfht *ht, unsigned long i, unsigned long start, unsigned long len);
};
...
static void cds_lfht_iter_debug_set_ht(struct cds_lfht *ht, struct cds_lfht_iter *iter)
{
    iter->lfht = ht;
}
...
static void cds_lfht_iter_debug_set_ht(struct cds_lfht *ht __attribute__((unused)),
...
static uint8_t bit_reverse_u8(uint8_t v)
{
    return BitReverseTable256[v];
}

#if (CAA_BITS_PER_LONG == 32)
static uint32_t bit_reverse_u32(uint32_t v)
{
    return ((uint32_t)bit_reverse_u8(v) << 24) | ((uint32_t)bit_reverse_u8(v >> 8) << 16)
           | ((uint32_t)bit_reverse_u8(v >> 16) << 8) | ((uint32_t)bit_reverse_u8(v >> 24));
}
#else
static uint64_t bit_reverse_u64(uint64_t v)
{
    return ((uint64_t)bit_reverse_u8(v) << 56) | ((uint64_t)bit_reverse_u8(v >> 8) << 48)
           | ((uint64_t)bit_reverse_u8(v >> 16) << 40) | ((uint64_t)bit_reverse_u8(v >> 24) << 32)
           | ((uint64_t)bit_reverse_u8(v >> 32) << 24) | ((uint64_t)bit_reverse_u8(v >> 40) << 16)
           | ((uint64_t)bit_reverse_u8(v >> 48) << 8) | ((uint64_t)bit_reverse_u8(v >> 56));
}
#endif

static unsigned long bit_reverse_ulong(unsigned long v)
{
#if (CAA_BITS_PER_LONG == 32)
 ...
 * Return -1 if x is 0.
 */
static int cds_lfht_get_count_order_u32(uint32_t x)
{
    if (!x) return -1;
...
static void cds_lfht_resize_lazy_grow(struct cds_lfht *ht, unsigned long size, int growth);

static void cds_lfht_resize_lazy_count(struct cds_lfht *ht, unsigned long size, unsigned long count);
...
static void mutex_lock(pthread_mutex_t *mutex)
{
    ...
    while ((ret = pthread_mutex_trylock(mutex)) != 0) {
        if (ret != EBUSY && ret != EINTR) urcu_die(ret);
...
static void alloc_split_items_count(struct cds_lfht *ht)
{
    if (nr_cpus_mask == -1) {
        ...
        split_count_order = cds_lfht_get_count_order_ulong(split_count_mask + 1);
    }

    urcu_posix_assert(split_count_mask >= 0);

    if (ht->flags & CDS_LFHT_ACCOUNTING) {
        ht->split_count = calloc(split_count_mask + 1, sizeof(struct ht_items_count));
...
static int ht_get_split_count_index(unsigned long hash)
{
    int cpu;
...
static void ht_count_add(struct cds_lfht *ht, unsigned long size, unsigned long hash)
{
    unsigned long split_count, count;
    int index;

    if (caa_unlikely(!ht->split_count)) return;
    index = ht_get_split_count_index(hash);
    split_count = uatomic_add_return(&ht->split_count[index].add, 1);
    if (caa_likely(split_count & ((1UL << COUNT_COMMIT_ORDER) - 1))) return;
    /* Only if number of add multiple of 1UL << COUNT_COMMIT_ORDER */

    dbg_printf("add split count %lu\n", split_count);
    count = uatomic_add_return(&ht->count, 1UL << COUNT_COMMIT_ORDER);
    if (caa_likely(count & (count - 1))) return;
    /* Only if global count is power of 2 */

    if ((count >> CHAIN_LEN_RESIZE_THRESHOLD) < size) return;
    dbg_printf("add set global %lu\n", count);
    cds_lfht_resize_lazy_count(ht, size, count >> (CHAIN_LEN_TARGET - 1));
}

static void ht_count_del(struct cds_lfht *ht, unsigned long size, unsigned long hash)
{
    unsigned long split_count, count;
    int index;
     ...
     * certain threshold.
     */
    if (count < (1UL << COUNT_COMMIT_ORDER) * (split_count_mask + 1)) return;
    cds_lfht_resize_lazy_count(ht, size, count >> (CHAIN_LEN_TARGET - 1));
}

static void check_resize(struct cds_lfht *ht, unsigned long size, uint32_t chain_len)
{
    unsigned long count;

    if (!(ht->flags & CDS_LFHT_AUTO_RESIZE)) return;
    count = uatomic_read(&ht->count);
    /*
     * Use bucket-local length for small table expand and for
     * environments lacking per-cpu data support.
     */
    if (count >= (1UL << (COUNT_COMMIT_ORDER + split_count_order))) return;
    if (chain_len > 100) dbg_printf("WARNING: large chain length: %u.\n", chain_len);
    if (chain_len >= CHAIN_LEN_RESIZE_THRESHOLD) {
         ...
         */
        growth = cds_lfht_get_count_order_u32(chain_len - (CHAIN_LEN_TARGET - 1));
        if ((ht->flags & CDS_LFHT_ACCOUNTING)
            && (size << growth) >= (1UL << (COUNT_COMMIT_ORDER + split_count_order))) {
            /*
             ...
             * table in every case.
             */
            growth = COUNT_COMMIT_ORDER + split_count_order - cds_lfht_get_count_order_ulong(size);
            if (growth <= 0) return;
        }
...
static int is_removed(const struct cds_lfht_node *node)
{
    return ((unsigned long)node) & REMOVED_FLAG;
}

static int is_bucket(struct cds_lfht_node *node)
{
...
static int is_removal_owner(struct cds_lfht_node *node)
{
    return ((unsigned long)node) & REMOVAL_OWNER_FLAG;
}

static struct cds_lfht_node *flag_removed(struct cds_lfht_node *node)
{
...
static int is_end(struct cds_lfht_node *node)
{
    return clear_flag(node) == (struct cds_lfht_node *)END_VALUE;
}

static unsigned long _uatomic_xchg_monotonic_increase(unsigned long *ptr, unsigned long v)
{
 ...
 * lfht is destroyed.
 */
static void cds_lfht_free_bucket_table(struct cds_lfht *ht, unsigned long order)
{
    return ht->mm->free_bucket_table(ht, order);
}

static inline struct cds_lfht_node *bucket_at(struct cds_lfht *ht, unsigned long index)
{
    return ht->bucket_at(ht, index);
}

static inline struct cds_lfht_node *lookup_bucket(struct cds_lfht *ht, unsigned long size, unsigned long hash)
{
...
static void _cds_lfht_gc_bucket(struct cds_lfht_node *bucket, struct cds_lfht_node *node)
{
    ...
    for (;;) {
        ...
        for (;;) {
            if (caa_unlikely(is_end(iter))) return;
            if (caa_likely(clear_flag(iter)->reverse_hash > node->reverse_hash)) return;
            next = rcu_dereference(clear_flag(iter)->next);
            if (caa_likely(is_removed(next))) break;
...
static int _cds_lfht_replace(struct cds_lfht *ht, unsigned long size, struct cds_lfht_node *old_node,
                             struct cds_lfht_node *old_next, struct cds_lfht_node *new_node)
{
    ...
    for (;;) {
         ...
         */
        ret_next = uatomic_cmpxchg(&old_node->next, old_next, flag_removed_or_removal_owner(new_node));
        if (ret_next == old_next) break; /* We performed the replacement. */
        old_next = ret_next;
    }
 ...
 */
static void _cds_lfht_add(struct cds_lfht *ht, unsigned long hash, cds_lfht_match_fct match, const void *key,
                          unsigned long size, struct cds_lfht_node *node, struct cds_lfht_iter *unique_ret,
                          int bucket_flag)
{
    struct cds_lfht_node *iter_prev, *iter, *next, *new_node, *new_next, *return_node;
    ...
    for (;;) {
        ...
        urcu_posix_assert(iter_prev->reverse_hash <= node->reverse_hash);
        for (;;) {
            if (caa_unlikely(is_end(iter))) goto insert;
            if (caa_likely(clear_flag(iter)->reverse_hash > node->reverse_hash)) goto insert;

            /* bucket node is the first node of the identical-hash-value chain */
            if (bucket_flag && clear_flag(iter)->reverse_hash == node->reverse_hash) goto insert;

            next = rcu_dereference(clear_flag(iter)->next);
            if (caa_unlikely(is_removed(next))) goto gc_node;

            /* uniquely add */
            if (unique_ret && !is_bucket(next) && clear_flag(iter)->reverse_hash == node->reverse_hash) {
            ...
            if (iter_prev->reverse_hash != clear_flag(iter)->reverse_hash && !is_bucket(next))
    ...
    insert:
        ...
        if (is_bucket(iter))
            ...
            new_node = node;
        if (uatomic_cmpxchg(&iter_prev->next, iter, new_node) != iter) {
            continue; /* retry */
        } else {
            return_node = node;
            goto end;
...
static int _cds_lfht_del(struct cds_lfht *ht, unsigned long size, struct cds_lfht_node *node)
{
    ...
    if (caa_unlikely(is_removed(next))) return -ENOENT;
     ...
     * was already set).
     */
    if (!is_removal_owner(uatomic_xchg(&node->next, flag_removal_owner(node->next))))
        return 0;
    else
        return -ENOENT;
}
...
static void partition_resize_helper(struct cds_lfht *ht, unsigned long i, unsigned long len,
                                    void (*fct)(struct cds_lfht *ht, unsigned long i, unsigned long start,
                                                unsigned long len))
{
    ...
    if (nr_cpus_mask < 0 || len < 2 * MIN_PARTITION_PER_THREAD) goto fallback;
     ...
     */
    if (nr_cpus_mask > 0) {
        nr_threads = min_t(unsigned long, nr_cpus_mask + 1, len >> MIN_PARTITION_PER_THREAD_ORDER);
    } else {
        nr_threads = 1;
    }
    ...
    for (thread = 0; thread < nr_threads; thread++) {
        ...
        work[thread].fct = fct;
        ret = pthread_create(&(work[thread].thread_id), ht->resize_attr, partition_resize_thread, &work[thread]);
        if (ret == EAGAIN) {
            /*
     ...
     */
    if (start == 0 && nr_threads > 0) return;
fallback:
    fct(ht, i, start, len);
}
 ...
 */
static void init_table_populate_partition(struct cds_lfht *ht, unsigned long i, unsigned long start, unsigned long len)
{
...
static void init_table_populate(struct cds_lfht *ht, unsigned long i, unsigned long len)
{
    partition_resize_helper(ht, i, len, init_table_populate_partition);
}

static void init_table(struct cds_lfht *ht, unsigned long first_order, unsigned long last_order)
{
    unsigned long i;
    ...
    for (i = first_order; i <= last_order; i++) {
        ...
        if (CMM_LOAD_SHARED(ht->resize_target) < (1UL << i)) break;
        ...
        if (CMM_LOAD_SHARED(ht->in_progress_destroy)) break;
    }
}
 ...
 */
static void remove_table_partition(struct cds_lfht *ht, unsigned long i, unsigned long start, unsigned long len)
{
    unsigned long j, size = 1UL << (i - 1);
...
static void remove_table(struct cds_lfht *ht, unsigned long i, unsigned long len)
{
 ...
 * be called.
 */
static void fini_table(struct cds_lfht *ht, unsigned long first_order, unsigned long last_order)
{
    ...
    for (i = last_order; i >= first_order; i--) {
        ...
        if (CMM_LOAD_SHARED(ht->resize_target) > (1UL << (i - 1))) break;
        ...
        if (free_by_rcu_order) cds_lfht_free_bucket_table(ht, free_by_rcu_order);
 ...
 */
static void cds_lfht_create_bucket(struct cds_lfht *ht, unsigned long size)
{
 ...
 */
static const struct cds_lfht_mm_type *get_mm_type(unsigned long max_nr_buckets)
{
 ...
 */
static const struct cds_lfht_mm_type *get_mm_type(unsigned long max_nr_buckets __attribute__((unused)))
{
    return &cds_lfht_mm_order;
}
#endif
...
struct cds_lfht *_cds_lfht_new(unsigned long init_size, unsigned long min_nr_alloc_buckets,
                               unsigned long max_nr_buckets, int flags, const struct cds_lfht_mm_type *mm,
                               const struct rcu_flavor_struct *flavor, pthread_attr_t *attr)
{
    struct cds_lfht *ht;
    unsigned long order;

    /* min_nr_alloc_buckets must be power of two */
    if (!min_nr_alloc_buckets || (min_nr_alloc_buckets & (min_nr_alloc_buckets - 1))) return NULL;

    /* init_size must be power of two */
    if (!init_size || (init_size & (init_size - 1))) return NULL;

    /*
     * Memory management plugin default.
     */
    if (!mm) mm = get_mm_type(max_nr_buckets);

    /* max_nr_buckets == 0 for order based mm means infinite */
    if (mm == &cds_lfht_mm_order && !max_nr_buckets) max_nr_buckets = 1UL << (MAX_TABLE_ORDER - 1);

    /* max_nr_buckets must be power of two */
    if (!max_nr_buckets || (max_nr_buckets & (max_nr_buckets - 1))) return NULL;

    if (flags & CDS_LFHT_AUTO_RESIZE) cds_lfht_init_worker(flavor);
...
void cds_lfht_lookup(struct cds_lfht *ht, unsigned long hash, cds_lfht_match_fct match, const void *key,
                     struct cds_lfht_iter *iter)
{
    ...
    for (;;) {
        ...
        if (caa_likely(!is_removed(next)) && !is_bucket(next) && node->reverse_hash == reverse_hash
...
void cds_lfht_next_duplicate(struct cds_lfht *ht __attribute__((unused)), cds_lfht_match_fct match, const void *key,
                             struct cds_lfht_iter *iter)
{
    ...
    for (;;) {
        ...
        next = rcu_dereference(node->next);
        if (caa_likely(!is_removed(next)) && !is_bucket(next) && caa_likely(match(node, key))) {
            break;
        }
        node = clear_flag(next);
    }
...
void cds_lfht_next(struct cds_lfht *ht __attribute__((unused)), struct cds_lfht_iter *iter)
{
    struct cds_lfht_node *node, *next;
    ...
    for (;;) {
        ...
        next = rcu_dereference(node->next);
        if (caa_likely(!is_removed(next)) && !is_bucket(next)) {
            break;
        }
        node = clear_flag(next);
    }
...
struct cds_lfht_node *cds_lfht_add_unique(struct cds_lfht *ht, unsigned long hash, cds_lfht_match_fct match,
                                          const void *key, struct cds_lfht_node *node)
{
    ...
    if (iter.node == node) ht_count_add(ht, size, hash);
    return iter.node;
}

struct cds_lfht_node *cds_lfht_add_replace(struct cds_lfht *ht, unsigned long hash, cds_lfht_match_fct match,
                                           const void *key, struct cds_lfht_node *node)
{
    unsigned long size;
    struct cds_lfht_iter iter;
    ...
    for (;;) {
        ...
        if (!_cds_lfht_replace(ht, size, iter.node, iter.next, node)) return iter.node;
    }
}

int cds_lfht_replace(struct cds_lfht *ht, struct cds_lfht_iter *old_iter, unsigned long hash, cds_lfht_match_fct match,
                     const void *key, struct cds_lfht_node *new_node)
{
    unsigned long size;

    new_node->reverse_hash = bit_reverse_ulong(hash);
    if (!old_iter->node) return -ENOENT;
    if (caa_unlikely(old_iter->node->reverse_hash != new_node->reverse_hash)) return -EINVAL;
    if (caa_unlikely(!match(old_iter->node, key))) return -EINVAL;
    size = rcu_dereference(ht->size);
    return _cds_lfht_replace(ht, size, old_iter->node, old_iter->next, new_node);
}
...
static int cds_lfht_delete_bucket(struct cds_lfht *ht)
{
    ...
    do {
        ...
        if (!is_bucket(node)) return -EPERM;
        urcu_posix_assert(!is_removed(node));
        urcu_posix_assert(!is_removal_owner(node));
    } while (!is_end(node));
    /*
    ...
    for (order = cds_lfht_get_count_order_ulong(size); (long)order >= 0; order--) cds_lfht_free_bucket_table(ht, order);
...
int cds_lfht_destroy(struct cds_lfht *ht, pthread_attr_t **attr)
{
    ...
    if (attr) *attr = ht->resize_attr;
    ret = pthread_mutex_destroy(&ht->resize_mutex);
    if (ret) ret = -EBUSY;
    if (ht->flags & CDS_LFHT_AUTO_RESIZE) cds_lfht_fini_worker(ht->flavor);
    poison_free(ht);
    return ret;
}

void cds_lfht_count_nodes(struct cds_lfht *ht, long *approx_before, unsigned long *count, long *approx_after)
{
    struct cds_lfht_node *node, *next;
...
static void _do_cds_lfht_grow(struct cds_lfht *ht, unsigned long old_size, unsigned long new_size)
{
...
/* called with resize mutex held */
static void _do_cds_lfht_shrink(struct cds_lfht *ht, unsigned long old_size, unsigned long new_size)
{
    unsigned long old_order, new_order;
...
static unsigned long resize_target_grow(struct cds_lfht *ht, unsigned long new_size)
{
    return _uatomic_xchg_monotonic_increase(&ht->resize_target, new_size);
}

static void resize_target_update_count(struct cds_lfht *ht, unsigned long count)
{
...
static void do_resize_cb(struct urcu_work *work)
{
    struct resize_work *resize_work = caa_container_of(work, struct resize_work, work);
...
static void __cds_lfht_resize_lazy_launch(struct cds_lfht *ht)
{
    ...
    if (!CMM_LOAD_SHARED(ht->resize_initiated)) {
        ...
        work->ht = ht;
        urcu_workqueue_queue_work(cds_lfht_workqueue, &work->work, do_resize_cb);
        CMM_STORE_SHARED(ht->resize_initiated, 1);
    }
}
...
static void cds_lfht_resize_lazy_grow(struct cds_lfht *ht, unsigned long size, int growth)
{
    ...
    if (resize_target_grow(ht, target_size) >= target_size) return;
...
static void cds_lfht_resize_lazy_count(struct cds_lfht *ht, unsigned long size, unsigned long count)
{
    ...
    count = max(count, MIN_TABLE_SIZE);
    count = min(count, ht->max_nr_buckets);
    if (count == size) return; /* Already the right size, no resize needed */
    if (count > size) {        /* lazy grow */
        if (resize_target_grow(ht, count) >= count) return;
    } else { /* lazy shrink */
        for (;;) {
            unsigned long s;

            s = uatomic_cmpxchg(&ht->resize_target, size, count);
            if (s == size) break;   /* no resize needed */
            if (s > size) return;   /* growing is/(was just) in progress */
            if (s <= count) return; /* some other thread do shrink */
            size = s;
        }
    }
    __cds_lfht_resize_lazy_launch(ht);
...
{
    if (cds_lfht_workqueue_atfork_nesting++) return;
    mutex_lock(&cds_lfht_fork_mutex);
    if (!cds_lfht_workqueue) return;
...
{
    if (--cds_lfht_workqueue_atfork_nesting) return;
    if (!cds_lfht_workqueue) goto end;
 ...
 */
static void cds_lfht_worker_init(struct urcu_workqueue *workqueue __attribute__((unused)),
...
static void cds_lfht_init_worker(const struct rcu_flavor_struct *flavor)
{
    ...
    mutex_lock(&cds_lfht_fork_mutex);
    if (cds_lfht_workqueue_user_count++) goto end;
    cds_lfht_workqueue = urcu_workqueue_create(0, -1, NULL, NULL, cds_lfht_worker_init, NULL, NULL, NULL, NULL, NULL);
end:
    mutex_unlock(&cds_lfht_fork_mutex);
}
