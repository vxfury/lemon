 ...
 *  0: unavailable
 */
__attribute__((weak)) int __rcu_cas_avail = -1;

__attribute__((weak)) pthread_mutex_t __urcu_x86_compat_mutex = PTHREAD_MUTEX_INITIALIZER;
...
unsigned long _compat_uatomic_cmpxchg(void *addr, unsigned long old, unsigned long _new, int len)
{
    unsigned long retval;
    sigset_t mask;
    ...
    switch (len) {
        case 1: {
            ...
            if (result == (unsigned char)old) *(unsigned char *)addr = (unsigned char)_new;
            retval = result;
            break;
        }
        case 2: {
            unsigned short result = *(unsigned short *)addr;
            if (result == (unsigned short)old) *(unsigned short *)addr = (unsigned short)_new;
            retval = result;
            break;
        }
        case 4: {
            unsigned int result = *(unsigned int *)addr;
            if (result == (unsigned int)old) *(unsigned int *)addr = (unsigned int)_new;
            retval = result;
            break;
        }
        default:
...
{
    if (__rcu_cas_avail < 0) __rcu_cas_avail = compare_and_swap_is_available();
