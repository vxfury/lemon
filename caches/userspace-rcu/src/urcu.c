...
static void smp_mb_master(void)
{
    if (caa_likely(urcu_memb_has_sys_membarrier)) {
        if (membarrier(urcu_memb_has_sys_membarrier_private_expedited ? MEMBARRIER_CMD_PRIVATE_EXPEDITED
...
static void force_mb_all_readers(void)
{
     ...
     */
    if (cds_list_empty(&registry)) return;
    /*
...
static void rcu_sys_membarrier_status(bool available)
{
    if (!available) abort();
}
#else
...
static void rcu_sys_membarrier_init(void)
{
...
#ifdef RCU_SIGNAL
static void sigrcu_handler(int signo __attribute__((unused)), siginfo_t *siginfo __attribute__((unused)),
