...
static int enqueue_values(struct cds_wfcq_head *head, struct cds_wfcq_tail *tail, int *values, size_t nr_values)
{
    int ret = 0;
    unsigned int i;
...
static void print_queue(struct cds_wfcq_head *head, struct cds_wfcq_tail *tail, const char *qname)
{
    ...
    {
        struct mynode *node = caa_container_of(qnode, struct mynode, node);
        printf(" %d", node->value);
    }
    printf("\n");
}
...
int main(void)
{
    ...
    ret = enqueue_values(&head_A, &tail_A, values_A, CAA_ARRAY_SIZE(values_A));
    if (ret) goto end;
    cds_wfcq_init(&head_B, &tail_B);
    /* Enqueue nodes into B. */
    ret = enqueue_values(&head_B, &tail_B, values_B, CAA_ARRAY_SIZE(values_B));
    if (ret) goto end;
    ...
    (void)cds_wfcq_splice_blocking(&head_B, &tail_B, &head_A, &tail_A);
