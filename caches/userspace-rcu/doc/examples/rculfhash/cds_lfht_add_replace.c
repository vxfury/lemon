...
int main(void)
{
     ...
     * Allocate hash table.
     */
    ht = cds_lfht_new_flavor(1, 1, 0, CDS_LFHT_AUTO_RESIZE | CDS_LFHT_ACCOUNTING, &urcu_memb_flavor, NULL);
    if (!ht) {
    ...
    for (i = 0; i < CAA_ARRAY_SIZE(values); i++) {
         ...
         */
        urcu_memb_read_lock();
        ht_node = cds_lfht_add_replace(ht, hash, match, &value, &node->node);
        if (ht_node) {
            struct mynode *ret_node = caa_container_of(ht_node, struct mynode, node);
