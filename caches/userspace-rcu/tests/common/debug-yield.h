...
static inline void rcu_debug_yield_read(void)
{
    if (rcu_yield_active & RCU_YIELD_READ)
        if (rand_r(&URCU_TLS(rcu_rand_yield)) & 0x1) usleep(rand_r(&URCU_TLS(rcu_rand_yield)) % MAX_SLEEP);
}
...
static inline void rcu_debug_yield_enable(unsigned int flags __attribute__((unused))) {}

static inline void rcu_debug_yield_disable(unsigned int flags __attribute__((unused))) {}
