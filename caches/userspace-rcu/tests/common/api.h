...
#define ____cacheline_internodealigned_in_smp __attribute__((__aligned__(CAA_CACHE_LINE_SIZE)))
...
#define for_each_thread(t) for (t = 0; t < NR_THREADS; t++)

#define for_each_running_thread(t)   \
    for (t = 0; t < NR_THREADS; t++) \
        if ((__thread_id_map[t] != __THREAD_ID_MAP_EMPTY) && (__thread_id_map[t] != __THREAD_ID_MAP_WAITING))

#define for_each_tid(t, tid)         \
    for (t = 0; t < NR_THREADS; t++) \
        if ((((tid) = __thread_id_map[t]) != __THREAD_ID_MAP_EMPTY) && ((tid) != __THREAD_ID_MAP_WAITING))
...
static int smp_thread_id(void)
{
    ...
    if (id == NULL) return __smp_thread_id();
    return ((long)id - 1);
}
...
static thread_id_t create_thread(void *(*func)(void *), void *arg)
{
    ...
    spin_lock(&__thread_id_map_mutex);
    for (i = 0; i < NR_THREADS; i++) {
        if (__thread_id_map[i] == __THREAD_ID_MAP_EMPTY) break;
    }
    if (i >= NR_THREADS) {
        spin_unlock(&__thread_id_map_mutex);
...
static void *wait_thread(thread_id_t tid)
{
    ...
    for (i = 0; i < NR_THREADS; i++) {
        if (__thread_id_map[i] == tid) break;
    }
    if (i >= NR_THREADS) {
...
static void wait_all_threads(void)
{
    ...
    for (i = 1; i < NR_THREADS; i++) {
        ...
        if (tid != __THREAD_ID_MAP_EMPTY && tid != __THREAD_ID_MAP_WAITING) (void)wait_thread(tid);
    }
}
...
static inline long long get_microseconds(void)
{
    struct timeval tv;
...
#define DEFINE_PER_THREAD(type, name)                                         \
    struct {                                                                  \
        __typeof__(type) v __attribute__((__aligned__(CAA_CACHE_LINE_SIZE))); \
...
#define init_per_thread(name, v)                                                                  \
    do {                                                                                          \
        int __i_p_t_i;                                                                            \
        for (__i_p_t_i = 0; __i_p_t_i < NR_THREADS; __i_p_t_i++) per_thread(name, __i_p_t_i) = v; \
    } while (0)
...
static void smp_init(void)
{
    ...
    for (i = 1; i < NR_THREADS; i++) __thread_id_map[i] = __THREAD_ID_MAP_EMPTY;
