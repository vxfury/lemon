...
#define diag_gen0(...)                           \
    do {                                         \
        if (!fork_generation) diag(__VA_ARGS__); \
    } while (0)
...
static int do_fork(const char *execname)
{
    ...
    diag_gen0("%s parent pid: %d, before fork", execname, (int)getpid());
    ...
    if (pid == 0) {
        ...
        diag_gen0("%s child pid: %d, after fork", execname, (int)getpid());
        test_rcu();
        diag_gen0("%s child pid: %d, after rcu test", execname, (int)getpid());
        if (fork_generation >= FORK_DEPTH) exit(EXIT_SUCCESS);
        return 0;
    } else if (pid > 0) {
        int status;
        ...
        diag_gen0("%s parent pid: %d, after fork", execname, (int)getpid());
        test_rcu();
        diag_gen0("%s parent pid: %d, after rcu test", execname, (int)getpid());
        for (;;) {
            pid = wait(&status);
            if (pid < 0) {
                if (!fork_generation) perror("wait");
                return -1;
            }
            if (WIFEXITED(status)) {
                diag_gen0("child %u exited normally with status %u", pid, WEXITSTATUS(status));
                if (WEXITSTATUS(status)) return -1;
                break;
            } else if (WIFSIGNALED(status)) {
                diag_gen0("child %u was terminated by signal %u", pid, WTERMSIG(status));
                return -1;
            } else {
                continue;
            }
        ...
        if (!fork_generation) perror("fork");
        return -1;
    }
}
