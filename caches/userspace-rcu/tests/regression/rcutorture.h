...
volatile int goflag __attribute__((__aligned__(CAA_CACHE_LINE_SIZE))) = GOFLAG_INIT;
...
#define mark_rcu_quiescent_state() \
    do {                           \
    } while (0)
...
#define put_thread_online() \
    do {                    \
    } while (0)
#define put_thread_online_delay() \
    do {                          \
    } while (0)
...
static void *rcu_read_perf_test(void *arg)
{
    ...
    put_thread_offline();
    while (goflag == GOFLAG_INIT) (void)poll(NULL, 0, 1);
    put_thread_online();
...
static void *rcu_update_perf_test(void *arg __attribute__((unused)))
{
...
static int perftestrun(int nthreads, int nreaders, int nupdaters)
{
    int t;
    int duration = 1;

    cmm_smp_mb();
    while (uatomic_read(&nthreadsrunning) < nthreads) (void)poll(NULL, 0, 1);
    goflag = GOFLAG_RUN;
    cmm_smp_mb();
    sleep(duration);
    cmm_smp_mb();
    ...
    diag("n_reads: %lld  n_updates: %ld  nreaders: %d  nupdaters: %d duration: %d", n_reads, n_updates, nreaders,
         nupdaters, duration);
    diag("ns/read: %g  ns/update: %g", ((duration * 1000 * 1000 * 1000. * (double)nreaders) / (double)n_reads),
         ((duration * 1000 * 1000 * 1000. * (double)nupdaters) / (double)n_updates));
...
static int perftest(int nreaders, int cpustride)
{
    int i;
    long arg;
...
static int rperftest(int nreaders, int cpustride)
{
    int i;
    long arg;
...
static int uperftest(int nupdaters, int cpustride)
{
    int i;
    long arg;
...
static void *rcu_read_stress_test(void *arg __attribute__((unused)))
{
    int i;
    int itercnt = 0;
    struct rcu_stress *p;
    ...
    while (goflag == GOFLAG_RUN) {
        ...
        if (p->mbtest == 0) n_mberror++;
        rcu_read_lock_nest();
        for (i = 0; i < 100; i++) garbage++;
        rcu_read_unlock_nest();
        pc = p->pipe_count;
        rcu_read_unlock();
        if ((pc > RCU_STRESS_PIPE_LEN) || (pc < 0)) pc = RCU_STRESS_PIPE_LEN;
...
static void rcu_update_stress_test_rcu(struct rcu_head *head __attribute__((unused)))
{
    int ret;
    ...
    if (ret) {
        errno = ret;
        diag("pthread_mutex_lock: %s", strerror(errno));
        abort();
    }
    ret = pthread_cond_signal(&call_rcu_test_cond);
    if (ret) {
        errno = ret;
        diag("pthread_cond_signal: %s", strerror(errno));
        abort();
    }
    ret = pthread_mutex_unlock(&call_rcu_test_mutex);
    if (ret) {
        errno = ret;
        diag("pthread_mutex_unlock: %s", strerror(errno));
        abort();
    }
}

static void *rcu_update_stress_test(void *arg __attribute__((unused)))
{
    int i;
    struct rcu_stress *p;
    struct rcu_head rh;
    ...
    while (goflag == GOFLAG_RUN) {
        ...
        for (i = 0; i < RCU_STRESS_PIPE_LEN; i++)
            if (i != rcu_stress_idx) rcu_stress_array[i].pipe_count++;
        if (n_updates & 0x1)
            synchronize_rcu();
        else {
            int ret;
...
static void *rcu_fake_update_stress_test(void *arg __attribute__((unused)))
{
    if (callrcu_type == CALLRCU_PERTHREAD) {
        struct call_rcu_data *crdp;
...
static int stresstest(int nreaders)
{
    int i;
    int t;
    long long *p;
    ...
    for_each_thread(t)
    {
        ...
        for (i = 0; i <= RCU_STRESS_PIPE_LEN; i++) p[i] = 0LL;
    }
    rcu_stress_current = &rcu_stress_array[0];
    rcu_stress_current->pipe_count = 0;
    rcu_stress_current->mbtest = 1;
    for (i = 0; i < nreaders; i++) create_thread(rcu_read_stress_test, NULL);
    create_thread(rcu_update_stress_test, NULL);
    for (i = 0; i < 5; i++) create_thread(rcu_fake_update_stress_test, NULL);
    cmm_smp_mb();
    goflag = GOFLAG_RUN;
    cmm_smp_mb();
    sleep(10);
    ...
    cmm_smp_mb();
    wait_all_threads();
    for_each_thread(t) n_reads += per_thread(n_reads_pt, t);
...
{
    diag("Usage: %s nreaders [ perf | rperf | uperf | stress ] [ stride ] [ callrcu_global | callrcu_percpu | "
         "callrcu_perthread ]\n",
         argv[0]);
    exit(-1);
}
...
int main(int argc, char *argv[])
{
    ...
    switch (callrcu_type) {
        ...
        case CALLRCU_PERCPU:
            ...
            if (create_all_cpu_call_rcu_data(0)) diag("create_all_cpu_call_rcu_data: %s", strerror(errno));
            break;
    ...
    if (argc > 1) {
        ...
        if (argc == 2) {
            ok(!perftest(nreaders, cpustride), "perftest readers: %d, stride: %d", nreaders, cpustride);
            goto end;
        }
        if (argc > 3) cpustride = strtoul(argv[3], NULL, 0);
        if (strcmp(argv[2], "perf") == 0)
            ok(!perftest(nreaders, cpustride), "perftest readers: %d, stride: %d", nreaders, cpustride);
        else if (strcmp(argv[2], "rperf") == 0)
            ok(!rperftest(nreaders, cpustride), "rperftest readers: %d, stride: %d", nreaders, cpustride);
        else if (strcmp(argv[2], "uperf") == 0)
            ok(!uperftest(nreaders, cpustride), "uperftest readers: %d, stride: %d", nreaders, cpustride);
        else if (strcmp(argv[2], "stress") == 0)
            ok(!stresstest(nreaders), "stresstest readers: %d, stride: %d", nreaders, cpustride);
        else
            usage(argv);
    } else {
        usage(argv);
