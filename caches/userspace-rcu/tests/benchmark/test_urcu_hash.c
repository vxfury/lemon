...
static struct test_hash_cb test_hash_cb[] = {
    [TEST_HASH_RW] =
        {
...
static void (*get_sigusr1_cb(void))(int)
{
    return test_hash_cb[test_choice].sigusr1;
}

static void (*get_sigusr2_cb(void))(int)
{
    return test_hash_cb[test_choice].sigusr2;
}

static void *(*get_thr_reader_cb(void))(void *)
{
    return test_hash_cb[test_choice].thr_reader;
}

static void *(*get_thr_writer_cb(void))(void *)
{
    return test_hash_cb[test_choice].thr_writer;
}

static int (*get_populate_hash_cb(void))(void)
{
...
unsigned long init_pool_size = DEFAULT_RAND_POOL, lookup_pool_size = DEFAULT_RAND_POOL,
...
{
    if (caa_unlikely(key1_len != key2_len)) return -1;
...
static void *thr_count(void *arg __attribute__((unused)))
{
...
{
    struct lfht_test_node *node = caa_container_of(head, struct lfht_test_node, head);
    free(node);
}
...
int main(int argc, char **argv)
{
    ...
    unsigned long long tot_reads = 0, tot_writes = 0, tot_add = 0, tot_add_exist = 0, tot_remove = 0;
    unsigned long count;
    long approx_before, approx_after;
    ...
    if (memory_backend) {
        test_ht = _cds_lfht_new(init_hash_size, min_hash_alloc_size, max_hash_buckets_size,
                                (opt_auto_resize ? CDS_LFHT_AUTO_RESIZE : 0) | CDS_LFHT_ACCOUNTING, memory_backend,
                                &rcu_flavor, NULL);
    } else {
        test_ht = cds_lfht_new(init_hash_size, min_hash_alloc_size, max_hash_buckets_size,
                               (opt_auto_resize ? CDS_LFHT_AUTO_RESIZE : 0) | CDS_LFHT_ACCOUNTING, NULL);
    }
    if (!test_ht) {
    ...
    err = pthread_create(&tid_count, NULL, thr_count, NULL);
    if (err != 0) {
        errno = err;
        mainret = 1;
    ...
    for (i_thr = 0; i_thr < nr_readers; i_thr++) {
        err = pthread_create(&tid_reader[i_thr], NULL, get_thr_reader_cb(), &count_reader[i_thr]);
        if (err != 0) {
            errno = err;
            mainret = 1;
            perror("pthread_create");
    ...
    for (i_thr = 0; i_thr < nr_writers; i_thr++) {
        err = pthread_create(&tid_writer[i_thr], NULL, get_thr_writer_cb(), &count_writer[i_thr]);
        if (err != 0) {
            errno = err;
            mainret = 1;
            perror("pthread_create");
...
end_online:
    ...
    printf("SUMMARY %-25s testdur %4lu nr_readers %3u rdur %6lu "
           ...
           tot_add, tot_add_exist, tot_remove, nr_leaked);
    if (nr_leaked != 0) {
        mainret = 1;
