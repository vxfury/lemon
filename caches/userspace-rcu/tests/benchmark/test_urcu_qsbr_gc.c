...
static void *thr_reader(void *_count)
{
    ...
    for (;;) {
        ...
        if (local_ptr) urcu_posix_assert(local_ptr->a == 8);
        if (caa_unlikely(rduration)) loop_sleep(rduration);
        _rcu_read_unlock();
        URCU_TLS(nr_reads)++;
        /* QS each 1024 reads */
        if (caa_unlikely((URCU_TLS(nr_reads) & ((1 << 10) - 1)) == 0)) _rcu_quiescent_state();
        if (caa_unlikely(!test_duration_read())) break;
    }
...
static void rcu_gc_clear_queue(unsigned long wtidx)
{
    ...
    for (p = pending_reclaims[wtidx].queue; p < pending_reclaims[wtidx].head; p++) {
        /* poison */
        if (*p) ((struct test_array *)*p)->a = 0;
        free(*p);
    }
...
static void rcu_gc_reclaim(unsigned long wtidx, void *old)
{
    ...
    *pending_reclaims[wtidx].head = old;
    ...
    if (caa_likely(pending_reclaims[wtidx].head - pending_reclaims[wtidx].queue < reclaim_batch)) return;

    rcu_gc_clear_queue(wtidx);
}

static void *thr_writer(void *data)
{
...
#endif
        if (caa_unlikely(wduration)) loop_sleep(wduration);
        rcu_gc_reclaim(wtidx, old);
        URCU_TLS(nr_writes)++;
        if (caa_unlikely(!test_duration_write())) break;
...
int main(int argc, char **argv)
{
    ...
    if (reclaim_batch * sizeof(*pending_reclaims[0].queue) < CAA_CACHE_LINE_SIZE)
        ...
        for (i_thr = 0; i_thr < nr_writers; i_thr++)
            pending_reclaims[i_thr].queue = calloc(reclaim_batch, sizeof(*pending_reclaims[i_thr].queue));
    for (i_thr = 0; i_thr < nr_writers; i_thr++) pending_reclaims[i_thr].head = pending_reclaims[i_thr].queue;

    next_aff = 0;

    for (i_thr = 0; i_thr < nr_readers; i_thr++) {
        err = pthread_create(&tid_reader[i_thr], NULL, thr_reader, &count_reader[i_thr]);
        if (err != 0) exit(1);
    }
    for (i_thr = 0; i_thr < nr_writers; i_thr++) {
        err = pthread_create(&tid_writer[i_thr], NULL, thr_writer, (void *)(long)i_thr);
        if (err != 0) exit(1);
    }
    ...
    printf("SUMMARY %-25s testdur %4lu nr_readers %3u rdur %6lu wdur %6lu "
           ...
           "batch %ld\n",
           argv[0], duration, nr_readers, rduration, wduration, nr_writers, wdelay, tot_reads, tot_writes,
    ...
    for (i_thr = 0; i_thr < nr_writers; i_thr++) free(pending_reclaims[i_thr].queue);
