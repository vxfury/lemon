...
#define printf_verbose(fmt, args...)           \
    do {                                       \
        if (verbose_mode) printf(fmt, ##args); \
    } while (0)
...
static void *thr_enqueuer(void *_count)
{
    ...
    for (;;) {
        ...
        if (!was_nonempty) URCU_TLS(nr_empty_dest_enqueues)++;

        if (caa_unlikely(wdelay)) loop_sleep(wdelay);
    fail:
        URCU_TLS(nr_enqueues)++;
        if (caa_unlikely(!test_duration_enqueue())) break;
    }
    ...
    printf_verbose("enqueuer thread_end, tid %lu, "
                   ...
                   urcu_get_thread_id(), URCU_TLS(nr_enqueues), URCU_TLS(nr_successful_enqueues),
...
static void do_test_dequeue(enum test_sync sync)
{
    ...
    if (sync == TEST_SYNC_MUTEX)
        node = cds_wfcq_dequeue_with_state_blocking(&head, &tail, &state);
    else
        node = __cds_wfcq_dequeue_with_state_blocking(&head, &tail, &state);

    if (state & CDS_WFCQ_STATE_LAST) URCU_TLS(nr_dequeue_last)++;
...
static void do_test_splice(enum test_sync sync)
{
    ...
    if (sync == TEST_SYNC_MUTEX)
        ret = cds_wfcq_splice_blocking(&tmp_head, &tmp_tail, &head, &tail);
    else
        ret = __cds_wfcq_splice_blocking(&tmp_head, &tmp_tail, &head, &tail);
...
static void *thr_dequeuer(void *_count)
{
    ...
    for (;;) {
        ...
        if (caa_unlikely(!test_duration_dequeue())) break;
        if (caa_unlikely(rduration)) loop_sleep(rduration);
    }

    printf_verbose("dequeuer thread_end, tid %lu, "
                   "dequeues %llu, successful_dequeues %llu, "
                   "nr_splice %llu\n",
                   urcu_get_thread_id(), URCU_TLS(nr_dequeues), URCU_TLS(nr_successful_dequeues), URCU_TLS(nr_splice));
...
static void test_end(unsigned long long *nr_dequeues_l, unsigned long long *nr_dequeue_last_l)
{
    struct cds_wfcq_node *node;
    int state;
    ...
    do {
        node = cds_wfcq_dequeue_with_state_blocking(&head, &tail, &state);
        if (node) {
            if (state & CDS_WFCQ_STATE_LAST) (*nr_dequeue_last_l)++;
            free(node);
            (*nr_dequeues_l)++;
        }
    } while (node);
...
int main(int argc, char **argv)
{
    ...
    unsigned long long tot_successful_enqueues = 0, tot_successful_dequeues = 0, tot_empty_dest_enqueues = 0,
    ...
    if (!test_dequeue && !test_splice) test_splice = 1;
    ...
    for (i_thr = 0; i_thr < nr_enqueuers; i_thr++) {
        err = pthread_create(&tid_enqueuer[i_thr], NULL, thr_enqueuer, &count_enqueuer[3 * i_thr]);
        if (err != 0) exit(1);
    }
    for (i_thr = 0; i_thr < nr_dequeuers; i_thr++) {
        err = pthread_create(&tid_dequeuer[i_thr], NULL, thr_dequeuer, &count_dequeuer[4 * i_thr]);
        if (err != 0) exit(1);
    }
    ...
    printf_verbose("total number of successful enqueues : %llu, "
                   ...
                   tot_successful_enqueues, tot_empty_dest_enqueues, tot_successful_dequeues, tot_splice,
                   tot_dequeue_last);
    printf("SUMMARY %-25s testdur %4lu nr_enqueuers %3u wdelay %6lu "
           ...
           argv[0], duration, nr_enqueuers, wdelay, nr_dequeuers, rduration, tot_enqueues, tot_dequeues,
           tot_successful_enqueues, tot_empty_dest_enqueues, tot_successful_dequeues, tot_splice, tot_dequeue_last,
           end_dequeues, tot_enqueues + tot_dequeues);
    ...
    if (tot_empty_dest_enqueues != tot_dequeue_last) {
        printf("WARNING! Discrepancy between empty enqueue (%llu) and "
               ...
               tot_empty_dest_enqueues, tot_dequeue_last);
        retval = 1;
    }
