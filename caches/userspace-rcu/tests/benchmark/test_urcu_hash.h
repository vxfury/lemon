...
static inline void lfht_test_node_init(struct lfht_test_node *node, void *key, size_t key_len)
{
...
extern unsigned long init_pool_size, lookup_pool_size, write_pool_size;
...
static inline __attribute__((unused)) uint32_t hash_u32(const uint32_t *k, /* the key, an array of uint32_t values */
...
static inline void hashword2(const uint32_t *k, /* the key, an array of uint32_t values */
                             size_t length,     /* the length of the key, in uint32_ts */
...
static inline unsigned long test_hash_mix(const void *_key, size_t length, unsigned long seed)
{
 ...
 * table.
 */
static inline unsigned long test_hash(const void *_key, size_t length, unsigned long seed)
{
    if (nr_hash_chains == 0) {
...
static inline int test_match(struct cds_lfht_node *node, const void *key)
{
    struct lfht_test_node *test_node = to_test_node(node);

    return !test_compare(test_node->key, test_node->key_len, key, sizeof(unsigned long));
}

static inline void cds_lfht_test_lookup(struct cds_lfht *ht, void *key, size_t key_len, struct cds_lfht_iter *iter)
{
    urcu_posix_assert(key_len == sizeof(unsigned long));

    cds_lfht_lookup(ht, test_hash(key, key_len, TEST_HASH_SEED), test_match, key, iter);
}
