...
static void rcu_copy_mutex_lock(void)
{
    int ret;
...
static void rcu_copy_mutex_unlock(void)
{
    int ret;
...
static struct test_array *test_array_alloc(void)
{
    ...
    urcu_posix_assert(test_array[index].a == ARRAY_POISON || test_array[index].a == 0);
    ret = &test_array[index];
    array_index++;
    if (array_index == ARRAY_SIZE) array_index = 0;
    return ret;
}
...
static void *thr_writer(void *_count)
{
    unsigned long long *count = _count;
...
int main(int argc, char **argv)
{
    ...
    for (i_thr = 0; i_thr < nr_writers; i_thr++) {
        err = pthread_create(&tid_writer[i_thr], NULL, thr_writer, &count_writer[i_thr]);
        if (err != 0) exit(1);
    }
