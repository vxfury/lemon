...
static void do_test_pop(enum test_sync sync)
{
    ...
    if (sync == TEST_SYNC_MUTEX) cds_wfs_pop_lock(&s);
    node = __cds_wfs_pop_with_state_blocking(&s, &state);
    if (sync == TEST_SYNC_MUTEX) cds_wfs_pop_unlock(&s);

    if (node) {
        if (state & CDS_WFS_STATE_LAST) URCU_TLS(nr_pop_last)++;
        free(node);
...
static void do_test_pop_all(enum test_sync sync)
{
    ...
    /* Check if empty */
    if (cds_wfs_first(head) == NULL) return;
...
static void *thr_dequeuer(void *_count)
{
    ...
    printf_verbose("dequeuer thread_end, tid %lu, "
                   ...
                   urcu_get_thread_id(), URCU_TLS(nr_dequeues), URCU_TLS(nr_successful_dequeues), URCU_TLS(nr_pop_all),
                   URCU_TLS(nr_pop_last));
...
static void test_end(unsigned long long *nr_dequeues_l, unsigned long long *nr_pop_last_l)
{
    struct cds_wfs_node *node;
    int state;
    ...
    do {
        ...
        if (node) {
            if (state & CDS_WFS_STATE_LAST) (*nr_pop_last_l)++;
            free(node);
            (*nr_dequeues_l)++;
        }
    } while (node);
...
int main(int argc, char **argv)
{
    ...
    if (!test_pop && !test_pop_all) test_pop_all = 1;
    ...
    if (test_pop) printf_verbose("pop test activated.\n");
    ...
    for (i_thr = 0; i_thr < nr_enqueuers; i_thr++) {
        err = pthread_create(&tid_enqueuer[i], NULL, thr_enqueuer, &count_enqueuer[3 * i_thr]);
        if (err != 0) exit(1);
    }
    ...
    printf_verbose("total number of successful enqueues : %llu, "
                   ...
                   "pop_all : %llu, pop_last : %llu\n",
                   tot_successful_enqueues, tot_empty_dest_enqueues, tot_successful_dequeues, tot_pop_all,
                   tot_pop_last);
