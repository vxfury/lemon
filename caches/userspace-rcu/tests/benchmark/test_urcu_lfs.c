...
{
    struct test *node = caa_container_of(head, struct test, rcu);
    free(node);
}
...
static void do_test_pop(enum test_sync sync)
{
    ...
    if (sync == TEST_SYNC_RCU) rcu_read_lock();
    snode = __cds_lfs_pop(&s);
    if (sync == TEST_SYNC_RCU) rcu_read_unlock();
    if (snode) {
        struct test *node;
...
int main(int argc, char **argv)
{
    ...
    unsigned long long tot_successful_enqueues = 0, tot_successful_dequeues = 0;
    ...
    for (i_thr = 0; i_thr < nr_enqueuers; i_thr++) {
        err = pthread_create(&tid_enqueuer[i_thr], NULL, thr_enqueuer, &count_enqueuer[2 * i_thr]);
        if (err != 0) exit(1);
    }
    for (i_thr = 0; i_thr < nr_dequeuers; i_thr++) {
        err = pthread_create(&tid_dequeuer[i_thr], NULL, thr_dequeuer, &count_dequeuer[2 * i_thr]);
        if (err != 0) exit(1);
    }
    ...
    printf("SUMMARY %-25s testdur %4lu nr_enqueuers %3u wdelay %6lu "
           ...
           tot_successful_enqueues, tot_successful_dequeues, end_dequeues, tot_enqueues + tot_dequeues);
