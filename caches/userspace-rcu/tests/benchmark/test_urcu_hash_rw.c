...
void *test_hash_rw_thr_reader(void *_count)
{
    ...
    for (;;) {
        ...
        cds_lfht_test_lookup(
            test_ht, (void *)(((unsigned long)rand_r(&URCU_TLS(rand_lookup)) % lookup_pool_size) + lookup_pool_offset),
            sizeof(void *), &iter);
        ...
        if (caa_unlikely((URCU_TLS(nr_reads) & ((1 << 10) - 1)) == 0)) rcu_quiescent_state();
    }
...
void *test_hash_rw_thr_writer(void *_count)
{
    ...
    for (;;) {
        ...
        if ((addremove == AR_ADD || add_only) || (addremove == AR_RANDOM && rand_r(&URCU_TLS(rand_lookup)) & 1)) {
            node = malloc(sizeof(struct lfht_test_node));
            lfht_test_node_init(
                node, (void *)(((unsigned long)rand_r(&URCU_TLS(rand_lookup)) % write_pool_size) + write_pool_offset),
                sizeof(void *));
            rcu_read_lock();
            if (add_unique) {
                ret_node = cds_lfht_add_unique(test_ht, test_hash(node->key, node->key_len, TEST_HASH_SEED), test_match,
                                               node->key, &node->node);
            } else {
                if (add_replace)
                    ret_node = cds_lfht_add_replace(test_ht, test_hash(node->key, node->key_len, TEST_HASH_SEED),
                                                    test_match, node->key, &node->node);
                else
                    cds_lfht_add(test_ht, test_hash(node->key, node->key_len, TEST_HASH_SEED), &node->node);
            }
            rcu_read_unlock();
        ...
        if (caa_unlikely((URCU_TLS(nr_writes) & ((1 << 10) - 1)) == 0)) rcu_quiescent_state();
    }
    ...
    printf_verbose("info tid %lu: nr_add %lu, nr_addexist %lu, nr_del %lu, "
                   "nr_delnoent %lu\n",
                   urcu_get_thread_id(), URCU_TLS(nr_add), URCU_TLS(nr_addexist), URCU_TLS(nr_del),
                   URCU_TLS(nr_delnoent));
...
int test_hash_rw_populate_hash(void)
{
    ...
    while (URCU_TLS(nr_add) < init_populate) {
        ...
        node = malloc(sizeof(struct lfht_test_node));
        lfht_test_node_init(
            node, (void *)(((unsigned long)rand_r(&URCU_TLS(rand_lookup)) % init_pool_size) + init_pool_offset),
            sizeof(void *));
        rcu_read_lock();
        if (add_unique) {
            ret_node = cds_lfht_add_unique(test_ht, test_hash(node->key, node->key_len, TEST_HASH_SEED), test_match,
