...
void *test_hash_unique_thr_reader(void *_count)
{
    ...
    for (;;) {
        ...
        cds_lfht_for_each_entry(test_ht, &iter, node, node)
        {
            ...
            dup_iter = iter;
            cds_lfht_next_duplicate(test_ht, test_match, node->key, &dup_iter);
...
void *test_hash_unique_thr_writer(void *_count)
{
    ...
    for (;;) {
         ...
         */
        if (1 || (addremove == AR_ADD || add_only) || (addremove == AR_RANDOM && rand_r(&URCU_TLS(rand_lookup)) & 1)) {
...
int test_hash_unique_populate_hash(void)
{
    ...
    while (URCU_TLS(nr_add) < init_populate) {
        ...
        lfht_test_node_init(
            ...
            sizeof(void *));
        rcu_read_lock();
        ret_node = cds_lfht_add_replace(test_ht, test_hash(node->key, node->key_len, TEST_HASH_SEED), test_match,
                                        node->key, &node->node);
        rcu_read_unlock();
        if (ret_node) {
            call_rcu(&to_test_node(ret_node)->head, free_node_cb);
