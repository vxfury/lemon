...
int main(void)
{
    ...
    for (i = 0; i < parse_test_data_len; i++) {
        ret = get_max_cpuid_from_mask(parse_test_data[i].buf, strlen(parse_test_data[i].buf));
        ok(ret == parse_test_data[i].expected, "get_max_cpuid_from_mask '%s', expected: '%d', result: '%d'",
