...
template <typename T>
class LockFreeQueueCpp11 {
  public:
    explicit LockFreeQueueCpp11(size_t capacity)
    {
        _capacityMask = capacity - 1;
        for (size_t i = 1; i <= sizeof(void *) * 4; i <<= 1) _capacityMask |= _capacityMask >> i;
    ...
    ~LockFreeQueueCpp11()
    {
        for (size_t i = _head; i != _tail; ++i) (&_queue[i & _capacityMask].data)->~T();
    ...
    bool push(const T &data)
    {
        ...
        for (;;) {
            ...
            if ((_tail.compare_exchange_weak(tail, tail + 1, std::memory_order_relaxed))) break;
        }
    ...
    bool pop(T &result)
    {
        ...
        for (;;) {
            ...
            if (_head.compare_exchange_weak(head, head + 1, std::memory_order_relaxed)) break;
        }
        result = node->data;
        (&node->data)->~T();
