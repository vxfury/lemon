...
class SimpleINIParser {
  public:
    ...
    // Note that these pointers are dependent on memory owned by SimpleINIParser
  ...
  public:
    SimpleINIParser(bool use_utf8 = false, bool allow_multikey = false, bool allow_multiline = false)
        : m_text(0),
          m_textlen(0),
          m_filecomment(NULL),
          m_is_utf8(use_utf8),
    ...
    int LoadText(const char *text, size_t textlen)
    {
        ...
        int rc = FindFileComment(pnext, needcopy);
        if (rc < 0) return rc;

        // add every entry in the file to the data table
        while (FindEntry(pnext, psection, pkey, pvalue, comment)) {
            rc = AddEntry(psection, pkey, pvalue, comment, false, needcopy);
            if (rc < 0) return rc;
        }
    ...
    int SetValue(const char *section, const char *key, long value, const char *comment = NULL, bool replace = false,
                 bool usehex = false)
    {
    ...
    int SetValue(const char *section, const char *key, double value, const char *comment = NULL, bool replace = false)
    {
        // use SetValue to create sections
        if (!section || !key) return INI_FAIL;
    ...
    int SetValue(const char *section, const char *key, bool value, const char *comment = NULL, bool replace = false)
    {
        // use SetValue to create sections
        if (!section || !key) return INI_FAIL;
  ...
  private:
    ...
    SimpleINIParser(const SimpleINIParser &);            // disabled
    SimpleINIParser &operator=(const SimpleINIParser &); // disabled
    ...
    int AddEntry(const char *section, const char *key, const char *value, const char *comment, bool replace, bool clone)
    {
        ...
        if (!key || !value) {
            // section only entries are specified with item and pvalue as NULL
...
class Reloading {
  public:
        ...
        : ping(args...), pong(args...), m_isping(true), m_reloading(false), m_interval(interval), m_reloaded_time(0)
    {
        ...
        PARSERS_TRACE("reloading procedure finished, activated = ping");
    }
        ...
        if (!m_reloading && m_reloaded_time + m_interval <= time(nullptr)) {
            ...
            if (m_reloading.compare_exchange_strong(expected, true, std::memory_order_release,
                ...
                std::thread([this]() {
                    ...
                    // PARSERS_TRACE("reloading worker started in background");
                    if (m_isping.load()) {
                        if (pong.Reload() == 0) {
                            m_isping.store(false, std::memory_order_relaxed);
                            PARSERS_TRACE("reloading succeed, activated = pong, elasped = %lld us", elasped());
                        } else {
                            // non-zero will returned if no changes or has errors
                            // PARSERS_TRACE("reloading slienced for a while 'cause failure, activated = ping, elasped =
                            // %lld us", elasped());
                        }
                    } else {
                        if (ping.Reload() == 0) {
                            m_isping.store(true, std::memory_order_relaxed);
                            PARSERS_TRACE("reloading succeed, activated = ping, elasped = %lld us", elasped());
                        } else {
                            // non-zero will returned if no changes or has errors
                            // PARSERS_TRACE("reloading slienced for a while 'cause failure, activated = pong, elasped =
                            // %lld us", elasped());
                        }
                    }
                    m_reloaded_time = time(nullptr); // update reloading time each attempt
                    m_reloading.store(false, std::memory_order_relaxed);
                    // PARSERS_TRACE("reloading procedure finished, elasped = %lld us", elasped());
                }).detach(); // detach to reload in background
            } else {
                // do nothing 'cause there is an worker running in background
                // PARSERS_TRACE("reloading worker started, no worker created this time");
            }
        }
...
using INIParser = typename lemon::SimpleINIParser<INI_GenericNoCase>;
using INICaseParser = typename lemon::SimpleINIParser<INI_GenericCase>;

template <typename Compare>
class INIFile {
  public:
    INIFile(std::string file) : m_file(std::move(file)), m_isping(true)
    {
        m_ping.LoadFile(m_file.c_str());
        m_reloaded_time = m_modified_time = time(nullptr);
        PARSERS_TRACE("reloading procedure finished, activated = ping");
    }
    ...
    SimpleINIParser<Compare> &GetActivated()
    {
        // start thread to switch config only if no worker running in background
        if (!m_reloading && m_reloaded_time + m_interval <= time(nullptr)) {
            bool expected = false;
            if (m_reloading.compare_exchange_strong(expected, true, std::memory_order_release,
                ...
                if (IsModified()) {
                    std::thread([this]() {
                        ...
                        // PARSERS_TRACE("reloading worker started in background");
                        if (m_isping.load()) {
                            if (m_pong.LoadFile(m_file.c_str()) == 0) {
                                m_isping.store(false, std::memory_order_relaxed);
                                PARSERS_TRACE("reloading succeed, activated = pong, elasped = %lld us", elasped());
                            } else {
                                // non-zero will returned if no changes or has errors
                                // PARSERS_TRACE("reloading slienced for a while 'cause failure, activated = ping,
                                // elasped = %lld us", elasped());
                            }
                        } else {
                            if (m_ping.LoadFile(m_file.c_str()) == 0) {
                                m_isping.store(true, std::memory_order_relaxed);
                                PARSERS_TRACE("reloading succeed, activated = ping, elasped = %lld us", elasped());
                            } else {
                                // non-zero will returned if no changes or has errors
                                // PARSERS_TRACE("reloading slienced for a while 'cause failure, activated = pong,
                                // elasped = %lld us", elasped());
                            }
                        }
                        m_reloaded_time = time(nullptr); // update reloading time each attempt
        ...
        return m_isping.load() ? m_ping : m_pong;
    }
    ...
    bool IsModified()
    {
        ...
        if (stat(m_file.c_str(), &st) == 0) {
            bool modified = st.st_mtime > m_modified_time;
            if (modified) {
                m_modified_time = st.st_mtime;
            }
            return modified;
        }
        return false;
  ...
  private:
    ...
    std::atomic_bool m_isping;                           // true if ping selected
    std::atomic_bool m_reloading;                        // true if reloading in background
    SimpleINIParser<Compare> m_ping, m_pong;             // double buffering for reloading
    time_t m_interval, m_reloaded_time, m_modified_time; // reloading interval and last reloaded time
};
} // namespace lemon
