...
#define ATOMIC_QUEUE_LIKELY(x)   (__builtin_expect((x), 1))
#define ATOMIC_QUEUE_UNLIKELY(x) (__builtin_expect((x), 0))
#else
#define ATOMIC_QUEUE_LIKELY(x)   (x)
#define ATOMIC_QUEUE_UNLIKELY(x) (x)
#endif
...
static inline void spin_loop_pause() noexcept
{
#if (defined(__ARM_ARCH_6K__) || defined(__ARM_ARCH_6Z__) || defined(__ARM_ARCH_6ZK__) || defined(__ARM_ARCH_6T2__) \
     || defined(__ARM_ARCH_7__) || defined(__ARM_ARCH_7A__) || defined(__ARM_ARCH_7R__) || defined(__ARM_ARCH_7M__) \
     || defined(__ARM_ARCH_7S__) || defined(__ARM_ARCH_8A__) || defined(__aarch64__))
...
class atomic_queue_pattern {
  protected:
    ...
    atomic_queue_pattern() noexcept = default;

    atomic_queue_pattern(atomic_queue_pattern const &b) noexcept
        : head_(b.head_.load(std::memory_order_relaxed)), tail_(b.tail_.load(std::memory_order_relaxed))
    {
    }

    atomic_queue_pattern &operator=(atomic_queue_pattern const &b) noexcept
    {
    ...
    void swap(atomic_queue_pattern &b) noexcept
    {
    ...
    static void do_push_atomic(T element, std::atomic<T> &q_element) noexcept
    {
        assert(element != NIL);
        if (Derived::spsc_) {
            while (ATOMIC_QUEUE_UNLIKELY(q_element.load(std::memory_order_relaxed) != NIL))
                if (Derived::maximize_throughput_) spin_loop_pause();
            q_element.store(element, std::memory_order_release);
        } else {
            for (T expected = NIL; ATOMIC_QUEUE_UNLIKELY(!q_element.compare_exchange_strong(
                     expected, element, std::memory_order_release, std::memory_order_relaxed));
                 expected = NIL) {
    ...
    {
        if (Derived::spsc_) {
            while (ATOMIC_QUEUE_UNLIKELY(state.load(std::memory_order_acquire) != STORED))
                if (Derived::maximize_throughput_) spin_loop_pause();
            ...
            for (;;) {
                ...
                if (ATOMIC_QUEUE_LIKELY(state.compare_exchange_strong(expected, LOADING, std::memory_order_acquire,
    ...
    {
        if (Derived::spsc_) {
            while (ATOMIC_QUEUE_UNLIKELY(state.load(std::memory_order_acquire) != EMPTY))
                if (Derived::maximize_throughput_) spin_loop_pause();
            ...
            for (;;) {
                ...
                if (ATOMIC_QUEUE_LIKELY(state.compare_exchange_strong(expected, STORING, std::memory_order_acquire,
                                                                      std::memory_order_relaxed))) {
                    q_element = std::forward<U>(element);
                    state.store(STORED, std::memory_order_release);
                    return;
  ...
  public:
    ...
    bool try_push(T &&element) noexcept
    {
        ...
        if (Derived::spsc_) {
            if (static_cast<int>(head - tail_.load(std::memory_order_relaxed))
            ...
            do {
                if (static_cast<int>(head - tail_.load(std::memory_order_relaxed))
                    >= static_cast<int>(static_cast<Derived &>(*this).size_))
                    return false;
            } while (ATOMIC_QUEUE_UNLIKELY(!head_.compare_exchange_strong(
                head, head + 1, std::memory_order_acquire, std::memory_order_relaxed))); // This loop is not FIFO.
        }
    ...
    bool try_pop(T &element) noexcept
    {
        ...
        if (Derived::spsc_) {
            ...
            do {
                if (static_cast<int>(head_.load(std::memory_order_relaxed) - tail) <= 0) return false;
            } while (ATOMIC_QUEUE_UNLIKELY(!tail_.compare_exchange_strong(
                tail, tail + 1, std::memory_order_acquire, std::memory_order_relaxed))); // This loop is not FIFO.
        }
    ...
    unsigned was_size() const noexcept
    {
        ...
        return std::max(static_cast<int>(head_.load(std::memory_order_relaxed) - tail_.load(std::memory_order_relaxed)),
                        0);
    }
...
// This queue is for atomic elements only. AtomicQueue2 is for non-atomic ones.
template <class T, unsigned SIZE, T NIL = T{}, bool MINIMIZE_CONTENTION = true, bool MAXIMIZE_THROUGHPUT = true,
          bool TOTAL_ORDER = false, bool SPSC = false, typename = std::enable_if_t<std::atomic<T>{NIL}.is_lock_free()>>
class atomic_queue : public atomic_queue_pattern<
                         atomic_queue<T, SIZE, NIL, MINIMIZE_CONTENTION, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>> {
    using Base =
        atomic_queue_pattern<atomic_queue<T, SIZE, NIL, MINIMIZE_CONTENTION, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>>;
    friend Base;
  ...
  public:
    ...
    atomic_queue &operator=(atomic_queue const &) = delete;
};

template <class T, unsigned SIZE, bool MINIMIZE_CONTENTION = true, bool MAXIMIZE_THROUGHPUT = true,
          bool TOTAL_ORDER = false, bool SPSC = false>
class AtomicQueue2
    : public atomic_queue_pattern<AtomicQueue2<T, SIZE, MINIMIZE_CONTENTION, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>> {
    using Base =
        atomic_queue_pattern<AtomicQueue2<T, SIZE, MINIMIZE_CONTENTION, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>>;
...
template <class T, class A = std::allocator<T>, T NIL = T{}, bool MAXIMIZE_THROUGHPUT = true, bool TOTAL_ORDER = false,
          bool SPSC = false>
class AtomicQueueB : public atomic_queue_pattern<AtomicQueueB<T, A, NIL, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>>,
                     private std::allocator_traits<A>::template rebind_alloc<std::atomic<T>> {
    using Base = atomic_queue_pattern<AtomicQueueB<T, A, NIL, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>>;
    friend Base;
    ...
    // atomic_queue_pattern members are stored into by readers and writers.
  ...
  public:
          ...
          elements_(AllocatorElements::allocate(size_))
    {
        assert(std::atomic<T>{NIL}
                   .is_lock_free()); // This queue is for atomic elements only. AtomicQueueB2 is for non-atomic ones.
...
template <class T, class A = std::allocator<T>, bool MAXIMIZE_THROUGHPUT = true, bool TOTAL_ORDER = false,
          bool SPSC = false>
class AtomicQueueB2 : public atomic_queue_pattern<AtomicQueueB2<T, A, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>>,
                      private A,
                      private std::allocator_traits<A>::template rebind_alloc<std::atomic<uint8_t>> {
    using Base = atomic_queue_pattern<AtomicQueueB2<T, A, MAXIMIZE_THROUGHPUT, TOTAL_ORDER, SPSC>>;
    using State = typename Base::State;
    friend Base;
  ...
  public:
    ...
    AtomicQueueB2(unsigned size)
        : size_(std::max(details::round_up_to_power_of_2(size), 1u << (SHUFFLE_BITS * 2))),
          ...
          elements_(AllocatorElements::allocate(size_))
    {
        for (auto p = states_, q = states_ + size_; p < q; ++p) p->store(Base::EMPTY, std::memory_order_relaxed);
